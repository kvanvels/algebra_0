...
\begin{proof}
  ...
  $a', a''$ in $A$,
  \[ [a']_\sim = [a'']_\sim \Rightarrow f(a') = f(a''). \]
  ...
  \[ \tilde{f}([a']_\sim) = \tilde{f}([a'']_\sim) \Rightarrow [a']_\sim = [a'']_\sim, \]
  proving injectivity.
...
\begin{defn}{}{defn:category}
  ...
  \begin{itemize}
  \item a class $\Obj(\categ{C})$ of \emph{objects} of
    the category; and
...
\item For every object $A$ of $\categ{C}$, there exists (at least)
  one morphism $1_A \in \Hom_{\categ{C}}(A, A)$, the
  `identity' on $A$.
\item One can compose morphisms: two morphisms
  $f \in \Hom_{\categ{C}}(A, B)$ and
  $g \in \Hom_{\categ{C}}(B, C)$ determine a morphism
  $gf \in \Hom_{\categ{C}}(A, C)$. That is, for every
  triple of objects $A$, $B$, $C$ of $\categ{C}$ there is a function
  (of sets)
  \[ \Hom_{\categ{C}}(A, B) \times
    \Hom_{\categ{C}}(B, C) \to
    \Hom_{\categ{C}}(A, C), \] and the image of the
  pair $(f, g)$ is denoted $gf$.
\item This `composition law' is associative: if
  $f \in \Hom_{\categ{C}}(A, B)$,
  $g \in \Hom_{\categ{C}}(B, C)$, and
...
$\Hom_{\categ{C}}(A, B)$ and
$\Hom_{\categ{C}}(C, D)$ be disjoint unless $A = C$,
...
called an \emph{endomorphism}; $\Hom_{\categ{C}}(A, A)$
is denoted $\End_{\categ{C}}(A)$. One of the axioms of
a category tells us that this is a `pointed' set, as
$1_A \in \End_{\categ{C}}(A)$.  The reader should note
that composition defines an `operation' on
$\End_{\categ{C}}(A)$: if $f$, $g$ are elements of
$\End_{\categ{C}}(A)$, so is their composition $gf$.

Writing `$f \in \Hom_{\categ{C}}(A, B)$' gets tiresome
...
\begin{exam}{}{}
  ...
  \item for $A$, $B$ in $\Obj(\categ{Set})$ (that is, for $A$,
    $B$ sets) $\Hom_{\categ{Set}}(A, B) = B^A$.
  \end{itemize}
...
\begin{exam}{}{}
  ...
  \item \emph{morphisms}: if $a$, $b$ are objects (that is, if $a, b \in S$),
    then let $\Hom(a, b)$ be the set consisting of the element
    $(a, b) \in S \times S$ if $a \sim b$, and let $\Hom(a, b) =
    \emptyset$ otherwise.
  \end{itemize}
  ...
  $1_a \in \Hom(a, a)$. This is precisely why we are assuming that
  $\sim$ is reflexive: this tells us that $\forall a$, $a \sim a$; that is,
  $\Hom(a, a)$ consists of the single element $(a, a)$. So we have
  no choice: we must let
  \[ 1_a = (a, a) \in \Hom(a, a). \]
  As for composition, let $a$, $b$, $c$ be objects (that is, elements of $S$)
  and $f \in \Hom(a, b)$, $g \in \Hom(b, c)$; we
  have to define a corresponding morphism $gf \in \Hom(a, c)$.
  Now, $f \in \Hom(a, b)$ tells us that $\Hom(a, b)$
  ...
  Similarly, $g \in \Hom(b, c)$ tells us $b \sim c$ and $g =
  (b, c)$. Now
  \[ a \sim b \text{ and } b \sim c \Rightarrow a \sim c \]
  since we are assuming that $\sim$ is transitive. This tells us that
  $\Hom(a, c)$ consists of the single element $(a, c)$. Thus we
  again have no choice: we must let
  \[ gf := (a, c) \in \Hom(a, c). \]
  Is this operation associative? If $f \in \Hom(a, b)$, $g \in
  \Hom(b, c)$, and $h \in \Hom(c, d)$, then
...
\begin{exam}{}{}
  ...
  by setting
  \begin{itemize}
  \item $\Obj(\hat{\categ{S}}) = \mathscr{P}(S)$, the power set
    of $S$ (cf.\ \S{}1.2 and Exercise~2.11);
  \item for $A$, $B$ objects of $\hat{\categ{S}}$ (that is, $A \subseteq S$
    and $B \subseteq S$) let $\Hom_{\hat{\categ{S}}}(A, B)$ be
...
\begin{exam}{}{}
  ...
  \item $\Obj(\categ{C}_A) = $ all morphisms from any object of
    ...
    \Hom_{\categ{C}}(Z, A)$ for some object $Z$ of $\categ{C}$.
...
\begin{exer}{}{}
  ...
  \item for $A$, $B$ objects of $\categ{C}^{\mathrm{op}}$ (hence objects of $\categ{C}$),
    $\Hom_{\categ{C}^{\mathrm{op}}}(A, B) := \Hom_{\categ{C}}(B, A)$.
  \end{itemize}
  Show how to make this into a category (that is, define composition of morphisms
  in $\categ{C}^{\mathrm{op}}$ and verify the properties listed in \S{}3.1). Intuitively,
  the `opposite' category $\categ{C}^{\mathrm{op}}$ is simply obtained by `reversing all
...
\begin{exer}{}{}
  ...
  $\categ{V}$ by taking $\Obj(\categ{V}) = \N$ and letting
  $\Hom_{\categ{V}}(n, m) = $ the set of $m \times n$ matrices
...
\begin{exer}{}{}
  ...
  $\Hom_{\categ{C}'}(A, B) \subseteq \Hom_{\categ{C}}(A, B)$
  for all objects $A$, $B$ in $\Obj(\categ{C}')$, such that
  identities and compositions in $\categ{C}$ make $\categ{C}'$ into a
  category. A subcategory $\categ{C}'$ is \emph{full} if
  $\Hom_{\categ{C}'}(A, B) = \Hom_{\categ{C}}(A, B)$
  for all $A$, $B$ in $\Obj(\categ{C}')$. Construct a category of
...
A morphism $f \in \Hom_{\categ{C}}(A,B)$ is an
\textbf{isomorphism} if it has a (two-sided) inverse under composition: that
is, if $\exists\, g \in \Hom_{\categ{C}}(B,A)$ such that
\[
...
\begin{itemize}
  ...
  \item If $f \in \Hom_{\categ{C}}(A,B)$ and
        $g \in \Hom_{\categ{C}}(B,C)$ are isomorphisms, then
...
the category $\categ{C}$ obtained from the relation $\leq$ on $\Z$,
...
$\Aut_{\categ{C}}(A)$; it is a subset of
$\End_{\categ{C}}(A)$. By \Cref{prop:prelims:iso-props},
composition confers on $\Aut_{\categ{C}}(A)$ a remarkable
structure:
\begin{itemize}
  \item the composition of two elements $f, g \in \Aut_{\categ{C}}(A)$
        is an element $g \circ f \in \Aut_{\categ{C}}(A)$;
  \item composition is associative;
  \item $\Aut_{\categ{C}}(A)$ contains the element $1_A$, which
...
\end{itemize}
In other words, $\Aut_{\categ{C}}(A)$ is a group, for all
...
Let $\categ{C}$ be a category. A morphism $f \in \Hom_{\categ{C}}(A,B)$
...
epimorphism. But in the category defined by $\leq$ on $\Z$, every
...
$\Hom_{\categ{C}}(A,B)$ consisting of monomorphisms, for all
...
\[
  \forall A \in \Obj(\categ{C}) \colon \quad
  \Hom_{\categ{C}}(I,A) \text{ is a singleton.}
\]
...
\[
  \forall A \in \Obj(\categ{C}) \colon \quad
  \Hom_{\categ{C}}(A,F) \text{ is a singleton.}
\]
\end{defn}
...
there is at least one element in $\Hom_{\categ{C}}(A,A)$, namely
...
and indeed
\[
  [a_1]_\sim = [a_2]_\sim \Rightarrow a_1 \sim a_2 \Rightarrow \varphi(a_1) = \varphi(a_2).
\]
...
on $\Z$, as in Example~3.3. Does this category have products? Objects
of this category are simply integers $a, b \in \Z$; call $a \times b$
for a moment the `categorical' product of $a$ and $b$. The universal property
written out above becomes, in this case: for all $z \in \Z$ such that
...
this point) the category obtained from $\leq$ on $\Z$ does have
...
$\Z^+$ of positive integers with the divisibility relation. Thus there
