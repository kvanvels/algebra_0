...
group (end of \ref{subsec:rings:the-category-r-mod}); a functor
...
\begin{exam}{}{linrep:exam:1.4}
  In \ref{subsec:rings:prime-and-maximal-ideals} I have defined the spectrum of a commutative ring
...
\begin{exam}{(Products)}{linrep:exam:1.10}
  ...
  $A_2$ in $\categ{C}$, as defined in \ref{subsec:prelims:products}: a limit exists if and
...
of a field (in \ref{subsec:fields:algebraic-closure}) or whenever I have mentioned polynomial
...
\begin{exam}{}{linrep:exam:1.16}
  The construction of the free group on a given set (\ref{sec:groups:free-groups}) is
  ...
  (\ref{subsec:groups:universal-property}). What this really means is that for all sets $A$ and
...
\begin{exam}{(Exact functors)}{linrep:exam:1.18}
  ...
  (\ref{subsec:rings:complexes-and-exact-sequences}), so let $R$, $S$ be rings and let
...
\begin{exer}{}{linrep:exer:1.3}
  Recall (\ref{sec:groups:definition-of-group}) that a group $G$ may be thought of as a groupoid
...
\begin{exer}{$*$}{linrep:exer:1.5}
  ...
  extension\footnote{Recall that there are no `zero-homomorphisms'
    between fields; cf.\ \ref{subsec:fields:basic-definitions}.}) to the restriction
...
\begin{exer}{$\lnot$}{linrep:exer:1.6}
  ...
  making $\mathcal{C}$ a sheaf. [IX.1.15]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:1.7}
  ...
  \item Relate this topology to the Zariski topology defined in
    \ref{subsec:fields:a-little-affine-algebraic-geometry}.
...
\begin{exer}{$\lnot$}{linrep:exer:1.17}
  ...
  \item Let $I = (x)$ in $R[x]$. Prove that the completion
    ...
    defined in \ref{subsec:rings:polynomial-rings}.
...
the end of \ref{subsec:rings:the-category-r-mod}). A tensor product $M \otimes_R N$ can only be
...
(\ref{subsec:rings:products-and-coproducts}) that $M \oplus N$ serves as both the product and
...
\begin{proof}
  ...
  $M \times N$ (\ref{subsec:rings:free-modules-and-free-algebras}). This module comes equipped with a
...
\begin{exam}{}{linrep:exam:2.14}
  ...
  terms of coordinate rings (cf.\ \ref{subsec:fields:a-little-affine-algebraic-geometry}), this map corresponds
...
take the homology of this complex (cf.\ \ref{subsec:rings:homology-and-the-snake-lemma}). Astoundingly,
...
\[
  ...
  \end{array}
\]
(This will be proven in gory detail in \ref{sec:homol:derived-functors}.) Tensor the two
...
considerations at the end of \ref{subsec:linalg:pids-and-resolutions}: a bound on the length of free
...
\begin{exer}{$\lnot$}{linrep:exer:2.4}
  ...
  \ref{subsec:rings:products}. [\ref{linrep:exer:2.10}]
\end{exer}
...
\begin{exer}{}{linrep:exer:2.10}
  ...
  \end{itemize}
  (Use \Cref{linrep:2.8} to `compute' the tensor. The CRT from \ref{subsec:irred:chinese-remainder-theorem}
...
\begin{exer}{$?$}{linrep:exer:2.15}
  ...
  such that $rn = 0$ (cf.\ \ref{subsec:linalg:torsion}). (This is the reason why Tor is
...
\begin{exer}{}{linrep:exer:2.20}
  ...
  classification theorem of \ref{subsec:linalg:the-classification-theorem} makes this particularly easy.
...
homomorphism, and let $N$ be an $S$-module. Recall (\ref{subsec:rings:definition-of-left-r-module}) that
...
(cf.\ \ref{subsec:linalg:finitely-presented-modules-and-free-resolutions}), tensoring by $S$ gives (by the right-exactness of
...
(obtained in \ref{subsec:linalg:the-determinant}) prove that it is a multilinear, alternating
...
using graded rings along the same lines we sketched in \ref{subsec:fields:a-little-affine-algebraic-geometry} for
the affine case. As I mentioned at the end of \ref{subsec:fields:a-little-affine-algebraic-geometry}, this
...
algebraic geometry reviewed in \ref{subsec:fields:a-little-affine-algebraic-geometry}, but using only homogeneous
...
\begin{exam}{}{linrep:4.17}
  ...
  respect to mapping to commutative rings (cf.\ \ref{subsec:rings:universal-property-of-polynomial-rings} and
  \ref{subsec:rings:submodule-generated-by-a-subset-noetherian-modules}). Likewise, $\mathrm{T}^*_R(R^{\oplus r})$ should be
  thought of as a `noncommutative' polynomial ring, in which the $r$
  indeterminates do not commute with each other (cf.\ \ref{subsec:rings:free-modules-and-free-algebras}).
\end{exam}
...
\begin{exer}{}{linrep:exer:4.13}
  ...
  taking a formal derivative (as in \ref{subsec:fields:separable-polynomials}). With the evident
...
\begin{rem}{}{linrep:5.9}
  ...
  subset of $F$, is not. In fact, recall (\ref{subsec:linalg:linear-independence-and-bases}) that the choice
...
(cf.\ the end of \ref{subsec:rings:complexes-and-exact-sequences}). For example, assume that $P$ is
...
\ref{subsec:rings:complexes-and-exact-sequences}, a module $P$ is projective if and only if every
...
\ref{sec:homol:double-complexes}.
...
\item One could start from a projective resolution of $M$:
  ...
  take cohomology,\footnote{Recall from \ref{subsec:rings:complexes-and-exact-sequences} that the homology
...
suitable isomorphism relation (cf.\ \ref{subsec:groups2:exact-sequences-of-groups-extension-problem}). For example, the
