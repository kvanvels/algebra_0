...
the beginning of \S{}\ref{subsec:linrep:functors} (such as viewing a
...
\begin{exam}{}{linrep:exam:1.4}
  In \S{}\ref{subsec:rings:prime-and-maximal-ideals} I have defined
...
in \Cref{linrep:1.7}. It is not at all immediate that this is indeed
...
isomorphisms' (see \S{}\ref{subsec:linrep:comparing-functors}) from
...
\begin{exam}{}{linrep:exam:1.9}
  Recall (\Cref{defn:fields:2.19}) that the coordinate ring of an
...
\begin{exam}{(Products)}{linrep:exam:1.10}
  ...
  \S{}\ref{subsec:prelims:products}: a limit exists if and only if a
...
of a field (in \S{}\ref{subsec:fields:algebraic-closure}) or whenever
...
\begin{exam}{}{linrep:exam:1.16}
  ...
  (\S{}\ref{sec:groups:free-groups}) is concocted so that giving a
  set-function from a set $A$ to a group $G$ is `the same as' giving a
  group homomorphism from $F(A)$ to $G$
  (\S{}\ref{subsec:groups:universal-property}). What this really means
...
\Cref{linrep:1.17} in gory detail, but I will endeavor to convince the
...
\Cref{linrep:1.17} implies, for example, that right-adjoint functors
...
\begin{exam}{(Exact functors)}{linrep:exam:1.18}
  ...
  (\S{}\ref{subsec:rings:complexes-and-exact-sequences}), so let $R$,
  $S$ be rings and let
  ...
  \S{}\ref{subsec:linrep:exactness-properties-of-tensor-flatness}). We
...
\begin{exer}{}{exer:linrep:grp-action-is-functor-from-groupoid}
  Recall (\S{}\ref{sec:groups:definition-of-group}) that a group $G$
...
\begin{exer}{$*$}{exer:linrep:field-units-functor-and-norm-contravariant}
  ...
  extension\footnote{Recall that there are no `zero-homomorphisms'
    between fields; cf.\ \S{}\ref{subsec:fields:basic-definitions}.})
...
\begin{exer}{}{exer:linrep:hom-functor-at-point-and-poly-fns}
  ...
  \item Denote by $h_S$ the functor
    ...
    \S{}\ref{subsec:linrep:examples-of-functors}), and let
...
\begin{exer}{$\lnot$}{exer:linrep:yoneda-embedding-is-functor}
  ...
  \S{}\ref{subsec:linrep:examples-of-functors}) defines a covariant
  functor $\categ{C} \to
...
\begin{exer}{$\lnot$}{exer:linrep:yoneda-lemma}
  ...
  \S{}\ref{subsec:linrep:examples-of-functors}). For every
...
\begin{exer}{}{exer:linrep:small-category-embeds-in-functor-category}
  ...
  \S{}\ref{subsec:linrep:examples-of-functors}). In this case, $X$
...
\begin{exer}{$\lnot$}{exer:linrep:construction-recovers-kernel}
  Verify that the construction described in \Cref{linrep:exam:1.11}
...
\begin{exer}{$\lnot$}{exer:linrep:construction-is-inverse-limit}
  ...
  \Cref{linrep:1.13} is an inverse limit, as
  claimed. [\S{}\ref{subsec:linrep:limits-and-colimits}]
\end{exer}
...
\begin{exer}{$\lnot$}{exer:linrep:colimits-in-set-and-mod-directed-poset}
  ...
  \S{}\ref{subsec:linrep:limits-and-colimits}, for any indexing poset.
  ...
  that $i \leq k$, $j \leq
  k$. [\S{}\ref{subsec:linrep:limits-and-colimits}]
\end{exer}
...
\begin{exer}{$\lnot$}{exer:linrep:exact-functor-characterization}
  ...
  complexes to exact
  complexes. [\S{}\ref{subsec:linrep:comparing-functors},
...
\begin{exer}{$\lnot$}{exer:linrep:right-adjoint-left-exact-left-adjoint-right-exact}
  ...
  right-exact. [\S{}\ref{subsec:linrep:comparing-functors}]
\end{exer}
...
the end of \S{}\ref{subsec:rings:the-category-r-mod}). A tensor
...
(\S{}\ref{subsec:rings:products-and-coproducts}) that $M \oplus N$
...
\begin{proof}
  ...
  $M \times N$
  (\S{}\ref{subsec:rings:free-modules-and-free-algebras}). This module
...
we have found out in \S{}\ref{subsec:linrep:comparing-functors}, the
...
\S{}\ref{subsec:linrep:examples-of-functors}. We have defined
...
\end{coro}
\begin{proof}
  The claim is that the isomorphism found in \Cref{linrep:2.4} is
  natural in the sense hinted at, but not fully explained, in
  \S{}\ref{subsec:linrep:comparing-functors}; the interested reader
  should have no problems checking this naturality.
\end{proof}
By \Cref{linrep:1.17} (or rather its co-version), we can conclude that
...
\S{}\ref{subsec:linrep:bilinear-maps-and-the-definition-of-tensor-product}. In
particular, and this is good material for another Pavlovian reaction,
$M \otimes_R \mathord{-}$ and $\mathord{-} \otimes_R N$ are
right-exact functors (cf.\ \Cref{linrep:exam:1.18}). These
...
This follows immediately from \Cref{linrep:2.8} and the `third
...
\Cref{linrep:2.8}: for an ideal $I$ of $R$ and an $R$-module $N$, the
...
\begin{exam}{}{linrep:exam:2.12}
  Since vector spaces are free (\Cref{linalg:1.7}), tensoring is exact
...
\begin{exam}{}{linrep:exam:2.14}
  ...
  (cf.\ \S{}\ref{subsec:fields:a-little-affine-algebraic-geometry}),
  ...
  \Cref{linrep:2.13}. From the geometric point of view, clearly
...
\S{}\ref{subsec:rings:homology-and-the-snake-lemma}). Astoundingly,
...
cf.\ \Cref{linrep:exam:2.2}), we could expect the same to apply to
...
In due time (\S{}\ref{sec:homol:derived-functors} and 8) we will prove
...
Exercises~2.15 and 2.17) and applications.  In fact, we know enough
...
\S{}\ref{sec:homol:derived-functors}.) Tensor the two `free' rows by
...
\S{}\ref{subsec:linalg:pids-and-resolutions}: a bound on the length of
...
\begin{exer}{$?$}{exer:linrep:coprime-cyclic-tensor-is-zero-by-hand}
  ...
  relatively prime
  integers. [\S{}\ref{subsec:linrep:adjunction-with-hom-and-explicit-computations}]
\end{exer}
...
\begin{exer}{$?$}{exer:linrep:localization-is-tensor-with-localized-ring}
  ...
  [\S{}\ref{subsec:linrep:adjunction-with-hom-and-explicit-computations},
...
\begin{exer}{$\lnot$}{exer:linrep:tensor-and-localization-commute}
  ...
  \Cref{linrep:2.3}. Both isomorphisms will be easy consequences of
...
\begin{exer}{}{exer:linrep:f-tensor-f-separable-galois-criteria}
  ...
  \end{itemize}
  (Use \Cref{linrep:2.8} to `compute' the tensor. The CRT from
  \S{}\ref{subsec:irred:chinese-remainder-theorem} will likely be
  helpful.)
\end{exer}
\begin{exer}{$?$}{exer:linrep:complete-proof-of-adjunction}
  Complete the proof of
  \Cref{linrep:2.4}. [\S{}\ref{subsec:linrep:adjunction-with-hom-and-explicit-computations}]
\end{exer}
...
\begin{exer}{$?$}{exer:linrep:tor0-iso-to-tensor-product}
  ...
  \S{}\ref{subsec:linrep:the-tor-functors}, $\Tor^R_0(M, N)$ is
...
\begin{exer}{$?$}{exer:linrep:tor1-r-torsion-of-n}
  ...
  (cf.\ \S{}\ref{subsec:linalg:torsion}). (This is the reason why Tor
  is called Tor.) [\S{}\ref{subsec:linrep:the-tor-functors},
...
\begin{exer}{}{exer:linrep:flat-iff-torsion-free-over-pid}
  ...
  \S{}\ref{subsec:linalg:the-classification-theorem} makes this
  ...
  picture in \Cref{linrep:exam:2.14}.
\end{exer}
\begin{exer}{$\lnot$}{exer:linrep:flatness-is-local-property}
  ...
  $\mathfrak{m}$. (Hint: Use Exercises~1.25 and 2.6.  The
...
\begin{exer}{$\lnot$}{exer:linrep:localization-commutes-with-tor}
  ...
  \S{}\ref{subsec:linrep:the-tor-functors} to show
...
in
\S{}\ref{subsec:linrep:bilinear-maps-and-the-definition-of-tensor-product}. Let
$M$, $N$ be modules over a commutative ring $R$, as in
\S{}\ref{subsec:linrep:bilinear-maps-and-the-definition-of-tensor-product},
...
upgrade the adjunction formula given in \Cref{linrep:2.4}. This
...
\begin{proof}
  ...
  $\Hom_S(N, P)$. Thus $\phi$ is $R$-balanced. By \Cref{linrep:3.2},
...
If $R = S$, we recover the adjunction formula of \Cref{linrep:2.4};
...
(\S{}\ref{subsec:rings:definition-of-left-r-module}) that this means
...
\Cref{linrep:exam:1.18}, this hints that $f_*$ may have both a
left-adjoint and a right-adjoint functor, and this will be precisely
the case (\Cref{linrep:3.6}).
...
\S{}\ref{subsec:linrep:bimodules-adjunction-again}) carries naturally
...
\S{}\ref{subsec:linrep:bimodules-adjunction-again} that
...
\begin{proof}
  ...
  \end{align*}
  where I have used \Cref{linrep:3.5}. These bijections are canonical,
  proving that $f^*$ is left-adjoint to $f_*$.

  Similarly, there is a canonical isomorphism $N \cong N \otimes_S S$
  (\Cref{linrep:exam:2.2}); thus $N \otimes_S S \cong f_*(N)$ as
  ...
  \begin{align*}
    ...
    &\cong \Hom_S(N, \Hom_R(S, M))
      = \Hom_S(N, f^!(M))
  \end{align*}
  again by \Cref{linrep:3.5}. This shows that $f^!$ is right-adjoint
...
\begin{exer}{$?$}{exer:linrep:tensor-relations-via-free-abelian-presentation}
  ...
  [\S{}\ref{subsec:linrep:balanced-maps}]
\end{exer}
...
\begin{exer}{$?$}{exer:linrep:associativity-of-tensor-product}
  ...
  \S{}\ref{subsec:linrep:multilinear-symmetric-alternating-maps}]
\end{exer}
...
\begin{exer}{}{exer:linrep:dim-of-restriction-of-scalars-field-ext}
  ...
  \S{}\ref{subsec:linrep:restriction-and-extension-of-scalars}).
\end{exer}
...
\begin{exer}{}{exer:linrep:composition-rules-for-base-change-functors}
  ...
  \Cref{linrep:rem:3.7}), one would actually work with categories
  which are opposite to the category of commutative rings; cf.\
  \Cref{linrep:exam:1.9}. A ring homomorphism $f : R \to S$
...
(obtained in \S{}\ref{subsec:linalg:the-determinant}) prove that it is
...
\S{}\ref{subsec:fields:a-little-affine-algebraic-geometry} for the
affine case. As I mentioned at the end of
\S{}\ref{subsec:fields:a-little-affine-algebraic-geometry}, this
...
\S{}\ref{subsec:fields:a-little-affine-algebraic-geometry}, but using
...
\begin{proof}
  We have observed in
  \S{}\ref{subsec:linrep:very-small-detour-graded-algebra} that the
  ...
  \S{}\ref{subsec:linrep:symmetric-and-exterior-powers}.
\end{proof}
...
\begin{exam}{}{linrep:4.17}
  ...
  \S{}\ref{subsec:rings:submodule-generated-by-a-subset-noetherian-modules}). Likewise,
...
\begin{exer}{$*$}{exer:linrep:wedge-power-universal-property-and-sym-power}
  ...
  \S{}\ref{subsec:linrep:symmetric-and-exterior-powers} does satisfy
  ...
  maps. [\S{}\ref{subsec:linrep:symmetric-and-exterior-powers}]
\end{exer}
...
\begin{exer}{$*$}{exer:linrep:multilinear-map-phi-i-alternating}
  ...
  Lemma~\ref{linrep:4.3} is
  alternating. [\S{}\ref{subsec:linrep:symmetric-and-exterior-powers}]
\end{exer}
...
\begin{exer}{}{exer:linrep:grassmannian-2-4-as-projective-quadric}
  ...
  Exercise~\ref{exer:linrep:plucker-embedding-well-defined} realizes
  ...
  Exercise~\ref{exer:linrep:homog-ideal-vanishing-well-defined-proj-set})
...
\begin{exer}{}{exer:linrep:weyl-algebra-presentation}
  ...
  \S{}\ref{subsec:fields:separable-polynomials}). With the evident
...
\begin{exer}{$\lnot$}{exer:linrep:artin-rees-lemma}
  ...
  By Exercise~\ref{exer:linrep:rees-algebra-is-noetherian},
...
\begin{exer}{$\lnot$}{exer:linrep:intersection-of-ideal-powers-via-artin-rees}
  ...
  (Hint: Exercise~\ref{exer:linrep:artin-rees-lemma}.)
...
\begin{exer}{}{exer:linrep:rees-algebra-example-iso-to-quotient}
  ...
  \S{}\ref{subsec:linrep:tensor-algebras}. It will be helpful to have
  ...
  help.) [\S{}\ref{subsec:linrep:very-small-detour-graded-algebra},
  \S{}\ref{subsec:linrep:tensor-algebras}]
\end{exer}
...
\S{}\ref{subsec:linrep:examples-of-functors})?
...
in
\S{}\ref{sec:linrep:projective-and-injective-modules-and-the-ext-functors}. Before
...
\begin{rem}{}{linrep:5.9}
  ...
  a free module $F = R^{\oplus S}$. We proved in \Cref{linalg:1.11}
  ...
  (\S{}\ref{subsec:linalg:linear-independence-and-bases}) that the
  choice of a specific isomorphism $F \cong R^{\oplus S}$ is
  equivalent to the choice of a basis of $F$. Now, the isomorphism
  appearing in Corollary~\ref{linrep:5.7} requires knowledge of $S$;
  indeed, we will verify in a moment (Example~\ref{linrep:5.11}) that
...
(\S{}\ref{subsec:linrep:duals-and-matrices-biduality}): we will
...
further, it is a consequence of Exercise 2.23. But here is a direct
argument:
...
\begin{exer}{$*$}{exer:linrep:hom-from-free-iso-dual-tensor-n}
  ...
  [\S{}\ref{subsec:linrep:duality-on-vector-spaces}]
\end{exer}
...
\begin{exer}{$*$}{exer:linrep:evaluation-map-dual-tensor-free-iso-hom}
  ...
  Proposition~\ref{linrep:5.5}. [\S{}\ref{subsec:linrep:dual-modules}]
\end{exer}
...
\begin{exer}{$*$}{exer:linrep:direct-computation-dual-of-free-module}
  ...
  [\S{}\ref{subsec:linrep:duals-of-free-modules}]
\end{exer}
...
\begin{exer}{$*$}{exer:linrep:compute-dual-of-fin-gen-module-over-pid}
  ...
  `Compute' the dual
  $M^\vee$. [\S{}\ref{subsec:linrep:duals-and-matrices-biduality}]
\end{exer}
...
We have seen in \S{}\ref{subsec:linrep:adjunction-again} that the Hom
...
\S{}\ref{subsec:linrep:the-tor-functors}) could be proven in detail by
...
(cf.\ the end of
\S{}\ref{subsec:rings:complexes-and-exact-sequences}). For example,
...
\S{}\ref{subsec:rings:complexes-and-exact-sequences}, a module $P$ is
...
especially \S{}\ref{sec:homol:double-complexes}.
...
defined in
\S{}\ref{subsec:linrep:restriction-and-extension-of-scalars} a
functor\footnote{Watch out --- in
  \S{}\ref{subsec:linrep:restriction-and-extension-of-scalars} the
...
\item One could start from a projective resolution of $M$:
  ...
  take cohomology,\footnote{Recall from
    \S{}\ref{subsec:rings:complexes-and-exact-sequences} that the
...
\S{}\ref{subsec:groups2:exact-sequences-of-groups-extension-problem}). For
...
\begin{exer}{$*$}{exer:linrep:projective-injective-splitting-characterization}
  ...
  splits. [\S{}\ref{subsec:linrep:projectives-and-injectives}]
\end{exer}
...
\begin{exer}{$\lnot$}{exer:linrep:restriction-of-scalars-of-projective-is-projective}
  ...
  $S$-module, then $f^*(P)$ is a projective
  $R$-module. [\S{}\ref{subsec:linrep:injective-modules}]
\end{exer}
...
\begin{exer}{$*$}{exer:linrep:projective-modules-are-flat-via-adjunction}
  ...
  [\S{}\ref{subsec:linrep:projective-modules},
  \ref{exer:linrep:combine-results-reprove-rank-thm}]
\end{exer}
...
\begin{exer}{$*$}{exer:linrep:injective-over-pid-iff-divisible}
  ...
  [\S{}\ref{subsec:linrep:injective-modules}]
\end{exer}
...
\begin{exer}{$*$}{exer:linrep:ext-vanishes-above-degree-1-over-pid}
  ...
  $N$. [\S{}\ref{subsec:linrep:ext-Z-G-Z}]
\end{exer}
