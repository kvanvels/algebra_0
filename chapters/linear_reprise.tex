...
more sophisticated constructions than those reviewed in \Cref{chap:linalg}. My
main goals are to discuss some aspects of multilinear algebra, tensor
products, and Hom, with special attention devoted to dual
modules. Along the way we will meet several other interesting notions,
including a first taste of projective and injective modules and of Tor
and Ext.

As in \Cref{chap:linalg}, I will attempt to deal with the more general case of
...
complement the preliminary notions described in the very first chapter
of this book.
...
$\categ{C}$ by `reversing the arrows'; cf.\ \Cref{exer:prelim:op-categ-def}. The fact
...
group (end of \S{}\ref{subsec:rings:the-category-r-mod}); a functor
...
the beginning of \S{}\ref{subsec:linrep:functors} (such as viewing a ring as an abelian group
under addition) are covariant functors: for example, the fact that a
homomorphism of rings is in particular a homomorphism of the
underlying abelian groups amounts to the statement that labeling each
ring with its underlying abelian group defines a covariant functor
$\categ{Ring} \to \categ{Ab}$. Since such functors are obtained by
`forgetting' part of the structure of a given object, they are
memorably called \emph{forgetful functors}. I have already mentioned
briefly a few slightly more imaginative examples; here they are again.
...
\begin{exam}{}{linrep:exam:1.3}
  The operation $\Aut_k(-)$ from Galois field extensions to
...
\begin{exam}{}{linrep:exam:1.4}
  In \S{}\ref{subsec:rings:prime-and-maximal-ideals} I have defined the spectrum of a commutative ring
  $R$, $\mathrm{Spec}\,R$, as the set of prime ideals of $R$. If $R$,
  $S$ are commutative rings and $\phi : R \to S$ is a ring
  homomorphism, then the inverse image $\phi^{-1}(\mathfrak{p})$ of a
  prime ideal $\mathfrak{p}$ is a prime ideal; thus, $\phi$ induces a
  set-function $\phi^\ast : \mathrm{Spec}(S) \to
  \mathrm{Spec}(R)$. This assignment is clearly functorial, so we can
  view $\mathrm{Spec}$ as a contravariant functor from the category of
  commutative rings to $\categ{Set}$. The reader will assign a
  topology to $\mathrm{Spec}(R)$ (\Cref{linrep:exer:1.7}), making this example
  (even) more interesting.
\end{exam}
\begin{exam}{}{linrep:exam:1.5}
  If $S$ is a set, recall (\Cref{exam:prelims:power-set-category}) that one can construct a
...
are only defined up to isomorphism (\Cref{prop:prelims:initial-final-unique}). Requiring
solutions in one context to match exactly with solutions in another
would be problematic. The structure of an object in a category is
adequately carried by its isomorphism class\footnote{One should not
  take this viewpoint too far.  It is tempting to think of isomorphic
  objects in a category as `the same', but this is also problematic
  and does not lead (as far as I know) to a workable alternative. For
  example, some objects in a category may have `more automorphisms'
  than others, and this information would be discarded if we simply
  chopped up the category into isomorphism classes, draconically
  promoting all isomorphisms to identity morphisms.}, and a natural
notion of `equivalence' of categories should aim at matching
isomorphism classes, rather than individual objects. The
\emph{morphisms} are a more essential piece of information; the
quality of a functor is first of all measured on how it acts on
morphisms.
...
in $\categ{D}$ (\Cref{linrep:exer:1.2}). For all intents and purposes, a fully
faithful functor $F : \categ{C} \to \categ{D}$ identifies $\categ{C}$
with a full\footnote{See \Cref{exer:prelim:subcat} for the definition of `full'
  subcategory.} subcategory of $\categ{D}$, even if `on objects' $F$
may be neither injective nor surjective onto that subcategory. This
...
in \Cref{linrep:1.7}. It is not at all immediate that this is indeed an
equivalence relation (why is it symmetric?), but this turns out to be
the case. In fact, an equivalence of categories $F$ has an `inverse',
in a suitably weakened sense: a functor $G : \categ{D} \to \categ{C}$
such that there are `natural isomorphisms' (see \S{}\ref{subsec:linrep:comparing-functors}) from
...
\begin{exam}{}{linrep:exam:1.8}
  (Cf.\ \Cref{exer:prelim:categ-matrix}.) Construct a category by taking the objects to
  be nonneg\-ative integers and $\Hom(m, n)$ to be the set of
  $n \times m$ matrices with entries in a field $k$, with composition
  defined by product of matrices (and suitable care concerning
  matrices with no rows or columns). The resulting category is
  equivalent to the category of finite-dimensional $k$-vector
  ...
  \S{}\ref{subsec:linalg:matrices}.
\end{exam}
\begin{exam}{}{linrep:exam:1.9}
  Recall (\Cref{defn:fields:2.19}) that the coordinate ring of an affine
  algebraic set over a field $K$ is a reduced, commutative,
  ...
  defining\footnote{The switch in the order of $S$ and $T$ is
    reasonable: as the reader has verified in \Cref{exer:fields:poly-fns-iso-to-coordinate-ring},
    $K[S]$ may be interpreted as the $K$-algebra of `polynomial
    functions' on $S$; any function $\phi : S \to T$ determines a
    pull-back of functions, $K[T] \to K[S]$.}
...
\begin{exam}{(Products)}{linrep:exam:1.10}
  ...
  morphisms\footnote{This latter prescription is what makes the
    category `discrete'; cf.\ \Cref{exam:prelims:category-leq}.}, and let $\mathscr{A}$
  be a functor from $\categ{I}$ to any category $\categ{C}$; let
  ...
  $A_2$ in $\categ{C}$, as defined in \S{}\ref{subsec:prelims:products}: a limit exists if and
...
\begin{exam}{(Equalizers and kernels)}{linrep:exam:1.11}
  ...
  the reader will verify (\Cref{linrep:exer:1.14}).
\end{exam}
...
of a field (in \S{}\ref{subsec:fields:algebraic-closure}) or whenever I have mentioned polynomial
rings with `infinitely many variables' (e.g., in
\Cref{exam:rings:infinite-poly-ring-fg}). More formally, $\bigcup_i A_i$ consists of
equivalence classes of pairs $(i, a_i)$, where $a_i \in A_i$ and
$(i, a_i)$ is equivalent to $(j, a_j)$ for $i \leq j$ if
$a_j = \psi_{ij}(a_i)$.
...
more general posets (\Cref{linrep:exer:1.16}). They are somewhat more explicit
and better behaved in the case of \emph{directed sets}: partially
ordered sets $\categ{I}$ such that $\forall i, j \in \categ{I}$ there
exists $k \in \categ{I}$ such that $i \leq k$, $j \leq k$.
...
\begin{exam}{}{linrep:exam:1.16}
  The construction of the free group on a given set (\S{}\ref{sec:groups:free-groups}) is
  concocted so that giving a set-function from a set $A$ to a group
  $G$ is `the same as' giving a group homomorphism from $F(A)$ to $G$
  (\S{}\ref{subsec:groups:universal-property}). What this really means is that for all sets $A$ and
  all groups $G$ there are natural identifications
  \[
...
rightly be termed \emph{cocontinuous}.)  We will not prove \Cref{linrep:1.17}
in gory detail, but I will endeavor to convince the reader that such
statements are easier than they look and just boil down to suitable
applications of the universal properties defining the various
concepts. By contrast, proving that a specific given functor preserves
products (for instance), without appealing to abstract nonsense, may
sometimes appear to involve some `real' work.
...
\Cref{linrep:1.17}. What is missing from this outline is the explicit
...
\Cref{linrep:1.17} implies, for example, that right-adjoint functors preserve
products and that they must `preserve kernels' when kernels make
sense.
\begin{exam}{(Exact functors)}{linrep:exam:1.18}
  ...
  (\S{}\ref{subsec:rings:complexes-and-exact-sequences}), so let $R$, $S$ be rings and let
  ...
  image of every exact complex is exact (\Cref{linrep:exer:1.23}).
  ...
  \S{}\ref{subsec:linrep:exactness-properties-of-tensor-flatness}). We say that an additive functor $F$ is \emph{left-exact}
  if whenever
  \[
...
\end{prop}
As the reader will verify (\Cref{linrep:exer:1.27}), this follows from the fact
that right-adjoint functors preserve kernels. (Note that exactness of
$0 \to A \xrightarrow{\phi} B$ amounts to the fact that
$\phi : A \hookrightarrow B$ identifies $A$ with $\ker \psi$.) Of
course the corresponding co-statements hold: since left-adjoint
functors preserve colimits and cokernels are colimits, it follows that
left-adjoint additive functors
...
\begin{exer}{}{linrep:exer:1.3}
  Recall (\S{}\ref{sec:groups:definition-of-group}) that a group $G$ may be thought of as a groupoid
  $\mathcal{G}$ with a single object. Prove that defining the action
  of $G$ on an object of a category $\categ{C}$ is equivalent to
  defining a functor $\mathcal{G} \to \categ{C}$.
\end{exer}

\begin{exer}{$\lnot$}{linrep:exer:1.4}
  Let $R$ be a commutative ring, and let $S \subseteq R$ be a
  multiplicative subset in the sense of \Cref{exer:irred:localization-construction}. Prove that
  `localization is a functor': associating with every $R$-module $M$
  the localization $S^{-1}M$ (\Cref{exer:irred:localization-of-module}) and with every
...
\begin{exer}{$*$}{linrep:exer:1.5}
  ...
  extension\footnote{Recall that there are no `zero-homomorphisms'
    between fields; cf.\ \S{}\ref{subsec:fields:basic-definitions}.}) to the restriction
  $\phi|_{k^*} : k^* \to F^*$ is clearly a covariant functor. On the
  other hand, we can consider the category $\categ{Fld}^f_k$ of finite
  extensions of a fixed field $k$. Prove that the assignment
  $F \mapsto F^*$ on objects, together with the prescription
  associating with every $F_1 \subseteq F_2$ the norm
  $N_{F_1 \subseteq F_2} : F_2^* \to F_1^*$ (cf.\ \Cref{exer:fields:norm-is-multiplicative}),
  gives a contravariant functor $\categ{Fld}^f_k \to \categ{Grp}$.
  State and prove an analogous statement for the trace (cf.\
...
\begin{exer}{$\lnot$}{linrep:exer:1.6}
  ...
  making $\mathcal{C}$ a sheaf. [\ref{exer:homol:1.15}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:1.7}
  ...
  \end{itemize} [\S{}\ref{subsec:linrep:examples-of-functors}]
\end{exer}

\begin{exer}{}{linrep:exer:1.8}
  Let $K$ be an algebraically closed field, and consider the category
  $K\text{-}\categ{Aff}$ defined in \Cref{linrep:exam:1.9}.
  \begin{itemize}
  \item Denote by $h_S$ the functor
    $\Hom_{K\text{-}\mathrm{Aff}}(\mathord{-}, S)$ (as in \S{}\ref{subsec:linrep:examples-of-functors}),
    and let $p = \A^0_K$ be a point. Show that there is a
    natural bijection between $S$ and $h_S(p)$. (Use
  ...
  \item If $S \subseteq \A^m_K$, $T \subseteq \A^n_K$,
    show that the function $S \to T$ determined by a morphism
    $\phi \in \Hom_{K\text{-}\mathrm{Aff}}(S, T)$ is the restriction
    of a `polynomial function' $\A^m_K \to
    \A^n_K$. (Part of this exercise is to make sense of what
    this means!)
  \end{itemize}
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:1.9}
  ...
  $X \mapsto h_X := \Hom_{\categ{C}} (\mathord{-}, X)$ (cf.\ \S{}\ref{subsec:linrep:examples-of-functors})
  defines a covariant functor
  $\categ{C} \to \categ{Set}^{\categ{C}^{\mathrm{op}}}$. (Define the
  action on morphisms in the natural way.) [\ref{linrep:exer:1.11}, \ref{exer:homol:1.11}]
\end{exer}

\begin{exer}{$\lnot$}{linrep:exer:1.10}
  Let $\categ{C}$ be a category, $X$ an object of $\categ{C}$, and
  consider the contravariant functor
  $h^X := \Hom_{\categ{C}} (\mathord{-}, X)$ (cf.\ \S{}\ref{subsec:linrep:examples-of-functors}). For every
  ...
  natural transformation $h^X \leadsto F$ consists of a morphism from
  $h^X(A) = \Hom_{\categ{C}}(A, X)$ to $F(A)$ for every object $A$ of
  $\categ{C}$. Map $h^X$ to the image of $\id_X \in h^X(X)$ in
  ...
  \emph{Yoneda lemma}.  [\ref{linrep:exer:1.11}, \ref{exer:homol:2.17}]
\end{exer}

\begin{exer}{}{linrep:exer:1.11}
  (Cf.\ \Cref{linrep:exer:1.9}.) Let $\categ{C}$ be a small category. A
  contravariant functor $\categ{C} \to \categ{Set}$ is
  \emph{representable} if it is naturally isomorphic to a functor
  $h^X$ (cf.\ \S{}\ref{subsec:linrep:examples-of-functors}). In this case, $X$ `represents' the
  functor. Prove that $\categ{C}$ is equivalent to the subcategory of
  representable functors in
  $\categ{Set}^{\categ{C}^{\mathrm{op}}}$. (Hint: Yoneda; see
  \Cref{linrep:exer:1.10}.) Thus, every (small) category is equivalent to a
...
\begin{exer}{$\lnot$}{linrep:exer:1.14}
  Verify that the construction described in \Cref{linrep:exam:1.11} indeed
  recovers the kernel of a homomorphism of $R$-modules, as
  claimed. [\S{}\ref{subsec:linrep:limits-and-colimits}]
\end{exer}

\begin{exer}{$\lnot$}{linrep:exer:1.15}
  Verify that the construction given in the proof of \Cref{linrep:1.13}
  is an inverse limit, as claimed. [\S{}\ref{subsec:linrep:limits-and-colimits}]
\end{exer}

\begin{exer}{$\lnot$}{linrep:exer:1.16}
  Flesh out the sketch of the constructions of colimits in
  $\categ{Set}$ and $R\text{-}\categ{Mod}$ given in \S{}\ref{subsec:linrep:limits-and-colimits}, for any
  indexing poset.  In $\categ{Set}$, observe that the construction of
  the colimit is simpler if the poset $\categ{I}$ is directed; that
  is, if $\forall i, j \in \categ{I}$, there exists $k \in \categ{I}$
  such that $i \leq k$, $j \leq k$. [\S{}\ref{subsec:linrep:limits-and-colimits}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:1.17}
  ...
  \item Let $I = (x)$ in $R[x]$. Prove that the completion
    ...
    defined in \S{}\ref{subsec:rings:polynomial-rings}.
  \end{itemize} [\ref{linrep:exer:1.18}, \ref{linrep:exer:1.19}]
\end{exer}
...
\begin{exer}{}{linrep:exer:1.18}
  ...
  will prove this in \Cref{linrep:exer:4.20}. Assume this, and prove that
  $\bigcap_n I^n$ equals the set of $r \in R$ such that $(1-a)r = 0$
  for some $a \in I$. (Hint: One inclusion is elementary. For the
  other, use the Nakayama lemma in the form of \Cref{exer:linalg:3.7}. This
  result is attributed to Krull.) For example, if $I$ is proper, then
  $\bigcap_n I^n = (0)$ if $R$ is an integral domain or if it is
  local.  In these cases, the natural map $R \to \hat{R}_I$ to the
  $I$-adic completion is injective (cf.\ \Cref{linrep:exer:1.17}).
\end{exer}

\begin{exer}{$\lnot$}{linrep:exer:1.19}
  An important example of the construction presented in \Cref{linrep:exer:1.17}
  is the ring $\Z_p$ of \emph{$p$-adic integers}: this is the limit
  ...
  \item Prove that $\Z_p$ is a DVR (cf.\ \Cref{exer:irred:discrete-valuation-ring-basics}).  (There is
    an evident valuation on $\Q_p$.)
  \end{itemize} [\S{}\ref{subsec:groups:cyclic-groups-and-modular-arithmetic}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:1.20}
  ...
  elements of $\hat{\Z}$ may be described.) [\ref{linrep:exer:1.21}]
\end{exer}

\begin{exer}{}{linrep:exer:1.21}
  Let $\hat{\Z}$ be as in \Cref{linrep:exer:1.20}.
  \begin{itemize}
...
\begin{exer}{$\lnot$}{linrep:exer:1.23}
  ...
  complexes to exact complexes. [\S{}\ref{subsec:linrep:comparing-functors}, \ref{exer:homol:3.7}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:1.25}
  Prove that localization (\Cref{linrep:exer:1.4}) is an exact functor. In fact,
  ...
  \[
    ...
    S^{-1}M_{i-1} \to \cdots.
  \] [\ref{linrep:exer:2.12}, \ref{linrep:exer:2.21}, \ref{linrep:exer:2.22}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:1.27}
  ...
  left-adjoint functors are right-exact. [\S{}\ref{subsec:linrep:comparing-functors}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:1.28}
  ...
  isomorphic to the center of $R$. [\ref{linrep:exer:3.15}]
\end{exer}
...
the end of \S{}\ref{subsec:rings:the-category-r-mod}). A tensor product $M \otimes_R N$ can only be
defined if $M$ is a right-$R$-module and $N$ is a left-$R$-module (in
a sense, the two module structures annihilate each other, and what is
left is an abelian group). By contrast, in the commutative case we
will be able to define $M \otimes_R N$ simply as an $R$-module. In
general, the theory goes through as in the commutative case if the
modules carry compatible left- and right-module structures, except in
questions such as the commutativity of tensors, where it would be
unreasonable to expect the commutativity of $R$ to have no
bearing. All in all, the commutative case is a little leaner, and (I
believe) it suffices in terms of conveying the basic intuition on the
general features of the theory.  Thus, $R$ will denote a fixed
...
(\S{}\ref{subsec:rings:products-and-coproducts}) that $M \oplus N$ serves as both the product and
coproduct of $M$ and $N$: a situation in which a limit coincides with
a colimit. As a set, $M \oplus N$ is just $M \times N$; the $R$-module
structure on $M \oplus N$ is defined by componentwise addition and
multiplication by scalars. An $R$-module homomorphism
$M \oplus N \to P$ is determined by $R$-module homomorphisms $M \to P$
and $N \to P$ (this is what makes $M \oplus N$ into a coproduct). But
there is another way to map $M \times N$ to an $R$-module $P$,
compatibly with the $R$-module structures.
...
(\Cref{prop:prelims:initial-final-unique} once more), and we could proceed to study it by
systematically using the universal property.
...
\begin{proof}
  ...
  $M \times N$ (\S{}\ref{subsec:rings:free-modules-and-free-algebras}). This module comes equipped with a
  set-map $j : M \times N \to F_R(M \times N)$, universal with respect
  to all set-maps from $M \times N$ to any $R$-module $P$; the main
  task is to make this into an $R$-bilinear map. For example, we have
  to identify elements in $F_R(M \times N)$ of the form
  $j(m, n_1 + n_2)$ with elements $j(m, n_1) + j(m, n_2)$, etc. Thus,
  let $K$ be the $R$-submodule of $F_R(M \times N)$ generated by all
  elements
  \[
...
is a pure tensor. See \Cref{linrep:exer:2.1} for one situation in which every
...
we have found out in \S{}\ref{subsec:linrep:comparing-functors}, the mere fact that $\otimes_R$ is
...
functor $P \mapsto \Hom_R(N, P)$; cf.\ \S{}\ref{subsec:linrep:examples-of-functors}. We have defined
...
\begin{proof}
  ...
  (\Cref{linrep:exer:2.11}) that the map $\alpha \mapsto \tilde\phi$ is
  $R$-linear and construct an inverse.
\end{proof}
...
\end{coro}
\begin{proof}
  The claim is that the isomorphism found in \Cref{linrep:2.4} is natural in
  the sense hinted at, but not fully explained, in \S{}\ref{subsec:linrep:comparing-functors}; the
  interested reader should have no problems checking this naturality.
\end{proof}
By \Cref{linrep:1.17} (or rather its co-version), we can conclude that for
each $R$-module $N$, the functor $\otimes_R N$ preserves colimits, and
so does $M \otimes_R \mathord{-}$, by the basic commutativity of
tensor products verified in \S{}\ref{subsec:linrep:bilinear-maps-and-the-definition-of-tensor-product}. In particular, and this is good
material for another Pavlovian reaction, $M \otimes_R \mathord{-}$ and
$\mathord{-} \otimes_R N$ are right-exact functors (cf.\
\Cref{linrep:exam:1.18}). These observations have several consequences, which
make `computations' with tensor products more reasonable. Here is a
sample:
...
to the right-hand side (\Cref{exer:rings:free-module-product-sets}). For finitely generated free
modules, this simply says that
...
(\Cref{linalg:1.7}). Tensor products are more interesting over more
general rings.
...
This follows immediately from \Cref{linrep:2.8} and the `third isomorphism
theorem', \Cref{prop:rings:third-iso-modules}. Indeed, $I(R/J) = (I+J)/J$.
...
\begin{exam}{}{linrep:exam:2.10}
  ...
  (cf.\ \Cref{linrep:exer:2.2}).
\end{exam}

\Cref{linrep:2.8} is a template example for a basic application of
...
will take a look at localization in \Cref{linrep:exer:2.5}.
...
\Cref{linrep:2.8}: for an ideal $I$ of $R$ and an $R$-module $N$, the map
...
identified (via \Cref{linrep:2.6}) with the inclusion
$M_1^{\oplus A} \subseteq M_2^{\oplus A}$.

\begin{exam}{}{linrep:exam:2.12}
  Since vector spaces are free (\Cref{linalg:1.7}), tensoring is
  exact in $k\text{-}\categ{Vect}$: if
  \[
...
\begin{exam}{}{linrep:exam:2.14}
  Consider the affine algebraic set $V(xy)$ in the plane
  $\A^2$ (over a fixed field $k$) and the `projection on the
  first coordinate' $V(xy) \to \A^1$, $(x,y) \mapsto x$. In
  terms of coordinate rings (cf.\ \S{}\ref{subsec:fields:a-little-affine-algebraic-geometry}), this map corresponds
  to the homomorphism of $k$-algebras $k[x] \to k[x,y]/(xy)$ defined
  by mapping $x$ to the coset $x + (xy)$ (this will be completely
  clear to the reader who has worked out \Cref{exer:fields:poly-fns-iso-to-coordinate-ring}!). This
  homomorphism defines a $k[x]$-module structure on $k[x,y]/(xy)$, and
  we can wonder whether the latter is flat in the sense of
  \Cref{linrep:2.13}. From the geometric point of view, clearly something
  `not flat' is going on over the point $x = 0$, so we consider the
  inclusion of the ideal $(x)$ in $k[x]$:
  \[
...
$\Tor^R_1(\mathord{-}, N)$: if $N$ is flat (for example, if it
is free), then $\Tor^R_1(M, N) = 0$ for all modules $M$. In
fact (amazingly) if
\[
  0 \to A \to B \to C \to 0
\]
...
\[
  \Tor^R_1(C, N) \to A \otimes_R N \to B \otimes_R N \to C
  \otimes_R N \to 0,
\]
so if $\Tor^R_1(C, N) = 0$, then the module on the left
...
\[
  \Tor^R_1(A, N) \to \Tor^R_1(B, N) \to
  \Tor^R_1(C, N) \to A \otimes_R N \to B \otimes_R N \to C
  \otimes_R N \to 0.
\]
This is not the end of the story: the complex may be continued even
further by invoking new functors $\Tor^R_2(\mathord{-}, N)$,
$\Tor^R_3(\mathord{-}, N)$, etc. These are the derived
functors of tensor. To `compute' these functors, one may apply the
following procedure: given an $R$-module $M$, find a free resolution
...
take the homology of this complex (cf.\ \S{}\ref{subsec:rings:homology-and-the-snake-lemma}). Astoundingly,
...
$\Tor^R_0(M, N) \cong M \otimes_R N$ (\Cref{linrep:exer:2.14}), and
$\Tor^R_i(M, N) = 0$ for all $i > 0$ and all $M$ if $N$ is
...
$\Tor^R_i(M, N) = 0$ for all $i > 0$ for all modules $M$.
Indeed, $N$ is then flat.  At this point you may feel that something
is a little out of balance: why focus on the functor
$\mathord{-} \otimes_R N$, rather than $M \otimes_R \mathord{-}$?
Since $M \otimes_R N$ is canonically isomorphic to $N \otimes_R M$ (in
the commutative case; cf.\ \Cref{linrep:exam:2.2}), we could expect the same to
apply to every $\Tor^R_i$: $\Tor^R_i(M, N)$ ought to
be canonically isomorphic to $\Tor^R_i(N, M)$ for all
$i$. Equivalently, we should be able to compute
$\Tor^R_i(M, N)$ as the homology of $M \otimes_R N_\bullet$,
where $N_\bullet$ is a free resolution of $N$.  This is indeed the
case.

In due time (\S{}\ref{sec:homol:derived-functors} and 8) we will prove this and all the other
...
applications.  In fact, we know enough about finitely generated
modules over PIDs to get a preliminary sense of what is involved in
proving such general facts. Recall that we have been able to establish
that every finitely generated module $M$ over a PID $R$ has a free
resolution of length 1:
\[
  0 \to R^{\oplus m_1} \to R^{\oplus m_0} \to M \to 0.
\]
This property characterizes PIDs (\Cref{linalg:5.4}). If
...
\[
  ...
  \end{array}
\]
(This will be proven in gory detail in \S{}\ref{sec:homol:derived-functors}.) Tensor the two
`free' rows by $N$; they remain exact (tensoring commutes with direct
sums):
\[
  \begin{array}{ccccccc}
...
Applying the snake lemma (\Cref{lem:rings:snake-lemma}; cf.\ \Cref{rem:rings:snake-lemma-homology-restatement}) gives
the exact sequence
\[
...
\[
  \Tor^R_1(A, N) \to \Tor^R_1(B, N) \to
  \Tor^R_1(C, N) \xrightarrow{\delta} A \otimes_R N \to B
  \otimes_R N \to C \otimes_R N \to 0,
\]
with a $0$ on the left for good measure (due to the fact that
$\Tor^R_2$ vanishes if $R$ is a PID; cf.\ \Cref{linrep:exer:2.17}).

Note that $\Tor^k_i$ vanishes for $i > 0$ if $k$ is a field,
as vector spaces are flat, and $\Tor^R_i$ vanishes for $i > 1$
if $R$ is a PID (\Cref{linrep:exer:2.17}). These facts are not surprising, in
view of the procedure described above for computing Tor and of the
considerations at the end of \S{}\ref{subsec:linalg:pids-and-resolutions}: a bound on the length of free
...
`smooth' points in algebraic geometry) this bound agrees with the
Krull dimension; but precise results of this sort are beyond the scope
of this book.
...
\begin{exer}{$?$}{linrep:exer:2.1}
  ...
  tensor. [\S{}\ref{subsec:linrep:bilinear-maps-and-the-definition-of-tensor-product}]
\end{exer}
\begin{exer}{$?$}{linrep:exer:2.2}
  ...
  relatively prime integers. [\S{}\ref{subsec:linrep:adjunction-with-hom-and-explicit-computations}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:2.4}
  ...
  \S{}\ref{subsec:rings:products}. [\ref{linrep:exer:2.10}]
\end{exer}
\begin{exer}{$?$}{linrep:exer:2.5}
  ...
  [\S{}\ref{subsec:linrep:adjunction-with-hom-and-explicit-computations}, \ref{linrep:exer:2.8}, \ref{linrep:exer:2.12}, \ref{linrep:exer:3.4}]
\end{exer}
\begin{exer}{$\lnot$}{linrep:exer:2.6}
  ...
  \Cref{linrep:2.3}. Both isomorphisms will be easy consequences of the
  associativity of tensor products; cf.\ \Cref{linrep:exer:3.3}.) [\ref{linrep:exer:2.21}, \ref{linrep:exer:3.4}]
\end{exer}
...
\begin{exer}{}{linrep:exer:2.8}
  ...
  $V := M \otimes_R K$ is a $K$-vector space (\Cref{linrep:exer:2.5}). Prove
  that $\dim_K V$ equals the rank of $M$ as an $R$-module, in the
  sense of \Cref{linalg:5.5}.
\end{exer}
...
\begin{exer}{}{linrep:exer:2.10}
  ...
  \end{itemize}
  (Use \Cref{linrep:2.8} to `compute' the tensor. The CRT from \S{}\ref{subsec:irred:chinese-remainder-theorem}
  will likely be helpful.)
\end{exer}
\begin{exer}{$?$}{linrep:exer:2.11}
  Complete the proof of \Cref{linrep:2.4}. [\S{}\ref{subsec:linrep:adjunction-with-hom-and-explicit-computations}]
\end{exer}
\begin{exer}{}{linrep:exer:2.12}
  Let $S$ be a multiplicative subset of $R$ (cf.\ \Cref{exer:irred:localization-construction}).
...
\begin{exer}{$?$}{linrep:exer:2.14}
  Prove that, according to the definition given in \S{}\ref{subsec:linrep:the-tor-functors},
  $\Tor^R_0(M, N)$ is isomorphic to $M \otimes_R N$.
  [\S{}\ref{subsec:linrep:the-tor-functors}]
\end{exer}
\begin{exer}{$?$}{linrep:exer:2.15}
  Prove that for $r \in R$ a non-zero-divisor and $N$ an $R$-module,
  the module $\Tor^R_1(R/(r), N)$ is isomorphic to the
  $r$-torsion of $N$, that is, the submodule of elements $n \in N$
  such that $rn = 0$ (cf.\ \S{}\ref{subsec:linalg:torsion}). (This is the reason why Tor is
  called Tor.) [\S{}\ref{subsec:linrep:the-tor-functors}, \ref{linrep:exer:6.21}]
\end{exer}
\begin{exer}{}{linrep:exer:2.16}
  Let $I$, $J$ be ideals of $R$. Prove that
  $\Tor^R_1(R/I, R/J) \cong (I \cap J)/IJ$. (For example, this
  $\Tor^R_1$ vanishes if $I + J = R$, by \Cref{lem:irred:6.3}.)  Prove
  that $\Tor^R_i(R/I, R/J)$ is isomorphic to
  $\Tor^R_{i-1}(I, R/J)$ for $i > 1$.
\end{exer}
\begin{exer}{$?$}{linrep:exer:2.17}
  Let $M$, $N$ be modules over a PID $R$. Prove that
  $\Tor^R_i(M, N) = 0$ for $i \geq 2$. (Assume $M$, $N$ are
  finitely generated, for simplicity.) [\S{}\ref{subsec:linrep:the-tor-functors}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:2.19}
  ...
  $M$ is flat if and only if $\Tor^R_1(R/I, M) = 0$ for all
  ideals $I$ of $R$. [\ref{linrep:exer:2.20}]
\end{exer}
\begin{exer}{}{linrep:exer:2.20}
  ...
  classification theorem of \S{}\ref{subsec:linalg:the-classification-theorem} makes this particularly easy.
  Otherwise, use \Cref{linrep:exer:2.19}.) Geometrically, this says roughly that
  ...
  phenomenon is displayed in the picture in \Cref{linrep:exam:2.14}.
\end{exer}
\begin{exer}{$\lnot$}{linrep:exer:2.21}
  (Cf.\ \Cref{exer:irred:localization-at-prime-is-local}.) Prove that flatness is a local property: an
  $R$-module $M$ is flat if and only if $M_{\mathfrak{p}}$ is a flat
  $R_{\mathfrak{p}}$-module for all prime ideals $\mathfrak{p}$, if
  and only if $M_{\mathfrak{m}}$ is a flat $R_{\mathfrak{m}}$-module
  for all maximal ideals $\mathfrak{m}$. (Hint: Use Exercises~1.25 and
  2.6.  The $\Rightarrow$ direction will be straightforward. For the
  converse, let $A \subseteq B$ be $R$-modules, and let $K$ be the
  kernel of the induced homomorphism
  $A \otimes_R M \to B \otimes_R M$. Prove that the kernel of the
  ...
  isomorphic to $K_{\mathfrak{m}}$, and use \Cref{exer:irred:module-zero-iff-zero-locally}.) [\ref{linrep:exer:2.22}]
\end{exer}
\begin{exer}{$\lnot$}{linrep:exer:2.22}
  Let $M$, $N$ be $R$-modules, and let $S$ be a multiplicative subset
  of $R$. Use the definition of Tor given in \S{}\ref{subsec:linrep:the-tor-functors} to show
  $S^{-1}\Tor^R_i(M, N) \cong
  \Tor^{S^{-1}R}_i(S^{-1}M, S^{-1}N)$. (Use \Cref{linrep:exer:1.25}.)
  Use this fact to give a leaner proof that flatness is a local
  property (\Cref{linrep:exer:2.21}). [\ref{linrep:exer:2.25}]
\end{exer}
\begin{exer}{$?$}{linrep:exer:2.23}
  ...
  \item Prove that for all $R$-modules $Q$, the induced sequence
    ...
    exact.
  \end{itemize} [\ref{linrep:exer:2.24}, \S{}\ref{subsec:linrep:duality-and-exactness}]
\end{exer}
\begin{exer}{$\lnot$}{linrep:exer:2.24}
  ...
  \item Prove that this sequence induces an exact sequence
    ...
    M/\mathfrak{m}M \to 0$. (Use \Cref{linrep:exer:2.23}.)
  ...
  flat if and only if it is free. Compare with \Cref{exer:linalg:5.5}.
  [\ref{linrep:exer:2.25}, \ref{linrep:exer:6.8}, \ref{linrep:exer:6.12}]
\end{exer}
...
\begin{exer}{}{linrep:exer:2.25}
  ...
  $\mathfrak{m}$ of $R$. (Use \Cref{linrep:exer:2.21}, and refine the argument
  you used in \Cref{linrep:exer:2.24}; remember that Tor localizes, by
  \Cref{linrep:exer:2.22}. The Noetherian hypothesis is actually unnecessary,
  but the proofs are harder without it.)
\end{exer}
...
in \S{}\ref{subsec:linrep:bilinear-maps-and-the-definition-of-tensor-product}. Let $M$, $N$ be modules over a commutative ring $R$, as in
\S{}\ref{subsec:linrep:bilinear-maps-and-the-definition-of-tensor-product}, and let $G$ be an abelian group, i.e., a $\Z$-module.
...
$R^{\oplus(M \times N)}$. The reader will verify (\Cref{linrep:exer:2.25}) that
...
The universal property explored in \S{}\ref{subsec:linrep:bilinear-maps-and-the-definition-of-tensor-product} is recovered as the
statement that if $G$ is an $R$-module and $\phi$ is $R$-bilinear,
then the induced group homomorphism $M \otimes_R N \to G$ is in fact
an $R$-linear map.
...
upgrade the adjunction formula given in \Cref{linrep:2.4}. This requires the
...
\begin{proof}
  ...
  $\Hom_S(N, P)$. Thus $\phi$ is $R$-balanced. By \Cref{linrep:3.2}, such a
  map determines (and is determined by) a homomorphism of abelian
  groups $\tilde\phi : M \otimes_R N \to P$, with
...
If $R = S$, we recover the adjunction formula of \Cref{linrep:2.4}; note that
...
homomorphism, and let $N$ be an $S$-module. Recall (\S{}\ref{subsec:rings:definition-of-left-r-module}) that
this means that we have chosen an action of the ring $S$ on the
abelian group $N$, that is, a ring homomorphism
...
\Cref{linrep:exam:1.18}, this hints that $f_*$ may have both a left-adjoint and a
right-adjoint functor, and this will be precisely the case
(\Cref{linrep:3.6}).
...
\S{}\ref{subsec:linrep:bimodules-adjunction-again}) carries naturally an $S$-module structure. This association
is evidently covariantly functorial. If
$R^{\oplus B} \to R^{\oplus A} \to M \to 0$ is a presentation of $M$
(cf.\ \S{}\ref{subsec:linalg:finitely-presented-modules-and-free-resolutions}), tensoring by $S$ gives (by the right-exactness of
...
is an $R$-module, I have pointed out in \S{}\ref{subsec:linrep:bimodules-adjunction-again} that
...
\begin{proof}
  ...
  \end{align*}
  where I have used \Cref{linrep:3.5}. These bijections are canonical, proving
  that $f^*$ is left-adjoint to $f_*$.

  Similarly, there is a canonical isomorphism $N \cong N \otimes_S S$
  (\Cref{linrep:exam:2.2}); thus $N \otimes_S S \cong f_*(N)$ as $R$-modules, and
  ...
  \begin{align*}
    ...
    &\cong \Hom_S(N, \Hom_R(S, M))
      = \Hom_S(N, f^!(M))
  \end{align*}
  again by \Cref{linrep:3.5}. This shows that $f^!$ is right-adjoint to $f_*$,
...
\begin{exer}{$?$}{linrep:exer:3.1}
  ...
  \begin{align*}
    ...
    &(rm, n) - (m, rn),
  \end{align*}
  with $m, m_1, m_2 \in M$, $n, n_1, n_2 \in N$, $r \in R$.  [\S{}\ref{subsec:linrep:balanced-maps}]
\end{exer}
...
\begin{exer}{$?$}{linrep:exer:3.3}
  ...
  \]
  In this sense, $\otimes$ is `associative'. [\ref{linrep:exer:3.4}, \S{}\ref{subsec:linrep:multilinear-symmetric-alternating-maps}]
\end{exer}
\begin{exer}{$?$}{linrep:exer:3.4}
  Use the associativity of the tensor product (\Cref{linrep:exer:3.2}) to prove
  again the formulas given in \Cref{linrep:exer:2.6}. (Use \Cref{linrep:exer:2.5}.) [\ref{linrep:exer:2.6}]
\end{exer}
...
\begin{exer}{}{linrep:exer:3.7}
  ...
  restriction of scalars; cf.\ \S{}\ref{subsec:linrep:restriction-and-extension-of-scalars}).
\end{exer}
...
\begin{exer}{}{linrep:exer:3.12}
  In `geometric' contexts (such as the one hinted at in \Cref{linrep:rem:3.7}),
  one would actually work with categories which are opposite to the
  category of commutative rings; cf.\ \Cref{linrep:exam:1.9}. A ring homomorphism
  $f : R \to S$ corresponds to a morphism
  $f^\circ : S^\circ \to R^\circ$ in the opposite category, and we can
  simply define $(f^\circ)_*$, etc., to be $f_*$, etc. For morphisms
...
\begin{exer}{}{linrep:exer:3.14}
  ...
  \item Prove that, for all $R$-modules $M$,
    ...
    $f^*(M) \cong M/IM$. (\Cref{exer:rings:7.7} may help.)
  \item Prove that, for all $S$-modules $N$, $f^! f_*(N) \cong N$ and
    $f^* f_*(N) \cong N$.
  \item Prove that $f_*$ is fully faithful (\Cref{linrep:1.6}).
...
\begin{exer}{}{linrep:exer:3.15}
  ...
  case worked out in this exercise. Recall (\Cref{linrep:exer:1.28}) that if $R$
  is any ring, then the center of $R\text{-}\categ{Mod}$ is isomorphic
  to the center of $R$. It is not difficult to deduce from this that
  if $R\text{-}\categ{Mod}$ and $S\text{-}\categ{Mod}$ are equivalent,
  then the centers of $R$ and $S$ are isomorphic. So if two
  commutative rings $R$ and $S$ are Morita equivalent, then they must
  be isomorphic.  The commutativity is crucial in this statement: for
  example, it can be shown that any ring $R$ is Morita equivalent to
  the ring of matrices\footnote{I was once told that
    $M_{n,n}(\C)$ is `not seriously noncommutative' since it
    is Morita equivalent to $\C$, which is commutative.}
  $M_{n,n}(R)$, for all $n > 0$.
\end{exer}
...
tensor product is associative\footnote{In fact, the tensor product is
  associative in a fancier sense; cf.\ \Cref{linrep:exer:3.2}.} in
...
m_\ell$.  The reader will verify (\Cref{linrep:exer:4.1}) that
...
\begin{proof}
  ...
  evidently alternating (\Cref{linrep:exer:4.5}). Thus, it factors uniquely
  through an $R$-linear map $\phi_I : \bigwedge^\ell_R(M) \to R$, with
  the property that
  \[
  ...
  \[
    ...
    \wedge \dots \wedge e_{j_\ell} \Bigr) = \phi_I(0) = 0.
  \]
...
(obtained in \S{}\ref{subsec:linalg:the-determinant}) prove that it is a multilinear, alternating
...
\begin{exam}{}{linrep:4.7}
  ...
  field $k$ (\Cref{linrep:exer:4.21}), which is the simplest interesting
  instance of this construction (well, the second simplest: the
  simplest example may be \Cref{exer:rings:rees-algebra}).
\end{exam}
...
using graded rings along the same lines we sketched in \S{}\ref{subsec:fields:a-little-affine-algebraic-geometry} for
the affine case. As I mentioned at the end of \S{}\ref{subsec:fields:a-little-affine-algebraic-geometry}, this
...
\begin{proof}
  ...
  (ii)$\iff$(iii) is left to the reader (\Cref{linrep:exer:4.10}).
...
\begin{exam}{}{linrep:4.10}
  ...
  Lemma~\ref{linrep:4.9}. The ideal $(y, y - x^2)$ is homogeneous,
...
algebraic geometry reviewed in \S{}\ref{subsec:fields:a-little-affine-algebraic-geometry}, but using only homogeneous
...
\begin{rem}{}{linrep:4.11}
  ...
  \]
  (\Cref{linrep:exer:4.16}), where I use $\wedge$ to denote the operation in
  $\bigwedge^*_R(M)$. The reader should recognize this formula from
  the theory of differential forms; differential forms provide perhaps
  the most important example of an exterior algebra.
\end{rem}
...
\end{lem}
\begin{proof}
  We have observed in \S{}\ref{subsec:linrep:very-small-detour-graded-algebra} that the kernel of a graded homomorphism
  is the direct sum of the kernels of the induced homomorphisms in
  each degree; the statement of the lemma then follows easily from the
  explicit descriptions of the kernels of the canonical projections
  from the tensor powers to the symmetric and exterior powers, given
  in \S{}\ref{subsec:linrep:symmetric-and-exterior-powers}.
\end{proof}
...
\begin{exam}{}{linrep:4.17}
  ...
  respect to mapping to commutative rings (cf.\ \S{}\ref{subsec:rings:universal-property-of-polynomial-rings} and
  \S{}\ref{subsec:rings:submodule-generated-by-a-subset-noetherian-modules}). Likewise, $\mathrm{T}^*_R(R^{\oplus r})$ should be
  thought of as a `noncommutative' polynomial ring, in which the $r$
  indeterminates do not commute with each other (cf.\ \S{}\ref{subsec:rings:free-modules-and-free-algebras}).
\end{exam}
...
\[
  ...
  \mathrm{S}^*_R(N) \to 0
\]
is exact. This is often useful in computations (cf.\ \Cref{linrep:exer:4.21}). I
will leave any further such explorations to the more motivated
readers.
...
\begin{exer}{$*$}{linrep:exer:4.1}
  Verify that the module $\bigwedge^\ell_R(M)$ constructed in \S{}\ref{subsec:linrep:symmetric-and-exterior-powers}
  does satisfy the universal property for alternating multilinear
  maps.  Construct a module $\mathrm{S}^\ell_R(M)$ satisfying the
  universal property for symmetric multilinear maps. [\S{}\ref{subsec:linrep:symmetric-and-exterior-powers}]
\end{exer}
...
\begin{exer}{}{linrep:exer:4.3}
  ...
  its wedge with itself would have to vanish.
\end{exer}
...
\begin{exer}{$*$}{linrep:exer:4.5}
  ...
  Lemma~\ref{linrep:4.3} is alternating. [\S{}\ref{subsec:linrep:symmetric-and-exterior-powers}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:4.7}
  ...
  $\ell$-di\-men\-sional subspaces of $V$ (\Cref{exer:linalg:2.13}) to the
  ...
  \emph{Pl\"ucker embedding} of the Grassmannian. [\ref{linrep:exer:4.8}]
\end{exer}
...
\begin{exer}{}{linrep:exer:4.8}
  ...
  corresponding minors, as in Example~\ref{linrep:4.5}. Prove that
  $\mathrm{Gr}_k(2,4)$ is the locus of zeros
  \[
    \mathbf{V}(x_{12}x_{34} - x_{13}x_{24} + x_{14}x_{23})
  \]
  (notation as in \Cref{exer:fields:homog-vanishing-well-defined-covering}). (Remember that you know how to
  ...
  Exercise~\ref{linrep:exer:4.11}) via the corresponding Pl\"ucker
  embeddings.

  Prove that $\mathrm{Gr}_k(2,4)$ may be covered with six copies of
  $\mathbf{A}^4_k$. (Recall from \Cref{exer:fields:affine-to-projective-space-covering} that
...
\begin{exer}{$*$}{linrep:exer:4.10}
  ...
  Lemma~\ref{linrep:4.9}. [\S{}\ref{subsec:linrep:very-small-detour-graded-algebra}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:4.11}
  ...
  for the point $(c_0 : \cdots : c_n)$. (Cf.\ \Cref{exer:fields:homog-vanishing-well-defined-covering}, but
  ...
  irreverently called the \emph{irrelevant ideal}. [\ref{linrep:exer:4.8}, \ref{linrep:exer:4.12}]
\end{exer}
...
\begin{exer}{}{linrep:exer:4.13}
  ...
  taking a formal derivative (as in \S{}\ref{subsec:fields:separable-polynomials}). With the evident
...
\begin{exer}{$*$}{linrep:exer:4.16}
  ...
  Remark~\ref{linrep:4.11}. [\S{}\ref{subsec:linrep:tensor-algebras}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:4.18}
  ...
  Noetherian if $R$ is Noetherian. [\ref{linrep:exer:4.19}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:4.19}
  ...
  \item Prove that $J_{s+\ell} = I^\ell J_s$ for all $\ell \geq 0$.
    This is a particular case of the `Artin-Rees' lemma. [\ref{linrep:exer:4.20}]
  \end{itemize}
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:4.20}
  ...
  (Hint: Exercise~\ref{linrep:exer:4.19}.) [\ref{linrep:exer:1.18}]
\end{exer}
...
\begin{exer}{}{linrep:exer:4.21}
  ...
  use the exact sequence mentioned at the end of \S{}\ref{subsec:linrep:tensor-algebras}. It will be
  helpful to have an explicit presentation of $I$ as a
  $k[x,y]$-module; for this, looking back at \Cref{exer:linalg:4.15} may
  help.) [\S{}\ref{subsec:linrep:very-small-detour-graded-algebra}, \S{}\ref{subsec:linrep:tensor-algebras}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:4.22}
  ...
  where $I = (a_1, \dots, a_n)$. This is called the \emph{Koszul
    complex} of $\mathbf{a}$.
  \begin{itemize}
  \item Check that the complexes constructed in Exercises VI.4.13 and
    VI.4.14 are Koszul complexes.
  \end{itemize}
  ...
  prove this in general. [\ref{exer:linalg:4.14}]
\end{exer}
...
functor.  This fact is spelled out in Lemma~\ref{linrep:2.4}, whose
...
discussed in general terms in \S{}\ref{subsec:linrep:examples-of-functors})?
...
contravariant functor. The statement of Proposition~\ref{linrep:5.2}
...
precise statement of Proposition~\ref{linrep:5.2} is that $h_N$ is
right-adjoint to $h_N^{\mathrm{op}}$.

\begin{proof}
  Let $L$, $M$, $N$ denote $R$-modules. Recall (cf.\ the
  considerations preceding Lemma~\ref{linrep:2.4}) that $R$-bilinear
...
As both $\Hom_R(N, -)$ (by Corollary~\ref{linrep:5.1}) and
$\Hom_R(-, N)$ (by Proposition~\ref{linrep:5.2}) are right-adjoint
functors, Lemma~\ref{linrep:1.17} tells us they commute with
...
Also, as pointed out in Claim~\ref{linrep:1.19}, the fact that both
...
in the distant past (\Cref{exer:rings:7.7}), without appealing to
...
in \S{}\ref{sec:linrep:projective-and-injective-modules-and-the-ext-functors}. Before doing that, however, I want to examine one important
...
this gives a more precise version of Proposition~\ref{linrep:5.5}.
...
Corollary~\ref{linrep:5.7}: direct sums become direct products through
the contravariant Hom (Corollary~\ref{linrep:5.3}).  However, a direct
product of finitely many modules happens to be isomorphic to their
direct sum (as we have known for a long time: \Cref{prop:rings:direct-sum-product-coproduct}),
...
\begin{rem}{}{linrep:5.9}
  ...
  a free module $F = R^{\oplus S}$. We proved in \Cref{linalg:1.11} (if
  ...
  subset of $F$, is not. In fact, recall (\S{}\ref{subsec:linalg:linear-independence-and-bases}) that the choice
  of a specific isomorphism $F \cong R^{\oplus S}$ is equivalent to
  the choice of a basis of $F$. Now, the isomorphism appearing in
  Corollary~\ref{linrep:5.7} requires knowledge of $S$; indeed, we
  will verify in a moment (Example~\ref{linrep:5.11}) that this
...
The isomorphism $(R^n)^\vee \cong R^n$ of Corollary~\ref{linrep:5.7}
is obtained precisely by matching $e_i$ and $\check{e}_i$: this will
be crystal clear to the reader who works out \Cref{linrep:exer:5.7}. In
...
unpleasant, but something magical will happen soon (\S{}\ref{subsec:linrep:duals-and-matrices-biduality}): we will
...
\end{equation}
since the dual of $\Z/2\Z$ is zero (\Cref{linrep:exer:5.6}). The map $\gamma$ in
this sequence is the dual of the multiplication by $2$.  If
$f : \Z \to \Z$ is an element of $\Z^\vee$, then $\gamma(f)$ is
...
this better after digesting \S{}\ref{sec:linrep:projective-and-injective-modules-and-the-ext-functors}, but the following observation will
suffice for now.
...
case of \Cref{exer:rings:7.8} and, further, it is a consequence of Exercise
2.23. But here is a direct argument:

\begin{proof}
  Lemma~\ref{linrep:5.12} takes care of all but the surjectivity of
  ...
  \[
    ...
    \end{tikzcd}
  \]
  By construction, $\nu \circ \rho = \id_P$. Now let
  $n \in N$. Then $\nu(\rho(\nu(n))) = \nu(n)$, giving
  $\nu(n - \rho(\nu(n))) = 0$. By the exactness of the given sequence,
  $\exists m \in M$ such that $n - \rho(\nu(n)) = \mu(m)$. The element
  $m$ is unique, since $\mu$ is injective. Setting $g(n) := f(m)$
  gives the necessary extension. Indeed, $g$ is immediately checked to
  be $R$-linear, and $g(\mu(m)) = f(m)$ by definition.
\end{proof}
...
and Corollary~\ref{linrep:5.7} reduce (in principle) the computation
...
The diligent reader has proved this already in \Cref{exer:linalg:4.2} and
...
this by working out \Cref{linrep:exer:5.11}.  Even if not every module is the
dual of a module, we may wonder whether an arbitrarily given module
$M$ has some relation with a suitably chosen dual, and this is indeed
the case: there is a canonical map to the \emph{double-dual},
\[
  \omega : M \to M^{\vee\vee}.
\]
...
sending $(f, m)$ to $f(m)$. For every $m \in M$, we get\footnote{The
  ...
  formally by working out \Cref{linrep:exer:5.12}.} an $R$-linear map
$M^\vee \to R$, $f \mapsto f(m)$, in other words, an element
$\omega(m)$ of $M^{\vee\vee}$. It is immediately checked that $\omega$
is $R$-linear.

By Proposition~\ref{linrep:5.16}, $M^{\vee\vee}$ is torsion-free. In
...
Corollary~\ref{linrep:5.7}, if $F$ is a finite-rank free $R$-module,
...
$\omega$ (\Cref{linrep:exer:5.13}) and $\omega$ knows nothing about choices of
...
double-duals.  In terms of matrices (and keeping in mind that duality
is exact on free modules) this is perhaps not too surprising. After
all, taking the transpose of a matrix twice simply gives the matrix
back; thus, even if making sense of the homomorphism corresponding to
the transpose of a matrix may depend on the choice of a basis, surely
such a choice is inessential if the transpose is taken twice.
...
foregoing observations (including \Cref{linrep:exer:5.1}) for this particular
...
\begin{exer}{$*$}{linrep:exer:5.1}
  ...
  $R$-module, then $\Hom_R(F, N) \cong F^\vee \otimes_R N$.  [\S{}\ref{subsec:linrep:duality-on-vector-spaces}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:5.2}
  ...
  $\alpha^*$ is surjective for all $N$. [\ref{linrep:exer:5.3}, \ref{linrep:exer:6.3}]
\end{exer}
...
\begin{exer}{}{linrep:exer:5.3}
  ...
  already, in \Cref{linrep:exer:5.2}. To show
  $\ker\beta \subseteq \image\alpha$, choose
  $N = B/\image(\alpha)$.) Remember that the converse does
  not hold, since in general $\Hom_R(-,N)$ is not exact.  What extra
  hypothesis on $\alpha$ would guarantee the exactness of $(*)$ for
  all $N$?
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:5.4}
  ...
  bundle' in good situations.)  [\ref{linrep:exer:6.20}]
\end{exer}
...
\begin{exer}{$*$}{linrep:exer:5.5}
  ...
  alternative proof of Proposition~\ref{linrep:5.5}. [\S{}\ref{subsec:linrep:dual-modules}]
\end{exer}

\begin{exer}{$*$}{linrep:exer:5.6}
  Show that $(\Z/2\Z)^\vee = \Hom_{\Z}(\Z/2\Z, \Z) = 0$.  [\S{}\ref{subsec:linrep:duality-and-exactness}]
\end{exer}
...
\begin{exer}{$*$}{linrep:exer:5.7}
  ...
  and what is the inverse of this correspondence? [\S{}\ref{subsec:linrep:duals-of-free-modules}]
\end{exer}
...
\begin{exer}{}{linrep:exer:5.9}
  ...
  \Cref{exer:linalg:6.18}) is nondegenerate. If $M$ is free of rank $n$, let
  $(e_1, \dots, e_n)$ be a basis of $M$, and let $A = (a_{ij})$ be the
  matrix with entries $a_{ij} = \phi(e_i, e_j)$. Prove that $\phi$ is
  nondegenerate if and only if $\det(A)$ is nonzero, and it is
  nonsingular if and only if $\det(A)$ is a unit. (Cf.\ Proposition
  VI.6.5.)
\end{exer}
...
\begin{exer}{$*$}{linrep:exer:5.11}
  ...
  `Compute' the dual $M^\vee$. [\S{}\ref{subsec:linrep:duals-and-matrices-biduality}]
\end{exer}
...
\begin{exer}{$*$}{linrep:exer:5.12}
  ...
  defined in \S{}\ref{subsec:linrep:duals-and-matrices-biduality}. [\S{}\ref{subsec:linrep:duals-and-matrices-biduality}]
\end{exer}
...
\begin{exer}{$*$}{linrep:exer:5.13}
  ...
  $F \cong F^\vee \cong F^{\vee\vee}$ from Corollary~\ref{linrep:5.7}
  is the (canonical) isomorphism $\omega$ defined in
  \S{}\ref{subsec:linrep:duals-and-matrices-biduality}. [\S{}\ref{subsec:linrep:duals-and-matrices-biduality}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:5.15}
  ...
  The \emph{annihilator} of $W$ is\footnote{The notation is inspired
    by \Cref{exer:linalg:6.17}, which is a particular case of this
  ...
  $W$. [\ref{linrep:exer:5.16}, \ref{linrep:exer:5.17}]
\end{exer}
...
\begin{exer}{}{linrep:exer:5.16}
  ...
  the notation introduced for vector spaces in \Cref{linrep:exer:5.15}, prove
  that $F_1^\perp \cong F_3^\vee$.
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:5.17}
  ...
  $r$-dimensional subspaces of $V$ (cf.\ \Cref{exer:linalg:2.13}) and the
  Grassmannian $\mathrm{Gr}(n-r, V^\vee)$ of $(n-r)$-dimensional
  subspaces of the dual of $V$. (Use \Cref{linrep:exer:5.15}.) In particular,
...
\begin{exer}{}{linrep:exer:5.19}
  ...
  to the opposite ring $R^\circ$. (Cf.\ \Cref{exer:rings:opposite-ring}.)
\end{exer}
...
We have seen in \S{}\ref{subsec:linrep:adjunction-again} that the Hom functors are left-exact, and not
...
about Tor that were left without proof in \S{}\ref{subsec:linrep:the-tor-functors}) could be proven in
...
us throughout \Cref{chap:homol} will see complete proofs of all the facts
...
as $\Ext^i_R(M, N)$ for two $R$-modules $M$ and $N$) are
...
\begin{exam}{}{linrep:6.3}
  ...
  $\Hom_R(-, R)$ is not exact in general; cf.\ \S{}\ref{subsec:linrep:adjunction-again}).
\end{exam}
...
(cf.\ the end of \S{}\ref{subsec:rings:complexes-and-exact-sequences}). For example, assume that $P$ is
...
\S{}\ref{subsec:rings:complexes-and-exact-sequences}, a module $P$ is projective if and only if every
...
the enterprising reader (\Cref{linrep:exer:6.1}).
...
Proposition~\ref{linrep:6.4} streamlines the verification of simple
...
\begin{proof}
  These statements follow easily from Proposition~\ref{linrep:6.4},
...
that projective modules are flat is straightforward (\Cref{linrep:exer:6.7}),
using the (nontrivial) fact that $R\text{-Mod}$ has enough injectives,
...
through some homological algebra machinery, in \Cref{chap:homol}, especially
...
injective modules in the style of Proposition~\ref{linrep:6.4}
(however, cf.\ Remark~\ref{linrep:6.13}), and while it is trivially
...
Theorem~\ref{linrep:6.6} completely clarifies the notion of injective
...
defined in \S{}\ref{subsec:linrep:restriction-and-extension-of-scalars} a functor\footnote{Watch out --- in \S{}\ref{subsec:linrep:restriction-and-extension-of-scalars} the
...
\begin{rem}{}{linrep:6.10}
  Similarly, $f^*$ preserves projectives (\Cref{linrep:exer:6.6}).
\end{rem}
...
\begin{proof}
  ...
  (Proposition~\ref{linrep:3.6}) and $\Hom_S(-, Q)$ is exact by
...
\begin{proof}
  ...
  $\Q^{\oplus A}/K$ is divisible (Example~\ref{linrep:6.8}).
\end{proof}
...
applying Corollary~\ref{linrep:6.12} to $M$ to construct $Q^0$ and
...
\begin{rem}{}{linrep:6.13}
  ...
  Corollary~\ref{linrep:6.11} and \Cref{linrep:exer:6.15}).  That is, products
  of $\iota^!(\Q/\Z) = \Hom_{\Z}(R, \Q/\Z)$ play for injectives the
  role played by free modules (= coproducts of $R = \iota^*(\Z)$) for
  projectives. (Not surprisingly, they are said to be \emph{cofree}.)
  It follows that every injective $R$-module is a direct summand of
...
\begin{itemize}
\item For all $i \geq 0$, $\Ext^i_R(M, -)$ and
  $\Ext^i_R(-, N)$ are covariant, resp., contravariant,
...
\end{itemize}
This will be proven in \Cref{chap:homol}. In any case, just knowing that such
...
more. Recall the procedure that defines $\Tor^R_i(-, N)$ from
the functor $- \otimes_R N$: to compute the value of these functors at
a module $M$,
\begin{itemize}
...
free resolutions.\footnote{In fact, resolutions by flat modules
  suffice; this will be proven in \Cref{chap:homol}.} This strategy is an
...
$*$ Welcome back. In the case of $\Ext^i_R(M,N)$, there would
seem to be two natural and possibly different ways to go:
\begin{itemize}
\item One could start from a projective resolution of $M$:
  ...
  take cohomology,\footnote{Recall from \S{}\ref{subsec:rings:complexes-and-exact-sequences} that the homology
    of complexes with increasing (rather than decreasing) indices is
...
other! Again, the reader will have to wait for \Cref{chap:homol} to see a
...
\begin{prop}{}{linrep:6.14}
  ...
  $\Ext^1_R(P, -) = 0$, if and only if
  $\Ext^i_R(P, -) = 0$ for all $i > 0$. An $R$-module $Q$ is
  injective if and only if $\Ext^1_R(-, Q) = 0$, if and only
  if $\Ext^i_R(-, Q) = 0$ for all $i > 0$.
\end{prop}

\begin{proof}
  The second assertion: we have seen that $Q$ is injective if
  $\Ext^1_R(-, Q) = 0$, and this is trivially the case if
  $\Ext^i_R(-, Q) = 0$ for all $i > 0$. So we just have to
  prove that $\Ext^i_R(-, Q) = 0$ for all $i > 0$ if $Q$ is
  ...
  $\Ext^0_R(M,Q) = \Hom_R(M,Q)$ and
  $\Ext^i_R(M,Q) = 0$ for $i > 0$ and all $M$. The first
...
$\Ext^*_{\Z}(G, \Z)$ for an arbitrary finitely generated
abelian group $G$.
\begin{itemize}
...
\item The second module: use the projective resolution
  ...
  confirming that
  $\Ext^0_{\Z}(\Z/m\Z, \Z) = \Hom_{\Z}(\Z/m\Z, \Z) = 0$ ($\Z$
  has no torsion) and computing
  $\Ext^1_{\Z}(\Z/m\Z, \Z) \cong \Z/m\Z$, with vanishing
  $\Ext^2$ and higher.
\item The conclusion is that $\Ext^0_{\Z}(G, \Z)$ picks up the
  free part of $G$, while $\Ext^1_{\Z}(G, \Z)$ is isomorphic
  to the torsion part, and $\Ext^i_{\Z}(G, \Z) = 0$ for
  $i \geq 2$ (as should have been expected; cf.\ \Cref{linrep:exer:6.18}).
\end{itemize}
In particular, if $G$ is a finite abelian group, then
$G \cong \Ext^1_{\Z}(G, \Z)$; as it happens, this isomorphism
is not canonical.

Another Ext computation gives a different interpretation of this
group. The exact sequence $0 \to \Z \to \Q \to \Q/\Z \to 0$ is an
injective resolution of $\Z$; thus, $\Ext^*_{\Z}(G, \Z)$ is
the cohomology of
\[
  0 \to \Hom_{\Z}(G, \Q) \to \Hom_{\Z}(G, \Q/\Z) \to 0 \to \dots.
\]
If $G$ is torsion (for example, if $G$ is finite), it follows that
$\Ext^1_{\Z}(G, \Z)$ is isomorphic to $\Hom_{\Z}(G,
\Q/\Z)$. This group is the \emph{Pontryagin dual} of $G$, denoted
$\hat{G}$. Thus, we have verified that if $G$ is a finite abelian
group, then $G$ is isomorphic (not canonically) to its Pontryagin dual
...
construct a bijection between the group $\Ext^1_R(M, N)$ and
the set of equivalence classes of \emph{extensions} of $M$ by $N$,
that is, of exact sequences $0 \to N \to E \to M \to 0$, modulo a
suitable isomorphism relation (cf.\ \S{}\ref{subsec:groups2:exact-sequences-of-groups-extension-problem}). For example, the
direct sum $E = M \oplus N$ corresponds to the $0$ element of this Ext
group. The diligent reader will construct this bijection in the
exercises.
...
\begin{exer}{$*$}{linrep:exer:6.1}
  ...
  every monomorphism $Q \to M$ splits. [\S{}\ref{subsec:linrep:projectives-and-injectives}]
\end{exer}

\begin{exer}{}{linrep:exer:6.2}
  Prove that the result of Proposition~\ref{linrep:5.13} holds more
...
\begin{exer}{$\lnot$}{linrep:exer:6.3}
  ...
  injectives.) This strengthens the result of \Cref{linrep:exer:5.2}.  [\ref{linrep:exer:6.7}]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:6.6}
  ...
  $S$-module, then $f^*(P)$ is a projective $R$-module. [\S{}\ref{subsec:linrep:injective-modules}]
\end{exer}
...
\begin{exer}{$*$}{linrep:exer:6.7}
  ...
  \Cref{linrep:exer:6.3}.) [\S{}\ref{subsec:linrep:projective-modules}, \ref{linrep:exer:6.8}]
\end{exer}

\begin{exer}{}{linrep:exer:6.8}
  Prove that Exercises~2.24 and~6.7 together imply the result of
  \Cref{exer:linalg:5.5}.
\end{exer}
...
\begin{exer}{}{linrep:exer:6.10}
  ...
  hand', without invoking \Cref{linrep:exer:6.12}.)
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:6.11}
  ...
  in \Cref{exer:linalg:5.5}.) Besides PIDs and local rings, there are other
...
\begin{exer}{$\lnot$}{linrep:exer:6.12}
  ...
  flat. (Use the results of Exercises~2.24 and~6.11.) The hypothesis
  of finite generation is necessary (cf.\ \Cref{linrep:exer:6.13}).  [\ref{linrep:exer:6.10}]
\end{exer}

\begin{exer}{$\lnot$}{linrep:exer:6.13}
  Prove that $\Q$ is flat, but not projective, as a
  $\Z$-module. [\ref{linrep:exer:6.12}]
\end{exer}

\begin{exer}{$*$}{linrep:exer:6.14}
  Prove that a module over a PID is injective if and only if it is
  divisible. (Use Baer's criterion.) [\S{}\ref{subsec:linrep:injective-modules}]
\end{exer}
...
\begin{exer}{$*$}{linrep:exer:6.15}
  ...
  is injective if and only if each $Q_i$ is injective.  [\S{}\ref{subsec:linrep:injective-modules},
  \ref{exer:homol:5.4}]
\end{exer}
...
\begin{exer}{}{linrep:exer:6.17}
  Prove that $\Hom_R(M, N) \cong \Ext^0_R(M, N)$, using any of
  the definitions provided for $\Ext^i_R(M, N)$.
\end{exer}

\begin{exer}{$*$}{linrep:exer:6.18}
  Prove that if $R$ is a PID, then $\Ext^i_R(M, N) = 0$ for
  all $i \geq 2$ and all $R$-modules $M$, $N$. [\S{}\ref{subsec:linrep:ext-Z-G-Z}]
\end{exer}
...
\begin{exer}{}{linrep:exer:6.20}
  ...
  \Cref{linrep:exer:5.4}. This says that this Ext module essentially computes
  the normal bundle of an embedding.)
\end{exer}

\begin{exer}{$\lnot$}{linrep:exer:6.21}
  In \Cref{linrep:exer:2.15} we have seen why Tor is called Tor. Why is Ext
  called Ext? Let $M$, $N$, $E$ be $R$-modules. An \emph{extension} of
  $M$ by $N$ is an exact sequence
  \[
  ...
  then be an isomorphism: cf.\ \Cref{exer:rings:7.10}.) Extensions that are
  isomorphic to the standard sequence
  ...
  \[
    \dots \to \Hom_R(E, N) \to \Hom_R(N, N) \to \Ext^1_R(M, N)
    \to \dots,
  \]
  ...
  $\xi(E) = 0$. [\ref{linrep:exer:6.22}]
\end{exer}

\begin{exer}{}{linrep:exer:6.22}
  \Cref{linrep:exer:6.21} teaches us that extensions determine elements of
  $\Ext^1_R$. We get an even sharper statement by constructing
  an inverse to the map $\xi$: for every element
  $\kappa \in \Ext^1_R(M, N)$, we will construct an extension
  ...
  \item Let $F$ be any free module surjecting onto $M$, and let
    ...
    \[
      \dots \to \Hom_R(K, N) \to \Ext^1_R(M, N) \to
      \Ext^1_R(F, N) = 0 \to \dots,
    \]
  ...
  $\Ext^1_R(M, N)$ and the set of isomorphism classes of
  extensions of $M$ by $N$. Hence the name. `Higher'
  $\Ext^i_R$ ($i > 1$) may also be treated similarly: for
  example, $\Ext^2_R(M, N)$ can be analyzed in terms of
