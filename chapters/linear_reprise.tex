...
more sophisticated constructions than those reviewed in Chapter~VI. My
...
\begin{exam}{}{linrep:exam:1.4}
  ...
  topology to $\mathrm{Spec}(R)$ (Exercise~1.7), making this example
...
are only defined up to isomorphism (Proposition~I.5.4). Requiring
...
in Definition~1.7. It is not at all immediate that this is indeed an
...
\begin{exam}{}{linrep:exam:1.9}
  ...
  defining\footnote{The switch in the order of $S$ and $T$ is
    reasonable: as the reader has verified in Exercise~VII.2.12,
...
\begin{exam}{(Products)}{linrep:exam:1.10}
  ...
  morphisms\footnote{This latter prescription is what makes the
    category `discrete'; cf.\ Example~I.3.3.}, and let $\mathscr{A}$
...
\begin{exam}{(Equalizers and kernels)}{linrep:exam:1.11}
  ...
  the reader will verify (Exercise~1.14).
\end{exam}
...
Example~III.6.5). More formally, $\bigcup_i A_i$ consists of
...
more general posets (Exercise~1.16). They are somewhat more explicit
...
rightly be termed \emph{cocontinuous}.)  We will not prove Lemma~1.17
...
Lemma~1.17. What is missing from this outline is the explicit
...
\begin{exam}{(Exact functors)}{linrep:exam:1.18}
  ...
  image of every exact complex is exact (Exercise~1.23).
...
\begin{exer}{$\lnot$}{linrep:exer:1.10}
  ...
  $\categ{C}$. Map $h^X$ to the image of $\mathrm{id}_X \in h^X(X)$ in
  ...
  \emph{Yoneda lemma}.  [1.11, IX.2.17]
\end{exer}
...
\begin{exer}{}{linrep:exer:1.11}
  ...
  Exercise~1.10.) Thus, every (small) category is equivalent to a
...
\begin{exer}{$\lnot$}{linrep:exer:1.14}
  Verify that the construction described in Example~1.11 indeed
...
\begin{exer}{}{linrep:exer:1.18}
  ...
  other, use the Nakayama lemma in the form of Exercise~VI.3.7. This
  ...
  $I$-adic completion is injective (cf.\ Exercise~1.17).
\end{exer}

\begin{exer}{$\lnot$}{linrep:exer:1.19}
  An important example of the construction presented in Exercise~1.17
  ...
  \item Prove that $\Z_p$ is a DVR (cf.\ Exercise~V.2.19).  (There is
    an evident valuation on $\Q_p$.)
  \end{itemize} [\S{}II.2.3]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:1.20}
  ...
  elements of $\hat{\Z}$ may be described.) [1.21]
\end{exer}
...
(Proposition~I.5.4 once more), and we could proceed to study it by
...
is a pure tensor. See Exercise~2.1 for one situation in which every
...
\begin{proof}
  ...
  (Exercise~2.11) that the map $\alpha \mapsto \tilde\phi$ is
...
Example~1.18). These observations have several consequences, which
...
to the right-hand side (Exercise~III.6.5). For finitely generated free
...
(Proposition~VI.1.7). Tensor products are more interesting over more
general rings.
...
\begin{exam}{}{linrep:exam:2.12}
  Since vector spaces are free (Proposition~VI.1.7), tensoring is
...
\begin{exam}{}{linrep:exam:2.14}
  ...
  clear to the reader who has worked out Exercise~VII.2.12!). This
  homomorphism defines a $k[x]$-module structure on $k[x,y]/(xy)$, and
  we can wonder whether the latter is flat in the sense of
  Definition~2.13. From the geometric point of view, clearly something
...
$\mathrm{Tor}^R_1(\mathord{-}, N)$: if $N$ is flat (for example, if it
is free), then $\mathrm{Tor}^R_1(M, N) = 0$ for all modules $M$. In
fact (amazingly) if
\[
  0 \to A \to B \to C \to 0
\]
...
\[
  \mathrm{Tor}^R_1(C, N) \to A \otimes_R N \to B \otimes_R N \to C
  \otimes_R N \to 0,
\]
...
$\mathrm{Tor}^R_3(\mathord{-}, N)$, etc. These are the derived
...
the commutative case; cf.\ Example~2.2), we could expect the same to
apply to every $\mathrm{Tor}^R_i$: $\mathrm{Tor}^R_i(M, N)$ ought to
be canonically isomorphic to $\mathrm{Tor}^R_i(N, M)$ for all
$i$. Equivalently, we should be able to compute
$\mathrm{Tor}^R_i(M, N)$ as the homology of $M \otimes_R N_\bullet$,
...
\[
  \mathrm{Tor}^R_1(A, N) \to \mathrm{Tor}^R_1(B, N) \to
  \mathrm{Tor}^R_1(C, N) \xrightarrow{\delta} A \otimes_R N \to B
...
\begin{exer}{$?$}{linrep:exer:2.2}
  ...
  relatively prime integers. [\S{}2.2]
\end{exer}
...
\begin{exer}{}{linrep:exer:2.16}
  ...
  $\mathrm{Tor}^R_1(R/I, R/J) \cong (I \cap J)/IJ$. (For example, this
  $\mathrm{Tor}^R_1$ vanishes if $I + J = R$, by Lemma~V.6.3.)  Prove
  that $\mathrm{Tor}^R_i(R/I, R/J)$ is isomorphic to
...
\begin{exer}{$\lnot$}{linrep:exer:2.19}
  ...
  $M$ is flat if and only if $\mathrm{Tor}^R_1(R/I, M) = 0$ for all
  ideals $I$ of $R$. [2.20]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:2.22}
  ...
  \mathrm{Tor}^{S^{-1}R}_i(S^{-1}M, S^{-1}N)$. (Use Exercise~1.25.)
...
\begin{exer}{$\lnot$}{linrep:exer:2.24}
  ...
  flat if and only if it is free. Compare with Exercise~VI.5.5.
  [2.25, 6.8, 6.12]
\end{exer}
...
\begin{exer}{}{linrep:exer:2.25}
  ...
  Exercise~2.22. The Noetherian hypothesis is actually unnecessary,
...
upgrade the adjunction formula given in Lemma~2.4. This requires the
...
\begin{proof}
  ...
  $\Hom_S(N, P)$. Thus $\phi$ is $R$-balanced. By Lemma~3.2, such a
...
Example~1.18, this hints that $f_*$ may have both a left-adjoint and a
...
\begin{exer}{$?$}{linrep:exer:3.1}
  ...
  \begin{align*}
    ...
    &(rm, n) - (m, rn),
  \end{align*}
  with $m, m_1, m_2 \in M$, $n, n_1, n_2 \in N$, $r \in R$.  [\S{}3.1]
\end{exer}
...
\begin{exer}{}{linrep:exer:3.15}
  ...
  \item Prove that there is a homomorphism of rings
    ...
    studied in Proposition~III.2.7).
...
tensor product is associative\footnote{In fact, the tensor product is
  associative in a fancier sense; cf.\ Exercise~3.3.} in
...
m_\ell$.  The reader will verify (Exercise 4.1) that
...
\begin{proof}
  ...
  evidently alternating (Exercise 4.5). Thus, it factors uniquely
...
\begin{exam}{}{linrep:4.7}
  ...
  field $k$ (Exercise 4.21), which is the simplest interesting
  instance of this construction (well, the second simplest: the
  simplest example may be Exercise III.5.17).
\end{exam}
...
\begin{exer}{$*$}{linrep:exer:4.1}
  ...
  universal property for symmetric multilinear maps. [\S{}4.2]
\end{exer}
...
\begin{exer}{}{linrep:exer:4.8}
  ...
  $\mathbf{A}^4_k$. (Recall from Exercise VII.2.20 that
...
\begin{exer}{}{linrep:exer:4.21}
  ...
  $k[x,y]$-module; for this, looking back at Exercise VI.4.15 may
  help.) [\S{}4.3, \S{}4.4]
\end{exer}
...
direct sum (as we have known for a long time: Proposition III.6.1),
...
\begin{rem}{}{linrep:5.9}
  ...
  a free module $F = R^{\oplus S}$. We proved in Corollary VI.1.11 (if
...
be crystal clear to the reader who works out Exercise 5.7. In
...
\end{equation}
since the dual of $\Z/2\Z$ is zero (Exercise 5.6). The map $\gamma$ in
...
\begin{proof}
  ...
  \[
    ...
    \end{tikzcd}
  \]
  By construction, $\nu \circ \rho = \mathrm{id}_P$. Now let
  $n \in N$. Then $\nu(\rho(\nu(n))) = \nu(n)$, giving
...
this by working out Exercise 5.11.  Even if not every module is the
...
sending $(f, m)$ to $f(m)$. For every $m \in M$, we get\footnote{The
  ...
  formally by working out Exercise~5.12.} an $R$-linear map
...
\begin{exer}{$*$}{linrep:exer:5.1}
  ...
  $R$-module, then $\Hom_R(F, N) \cong F^\vee \otimes_R N$.  [\S{}5.6]
\end{exer}
...
\begin{exer}{}{linrep:exer:5.3}
  ...
  already, in Exercise~5.2. To show
  $\ker\beta \subseteq \operatorname{im}\alpha$, choose
  $N = B/\operatorname{im}(\alpha)$.) Remember that the converse does
...
\begin{exer}{$\lnot$}{linrep:exer:5.4}
  ...
  bundle' in good situations.)  [6.20]
\end{exer}
...
\begin{exer}{$*$}{linrep:exer:5.6}
  Show that $(\Z/2\Z)^\vee = \Hom_{\Z}(\Z/2\Z, \Z) = 0$.  [\S{}5.4]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:5.15}
  ...
  The \emph{annihilator} of $W$ is\footnote{The notation is inspired
    by Exercise VI.6.17, which is a particular case of this
...
\begin{exer}{}{linrep:exer:5.16}
  ...
  the notation introduced for vector spaces in Exercise~5.15, prove
...
that projective modules are flat is straightforward (Exercise~6.7),
...
\begin{rem}{}{linrep:6.13}
  ...
  Corollary~\ref{linrep:6.11} and Exercise~6.15).  That is, products
...
$\mathrm{Tor}^R_i$ quantify the lack of exactness of $\otimes_R$. The
corresponding functors are called $\mathrm{Ext}^i_R$.
...
the same `bifunctors' $\mathrm{Ext}^i_R(-, -)$ work for both! For all
...
\item For $i = 0$, $\mathrm{Ext}^0_R(M, N) = \Hom_R(M, N)$.
...
more. Recall the procedure that defines $\mathrm{Tor}^R_i(-, N)$ from
...
$*$ Welcome back. In the case of $\mathrm{Ext}^i_R(M,N)$, there would
...
\begin{prop}{}{linrep:6.14}
  ...
  $\mathrm{Ext}^i_R(P, -) = 0$ for all $i > 0$. An $R$-module $Q$ is
  injective if and only if $\mathrm{Ext}^1_R(-, Q) = 0$, if and only
  if $\mathrm{Ext}^i_R(-, Q) = 0$ for all $i > 0$.
\end{prop}
...
$\mathrm{Ext}^*_{\Z}(G, \Z)$ for an arbitrary finitely generated
abelian group $G$.
\begin{itemize}
...
\item By the classification theorem for finitely generated abelian
  groups, we are reduced to computing $\mathrm{Ext}^i_{\Z}(\Z, \Z)$
  and $\mathrm{Ext}^i_{\Z}(\Z/m\Z, \Z)$, for all $m > 0$ and
  $i \geq 0$.
...
\item The second module: use the projective resolution
  ...
  that $\mathrm{Ext}^i_{\Z}(\Z/m\Z, \Z)$ is the cohomology of the
  complex
  \[
  ...
  confirming that
  $\mathrm{Ext}^0_{\Z}(\Z/m\Z, \Z) = \Hom_{\Z}(\Z/m\Z, \Z) = 0$ ($\Z$
  has no torsion) and computing
  $\mathrm{Ext}^1_{\Z}(\Z/m\Z, \Z) \cong \Z/m\Z$, with vanishing
  $\mathrm{Ext}^2$ and higher.
\item The conclusion is that $\mathrm{Ext}^0_{\Z}(G, \Z)$ picks up the
  free part of $G$, while $\mathrm{Ext}^1_{\Z}(G, \Z)$ is isomorphic
  to the torsion part, and $\mathrm{Ext}^i_{\Z}(G, \Z) = 0$ for
  $i \geq 2$ (as should have been expected; cf.\ Exercise~6.18).
\end{itemize}
In particular, if $G$ is a finite abelian group, then
$G \cong \mathrm{Ext}^1_{\Z}(G, \Z)$; as it happens, this isomorphism
is not canonical.

Another Ext computation gives a different interpretation of this
group. The exact sequence $0 \to \Z \to \Q \to \Q/\Z \to 0$ is an
injective resolution of $\Z$; thus, $\mathrm{Ext}^*_{\Z}(G, \Z)$ is
the cohomology of
\[
  0 \to \Hom_{\Z}(G, \Q) \to \Hom_{\Z}(G, \Q/\Z) \to 0 \to \dots.
\]
If $G$ is torsion (for example, if $G$ is finite), it follows that
$\mathrm{Ext}^1_{\Z}(G, \Z)$ is isomorphic to $\Hom_{\Z}(G,
...
\begin{exer}{$\lnot$}{linrep:exer:6.3}
  ...
  injectives.) This strengthens the result of Exercise~5.2.  [6.7]
\end{exer}
...
\begin{exer}{$\lnot$}{linrep:exer:6.12}
  ...
  of finite generation is necessary (cf.\ Exercise~6.13).  [6.10]
\end{exer}
...
\begin{exer}{}{linrep:exer:6.17}
  Prove that $\Hom_R(M, N) \cong \mathrm{Ext}^0_R(M, N)$, using any of
  the definitions provided for $\mathrm{Ext}^i_R(M, N)$.
\end{exer}
...
\begin{exer}{}{linrep:exer:6.20}
  ...
  $\mathrm{Ext}^1_R(R/I, R/I) \cong \Hom_R(I/I^2, R/I)$.  (Cf.\
  Exercise~5.4. This says that this Ext module essentially computes
...
\begin{exer}{$\lnot$}{linrep:exer:6.21}
  ...
  then be an isomorphism: cf.\ Exercise~III.7.10.) Extensions that are
  ...
  \[
    \dots \to \Hom_R(E, N) \to \Hom_R(N, N) \to \mathrm{Ext}^1_R(M, N)
    \to \dots,
  \]
  and we let $\xi(E)$ be the image in $\mathrm{Ext}^1_R(M, N)$ of the
  distinguished element $\mathrm{id}_N \in \Hom_R(N, N)$.
...
\begin{exer}{}{linrep:exer:6.22}
  ...
  $\mathrm{Ext}^1_R$. We get an even sharper statement by constructing
  an inverse to the map $\xi$: for every element
  $\kappa \in \mathrm{Ext}^1_R(M, N)$, we will construct an extension
  ...
  \item Let $F$ be any free module surjecting onto $M$, and let
    ...
    \[
      \dots \to \Hom_R(K, N) \to \mathrm{Ext}^1_R(M, N) \to
      \mathrm{Ext}^1_R(F, N) = 0 \to \dots,
    \]
    tells us that there exists a homomorphism $k : K \to N$ mapping to
    the element $\kappa \in \mathrm{Ext}^1_R(M, N)$.
