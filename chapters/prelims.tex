...
That is, a function must send each element $a$ of $A$  
 to exactly one element of $B$,
depending on $a$. `Multivalued functions' such as $\pm  \sqrt{x}$ (which are very important
...
\[f (S) := \{b \in B | (\exists a \in S) b = f (a)\}.\] That is,
$f (S)$ is the subset of B consisting of all elements that are images
of elements of $S$ by the function $f$. The largest such subset, that
is, $f (A)$, is called the image of $f$, denoted $\image f$.
...
be integers \dots, we really mean consider a function
...
functions are called \emph{diagrams}. We will draw many diagrams, and in contexts substantially more general than the one at hand right now.
...
the
same. This is precisely the content of the statement \eqref{eq:comp-def}.
...
B$
is any function, then $\id_B \circ f = f$ and $f \circ \id_A = f$. Graphically, the
diagrams
\[
  \begin{tikzcd}
