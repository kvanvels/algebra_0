...
changing the set---but that has to do with properties of $\field{Z}$, not with
...
integer; but
\[(\exists b \in Z) (\forall a \in Z)\quad b = 2a\] is false: it says that
...
\begin{exam}{}{}
  ...
  the isomorphic copy $A'$ of $A$ (resp., $B'$ of $B$) in $A \amalg B$.
\end{exam}
...
and quotients, made in \S{}1.4. Our definition of $A \amalg B$ was the
...
the fact that all of them produce acceptable candidates for $A \amalg B$. From
this egalitarian standpoint, the result of the operation $A \amalg B$ is not
`well-defined' as a set in the sense specified above. However, it is easy to see
(Exercise~2.9) that $A \amalg B$ is well-defined up to isomorphism: that is,
...
\begin{exer}{}{}
  ...
  $A' \cup B' \cong A'' \cup B''$. Conclude that the operation $A \amalg B$ (as
...
\begin{defn}{}{defn:category}
  ...
  \begin{itemize}
  \item a class $\operatorname{Obj}(\categ{C})$ of \emph{objects} of
    the category; and
  \item for every two objects $A$, $B$ of $\categ{C}$, a set
    $\operatorname{Hom}_{\categ{C}}(A, B)$ of \emph{morphisms},
  \end{itemize}
...
\begin{itemize}
\item For every object $A$ of $\categ{C}$, there exists (at least)
  one morphism $1_A \in \operatorname{Hom}_{\categ{C}}(A, A)$, the
  `identity' on $A$.
\item One can compose morphisms: two morphisms
  $f \in \operatorname{Hom}_{\categ{C}}(A, B)$ and
  $g \in \operatorname{Hom}_{\categ{C}}(B, C)$ determine a morphism
  $gf \in \operatorname{Hom}_{\categ{C}}(A, C)$. That is, for every
  triple of objects $A$, $B$, $C$ of $\categ{C}$ there is a function
  (of sets)
  \[ \operatorname{Hom}_{\categ{C}}(A, B) \times
    \operatorname{Hom}_{\categ{C}}(B, C) \to
    \operatorname{Hom}_{\categ{C}}(A, C), \] and the image of the
  pair $(f, g)$ is denoted $gf$.
\item This `composition law' is associative: if
  $f \in \operatorname{Hom}_{\categ{C}}(A, B)$,
  $g \in \operatorname{Hom}_{\categ{C}}(B, C)$, and
  $h \in \operatorname{Hom}_{\categ{C}}(C, D)$, then $(hg)f = h(gf)$.
\item The identity morphisms are identities with respect to
  composition: that is, for all
  $f \in \operatorname{Hom}_{\categ{C}}(A, B)$ we have
...
$\operatorname{Hom}_{\categ{C}}(A, B)$ and
$\operatorname{Hom}_{\categ{C}}(C, D)$ be disjoint unless $A = C$,
$B = D$; this is something you do not usually think about, but again
it holds for ordinary set-functions.\footnote{I will often use the
  term `set-function' to emphasize that we are dealing with a function
  in the context of sets.} That is, if two functions are one and the
same, then necessarily they have the same source and the same target:
source and target are part of the datum of a set-function.

A morphism of an object $A$ of a category $\categ{C}$ to itself is
called an \emph{endomorphism}; $\operatorname{Hom}_{\categ{C}}(A, A)$
is denoted $\operatorname{End}_{\categ{C}}(A)$. One of the axioms of
a category tells us that this is a `pointed' set, as
$1_A \in \operatorname{End}_{\categ{C}}(A)$.  The reader should note
that composition defines an `operation' on
$\operatorname{End}_{\categ{C}}(A)$: if $f$, $g$ are elements of
$\operatorname{End}_{\categ{C}}(A)$, so is their composition $gf$.

Writing `$f \in \operatorname{Hom}_{\categ{C}}(A, B)$' gets tiresome
in the long run. If the category is understood, one may safely drop
the index $\categ{C}$, or even use arrows as we do with
set-functions: $f : A \to B$.  This also allows us to draw
\emph{diagrams} of morphisms in any category; a diagram is said to
`commute' (or to be a `commutative' diagram) if all ways to traverse
it lead to the same results of composing morphisms along the way, just
...
\begin{exam}{}{}
  ...
  denote categories; thus, $\categ{Set}$ will denote the category of sets. Thus
  \begin{itemize}
  \item $\operatorname{Obj}(\categ{Set}) = $ the class of all sets;
  \item for $A$, $B$ in $\operatorname{Obj}(\categ{Set})$ (that is, for $A$,
    $B$ sets) $\operatorname{Hom}_{\categ{Set}}(A, B) = B^A$.
  \end{itemize}
  ...
  features of $\categ{Set}$, which may or may not be shared by other
  categories. We will soon come back to some of these operations and understand
  more precisely what they say about $\categ{Set}$.
\end{exam}
\begin{exam}{}{}
  ...
  \item \emph{morphisms}: if $a$, $b$ are objects (that is, if $a, b \in S$),
    ...
    \emptyset$ otherwise.
  \end{itemize}
  Note that (unlike in $\categ{Set}$) there are very few morphisms: at most one
  ...
  $\categ{Set}$. Also note that these categories are all small.
\end{exam}
\begin{exam}{}{}
  ...
  small category. Let $S$ again be a set. Define a category $\hat{\categ{S}}$
  by setting
  \begin{itemize}
  \item $\operatorname{Obj}(\hat{\categ{S}}) = \mathscr{P}(S)$, the power set
    of $S$ (cf.\ \S{}1.2 and Exercise~2.11);
  \item for $A$, $B$ objects of $\hat{\categ{S}}$ (that is, $A \subseteq S$
    and $B \subseteq S$) let $\operatorname{Hom}_{\hat{\categ{S}}}(A, B)$ be
    the pair $(A, B)$ if $A \subseteq B$, and let
    $\operatorname{Hom}_{\hat{\categ{S}}}(A, B) = \emptyset$ otherwise.
  \end{itemize}
  ...
  in $\hat{\categ{S}}$, then $A \subseteq B$ and $B \subseteq C$; hence $A
...
\begin{exam}{}{}
  ...
  Let $\categ{C}$ be a category, and let $A$ be an object of $\categ{C}$. We
  are going to define a category $\categ{C}_A$ whose objects are certain
  morphisms in $\categ{C}$ and whose morphisms are certain diagrams of
  $\categ{C}$ (surprise!).
  \begin{itemize}
  \item $\operatorname{Obj}(\categ{C}_A) = $ all morphisms from any object of
    $\categ{C}$ to $A$; thus, an object of $\categ{C}_A$ is a morphism $f \in
    \operatorname{Hom}_{\categ{C}}(Z, A)$ for some object $Z$ of $\categ{C}$.
    Pictorially, an object of $\categ{C}_A$ is an arrow $Z \xrightarrow{f} A$
    in $\categ{C}$; these are often drawn `top-down', as in
    \[
      \begin{tikzcd} Z \arrow[d, "f"] \\ A \end{tikzcd}
    \]
  \end{itemize}
  What are morphisms in $\categ{C}_A$ going to be? There really is only one
  ...
  definition of morphism in $\categ{C}_A$.

  Welcome back.
  \begin{itemize}
  \item Let $f_1$, $f_2$ be objects of $\categ{C}_A$, that is, two arrows
    $Z_1 \xrightarrow{f_1} A$ and $Z_2 \xrightarrow{f_2} A$ in $\categ{C}$.
  ...
  $\sigma : Z_1 \to Z_2$ in $\categ{C}$ such that $f_1 = f_2 \sigma$.
  ...
  from the identities in $\categ{C}$: for $f : Z \to A$ in $\categ{C}_A$, the
  ...
  Composition is also a subproduct of composition in $\categ{C}$. Two morphisms
  $f_1 \to f_2 \to f_3$ in $\categ{C}_A$ correspond to putting two commutative
  ...
  \[
    ...
    \end{tikzcd}
  \]
  and then it follows (again because $\categ{C}$ is a category!) that the
  ...
  \[
    ...
    \end{tikzcd}
  \]
  also commutes. Check all this(!), and verify that composition in $\categ{C}_A$
  is associative (again, this follows immediately from the fact that composition
  is associative in $\categ{C}$).
...
\begin{exam}{}{}
  ...
  relation $\leq$. Call $\categ{C}$ this category, and choose an object $A$ of
  $\categ{C}$---that is, an integer, for example, $A = 3$. Then the objects of
  $\categ{C}_A$ are morphisms in $\categ{C}$ with target $3$, that is, pairs
  $(n, 3) \in \Z \times \Z$ with $n \leq 3$. There is a morphism $(m, 3) \to
  (n, 3)$ if and only if $m \leq n$. In this case $\categ{C}_A$ may be
  harmlessly identified with the `subcategory' of integers $\leq 3$, with `the
  same' morphisms as in $\categ{C}$.
\end{exam}

\begin{exam}{}{}
  An entirely similar example to the one explored in Example~3.5 may be obtained
  by considering morphisms in a category $\categ{C}$ \emph{from} a fixed object
  $A$ to all objects in $\categ{C}$, again with morphisms defined by suitable
...
\begin{exam}{}{}
  As a `concrete' instance of a category as in Example~3.7, let $\categ{C} =
  \categ{Set}$ and $A = $ a fixed singleton $\{*\}$. Call the resulting category
  $\categ{Set}^*$.

  An object in $\categ{Set}^*$ is then a morphism $f : \{*\} \to S$ in
  $\categ{Set}$, where $S$ is any set. The information of an object in
  $\categ{Set}^*$ consists therefore of the choice of a nonempty set $S$ and of
  an element $s \in S$---that is, the element $f(*)$: this element determines,
  and is determined by, $f$.

  Thus, we may denote objects of $\categ{Set}^*$ as pairs $(S, s)$, where $S$
  ...
  Objects of $\categ{Set}^*$ are called \emph{pointed sets}. Many of the
...
\begin{exam}{}{}
  ...
  This time we start from a given category $\categ{C}$ and \emph{two} objects
  $A$, $B$ of $\categ{C}$. We can define a new category $\categ{C}_{A,B}$ by
  essentially the same procedure that we used in order to define $\categ{C}_A$:
  \begin{itemize}
  \item $\operatorname{Obj}(\categ{C}_{A,B})$ = diagrams
    % TODO: commutative diagram with Z mapping to both A and B (two-legged span)
    $Z \xrightarrow{f} A$ and $Z \xrightarrow{g} B$ in $\categ{C}$; and
  \item morphisms from $(f_1 : Z_1 \to A,\, g_1 : Z_1 \to B)$ to
    $(f_2 : Z_2 \to A,\, g_2 : Z_2 \to B)$ are those morphisms $\sigma : Z_1
    \to Z_2$ in $\categ{C}$ making both diagrams commute simultaneously:
    $f_1 = f_2 \sigma$ and $g_1 = g_2 \sigma$.
  \end{itemize}
  I will leave to the reader the task of formalizing this rough description. This
  example is really nothing more than a mixture of $\categ{C}_A$ and
  $\categ{C}_B$, where the two structures interact because of the stringent
  ...
  $\categ{C}^{A,B}$; details are left to the reader.
\end{exam}

\begin{exam}{}{}
  As a final variation on these examples, we conclude by considering the
  \emph{fibered} version of $\categ{C}_{A,B}$ (and $\categ{C}^{A,B}$). Take
  this as a test to see if you have really understood $\categ{C}_{A,B}$---experts
  ...
  $\categ{C}$, and this time choose two fixed morphisms $\alpha : A \to C$,
  $\beta : B \to C$ in $\categ{C}$, with the same target $C$. We can then
  consider a category $\categ{C}_{\alpha, \beta}$ as follows:
  \begin{itemize}
  \item $\operatorname{Obj}(\categ{C}_{\alpha,\beta})$ = commutative diagrams
    % TODO: diagram with Z mapping to A and B, composed with α and β to a common C
    in $\categ{C}$: pairs $(f : Z \to A,\, g : Z \to B)$ such that $\alpha f =
    \beta g$; and
  ...
  $\categ{C}^{\alpha,\beta}$, starting from two morphisms $\alpha : C \to A$,
...
\begin{exer}{}{}
  \exerused Let $\categ{C}$ be a category. Consider a structure $\categ{C}^{\mathrm{op}}$ with
  \begin{itemize}
  \item $\operatorname{Obj}(\categ{C}^{op}) := \operatorname{Obj}(\categ{C})$;
  \item for $A$, $B$ objects of $\categ{C}^{op}$ (hence objects of $\categ{C}$),
    $\operatorname{Hom}_{\categ{C}^{op}}(A, B) := \operatorname{Hom}_{\categ{C}}(B, A)$.
  \end{itemize}
  Show how to make this into a category (that is, define composition of morphisms
  in $\categ{C}^{op}$ and verify the properties listed in \S{}3.1). Intuitively,
  the `opposite' category $\categ{C}^{op}$ is simply obtained by `reversing all
  the arrows' in $\categ{C}$. [5.1, \S{}VIII.1.1, \S{}IX.1.2, IX.1.10]
\end{exer}

\begin{exer}{}{}
  If $A$ is a finite set, how large is $\operatorname{End}_{\categ{Set}}(A)$?
\end{exer}
...
\begin{exer}{}{}
  ...
  $\categ{V}$ by taking $\operatorname{Obj}(\categ{V}) = \N$ and letting
  $\operatorname{Hom}_{\categ{V}}(n, m) = $ the set of $m \times n$ matrices
...
\begin{exer}{}{}
  \exerused A \emph{subcategory} $\categ{C}'$ of a category $\categ{C}$
  consists of a collection of objects of $\categ{C}$ with sets of morphisms
  $\operatorname{Hom}_{\categ{C}'}(A, B) \subseteq \operatorname{Hom}_{\categ{C}}(A, B)$
  for all objects $A$, $B$ in $\operatorname{Obj}(\categ{C}')$, such that
  identities and compositions in $\categ{C}$ make $\categ{C}'$ into a
  category. A subcategory $\categ{C}'$ is \emph{full} if
  $\operatorname{Hom}_{\categ{C}'}(A, B) = \operatorname{Hom}_{\categ{C}}(A, B)$
  for all $A$, $B$ in $\operatorname{Obj}(\categ{C}')$. Construct a category of
...
\begin{exer}{}{}
  ...
  $\categ{MSet}$ containing (a `copy' of) $\categ{Set}$ as a full subcategory.
  (There may be more than one reasonable way to do this! This is intentionally an
  open-ended exercise.) Which objects in $\categ{MSet}$ determine ordinary
  ...
  morphisms of multisets. Try to define morphisms in $\categ{MSet}$ so that the
  notion you obtain for ordinary multisets captures your intuitive understanding
  of these objects.) [\S{}2.2, \S{}3.2, 4.5]
\end{exer}

\begin{exer}{}{}
  Since the objects of a category $\categ{C}$ are not (necessarily interpreted
  ...
  the subobjects of any given object $A$ in $\categ{C}$ are in one-to-one
  correspondence with the morphisms $A \to \Omega$ for a fixed, special object
  $\Omega$ of $\categ{C}$, called a \emph{subobject classifier}. Show that
  $\categ{Set}$ has a subobject classifier.
\end{exer}

\begin{exer}{}{}
  \exerused Draw the relevant diagrams and define composition and identities for
  the category $\categ{C}^{A,B}$ mentioned in Example~3.9. Do the same for the
  category $\categ{C}^{\alpha,\beta}$ mentioned in Example~3.10.
  [\S{}5.5, 5.12]
\end{exer}
...
Just as in $\categ{Set}$ we highlight certain types of functions (injective,
...
A morphism $f \in \operatorname{Hom}_{\categ{C}}(A,B)$ is an
\textbf{isomorphism} if it has a (two-sided) inverse under composition: that
is, if $\exists\, g \in \operatorname{Hom}_{\categ{C}}(B,A)$ such that
\[
...
\begin{itemize}
  ...
  \item If $f$ is an isomorphism, then $f^{-1}$ is an isomorphism and
        $(f^{-1})^{-1} = f$.
  \item If $f \in \operatorname{Hom}_{\categ{C}}(A,B)$ and
        $g \in \operatorname{Hom}_{\categ{C}}(B,C)$ are isomorphisms, then
...
$\categ{Set}$; the same proof will work in any category.} If two objects $A$,
$B$ are isomorphic, one writes $A \cong B$.

\begin{exam}{}{}
Of course, the isomorphisms in the category $\categ{Set}$ are precisely the
...
the category $\categ{C}$ obtained from the relation $\leq$ on $\mathbb{Z}$,
as in \Cref{exam:prelims:category-leq}. Indeed, for $a$, $b$ objects of
$\categ{C}$ (that is, $a, b \in \mathbb{Z}$), there is a morphism
$f \colon a \to b$ and a morphism $g \colon b \to a$ only if $a \leq b$ and
$b \leq a$, that is, if $a = b$. So an isomorphism in $\categ{C}$ necessarily
acts from an object $a$ to itself; but in $\categ{C}$ there is only one such
...
An \textbf{automorphism} of an object $A$ of a category $\categ{C}$ is an
isomorphism from $A$ to itself. The set of automorphisms of $A$ is denoted
$\operatorname{Aut}_{\categ{C}}(A)$; it is a subset of
$\operatorname{End}_{\categ{C}}(A)$. By \Cref{prop:prelims:iso-props},
composition confers on $\operatorname{Aut}_{\categ{C}}(A)$ a remarkable
structure:
\begin{itemize}
  \item the composition of two elements $f, g \in \operatorname{Aut}_{\categ{C}}(A)$
        is an element $g \circ f \in \operatorname{Aut}_{\categ{C}}(A)$;
  \item composition is associative;
  \item $\operatorname{Aut}_{\categ{C}}(A)$ contains the element $1_A$, which
        is an identity for composition (that is,
        $f \circ 1_A = 1_A \circ f = f$);
  \item every element $f \in \operatorname{Aut}_{\categ{C}}(A)$ has an inverse
        $f^{-1} \in \operatorname{Aut}_{\categ{C}}(A)$.
\end{itemize}
In other words, $\operatorname{Aut}_{\categ{C}}(A)$ is a group, for all
objects $A$ of all categories $\categ{C}$. We will soon devote all our
attention to groups!
...
Let $\categ{C}$ be a category. A morphism $f \in \operatorname{Hom}_{\categ{C}}(A,B)$
is a \textbf{monomorphism} if the following holds: for all objects $Z$ of
$\categ{C}$ and all morphisms $\alpha', \alpha'' \in
\operatorname{Hom}_{\categ{C}}(Z,A)$,
\[
...
$\categ{C}$ and all morphisms $\beta', \beta'' \in
\operatorname{Hom}_{\categ{C}}(B,Z)$,
\[
...
\begin{exam}{}{}
As proven in Proposition~2.3, in the category $\categ{Set}$ the monomorphisms
are precisely the injective functions. The reader should have by now checked
that, likewise, in $\categ{Set}$ the epimorphisms are precisely the surjective
...
$\categ{Set}$, a function is an isomorphism if and only if it is both injective
...
special feature of $\categ{Set}$, and we should not expect it to hold
automatically in every category; it will not hold in the category $\categ{Ring}$
of rings. It will hold in every abelian category (of which $\categ{Set}$ is
not an example!), but that is a story for a very distant future.

Similarly, in $\categ{Set}$ a function is an epimorphism, that is, surjective,
if and only if it has a right-inverse (Proposition~2.1); this may fail in
general, even in respectable categories such as the category $\categ{Grp}$ of
groups.
...
$f \in \operatorname{Hom}_{\categ{C}}(A,B)$ be a morphism.
\begin{itemize}
...
one can define a subcategory $\categ{C}_{\mathrm{mono}}$ of a category
$\categ{C}$ by taking the same objects as in $\categ{C}$ and defining
$\operatorname{Hom}_{\categ{C}_{\mathrm{mono}}}(A,B)$ to be the subset of
$\operatorname{Hom}_{\categ{C}}(A,B)$ consisting of monomorphisms, for all
objects $A$, $B$. (Cf.\ \Cref{exer:3.8}; of course, in general
$\categ{C}_{\mathrm{mono}}$ is not full in $\categ{C}$.) Do the same for
epimorphisms. Can you define a subcategory $\categ{C}_{\mathrm{nonmono}}$ of
$\categ{C}$ by restricting to morphisms that are \emph{not} monomorphisms?
\end{exer}

\begin{exer}{}{exer:4.5}
Give a concrete description of monomorphisms and epimorphisms in the category
$\categ{MSet}$ you constructed in \Cref{exer:3.9}. (Your answer will depend
...
certain universal properties having to do with the categories $\categ{C}_{A,B}$
and $\categ{C}^{A,B}$ considered in Example~3.9.
...
Let $\categ{C}$ be a category. We say that an object $I$ of $\categ{C}$ is
\textbf{initial} in $\categ{C}$ if for every object $A$ of $\categ{C}$ there
exists exactly one morphism $I \to A$ in $\categ{C}$:
\[
  \forall A \in \operatorname{Obj}(\categ{C}) \colon \quad
  \operatorname{Hom}_{\categ{C}}(I,A) \text{ is a singleton.}
\]
We say that an object $F$ of $\categ{C}$ is \textbf{final} in $\categ{C}$ if
for every object $A$ of $\categ{C}$ there exists exactly one morphism
$A \to F$ in $\categ{C}$:
\[
  \forall A \in \operatorname{Obj}(\categ{C}) \colon \quad
  \operatorname{Hom}_{\categ{C}}(A,F) \text{ is a singleton.}
\]
\end{defn}

One may use \emph{terminal} to denote either possibility, but in general I would
advise the reader to be explicit about which `end' of $\categ{C}$ one is
considering.
...
\begin{exam}{}{}
In $\categ{Set}$, the empty set $\emptyset$ is initial (the `empty graph'
...
$\categ{Set}$ also has final objects: for every set $A$, there is a unique
function from $A$ to a singleton $\{p\}$ (that is, the `constant' function).
Every singleton is final in $\categ{Set}$; thus, final objects are not unique
in this category.
\end{exam}
...
\end{prop}
\begin{proof}
Recall that (by definition of category!) for every object $A$ of $\categ{C}$
there is at least one element in $\operatorname{Hom}_{\categ{C}}(A,A)$, namely
the identity $1_A$. If $I$ is initial, then there is a unique morphism
$I \to I$, which therefore must be the identity $1_I$.

Now assume $I_1$ and $I_2$ are both initial in $\categ{C}$. Since $I_1$ is
initial, there is a unique morphism $f \colon I_1 \to I_2$ in $\categ{C}$; we
have to show that $f$ is an isomorphism. Since $I_2$ is initial, there is a
unique morphism $g \colon I_2 \to I_1$ in $\categ{C}$. Consider
...
final objects in $\categ{Set}$ are all isomorphic: no singleton is more
...
objects in the category $\categ{C}_{A,B}$ considered in Example~3.9, for
$\categ{C} = \categ{Set}$.
What is the advantage of viewing products this way? The main advantage is that
the universal property may be stated in \emph{any} category, while the
definition of products given in \S{}1.4 only makes sense in $\categ{Set}$ (and
possibly in other categories where one has a notion of `elements'). We say that
a category $\categ{C}$ \emph{has (finite) products}, or is a category `with
(finite) products', if for all objects $A$, $B$ in $\categ{C}$ the category
$\categ{C}_{A,B}$ considered in Example~3.9 has final objects. Such a final
object consists of the data of an object of $\categ{C}$, usually denoted
$A \times B$, and of two morphisms $A \times B \to A$, $A \times B \to B$.
Note that a `product' from this perspective does not need to `look like a
product'. Consider our recurring example of the category obtained from $\leq$
on $\mathbb{Z}$, as in Example~3.3. Does this category have products? Objects
of this category are simply integers $a, b \in \mathbb{Z}$; call $a \times b$
for a moment the `categorical' product of $a$ and $b$. The universal property
written out above becomes, in this case: for all $z \in \mathbb{Z}$ such that
$z \leq a$ and $z \leq b$, we have $z \leq a \times b$.

This universal problem does have a solution $\forall a, b$: it is conventionally
not called $a \times b$, but rather $\min(a, b)$. It is immediate to see that
$\min(a, b)$ satisfies the property. Thus this category has products, and in
fact we see that the product in this category amounts to the familiar operation
of taking the minimum of two integers.

Thus there is an unexpected connection between `the Cartesian product of two
sets' and `the minimum of two integers': both are examples of products, taken in
different categories; they both satisfy `the same' universal property, in
different contexts.

\subsection{Coproducts}
\label{subsec:prelims:coproducts}
The prefix \emph{co-} usually indicates that one is `reversing all arrows'.
Just as products are final objects in the categories $\categ{C}_{A,B}$ obtained
by considering morphisms in $\categ{C}$ with common source, whose targets are
$A$ and $B$, coproducts will be initial objects in the
categories\footnote{These categories were also considered in Example~3.9;
cf.\ \Cref{exer:3.11}.} $\categ{C}^{A,B}$ of morphisms with common target,
whose sources are $A$ and $B$. Dear reader, look away and spell this universal
...
$i_B \colon B \to A \amalg B$ and satisfying the following universal property:
...
\end{tikzcd}
\]
there exists a unique morphism $\sigma \colon A \amalg B \to Z$ such that the
diagram
\[
\begin{tikzcd}
...
\end{tikzcd}
\]
commutes. The symmetry with the universal property of products is hopefully
completely apparent. We say that a category $\categ{C}$ \emph{has coproducts}
if this universal problem has a solution for all pairs of objects $A$ and $B$.
Is the reader familiar with any coproduct? Yes!

\begin{prop}{The disjoint union is a coproduct in $\categ{Set}$.}{prop:prelims:disjoint-union-coproduct}
\end{prop}
\begin{proof}
Recall (\S{}1.4) that the disjoint union $A \amalg B$ is defined as the union
of two disjoint isomorphic copies $A'$, $B'$ of $A$, $B$, respectively; for
example, we may let $A' = \{0\} \times A$, $B' = \{1\} \times B$. The
...
Now let $f_A \colon A \to Z$, $f_B \colon B \to Z$ be arbitrary morphisms to a
common target. Define
\[
  \sigma \colon A \amalg B = (\{0\} \times A) \cup (\{1\} \times B) \to Z
\]
by
\[
...
\end{proof}
This observation tells us that the category $\categ{Set}$ has coproducts and
further sheds considerable light on the mysteries of disjoint unions. For
example, there was an element of arbitrariness in our choice of `a' disjoint
union, although different choices led to isomorphic notions. Now we see why:
terminal objects of a category are not unique in general, although they are
unique up to isomorphism (\Cref{prop:prelims:initial-final-unique}); there is
not a `most beautiful' disjoint union of two sets just as there is not a `most
...
categories we have encountered. For example (and probably not surprisingly at
this point) the category obtained from $\leq$ on $\mathbb{Z}$ does have
coproducts: the coproduct of two objects (i.e., integers) $a$, $b$ is simply
the maximum of $a$ and $b$.

\subsection*{Exercises}

\begin{exer}{}{exer:5.1}
Prove that a final object in a category $\categ{C}$ is initial in the opposite
category $\categ{C}^{op}$ (cf.\ \Cref{exer:3.1}).
\end{exer}

\begin{exer}{\exerused}{exer:5.2}
Prove that $\emptyset$ is the unique initial object in $\categ{Set}$. [\S{}5.1]
\end{exer}
...
What are the final objects in the category considered in \S{}5.3? [\S{}5.3]
\end{exer}

\begin{exer}{\exerused}{exer:5.6}
Consider the category corresponding to endowing (as in Example~3.3) the set
$\mathbb{Z}^+$ of positive integers with the divisibility relation. Thus there
is exactly one morphism $d \to m$ in this category if and only if $d$ divides
$m$ without remainder; there is no morphism between $d$ and $m$ otherwise. Show
that this category has products and coproducts. What are their `conventional'
names? [\S{}VII.5.1]
\end{exer}

\begin{exer}{}{exer:5.7}
Redo \Cref{exer:2.9}, this time using \Cref{prop:prelims:initial-final-unique}.
\end{exer}

\begin{exer}{}{exer:5.8}
Show that in every category $\categ{C}$ the products $A \times B$ and
$B \times A$ are isomorphic, if they exist.
\begin{hint}
Observe that they both satisfy the universal property for the product of $A$ and
$B$; then use \Cref{prop:prelims:initial-final-unique}.
\end{hint}
\end{exer}

\begin{exer}{}{exer:5.9}
Let $\categ{C}$ be a category with products. Find a reasonable candidate for
the universal property that the product $A \times B \times C$ of three objects
of $\categ{C}$ ought to satisfy, and prove that both $(A \times B) \times C$
and $A \times (B \times C)$ satisfy this universal property. Deduce that
$(A \times B) \times C$ and $A \times (B \times C)$ are necessarily isomorphic.
\end{exer}

\begin{exer}{}{exer:5.10}
Push the envelope a little further still, and define products and coproducts for
families (i.e., indexed sets) of objects of a category. Do these exist in
$\categ{Set}$? It is common to denote the product $A \times \cdots \times A$
($n$ times) by $A^n$.
\end{exer}
...
\begin{itemize}
  \item Use the universal property for quotients (\S{}5.3) to establish that
        there are functions $(A \times B)/{\sim} \to A/{\sim_A}$,
        $(A \times B)/{\sim} \to B/{\sim_B}$.
  \item Prove that $(A \times B)/{\sim}$, with these two functions, satisfies
...
objects of the categories $\categ{C}_{\alpha,\beta}$,
$\categ{C}^{\alpha,\beta}$ considered in Example~3.10 (cf.\ also
\Cref{exer:3.11}), by stating carefully the corresponding universal properties.
As it happens, $\categ{Set}$ has both fibered products and coproducts. Define
these objects `concretely', in terms of naive set theory.
