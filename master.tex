...
% TODO(remove): the fast-build path below (fastbuild/fonts_fast.tex/
% environments_fast.tex/check_fast) served its purpose while getting
% references correct and is no longer needed -- slated for removal
% (along with those two files and the Makefile's check_fast target).
% Left in place for now; \tolean{}'s tcolorbox-based box below is not
% fast-build-safe (plain pdflatex can't process tcolorbox here), so
% `make check_fast` is broken until this is actually removed.
%
% Set \fastbuildtrue for a fast plain-pdflatex reference/syntax-check
% build: fonts_fast.tex (no fontspec/unicode-math OpenType loading)
% and environments_fast.tex (plain-text theorem environments instead
% of tcolorbox's colored/breakable boxes -- the real bottleneck).
% Same \label/\ref/\Cref behavior, ~3x faster, but NEVER represents
% the real book's appearance -- only use it to check references and
% catch errors, never to review layout. Requires pdflatex, not
% lualatex (unicode-math needs lualatex/xelatex, which is the whole
% reason this switch exists). Run `make clean` after flipping this,
% since \include writes per-chapter aux files shared between both
% modes.
\newif\iffastbuild
\fastbuildfalse
...
\newcommand{\tolean}[2][]{%
  ...
  \ifblank{#1}{}{\marginpar{\footnotesize #1}}%
}
...
% LibertinusSans-Bold in fonts.tex) -- it doesn't exist without
% unicode-math, so \categ needs a plain-\mathsf fallback under
% fonts_fast.tex.
\iffastbuild
  \newcommand{\categ}[1]{{\color{UCLA_Blue}\mathsf{#1}}}
\else
