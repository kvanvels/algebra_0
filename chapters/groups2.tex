...
a less `general' nature than those considered in \Cref{chap:groups}\@. Most of
what we do here will apply exclusively to \emph{finite} groups; this
is an important example in its own right, as it has spectacular
applications (for example, in Galois theory; cf.\ \S{}\ref{sec:fields:short-march-through-applications-of-galois-theory}), and it
is a good subject from the expository point of view, since it gives us
the opportunity to see several general concepts at work in a context
that is complex enough to carry substance, but simple enough (in this
tiny selection of elementary topics) to be appreciated easily.
...
established back in~\S{}\ref{sec:groups:group-actions}.

Recall that we proved (\Cref{prop:groups:transitive-action-is-g-mod-h}) that every
...
(\Cref{defn:groups:stabilizer}) the subgroup of $G$ fixing $a$. This fact applies
to the orbits of every left-action of $G$ on a set; in particular, the
number of elements in a finite orbit $O$ equals the index of the
stabilizer of any $a \in O$; in particular (\Cref{coro:groups:orbit-divides-order}) the
number of elements $|O|$ of an orbit must divide the order $|G|$ of
$G$, if $G$ is finite.
...
\begin{proof}
  ...
  \[
    |S| = |Z| + \sum_{a \in A} |O_a|,
  \]
  where $O_a$ denotes the orbit of $a$. By \Cref{prop:groups:transitive-action-is-g-mod-h}, the
  order $|O_a|$ equals the index of the stabilizer of $a$, yielding
  the statement.
\end{proof}
...
\Cref{exer:groups2:1.1}. Such immediate consequences of
\cref{prop:groups2:class-formula-action} will assist us below, in the
...
Recall (\Cref{exam:groups:action-by-left-multiplication-conjugation}) that every group $G$ acts on itself in at
least two interesting ways: by (left-) multiplication and by
\emph{conjugation}. The latter action is defined by the following
$\rho : G \times G \to G$:
\[
  \rho(g, a) = gag^{-1}.
\]
As we know (\S{}\ref{subsec:groups:actions-on-sets}), this datum is equivalent to the datum of a
certain group homomorphism:
\[
  \sigma : G \to S_G
\]
...
\end{lem}
\begin{proof}
  (Cf.\ \Cref{exer:groups2:1.5}.) As $G/Z(G)$ is cyclic, there exists an element
  $g \in G$ such that the class $gZ(G)$ generates $G/Z(G)$. Then
  $\forall a \in G$
  \[
    aZ(G) = (gZ(G))^r
  \]
...
\cref{lem:groups2:quotient-center-cyclic}; cf.\ \Cref{exer:groups2:1.6}) that if
$p$ is prime, then every group of order $p^2$ is commutative. In
general, the class formula poses a strong constraint on what can go on
in a group.
\begin{exam}{}{exam:groups2:order-6-class-formula}
  ...
  noncommutative group of order 6 up to isomorphism (\Cref{exer:groups2:1.13}).
\end{exam}
...
This leads to terminology analogous to the one introduced in \S{}\ref{subsec:groups2:center-centralizer-conjugacy-classes}.
...
\end{lem}
\begin{proof}
  This is again an immediate consequence of \Cref{prop:groups:transitive-action-is-g-mod-h}.
\end{proof}
...
\begin{proof}
  ...
  \]
  (cf.\ \S{}\ref{subsec:groups:the-index-and-lagrange-s-theorem}).
\end{proof}
...
\Cref{lem:groups2:normalizer-index-mod-p}.  Another useful numerical tool is the observation that if
$H$ and $K$ are subgroups of a group $G$ and $H \subseteq N_G(K)$---so
that $gKg^{-1} = K$ for all $g \in H$---then conjugation by $g \in H$
gives an automorphism of $K$. Indeed, I have already observed that
conjugation is a bijection, and it is immediate to see that it is a
homomorphism: $\forall k_1, k_2 \in K$
\[
...
determine $\ker\gamma$ (\Cref{exer:groups2:1.21}).

This is especially useful if $H$ is finite and some information is
available concerning $\Aut_{\categ{Grp}}(K)$ (for an
example, see \Cref{exer:groups2:4.14}). A classic application is presented in
\Cref{exer:groups2:1.22}.
...
\begin{exer}{}{exer:groups2:1.1}
  ...
  on $S$, prove that the action must have fixed points. [\S{}\ref{subsec:groups2:actions-of-groups-on-sets-reminder},
  \S{}\ref{subsec:groups2:sylow-ii}]
\end{exer}
\begin{exer}{}{exer:groups2:1.2}
  Find the center of $D_{2n}$. (The answer depends on the parity of
  $n$. You have actually done this already: \Cref{exer:groups:dihedral-center}. This
  time, use a presentation.)
\end{exer}
...
\begin{exer}{}{exer:groups2:1.4}
  ...
  $Z(G)$. Prove that $N$ is normal in $G$. [\S{}
ef{subsec:groups2:sylow-i}]
\end{exer}
\begin{exer}{}{exer:groups2:1.5}
  ...
  of \Cref{exer:groups:inn-cyclic-iff-abelian}. [\S{}\ref{subsec:groups2:center-centralizer-conjugacy-classes}]
\end{exer}
\begin{exer}{}{exer:groups2:1.6}
  ...
  order $p^2$, for a prime $p$, is commutative. [\S{}
ef{subsec:groups2:the-class-formula}]
\end{exer}
...
\begin{exer}{}{exer:groups2:1.12}
  ...
  \Cref{exer:rings:quaternions}) is $8 = 2 + 2 + 2 + 2$. (Also note that
  $D_8 \not\cong Q_8$.)
\end{exer}
\begin{exer}{}{exer:groups2:1.13}
  \exerused{} Let $G$ be a noncommutative group of order 6. As
  observed in \Cref{exam:groups2:order-6-class-formula}, $G$ must have trivial center and exactly
  two conjugacy classes, of order 2 and 3.
  \begin{itemize}
  ...
  \end{itemize} [\S{}
ef{subsec:groups2:the-class-formula}, \S{}
ef{subsec:groups2:applications}]
\end{exer}
...
\begin{exer}{}{exer:groups2:1.16}
  ...
  in $H$. (Hint: Note that $H$ is normal in $G$, by \Cref{exer:groups:index-2-subgroup-normal};
  apply \Cref{prop:groups:second-isomorphism-theorem}.) [\S{}\ref{subsec:groups2:conjugacy-an-simplicity}]
\end{exer}
\begin{exer}{}{exer:groups2:1.17}
  ...
  overlap, at least at the identity.) [\ref{exer:groups2:1.18}, \ref{exer:groups2:1.20}]
\end{exer}
\begin{exer}{}{exer:groups2:1.18}
  ...
  $s \in S$. (Hint: By \Cref{prop:groups:transitive-action-is-g-mod-h}, you may assume $S = G/H$,
  with $H$ proper in $G$. Use \Cref{exer:groups2:1.17}.)
\end{exer}
...
\begin{exer}{}{exer:groups2:1.20}
  ...
  of upper triangular matrices (\Cref{exer:groups:upper-triangular-subgroup}). Prove that $G$ is
  the union of the conjugates of $H$. Thus, the finiteness hypothesis
  in \Cref{exer:groups2:1.17} is necessary. (Hint: Equivalently, prove that every
  $2 \times 2$ matrix is conjugate to a matrix in $H$. You will use
  the fact that $\C$ is algebraically closed; see \Cref{exam:rings:speck-prime-ideals-maximal}.)
\end{exer}
\begin{exer}{}{exer:groups2:1.21}
  \exerused{} Let $H$, $K$ be subgroups of a group $G$, with
  $H \subseteq N_G(K)$.  Verify that the function
  $\gamma : H \to \Aut_{\categ{Grp}}(K)$ defined by
  conjugation is a homomorphism of groups and that
  $\ker\gamma = H \cap Z_G(K)$, where $Z_G(K)$ is the centralizer of
  $K$.  [\S{}\ref{subsec:groups2:conjugation-of-subsets-and-subgroups}, \ref{exer:groups2:1.22}]
\end{exer}
\begin{exer}{}{exer:groups2:1.22}
  ...
  $H$ is contained in the center of $G$. (Hint: By \Cref{exer:groups2:1.21} there
  is a homomorphism
  $\gamma : G \to \Aut_{\categ{Grp}}(H)$; by
  \Cref{exer:groups:aut-cn-euler-phi}, $\Aut_{\categ{Grp}}(H)$ has order
  $p - 1$. What can you say about $\gamma$?) [\S{}\ref{subsec:groups2:conjugation-of-subsets-and-subgroups}]
\end{exer}
...
theorem for abelian groups (in \Cref{exer:groups:cauchy-abelian}), we could directly
move on to Sylow theorems.  However, there is a quick
...
then that subgroup must be normal (\Cref{exer:groups2:2.2}), suffices for
...
groups; we will see how this is done in \S{}\ref{subsec:groups2:the-jordan-h-older-theorem}. Thus, it is important
to be able to tell whether a group is simple or not.\footnote{In fact,
  a complete list of all finite simple groups is known: this is the
  classification result mentioned at the end of \S{}\ref{subsec:groups:example-subgroup-generated-by-a-subset}, arguably
  one of the deepest and hardest results in mathematics.}
...
\cref{thm:groups2:sylow-i} proves \cref{prop:groups2:sylow-i-strong},
and I see no reason to hide this fact. Here is the argument:
\begin{proof}[Proof of \cref{prop:groups2:sylow-i-strong}]
  ...
  By Cauchy's theorem\footnote{Note that, as mentioned in \S{}\ref{subsec:groups2:cauchy-s-theorem}, we
    only need the abelian case of this theorem.},
  $\exists\, a \in Z(G)$ such that $a$ has order $p$. The cyclic
  subgroup $N = \genalg{a}$ is contained in $Z(G)$, and hence it is
  normal in $G$ (\cref{exer:groups2:1.4}). Therefore we can consider
  the quotient $G/N$.  Since $|G/N| = |G|/p$ and $p^k$ divides $|G|$
  by hypothesis, we have that $p^{k-1}$ divides the order of $G/N$. By
  the induction hypothesis, we may conclude that $G/N$ contains a
  subgroup of order $p^{k-1}$. By the structure of the subgroups of a
  quotient (\S{}\ref{subsec:groups:subgroups-of-quotients}, especially \Cref{prop:groups:subgroups-of-quotients-bijection}), this subgroup
...
see a pretty (and insightful) alternative in \S{}\ref{subsec:groups2:sylow-ii}; but the proof
given above is easy to remember and is a good template for similar
arguments.

\begin{rem}{}{}
  The diligent reader worked out in \Cref{exer:groups:abelian-subgroup-of-order-d} a \emph{stronger}
...
\begin{proof}
  ...
  (\cref{exer:groups2:1.1}): let $gP$ be one of them. This means that
  $\forall h \in H$:
  \[
    hgP = gP;
  \]
...
\begin{proof}
  ...
  (cf.\ \S{}\ref{subsec:groups:subgroups-of-quotients}) of the form $H'/H$ for a subgroup $H'$ of
  $N_G(H)$. It is straightforward to verify that $H'$ satisfies the
  stated requirements.
\end{proof}
...
useful in establishing the existence of normal subgroups of $G$: since
all $p$-Sylow subgroups of a group are conjugates of each other (by
the second Sylow theorem), if there is only one $p$-Sylow subgroup,
then that subgroup must be normal.\footnote{For an alternative
  viewpoint, see \Cref{exer:groups2:2.2}.}
...
see for example \Cref{exer:groups2:2.11}.
...
\begin{fact}{}{fact:groups2:nonsimple-mpr}
  ...
  $1 < m < p$. Then $G$ is not simple. (Cf.\ \Cref{exam:groups2:mp-not-simple}.)
\end{fact}
\begin{proof}
...
of order $mp^r$, where $(m, p) = 1$ and the only divisor $d$ of $m$
...
\begin{proof}
  ...
  action of $G$ on $H$, hence (by \cref{exer:groups2:1.21}) a
  homomorphism $\gamma : G \to \Aut(H)$.  Now $H$ is
  cyclic of order $p$, so $|\Aut(H)| = p - 1$
  (\Cref{exer:groups:aut-cn-euler-phi}); the order of $\gamma(G)$ must divide both $pq$
  and $p - 1$, and it follows that $\gamma$ is the trivial map.
  Therefore, conjugation is trivial on $H$: that is,
  $H \subseteq Z(G)$.  \cref{lem:groups2:quotient-center-cyclic}
...
low-brow alternatives, see \Cref{exer:groups2:2.18} or \Cref{rem:groups2:5.4}.
...
semidirect products (\Cref{exer:groups2:5.12}). But we are in fact already in the
position of obtaining rather sophisticated information about this
group, even without knowing its construction in general
(\Cref{exer:groups2:2.19}).
...
\begin{proof}
  ...
  \[
    q \mid (r^2 - 1) = (r-1)(r+1)
  \]
  by \Cref{coro:groups:order-multiple}. Since $q$ is prime, this says that
  $q \mid (r-1)$ or $q \mid (r+1)$; since $0 \leq r \leq q-1$, it
  follows that $r = 1$ or $r = q - 1$.  If $r = 1$, then
  $xyx^{-1} = y$; that is, $xy = yx$. But then the order of $xy$ is
  $2q$ (by \Cref{exer:groups:order-product-coprime}), and $G$ is cyclic, a contradiction.
  ...
  $D_{2q}$, as the reader hopefully verified in \Cref{exer:groups:dihedral-generators}; the
  statement follows.
\end{proof}
...
\cref{exer:groups2:1.13}: every noncommutative group of order 6 is
isomorphic to $D_6 \cong S_3$.

\subsection*{Exercises}

\begin{exer}{}{exer:groups2:2.1}
  \exerused{} Prove \cref{fact:groups2:cauchy-cyclic-count}. [\S{}
ef{subsec:groups2:cauchy-s-theorem}]
\end{exer}
\begin{exer}{}{exer:groups2:2.2}
  ...
  \item Let $K$ be a normal subgroup of a finite group $G$, and assume
    ...
    characteristic in $G$.
  \end{itemize} [\S{}
ef{subsec:groups2:cauchy-s-theorem}, \S{}
ef{subsec:groups2:sylow-iii}, \ref{exer:groups2:2.13}, \S{}\ref{subsec:groups2:the-commutator-subgroup-derived-series-and-solvability}]
\end{exer}
...
\begin{exer}{}{exer:groups2:2.4}
  \exerused{} Prove that a nontrivial group $G$ is simple if and only
  if its only homomorphic images (i.e., groups $G'$ such that there is
  an onto homomorphism $G \to G'$) are the trivial group and $G$
  itself (up to isomorphism). [\S{}\ref{subsec:groups2:composition-factors-schreier-s-theorem}]
\end{exer}
...
\begin{exer}{}{exer:groups2:2.9}
  ...
  $N_G(P)$, so $PH$ is a subgroup of $N_G(P)$ by \Cref{prop:groups:second-isomorphism-theorem},
  and $|PH/P| = |H/(P \cap H)|$. Show that this implies that $PH$ is a
  $p$-group, and hence $PH = P$ since $P$ is a maximal $p$-subgroup of
  $G$. Deduce that $H \subseteq P$.) [\ref{exer:groups2:2.10}]
\end{exer}
\begin{exer}{}{exer:groups2:2.10}
  ...
  Use \Cref{exer:groups2:2.9}.) [\ref{exer:groups2:2.11}]
\end{exer}
\begin{exer}{}{exer:groups2:2.11}
  \exerused{} Use the second Sylow theorem,
  \cref{coro:groups2:conjugate-subgroups-divides-index}, and
  \Cref{exer:groups2:2.10} to paste together an alternative proof of the third
  Sylow theorem. [\S{}
ef{subsec:groups2:sylow-iii}]
\end{exer}
...
\begin{exer}{}{exer:groups2:2.13}
  ...
  \item Prove that if $P$ is normal in $G$, then it is in fact
    characteristic in $G$ (cf.\ \cref{exer:groups2:2.2}).
  ...
  \end{itemize} [\ref{exer:groups2:3.12}]
\end{exer}
...
\begin{exer}{}{exer:groups2:2.16}
  ...
  \end{itemize} [\ref{exer:groups2:6.2}, \ref{fields:exer:6.6}]
\end{exer}
\begin{exer}{}{exer:groups2:2.17}
  $\lnot$ Let $R$ be a division ring (\Cref{defn:rings:division-ring}), and assume
  $|R| = 64$. Prove that $R$ is necessarily commutative (hence, a
  field), as follows:
  \begin{itemize}
  ...
  \item Conclude that $G$ is contained in the center of $R$. Recall
    ...
    \Cref{exer:rings:center-is-subring}), and conclude that $R$ is commutative.
  \end{itemize}
  Like \Cref{exer:rings:division-ring-p2-commutative}, this is a particular case of a theorem of
  Wedderburn, according to which every finite division ring is a
  field. [\ref{fields:exer:5.16}]
\end{exer}
\begin{exer}{}{exer:groups2:2.18}
  ...
  nor $p$ nor $q$; deduce that $G$ is cyclic. [\S{}
ef{subsec:groups2:applications}]
\end{exer}
...
\begin{exer}{}{exer:groups2:2.22}
  Let $G$ be a finite group, $n = |G|$, and $p$ be a prime divisor of
  $n$.  Assume that the only divisor of $n$ that is congruent to $1$
  modulo $p$ is $1$. Prove that $G$ is not simple.
\end{exer}

\begin{exer}{}{exer:groups2:2.23}
  $\lnot$ Let $N_p$ denote the number of $p$-Sylow subgroups of a
  group $G$.  Prove that if $G$ is simple, then $|G|$ divides $N_p!$
  for all primes $p$ in the factorization of $|G|$. More generally,
  prove that if $G$ is simple and $H$ is a subgroup of $G$ of index
  $N > 1$, then $|G|$ divides $N!$. (Hint: \Cref{exer:groups:index-n-normal-subgroup-bound}.) This
  problem capitalizes on the idea behind \Cref{exam:groups2:no-simple-order-24}.  [\ref{exer:groups2:2.25}]
\end{exer}
...
\begin{exer}{}{exer:groups2:2.24}
  ...
  troublesome orders $> 60$.) [\S{}\ref{subsec:groups2:conjugacy-an-simplicity}]
\end{exer}
...
\begin{exer}{}{exer:groups2:2.25}
  ...
  \item Use Sylow's theorems and simple numerology to prove that $G$
    ...
    fifteen elements of order 2 or 4. (\cref{exer:groups2:2.23} will
    likely be helpful.)
  \item If there are fifteen 2-Sylow subgroups, prove that there
    exists an element $g \in G$ of order 2 contained in at least two
    of them. Prove that the centralizer of $g$ has index 5.
  \end{itemize}
  Conclude that every simple group\footnote{The reader will prove
    later (\Cref{exer:groups2:4.22}) that there is in fact only one simple group
    of order 60 up to isomorphism and that this group contains exactly
    five 2-Sylow subgroups. The result obtained here will be needed to
    establish this fact.} of order 60 contains a subgroup of index
  5. [\ref{exer:groups2:4.22}]
\end{exer}
...
(Exercise~\ref{exer:groups2:3.3}). It is also clear that if a normal
series has maximal length $\ell(G)$, then it is a composition
series. What is not clear is that the converse holds: conceivably,
there could exist composition series of different lengths (the longest
ones having length $\ell(G)$). For example, why can't there be a
finite group $G$ with $\ell(G) = 3$ and two different composition
series
\[
...
\begin{proof}
  ...
  normal in $G$ (Exercise~\ref{exer:groups2:3.5}), and
  ...
  \Cref{prop:groups:second-isomorphism-theorem} (the ``second isomorphism theorem''),
  \[
...
form a multiset\footnote{See~\S{}\ref{subsec:prelims:examples-multisets-indexed-sets} for a reminder on multisets:
  they are sets of elements counted with multiplicity.} of simple
groups: the `basic constituents' of our loose comment back in~\S{}\ref{sec:groups2:the-sylow-theorems}.
It is clear that two isomorphic groups must have the same composition
factors.  Unfortunately, it is not possible to reconstruct a group
from its composition factors alone
(Exercise~\ref{exer:groups2:3.4}). One has to take into account the
way the simple groups are `glued' together; we will come back to this
point in~\S{}\ref{subsec:groups2:exact-sequences-of-groups-extension-problem}.
...
\begin{proof}
  ...
  quotients, by \Cref{prop:groups:third-isomorphism-theorem} (the ``third isomorphism
  theorem''). Thus, if both $G/N$ and $N$ have composition series,
  juxtaposing them produces a composition series for $G$, with the
  stated consequence on composition factors.  The converse is a little
  trickier. Assume that $G$ has a composition series
  \[
    G = G_0 \supsetneq G_1 \supsetneq G_2 \supsetneq \dots \supsetneq
    G_n = \{e\}
  \]
  ...
  \Cref{thm:groups:quotient-universal-property}) there is an onto homomorphism
  \[
  ...
  (Exercise~\ref{exer:groups2:2.4}), as needed.  Summarizing, we have
  shown that if $G$ has a composition series and $N$ is normal in $G$,
  then both $N$ and $G/N$ have composition series. The first part of
  the argument yields the statement on lengths and composition
...
terms of the first appear in the second.
...
isomorphism theorem (cf.\ Exercise~\ref{exer:groups2:3.7}).
...
subgroup essentially immediate (cf.\ \Cref{exer:groups:commutator-subgroup-normal}):
...
\begin{proof}
  ...
  universal property of quotients (\Cref{thm:groups:quotient-universal-property}).
\end{proof}
...
\]
which is `normal' in the sense indicated in~\S{}\ref{subsec:groups2:the-jordan-h-older-theorem}.
...
(\S{}\ref{subsec:fields:solvability-of-polynomial-equations-by-radicals}).  But we can already appreciate it in the way it relates
to the material we just covered. A normal series is \emph{abelian},
resp., \emph{cyclic}, if all quotients are abelian, resp.,
...
\begin{coro}{}{coro:groups2:solvable-normal}
  Let $N$ be a normal subgroup of a group $G$. Then $G$ is solvable if
...
\begin{exer}{}{exer:groups2:3.3}
  ...
  series. [\S{}\ref{subsec:groups2:the-jordan-h-older-theorem}]
\end{exer}

\begin{exer}{}{exer:groups2:3.4}
  \exerused Find an example of two nonisomorphic groups with the same
  composition factors. [\S{}\ref{subsec:groups2:composition-factors-schreier-s-theorem}]
\end{exer}
\begin{exer}{}{exer:groups2:3.5}
  \exerused Show that if $H$, $K$ are \emph{normal} subgroups of a
  group $G$, then $HK$ is a normal subgroup of $G$. [\S{}\ref{subsec:groups2:the-jordan-h-older-theorem}]
\end{exer}
...
\begin{exer}{}{exer:groups2:3.7}
  ...
  theorem, using Schreier's. [\S{}\ref{subsec:groups2:composition-factors-schreier-s-theorem}]
\end{exer}

\begin{exer}{}{exer:groups2:3.8}
  \exerused Prove
  Lemma~\ref{lem:groups2:commutator-homomorphism}. [\S{}\ref{subsec:groups2:the-commutator-subgroup-derived-series-and-solvability}]
\end{exer}
...
\begin{exer}{}{exer:groups2:3.10}
  ...
  subgroup of $G$ corresponding (as in \Cref{prop:groups:subgroups-of-quotients-bijection}) to the
  center of $G/Z_{i-1}$.
  \begin{itemize}
  ...
  \end{itemize} [\ref{exer:groups2:3.11}, \ref{exer:groups2:3.12},
  \ref{exer:groups2:5.1}]
\end{exer}
\begin{exer}{}{exer:groups2:3.11}
  $\lnot$ Let $H$ be a nontrivial normal subgroup of a nilpotent group
  $G$ (cf.\ Exercise~\ref{exer:groups2:3.10}). Prove that $H$
  intersects $Z(G)$ nontrivially. (Hint: Let $r \geq 1$ be the
  smallest index such that $\exists h \neq e$, $h \in H \cap
  Z_r$. Contemplate a well-chosen commutator $[g,h]$.) Since
  $p$-groups are nilpotent, this strengthens the result of
  Exercise~\ref{exer:groups2:1.9}.  [\ref{exer:groups2:3.14}]
\end{exer}
\begin{exer}{}{exer:groups2:3.12}
  Let $H$ be a proper subgroup of a finite nilpotent group $G$ (cf.\
  Exercise~\ref{exer:groups2:3.10}). Prove that $H \subsetneq
  N_G(H)$. (Hint: $Z(G)$ is nontrivial. First dispose of the case in
  which $H$ does not contain $Z(G)$, and then use induction to deal
  with the case in which $H$ does contain $Z(G)$.) Deduce that every
  Sylow subgroup of a finite nilpotent group is normal\footnote{This
    property characterizes finite nilpotent groups; cf.\
...
\begin{exer}{}{exer:groups2:3.13}
  ...
  commutator, as in~\S{}\ref{subsec:groups2:the-commutator-subgroup-derived-series-and-solvability}. Prove that each $G^{(i)}$ is
  characteristic (hence normal) in $G$. [\ref{exer:groups2:3.14}]
\end{exer}
\begin{exer}{}{exer:groups2:3.14}
  ...
  \item Prove that $H$ contains a nontrivial commutative subgroup that
    ...
    and use Exercise~\ref{exer:groups2:3.13} to show it is normal in
    $G$.)
  \item Find an example showing that $H$ need not intersect the center
    of $G$ nontrivially (cf.\ Exercise~\ref{exer:groups2:3.11}).
  \end{itemize}
\end{exer}
...
\begin{exer}{}{exer:groups2:3.16}
  ...
  solvable.  [\S{}\ref{subsec:groups2:conjugacy-an-simplicity}, \S{}\ref{subsec:fields:solvability-of-polynomial-equations-by-radicals}]
\end{exer}
...
$\categ{Set}$) of the set $\{1,\dots,n\}$. In~\S{}\ref{sec:groups:examples-of-groups} we denoted
...
$\mathbf{8}$, $\mathbf{2}$ to $\mathbf{1}$, etc.
...
maximal enlightenment, try to do Exercise~\ref{exer:groups2:4.1} now,
and then try again after absorbing the material in~\S{}\ref{subsec:groups2:type-conjugacy-sn}. In short,
we should be able to do better.  As often is the case, thinking in
terms of actions helps. By its very definition, the group $S_n$ acts
on the set $\{1,\dots,n\}$; so does every subgroup of $S_n$. Given a
permutation $\sigma \in S_n$, consider the cyclic group
$\langle\sigma\rangle$ generated by $\sigma$ and its action on
$\{1,\dots,n\}$. The orbits of this action form a partition of
$\{1,\dots,n\}$; therefore, every $\sigma \in S_n$ determines a
partition of $\{1,\dots,n\}$. For example, the element
$\sigma \in S_8$ given above splits $\{1,\dots,8\}$ into three orbits:
\[
...
agreed in~\S{}\ref{subsec:groups:symmetric-groups} that we would let our permutations act on the
right, for consistency with the usual notation for products in groups.
\begin{proof}
...
that is, the class formula (cf.~\S{}\ref{subsec:groups2:center-centralizer-conjugacy-classes}) for $S_5$:
\[
  120 = 1 + 10 + 15 + 20 + 20 + 30 + 24.
\]
\begin{exam}{}{exam:groups2:4.9}
  There are no normal subgroups of size $30$ in $S_5$. Indeed, normal
  subgroups are unions of conjugacy classes (\S{}\ref{subsec:groups2:the-class-formula}); since the
  identity is in every subgroup and $30 - 1 = 29$ cannot be written as
  a sum of the numbers appearing in the class formula for $S_5$, there
  is no such subgroup.
\end{exam}

This observation will be dwarfed by much stronger results that we will
prove soon (such as Theorem~\ref{thm:groups2:4.20},
Corollary~\ref{coro:groups2:4.21}); but it is remarkable that such
...
\begin{proof}
  ...
  $Z_{A_n}(\sigma) = A_n \cap Z_{S_n}(\sigma)$: this follows
  ...
  \Cref{prop:groups:second-isomorphism-theorem}), and it properly contains $A_n$, so it must
  equal $S_n$ as $A_n$ has index $2$ in $S_n$. By index considerations
  (cf.\ \Cref{exer:groups:hk-cardinality-formula})
...
\begin{exam}{}{exam:groups2:4.16}
  Looking again at $A_5$, we have noted in~\S{}\ref{subsec:groups2:transpositions-parity-and-the-alternating-group} that the types of
  the even permutations in $S_5$ are $[1,1,1,1,1]$, $[2,2,1]$,
  $[3,1,1]$, and $[5]$.  By
...
(Exercise~\ref{exer:groups2:2.24}); and we now see why $60$ is
special:
...
(Exercise~\ref{exer:groups2:4.21}). It is in fact the case that all
groups $A_n$, $n \geq 5$, are simple (and noncommutative), implying
that $S_n$ is not solvable for $n \geq 5$, and these facts are rather
important in view of applications to Galois theory (cf., e.g.,
\Cref{coro:fields:7.16}). Note that $A_2$ is trivial, $A_3 \cong \Z/3\Z$ is
simple and abelian, and $A_4$ is not simple
(Exercise~\ref{exer:groups2:2.24}).
...
\begin{proof}
  ...
  (Exercise~\ref{exer:groups2:4.14}) and $3$-cycles generate $A_n$, we
  may assume that $\tau$ and $\sigma$ do not commute, that is, the
  commutator
  ...
  (Exercise~\ref{exer:groups2:4.21}), this implies $N \cap A_6 = A_6$.
...
\begin{exer}{}{exer:groups2:4.1}
  ...
  \]
  in $S_8$. [\S{}
ef{subsec:groups2:cycle-notation}]
\end{exer}
...
\begin{exer}{}{exer:groups2:4.7}
  ...
  [\cref{exer:groups2:4.9}, \S{}\ref{subsec:fields:galois-groups-of-polynomials}]
\end{exer}
...
\begin{exer}{}{exer:groups2:4.9}
  ...
  subgroups. For all $n \geq 3$ prove that $S_n$ contains a copy of
  the dihedral group $D_{2n}$, and find generators for it.
\end{exer}
...
\begin{exer}{}{exer:groups2:4.11}
  ...
  subgroups of $S_p$. (Use \cref{exer:groups2:4.10}.) Use this result
  and Sylow's third theorem to prove again the `only if' implication
  in Wilson's theorem (cf.\ \Cref{exer:groups:wilsons-theorem}).
\end{exer}
\begin{exer}{}{exer:groups2:4.12}
  ...
  \end{itemize} [\S{}\ref{subsec:fields:galois-groups-of-polynomials}]
\end{exer}
\begin{exer}{}{exer:groups2:4.13}
  ...
  the determinant of the matrix $M_\sigma$ defined in \Cref{exer:groups:permutation-matrix}.
\end{exer}
\begin{exer}{}{exer:groups2:4.14}
  \exerused{} Prove that the center of $A_n$ is trivial for
  $n \geq 4$.  [\S{}\ref{subsec:groups2:conjugacy-an-simplicity}]
\end{exer}
\begin{exer}{}{exer:groups2:4.15}
  Justify the `pictorial' recipe given in \S{}\ref{subsec:groups2:transpositions-parity-and-the-alternating-group} to decide whether a
  permutation is even.
\end{exer}
...
\begin{exer}{}{exer:groups2:4.19}
  ...
  on any set $S$ with $|S| < n$. Construct\footnote{You can think
    ...
    of opposite sides on a tetrahedron.} a nontrivial action of $A_4$
...
\begin{exer}{}{exer:groups2:4.21}
  ...
  \cref{coro:groups2:4.17}). [\S{}\ref{subsec:groups2:conjugacy-an-simplicity}]
\end{exer}
\begin{exer}{}{exer:groups2:4.22}
  $\lnot$ Verify that $A_5$ is the \emph{only} simple group of order
  $60$, up to isomorphism. (Hint: By \Cref{exer:groups2:2.25}, a simple group $G$
  ...
  possibility contemplated in \Cref{exer:groups2:2.25} does not occur. [\ref{exer:groups2:2.25}]
\end{exer}

\section{Products of groups}
\label{sec:groups2:products-of-groups}

We already know that products exist in \textsf{Grp} (see~\S{}\ref{subsec:groups:products-et-al});
...
Recall from~\S{}\ref{subsec:groups:products-et-al} that the \emph{(direct) product} of two groups
$H$, $K$ is the group supported on the set $H \times K$, with
operation defined componentwise. We have checked (\Cref{prop:groups:direct-product-universal})
that the direct product satisfies the universal property defining
products in the category \textsf{Grp}.  There are situations in which
the direct product of two subgroups $N$, $H$ of a group $G$ may be
realized as a subgroup of $G$. Recall (\Cref{prop:groups:second-isomorphism-theorem}) that if
one of the subgroups is normal, then the subset $NH$ of $G$ is in fact
a subgroup of $G$. The relation between $NH$ and $N \times H$ depends
on how $N$ and $H$ intersect in $G$, so we take a look at this
intersection.  The `commutator' $[A, B]$ of two subsets $A$, $B$ of
$G$ (see~\S{}\ref{subsec:groups2:the-commutator-subgroup-derived-series-and-solvability}) is the subgroup generated by all commutators
...
\begin{rem}{}{rem:groups2:5.4}
  ...
  \Cref{fact:groups2:order-pq-cyclic}: if $|G| = pq$, with $p < q$ prime integers, and $G$
...
in~\S{}\ref{subsec:rings:complexes-and-exact-sequences}. A (short) exact sequence of groups is a sequence of
...
$\Z$-modules) from \S{}\ref{subsec:rings:complexes-and-exact-sequences}; a notational difference is that here
the trivial group is denoted\footnote{This is not unreasonable, since
  groups are more often written `multiplicatively' rather than
  additively, so the identity element is more likely to be denoted $1$
  rather than $0$.}  `$1$' rather than `$0$'.
...
\begin{defn}{}{defn:groups2:5.6}
  ...
  \emph{split} if $H$ may be identified with a subgroup of $G$, so
  that $N \cap H = \{e\}$.
\end{defn}

We encountered this terminology in~\S{}\ref{subsec:rings:split-exact-sequences} for modules, thus for
abelian groups. Note that the notion examined there appears to be more
restrictive than \cref{defn:groups2:5.6}, since it requires $G$ to be
isomorphic to a direct product $N \times H$. This apparent mismatch
evaporates because of \cref{prop:groups2:5.3}: in the abelian case,
every split extension (according to \cref{defn:groups2:5.6}) is in
...
\cref{prop:groups2:5.3} is really \cref{coro:groups2:5.2}: if both $N$
...
\Cref{exer:groups2:1.21}) conjugation determines a homomorphism
\[\gamma : H \to \Aut_{\textsf{Grp}}(N), \quad
...
trivial. \Cref{coro:groups2:5.2} shows that if $N$ and $H$ are both
...
$\gamma : H \to \Aut_{\textsf{Grp}}(N)$. The reader is
...
\[\theta : H \to \Aut_{\textsf{Grp}}(N), \quad
...
\begin{prop}{}{prop:groups2:5.10}
  ...
  $\theta : H \to \Aut_{\textsf{Grp}}(N)$ be a
...
\begin{prop}{}{prop:groups2:5.11}
  ...
  $\gamma : H \to \Aut_{\textsf{Grp}}(N)$ be defined by
...
\begin{exam}{}{exam:groups2:5.13}
  ...
  are
  \[
    \id : \begin{cases} e \mapsto e, \\ y \mapsto y, \\ y^2
      \mapsto y^2,
    \end{cases}
  ...
  $C_2 \to \Aut_{\textsf{Grp}}(C_3)$: the trivial map,
  and the isomorphism sending the identity to $\id$ and the
  nonidentity element to $\sigma$. The semidirect product
  corresponding to the trivial map is the direct product
  $C_3 \times C_2 \cong C_6$; the other semidirect product
  $C_3 \rtimes C_2$ is isomorphic to $S_3$. This can of course be
  checked by hand (and you should do it, for fun); but it also follows
  immediately from \cref{prop:groups2:5.11}, since
  $N = \langle(123)\rangle$, $H = \langle(12)\rangle \subseteq S_3$
  satisfy the hypotheses of this result.
\end{exam}
The reader should contemplate carefully the slightly more general case
of dihedral groups (\cref{exer:groups2:5.11}); this enriches (and in a
sense explains) the discussion presented in \Cref{fact:groups2:order-2q-dihedral}.

In fact, semidirect products shed light on all groups of order $pq$,
for $p < q$ primes; the reader should be able to complete the
classification of these groups begun in \Cref{fact:groups2:order-pq-cyclic} and show that if
...
$\Aut_{\textsf{Grp}}(N)$ and can be carried out. See
\cref{exer:groups2:5.15} for a further illustration of this technique.
\begin{exer}{}{exer:groups2:5.1}
  ...
  normal. [\cref{exer:groups2:3.12}, \S{}\ref{subsec:groups2:classification-of-finite-abelian-groups}]
\end{exer}
...
\begin{exer}{}{exer:groups2:5.4}
  ...
  \[0 \longrightarrow \Z \xrightarrow{\cdot 2} \Z \longrightarrow
    \Z/2\Z \longrightarrow 0\] is exact but does not split. [\S{}\ref{subsec:groups2:exact-sequences-of-groups-extension-problem}]
\end{exer}
\begin{exer}{}{exer:groups2:5.5}
  In \Cref{prop:rings:split-sequence} we have seen that if an exact sequence
...
\begin{exer}{}{exer:groups2:5.9}
  ...
  $G \cong N \times H$. [\S{}\ref{subsec:groups2:classification-of-finite-abelian-groups}]
\end{exer}
...
\begin{exer}{}{exer:groups2:5.11}
  ...
  $C_n \rtimes_\theta C_2$, finding $\theta$ explicitly. [\S{}\ref{subsec:groups2:internal-semidirect-products}]
\end{exer}
\begin{exer}{}{exer:groups2:5.12}
  ...
  $\Aut_{\textsf{Grp}}(C_q) \cong C_{q-1}$ if $q$ is
  prime; cf.\ \Cref{exer:groups:aut-z-aut-cp}.) [\S{}
ef{subsec:groups2:applications}, \S{}\ref{subsec:groups2:internal-semidirect-products}]
\end{exer}
\begin{exer}{}{exer:groups2:5.13}
  ...
  $H$. [\cref{exer:groups2:5.14}]
\end{exer}
\begin{exer}{}{exer:groups2:5.14}
  Recall that
  $S_3 \cong \Aut_{\textsf{Grp}}(C_2 \times C_2)$
  (\Cref{exer:groups:aut-c2xc2-iso-s3}). Let $\iota$ be this isomorphism. Prove that
...
\begin{exer}{}{exer:groups2:5.15}
  ...
  \item Recall (or prove again) that, up to isomorphism, the only
    ...
    $C_4 \to \Aut_{\textsf{Grp}}(N)$ and two
    homomorphisms
    $C_2 \times C_2 \to \Aut_{\textsf{Grp}} (N)$ up to
    the choice of generators for the sources.
  ...
  \item Prove that $D_{28} \cong C_2 \times D_{14}$. The other
    noncommutative group of order $28$ is a \emph{generalized
      quaternionic group}.
  \end{itemize} [\S{}\ref{subsec:groups2:internal-semidirect-products}]
\end{exer}
\begin{exer}{}{exer:groups2:5.16}
  Prove that the quaternionic group $Q_8$ (cf.\ \Cref{exer:rings:quaternions})
...
\begin{exer}{}{exer:groups2:5.17}
  ...
  quaternions (cf.\ \Cref{exer:rings:quaternions}) is isomorphic to a semidirect
  product $\mathrm{SU}(2) \rtimes \R^+$. (Hint: \Cref{exer:rings:quaternion-norm}.) Is
  this semidirect product in fact direct?
\end{exer}
...
theorem for finite abelian groups mentioned in~\S{}\ref{subsec:groups:example-subgroup-generated-by-a-subset}.
...
position to classify all finite \emph{abelian} groups.\footnote{Of
  ...
  \cref{exer:groups2:5.9}.}

In due time (\Cref{linalg:2.11}, \Cref{exer:linalg:2.19}) we will in fact be
able to classify all finitely generated abelian groups: as mentioned
in UNCERTAIN:Example II.6.3, all such groups are products of cyclic
...
least as far back as\footnote{In fact, this observation will really
  ...
  Remainder Theorem, \Cref{thm:irred:6.1}.} \Cref{exer:groups:cmn-iso-cm-times-cn}.
...
\end{lem}
\begin{proof}
  By Lagrange's theorem (\Cref{coro:groups:lagrange}), $H \cap K = \{0\}$. Since
...
$p$-groups for different primes $p$, \cref{lem:groups2:6.1}
...
abelian groups are nilpotent; cf.\ \cref{exer:groups2:5.1}.)  Thus, we
already know that every finite abelian group is a direct sum of
...
$G \cong \langle g \rangle \oplus L$, by \cref{prop:groups2:5.3}.  The
...
\end{lem}
\cref{lem:groups2:6.4} is a special case of \cref{lem:groups2:6.3} in
...
\begin{proof}
  ...
  element of $P$ of maximal order. By \cref{lem:groups2:6.3}
...
(\cref{exer:groups2:6.1}).  The prime powers appearing in the first
form of \cref{thm:groups2:6.6} are called the \emph{elementary
  divisors} of $G$; the integers $d_i$ appearing in the second form
are called \emph{invariant factors}. To go from elementary divisors to
invariant factors, collect the elementary divisors in a table, listing
(for example) prime powers according to increasing primes in the
horizontal direction and decreasing exponents in the vertical
direction; then the invariant factors are obtained as products of the
factors in each row:
\[
...
in \cref{thm:groups2:6.6} are indeed equivalent. This will likely be
...
$s = 1$; that is, $G$ is cyclic.  \cref{lem:groups2:6.9} is the key to
...
of which\footnote{The diligent reader has proved that particular case
  in \Cref{exer:groups:zpz-star-cyclic}. The proof hinted at in that exercise upgrades
  easily to the general case presented here. The point is not that the
...
\Cref{exam:groups:cyclic-formal-intro}. Recall that the set $F^*$ of nonzero elements of a
field $F$ is a commutative group under multiplication. Also recall
(\Cref{exam:rings:remainder-degree-1}) that a polynomial $f(x) \in F[x]$ is divisible by
...
shows\footnote{Unique factorization in $F[x]$ is secretly needed
  ...
  \Cref{lem:irred:5.1}.} that if $f(x) \in F[x]$ has degree $n$, then
...
\Cref{exam:groups:cyclic-formal-intro}.  \emph{Preview of coming attractions:} Finitely
generated (as opposed to just finite) abelian groups are also direct
sums of cyclic groups.  The only difference between the classification
of finitely generated abelian groups and the classification of finite
abelian groups explored here is the possible presence of a `free'
factor $\Z^{\oplus r}$ in the decomposition. The reader will first
prove this fact in VI.2.19, as a consequence of `Gaussian
...
modules over PIDs, \Cref{linalg:5.6}. Neither Gaussian elimination nor
the very general \Cref{linalg:5.6} are any harder to prove than the
...
point of view is that, as a rule, proofs simplify. Technical work such
as that performed in order to prove \cref{lem:groups2:6.3} is absorbed
into the work necessary to build up the more general apparatus; the
need for such technicalities evaporates in the process.
...
\begin{exer}{}{exer:groups2:6.1}
  ...
  [\S{}\ref{subsec:groups2:invariant-factors-and-elementary-divisors}, \S{}\ref{subsec:linalg:the-classification-theorem}, \ref{exer:linalg:5.12}]
\end{exer}
\begin{exer}{}{exer:groups2:6.2}
  Complete the classification of groups of order $8$ (cf.\
  \cref{exer:groups2:2.16}).
\end{exer}
...
\begin{exer}{}{exer:groups2:6.6}
  ...
  there, up to isomorphism? [\S{}\ref{subsec:groups:example-subgroup-generated-by-a-subset}]
\end{exer}
\begin{exer}{}{exer:groups2:6.7}
  ...
  \item Prove that $p\ker\rho$ and $p(\coker\rho)$ are
    both $0$.
  ...
  \end{itemize} [\cref{exer:groups2:6.8}]
\end{exer}
\begin{exer}{}{exer:groups2:6.8}
  ...
  establish $s \leq r$; this also proves the statement if all
  $n_i = 1$.  For the general case use induction, noting that if
...
\begin{exer}{}{exer:groups2:6.10}
  ...
  $G^\vee := \Hom_{\textsf{Grp}}(G, \mathbb{C}^*)$, where
  ...
  \item Prove that if $G$ is a finite abelian group, then
    ...
    case.)
  \end{itemize}
  In~\S{}\ref{subsec:linrep:ext-Z-G-Z} we will encounter another notion of `dual' of a
  group.
\end{exer}
\begin{exer}{}{exer:groups2:6.11}
  ...
  \item Prove that if $p$ is prime, all finite modules over $\Z/p\Z$
    are free.\footnote{As we will see in \Cref{linalg:4.10}, this
...
\begin{exer}{}{exer:groups2:6.13}
  ...
  elements of order $2$, and $12$ elements of order $4$.) [\S{}\ref{subsec:groups:isomorphisms}]
\end{exer}
\begin{exer}{}{exer:groups2:6.14}
  ...
  sense a converse to \Cref{prop:groups:subgroups-of-znz}. You are welcome to try to
