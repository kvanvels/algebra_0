...
Using indexed sets in \cref{defn:linalg:1.1} takes care of obvious
...
Note that the singleton $\{2\} \subseteq \mathbb{Z}$ is a `maximal
linearly independent subset' of $\mathbb{Z}$, but it does not generate
$\mathbb{Z}$. In general this is an additional requirement, leading to the
...
\begin{proof}
This is immediate from \cref{defn:linalg:1.1}: if $B \subseteq M$ is
...
\end{proof}
By \cref{lem:linalg:1.5}, the choice of a basis $B$ of a free module $M$
...
\cref{lem:linalg:1.5} is all that is needed to prove the fundamental
...
By \cref{lem:linalg:1.5}, proving that vector spaces are free modules
amounts to proving that they admit bases; \cref{lem:linalg:1.2} reduces
...
a maximal linearly independent subset of $\mathbb{Z}$, but it is not a
basis.
\begin{proof}
...
\begin{proof}
Put \cref{lem:linalg:1.2}, \cref{lem:linalg:1.5}, and \cref{lem:linalg:1.6}
together.
\end{proof}
...
\cref{lem:linalg:1.8} also fails on more general rings (Exercise~1.5). To
