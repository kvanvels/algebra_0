...
about Tor and Ext stated in Chapter~VIII will be recovered as very
...
(co)homology': what information in a complex (see \S{}III.7) is really
...
\item It may make no sense to talk about kernels and cokernels in a
  ...
  modules over a ring; cf.\ Example~III.6.5).
\item Even when there are kernels and cokernels, monomorphisms need
  ...
  $\Z \to \Q$ is an epimorphism in $\categ{Ring}$; see \S{}III.2.3).
...
$\operatorname{coker}\varphi$.  We have to get used to the fact that
...
(Proposition~I.5.4); it is common to harmlessly abuse language and
...
epimorphism is the appropriate replacement. Recall (\S{}I.2.6) that
...
\begin{proof}
  ...
  $(\gamma \circ \operatorname{coker}\varphi) \circ \varphi$ is $0$;
  by definition of cokernel,
  $\gamma \circ \operatorname{coker}\varphi$ factors uniquely through
  $C$:
  \[\begin{tikzcd}
      A \arrow[r, "\varphi"] & B \arrow[r, "\gamma \circ
      \operatorname{coker}\varphi = 0"]
      \arrow[d, "\operatorname{coker}\varphi"'] & Z \\
      & C \arrow[ur, dashed, "\gamma"', "\exists!"']
    \end{tikzcd}\] Since
  $\gamma \circ \operatorname{coker}\varphi = 0 = 0 \circ
...
\begin{proof}
  ...
  \[\begin{tikzcd}
      & A & & \\
      0 \arrow[r] & A \arrow[u, "\mathrm{id}"] \arrow[r, "\varphi",
...
Exercise~III.5.12. Such set-theoretic arguments are not an option in
...
straightforward than what follows (Exercise~1.22). Here I will prove
...
\item $\pi_B : A \sqcup B \to B$ is the cokernel of
  ...
  $\pi_B \circ i_B = \mathrm{id}_B$.  We have to show that $\gamma$
  ...
  \item $(\gamma \circ i_B) \circ \pi_B \circ i_B = \gamma \circ i_B$
    (since $\pi_B \circ i_B = \mathrm{id}_B$).
  \end{itemize}
...
\item The morphism $(\pi_A, \pi_B) : A \sqcup B \to A \times B$
  ...
  composition $K \to B \xrightarrow{\mathrm{id}} B$. That is,
...
structure.\footnote{However, we will soon recover a sense in which
  ...
  terms: see Lemma~2.7.} But look at your mental image of what the
...
\begin{center}
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    ...
    \node at (0,1.4) {$A$}; \node at (2.6,1.4) {$B$}; \node at
    (2.6,-1.4) {$\operatorname{im}\varphi$};
  \end{tikzpicture}
\end{center}
...
\begin{proof}
  ...
  universal property of $\operatorname{coker}\varphi$, we have an
  induced map:
  \[\begin{tikzcd}
      & L \arrow[dr, "\lambda"] & & \\
      A \arrow[ur] \arrow[r] & K \arrow[r, hook] & B \arrow[r,
      "\operatorname{coker}\varphi"] \arrow[dr,
  ...
  \[
    K \to B \xrightarrow{\operatorname{coker}\lambda}
    \operatorname{Cok}\lambda
  \]
...
\begin{defn}{}{homol:1.15}
  ...
  $\ker(\operatorname{coker}\varphi)$. The \emph{coimage} of
...
\begin{exer}{$\lnot$}{exer:homol:1.3}
  ...
  functor' between preadditive categories makes sense.  [1.12]
\end{exer}
...
\begin{exer}{}{exer:homol:1.12}
  ...
  a single object determined by $R$ (Exercise~1.3). Prove that the
  ...
  category $\categ{Ab}^{\categ{R}}$ (cf.\ Exercise~1.11). What about
...
\begin{exer}{}{exer:homol:1.14}
  ...
  (Example~VIII.1.5). Define the \emph{category} of $\categ{A}$-valued
  presheaves on $T$. Prove that presheaves on $T$ with values in an
  abelian category form an abelian category. (Hint: Exercise~1.11.)
\end{exer}
\begin{exer}{}{exer:homol:1.15}
  ...
  presheaves\footnote{Watch out! Both presheaves $\mathscr{C}$,
    ...
    Exercise~VIII.1.6. Sheaves of abelian groups form a full
    ...
    particular case of Exercise~1.11), but the example given above
...
\begin{exer}{}{exer:homol:1.20}
  ...
  Lemma~\ref{homol:1.16} was used in its proof.) [\S{}1.5]
\end{exer}
...
through
$\ker(\operatorname{coker}\varphi) = \operatorname{im}\varphi$.  This
implies that $\operatorname{im}\varphi$ and $\ker\psi$ coincide.  The
...
for fibered coproducts (Exercise~2.2).  Going back to the snake lemma,
...
morphism\footnote{To avoid unbearably heavy notation, I will write
  ...
  etc.}  $\delta : \ker\nu \to \operatorname{coker}\lambda$; once this
...
$\ker\beta_1'$ have matching sources by Exercise~1.16. Analogous
...
\[\begin{tikzcd}[column sep=small]
    ...
    & (\operatorname{coker}\lambda) \amalg_{L_0} M_0 \arrow[r,
    "\tau"'] & N_0 \arrow[r] & 0
...
through $\ker\nu$, giving a morphism
\[
  \epsilon' : \ker\nu \to (\operatorname{coker}\lambda) \amalg_{L_0}
  M_0.
\]
...
$\operatorname{coker}\lambda$, finally yielding
\[
...
\[\begin{tikzcd}[column sep=small]
    ...
    \arrow[r, "\delta"] & \operatorname{coker}\lambda \arrow[r] &
    \operatorname{coker}\mu \arrow[r] & \operatorname{coker}\nu
    \arrow[r] & 0
...
from the diagrammatic point of view. Exercise~2.12 gives another
...
preliminary note, recall that in the distant past (Example~I.3.8) we
...
reader should check carefully (Exercise~2.5).  Now I want to make the
...
$0$, which we can do\footnote{Note that I am not saying that
  ...
  Exercise~2.10).} at the level of pointed sets (hence, by chasing
...
\begin{proof}
  ...
  $\operatorname{id}_B$. This `element' is represented by a morphism
...
\begin{lem}{}{homol:2.7}
  ...
  $\operatorname{im}\varphi : I \to B$, be the kernel and the image of
  $\varphi$, respectively. Then
  \begin{itemize}
  \item $\widehat{\ker\varphi}$ identifies $\hat{K}$ with the subset
    $\hat{\varphi}^{-1}(0)$ of $\hat{A}$;
  \item $\widehat{\operatorname{im}\varphi}$ identifies $\hat{I}$ with
...
\begin{proof}
  ...
  \[\begin{tikzcd}
      ...
      "\operatorname{im}\varphi", hook] & B,
    \end{tikzcd}\] and that $\operatorname{im}\varphi : I \to B$ is a
  kernel of $\operatorname{coker}\varphi$ (this is the definition of
  ...
  \[\begin{tikzcd}
      ...
      "\operatorname{coker}\varphi"] & \operatorname{Cok}
...
\begin{proof}
  ...
  in $\categ{A}$ means that $\operatorname{im}\varphi = \ker\psi$, and
...
Definition~VIII.1.6. Briefly, a functor $F : \categ{A} \to \categ{C}$
...
exactness (cf.\ Example~VIII.1.18) of the functor
...
\begin{exer}{}{exer:homol:2.6}
  ...
  `elements'. (Cf.\ Lemma~\ref{homol:2.6}.) [\S{}2.3]
\end{exer}
...
\begin{exer}{$\lnot$}{exer:homol:2.8}
  ...
  $-\operatorname{id}_A : A \to A$ determine the same `element' of $A$
  in the sense of \S{}2.3. [2.10]
\end{exer}
...
\begin{exer}{}{exer:homol:2.11}
  Upgrade the result of Exercise~2.4, by proving the
  \emph{five-lemma}: if
  \[\begin{tikzcd}
...
\begin{exer}{}{exer:homol:2.13}
  ...
  \End_{\R\text{-}\categ{Vect}}(\R)$.  But look at Exercise~VI.1.13.)
\end{exer}
...
\begin{exer}{}{exer:homol:2.17}
  ...
  thus, Exercise~2.14 doesn't help), and the Yoneda embedding of
...
of an application. In \S{}III.7 I chose homology; in this chapter I
...
resolution (Exercise~3.1). Note that the datum of a bounded-above
...
resolutions, in \S{}VI.4.2.  The complex $\iota(A)$,
\[
...
have in fact claimed (in \S{}VIII.6.4) that resolutions by
...
\begin{itemize}
\item
  $\operatorname{Obj}(\categ{C}(\categ{A})) = \{\text{cochain
...
\begin{proof}
  ...
  $\operatorname{coker}\alpha^i : N^i \to C^i$ give morphisms of
  complexes $\ker\alpha^\bullet$, $\operatorname{coker}\alpha^\bullet$
...
defined by setting\footnote{The sign in the differentials is inserted
  ...
  change the isomorphism class of the complex (Exercise~3.5).}
...
\S{}III.7.3 and can be summarized (cohomologically speaking) as
follows:
\begin{itemize}
...
\item Set $\delta(n) = $ the class of $\ell$ in
  $H^1(L^\bullet) = \operatorname{coker}(\lambda^0)$.
\end{itemize}
Pictorially,
...
absorbed into the snake lemma itself (cf.\ Exercise~3.10); but I have
...
$\mu^i(m) \in \operatorname{im}\mu^i$. The image of $\mu^i(m)$ in
...
\]
On the other hand, each $H^i$ is a functor (Lemma~3.4); thus there are
natural morphisms
\[
...
\begin{proof}
  ...
  for the reader (Exercise~3.9). But I will stress the extent to which
...
\begin{exer}{$\lnot$}{exer:homol:3.2}
  ...
  and is $\emptyset$ otherwise (cf.\ Example~I.3.2). Show that the
  category $\categ{C}(\categ{A})$ of cochain complexes is a full
  subcategory of the abelian category $\categ{A}^{\categ{I}}$. [3.3]
\end{exer}
...
\begin{exer}{$\lnot$}{exer:homol:3.4}
  ...
  \item Show that, for every object $X$ of $\categ{A}$,
    \[\begin{tikzcd}[column sep=small]
        0 \arrow[r] & 0 \arrow[r] & X \arrow[r, "\operatorname{id}_X"]
        \arrow[d, "\operatorname{id}_X"'] &
        X \arrow[r] & 0 \\
        0 \arrow[r] & X \arrow[r, "\operatorname{id}_X"'] & X
...
\begin{exer}{\exerused{}}{exer:homol:3.10}
  ...
  $\ker^\bullet$, with the object\footnote{Of course $\ker$ and
    ...
    $\ker$ and $\operatorname{coker}$, respectively.}
  $\ker\lambda^{i+1}$ in degree $i$, connected by zero-morphisms, and
  a complex $\operatorname{coker}^\bullet$, with the object
...
\begin{exer}{$\lnot$}{exer:homol:3.11}
  ...
  (cf.\ Exercise~3.4) of complexes to the category of
  complexes. [7.13]
\end{exer}
...
\begin{exer}{$\lnot$}{exer:homol:3.15}
  ...
  construction given in \S{}VI.3.4.
...
that $MC(\alpha)^\bullet$ is indeed a cochain complex (Exercise~4.1);
...
By Corollary~4.2, this condition is equivalent to the exactness of the
mapping cone of $\alpha^\bullet$.

\begin{exam}{}{homol:4.4}
  The datum of a resolution $M^\bullet$ of an object $A$ of an abelian
  category $\categ{A}$, as in Definition~3.2 and with $M^i = 0$ for
...
\begin{rem}{}{homol:4.5}
  ...
  as in Example~4.4, form a category.
\end{rem}
...
\begin{proof}
  ...
  that $\operatorname{id}_{\mathscr{F}(M^\bullet)} = 0$ and hence that
...
dimensions up, in Definition~4.8 it shifts degrees down.
...
Corollary~4.2 the cone of $\alpha^\bullet$ must be exact. It is a good
exercise to verify this fact directly (Exercise~4.8).
...
\]
is an additive functor (Definition~1.1). Then $\mathscr{F}$ induces an
...
\begin{exer}{\exerused{}}{exer:homol:4.6}
  ...
  \end{enumerate} [\S{}4.3, \S{}5.2, \S{}5.4, \S{}5.5]
\end{exer}
...
\begin{exer}{}{exer:homol:4.10}
  ...
  object leads to the algebraic mapping cylinder of Exercise~4.9.
  ...
  homotopic in the sense of Definition~4.8 if and only if they extend
  to a cochain morphism
  $(-\alpha^\bullet, h^\bullet, \beta^\bullet) :
  MCyl(\operatorname{id}_{L^\bullet}) \to M^\bullet$.
\end{exer}
...
\begin{exer}{$\lnot$}{exer:homol:4.13}
  ...
  morphisms. Note that
  $\sigma^\bullet \rho^\bullet = \operatorname{id}_{M^\bullet}$, and
...
applications. This is the moral of Theorem~4.14: homotopy equivalent
...
\begin{proof}
  ...
  \[
    L^i \to MC(\operatorname{id}_{L^\bullet})^i, \qquad b \mapsto (0,
    b)
  \]
...
$\alpha^\bullet \circ \beta^\bullet = \operatorname{id}$ in
...
follows directly from Proposition~4.10: homotopic maps induce the same
...
equivalences: keep Example~4.6 in mind. If we want to think of
...
General remarks such as Lemma~VIII.6.2 or the comments about splitting
of sequences at the end of \S{}VIII.6.1 hold in any abelian category,
...
the opposite of an abelian category is again abelian (Exercise~1.10),
...
in \S{}VIII.6: in particular, we have already observed that, for every
...
to be `split exact' if $\operatorname{id}_{N^\bullet}$ is homotopic to
...
\begin{proof}
  ...
  \[\begin{tikzcd}
      & P^0 \arrow[d, "\alpha^0"] \\
      L^{-1} \arrow[r, "d_{L^\bullet}^{-1}"'] & \operatorname{im}
...
\begin{proof}
  ...
  \[\begin{tikzcd}
    ...
    \end{tikzcd}\] so (Exercise~4.6, part (i)) the fact that
...
\begin{exam}{}{exam:homol:5.15}
  ...
  \[\begin{tikzcd}[column sep=small]
      ...
      \arrow[r, "\mathrm{id}"] \arrow[d, "\mathrm{id}"'] & \Z
...
\begin{proof}[Proof of \cref{thm:homol:5.9}]
  ...
  \operatorname{id}$ (Proposition~4.10). Since
  ...
  \]
  (Exercise~4.6), it follows that
  $\beta^\bullet \circ \alpha^\bullet \sim
  \operatorname{id}_{P_0^\bullet}$. Thus $\alpha^\bullet$ is a
...
\begin{exer}{\exerused{}}{exer:homol:5.1}
  ...
  category $\categ{A}$ are additive. [\S{}5.2]
\end{exer}
...
\begin{exer}{\exerused{}}{exer:homol:5.5}
  ...
  projective or injective objects. [\S{}5.3]
\end{exer}
...
\begin{exer}{$\lnot$}{exer:homol:5.8}
  ...
  injective in $\categ{A}^{\categ{C}}$. (Hint: Exercise~5.6.)  [5.9]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:5.9}
  ...
  \begin{itemize}
  \item With notation as in Exercise~5.7, let
    $\mathscr{Q} := \prod_{X \in \operatorname{Obj}(\categ{C})}
...
\begin{exer}{}{exer:homol:5.10}
  ...
  projectives\footnote{The category of \emph{sheaves} of abelian
    ...
    general it does not have enough projectives.}. (Use Exercise~5.9.)
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:5.11}
  ...
  sense analogous to the one given for modules in \S{}III.7.2) if and
  only if $\operatorname{id}_{N^\bullet}$ is homotopic to $0$ (cf.
  Proposition~III.7.5). Deduce that every additive functor sends split
...
\begin{exer}{$\lnot$}{exer:homol:5.14}
  ...
  \item Assume that there are quasi-isomorphisms
    ...
    wait until you have covered \S{}6, in particular Theorem~6.6.)
  \end{itemize} [6.5]
\end{exer}
...
(Exercise~4.4), while quasi-isomorphism is not (see the discussion
following Example~4.6). Further, homotopy equivalences are preserved
by every additive functor (Theorem~4.14), while general
quasi-isomorphisms are not (Exercise~4.15).
...
as a full subcategory of $\categ{K}(\categ{A})$ (Exercise~6.1). We
...
with target $\iota(A)$ (cf. Remark~4.5): this is a `homotopic category
...
\begin{proof}
  ...
  $\ker\mu = \operatorname{im} d^{-1}_{M^\bullet}$. Therefore, we have
  the diagram
  \[\begin{tikzcd}
      & P^{-1} \arrow[d, "\alpha^0 \circ d_{P^\bullet}^{-1}"] \\
      M^{-1} \arrow[r, "d_{M^\bullet}^{-1}"'] & \operatorname{im}
...
this assertion, for it is a key point (Exercise~6.2). If $\categ{A}$
...
\begin{proof}
  ...
  left is also an epimorphism, by Lemma~2.3. A projective object $P^i$
  ...
  isomorphically to the kernel of $d^i_{L^\bullet}$ (Exercise~1.16);
  ...
  $\operatorname{coker}\pi \to \operatorname{coker} d^i_{L^\bullet}$
  ...
  now established that there exists a homotopy lift\footnote{That is,
    ...
    cf. Exercise~6.10.} to any complex $P^\bullet$ mapping
...
\begin{rem}{}{rem:homol:6.8}
  ...
  isomorphism\footnote{`Natural transformations' were rapidly defined
    in \S{}VIII.1.5, Definition~VIII.1.15. They are the most sensible
...
\begin{proof}
  ...
  $\categ{D}$). The reader will verify (Exercise~6.12) that this
...
\begin{exer}{\exerused{}}{exer:homol:6.2}
  ...
  this subcategory.) [\S{}6.1, \S{}6.2, 6.13, \S{}7.1]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:6.3}
  ...
  \item Prove that there is a morphism of complexes
    $P^\bullet \to MC(\operatorname{id}_{P^\bullet})[-1]$, such that
...
\begin{exer}{\exerused{}}{exer:homol:6.4}
  ...
  also denote by $\mathscr{P}$.  (Use Lemma~5.14.) Next, let
  ...
  \operatorname{id}_{\categ{K}^- (\categ{A})}$. [\S{}7.1, \S{}7.2]
\end{exer}
...
\begin{exer}{}{exer:homol:6.8}
  ...
  argument at the beginning of the proof of Theorem~6.6.
\end{exer}
...
\begin{exer}{}{exer:homol:6.11}
  ...
  particularly easy if you have worked out Exercise~6.9).
\end{exer}
...
\begin{exer}{\exerused{}}{exer:homol:6.13}
  ...
  as in Proposition~6.5, we obtain a functor
  ...
  $\mathscr{P} \circ \iota \circ H^0$. [\S{}7.1]
\end{exer}
...
\begin{rem}{}{rem:homol:7.4}
  ...
  $\mathscr{I} \circ \mathscr{P} \rightsquigarrow \operatorname{id}$
  (Exercise~6.4), used in the proof of
  ...
  $\operatorname{id}_{\categ{K}^-(\categ{B})}$ takes this to
  \[
    \mathscr{P}_{\categ{C}} \circ \categ{K}(\mathscr{G}) \circ
    \categ{K}(\mathscr{F}) \circ \mathscr{I}_{\categ{A}} =
    \categ{L}(\mathscr{G} \circ \mathscr{F}).
...
directly implied by
\begin{itemize}
\item Proposition~6.4, showing that any two projective resolutions are
...
both enough projectives and injectives (as seen in \S{}VIII.6.2 and
\S{}VIII.6.3); thus every covariant/contravariant functor
...
\begin{exam}{}{exam:homol:7.6}
  ...
  $\operatorname{Tor}^R_i (\_ \otimes_R N)$: indeed, the construction
  of $\operatorname{Tor}^R_i (M, N)$ given in \S{}VIII.2.4 matches
  precisely the `concrete' interpretation of the $i$-th left-derived
  functor given above. The reader may note that in \S{}VIII.2.4 we
  ...
  compute $\operatorname{Tor}^R_i(M, N)$ was mentioned at the end of
...
Chapter~VIII will be particular cases. Surely the reader expects this
general fact to follow one way or the other from the long exact
cohomology sequence (Theorem~3.5); that reader will not be
disappointed.
...
\begin{proof}
  ...
  $P_M^i := P_L^i \oplus P_N^i$; this is projective (Exercise~5.4),
...
(Exercise~7.5) that $P_M^\bullet$ is the mapping cone of a morphism
...
\begin{proof}
  ...
  (Exercise~1.21). The statements follow. (If $M^\bullet$ is exact,
...
\begin{rem}{}{rem:homol:7.10}
  ...
  $\operatorname{im} d^i$, $\ker d^i$, and the cohomology
  $\ker d^i / \operatorname{im} d^{i-1}$ are all projective
...
\begin{proof}
  ...
  \[\begin{tikzcd}
    ...
    \end{tikzcd}\] splits (see the end of \S{}VIII.6.1). It follows
  that
  \[\begin{tikzcd}
      0 \arrow[r] & \mathscr{F}(L^i) \arrow[r] & \mathscr{F}(M^i)
      \arrow[r] & \mathscr{F}(P^i) \arrow[r] & 0
    \end{tikzcd}\] is (split and) exact for all $i$
  (Exercise~5.11). Exactness of a sequence of complexes is determined
...
\begin{proof}
  ...
  determined by this sequence (by Theorem~3.5) gives a long exact
  sequence as stated.
\end{proof}
...
reader should either check this directly or do Exercise~7.16 and then
remember Claim~VIII.1.19. The reader should in fact contemplate why
...
that $(\dagger)$ is exact (Exercise~7.17), at least for the part
...
$H^1(G, F^*) \cong \ker d_G^1 / \operatorname{im} d_G^0$, and we can
...
so by Exercise~VII.6.14 they must be linearly independent.  Therefore,
...
$\ker d_G^1 / \operatorname{im} d_G^0 = 0$, as claimed.
...
prove this particular case in Exercise~VII.6.16 and now will enjoy the
chance to verify that indeed Claim~\ref{prop:homol:7.18} implies
Hilbert's result (Exercise~7.18).
...
\begin{exer}{\exerused{}}{exer:homol:7.3}
  ...
  Example~4.6 as a projective resolution in $\Z\text{-}\categ{Mod}$,
  and take
...
\begin{exer}{\exerused{}}{exer:homol:7.5}
  ...
  define morphisms $P_N^i \to P_L^{i+1}$.) [\S{}7.5]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:7.6}
  ...
  0$. (Hint: With notation as in the proof of Corollary~7.9, for all
...
\begin{exer}{$\lnot$}{exer:homol:7.14}
  ...
  is effaced by $\iota : A \to B$, let $C = \operatorname{coker}\iota$
  and consider the exact sequence $0 \to A \to B \to C \to 0$.) [7.15]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:7.15}
  ...
  right before the statement of Proposition~4.1.)
...
\begin{exam}{}{exam:homol:8.2}
  Let $R$ be a commutative ring. Recall (Definition~VIII.2.13) that an
...
to an isomorphic double complex, Exercise~8.4; in fact, other choices
...
\begin{exam}{}{exam:homol:8.8}
  ...
  (Exercise~8.4), and by a slight abuse of language they can both be
  ...
  reader will verify (Exercise~8.5) that, with suitable positions:
  \begin{itemize}
...
\begin{exam}{}{exam:homol:8.10}
  ...
  Example~\ref{exam:homol:8.8} (cf.\ Exercise~8.5), this says that
...
$N^\bullet$: Theorem~6.6 accomplishes this. I have called the
...
\begin{proof}
  ...
  follows (Corollary~4.2) that this morphism is a quasi-isomorphism,
  as needed.
\end{proof}
...
a convenient bridge between resolving \emph{objects} (see Lemma~6.3)
and resolving \emph{complexes} (as in Theorem~6.6).  Indeed, by
Corollary~7.9 (which relies on the horseshoe lemma, therefore on
...
(and satisfying the additional condition explained in Remark~7.10) are
...
same object, as sides of an array.  Corollary~7.9 tells us how to fill
...
in \S{}VIII.2.4 (where we used free resolutions); see also
...
and Ext made in Chapter~VIII, in the sense that the extent to which we
...
\begin{exer}{}{exer:homol:8.2}
  ...
  resolution of $M$, and let $J^i := \operatorname{im} d_A^i$. Prove
  that
  \[
...
\begin{exer}{$\lnot$}{exer:homol:8.8}
  ...
  $\mathscr{F}M^\bullet$ (as constructed in Corollary~7.9). Consider
  the complex
  \[
    (\dagger) \qquad
    \begin{tikzcd}
...
\begin{exer}{$\lnot$}{exer:homol:8.11}
  ...
  adapt Exercise~\ref{exer:homol:7.12}.) [8.12, 8.13]
\end{exer}
...
\begin{exer}{}{exer:homol:8.13}
  ...
  $A \in \operatorname{Obj}(\categ{A})$, equals the supremum of
  $\operatorname{id}(B)$, $B \in \operatorname{Obj}(\categ{A})$, and
...
of the field of fractions (see \S{}V.4.2). This process, in a more
general version dealing with inverting `multiplicative subsets' of a
ring, is called \emph{localization} (Exercise V.4.7). Categories may
...
identified to one another and to $\operatorname{id}_A$ in the
...
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    ...
    below]
    {$\operatorname{id}_A$}; \draw[->] (C) -- (T) node[midway, left]
    {$+1$};
  \end{tikzpicture}
\]
...
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    ...
    node[midway, below]
    {$\operatorname{id}_{L^\bullet}$}; \draw[->] (Z) -- (T)
    node[midway, left] {$+1$};
  \end{tikzpicture}
\]
...
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    ...
    {$\operatorname{id}_{L^\bullet}$}; \draw[->] (B) -- (Z); \draw[->]
...
will not surprise the reader. The functors $\operatorname{Hom}(A,\_)$
and $\operatorname{Hom}(\_,A)$ (to $\categ{Ab}$) are cohomological
...
literature, references to results such as Theorem~8.12 are often
...
be an exact triangle\footnote{That is,
  ...
  $\operatorname{im}\beta = \ker\gamma$, and
  $\operatorname{im}\gamma = \ker\alpha$.} in an abelian category; I
...
$\gamma(e) \in \ker\beta = \operatorname{im}\alpha = A'$.) All needed
...
$E_{i+1} = \ker d_i / \operatorname{im} d_i$.
...
\begin{defn}{}{defn:homol:9.2}
  ...
  $E_{i+1} \cong \ker d_i / \operatorname{im} d_i$.
\end{defn}
...
$\overline{B}_{i+1} = \operatorname{im} d_i$; define $Z_{i+1}$,
...
Definition~8.5) that the diagram anticommutes, so that the
...
Theorems~8.9 and~8.12. To see that it implies Theorem~8.12, note again
that
\[
...
statement in Theorem~8.12. This is what is meant by the incantation
...
Theorem~8.12 can be analyzed by using the sequence ${}_hE_i$ obtained
...
derived functor of the composition of two functors\footnote{For a
  different viewpoint on this question, see Remark~7.4.}: for example,
...
\begin{exer}{}{exer:homol:9.1}
  ...
  bounded complexes in these categories (see Exercise~3.15). Prove
  that if
  \[
    \begin{tikzpicture}[baseline=(current bounding box.center)]
      \node (A) at (1,1.3) {$A$}; \node (C) at (0,0)
...
\begin{exer}{\exerused{}}{exer:homol:9.6}
  ...
  $H^i(N^\bullet) \to H^{i+1}(L^\bullet)$ in cohomology. [\S{}9.2]
\end{exer}
...
\begin{exer}{\exerused{}}{exer:homol:9.8}
  ...
  $\gamma^{-1}(\bigcap_r \operatorname{im}\alpha^r) / \beta(\bigcup_r
  \ker\alpha^r)$. (Hint: In fact,
  $Z_{r+1} = \gamma^{-1}(\operatorname{im}\alpha^r)$ and
  $B_{r+1} = \beta(\ker\alpha^r)$.) [\S{}9.3]
\end{exer}
