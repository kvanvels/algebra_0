...
homore sophisticated constructions than those reviewed in
...
\Cref{lem:linrep:right-adjoints-commute-with-limits} in gory detail,
but I will endeavor to convince the reader that such statements are
easier than they look and just boil down to suitable applications of
the universal properties defining the various concepts. By contrast,
proving that a specific given functor preserves products (for
instance), without appealing to abstract nonsense, may sometimes
appear to involve some `real' work.
...
\Cref{lem:linrep:right-adjoints-commute-with-limits}. What is missing
from this outline is the explicit
...
\Cref{lem:linrep:right-adjoints-commute-with-limits} implies, for example,
that right-adjoint functors
...
\end{proof}
By \Cref{lem:linrep:right-adjoints-commute-with-limits} (or rather its
co-version), we can conclude that for each $R$-module $N$, the functor
$\otimes_R N$ preserves colimits, and so does
$M \otimes_R \mathord{-}$, by the basic commutativity of tensor
products verified in
...
right-exact functors (cf.\
\Cref{exam:linrep:exact-functor-definition-example}). These
...
(\Cref{prop:linalg:lin-indep-extends-to-basis}). Tensor products are
more interesting over more general rings.
...
\Cref{coro:linrep:tensor-with-quotient-by-ideal} in
a template example for a basic application of
...
\Cref{coro:linrep:tensor-with-quotient-by-ideal}: for an
ideal $I$ of $R$ and an $R$-module $N$, the
...
identified (via \Cref{coro:linrep:tensor-distributes-over-direct-sum})
with the inclusion $M_1^{\oplus A} \subseteq M_2^{\oplus A}$.

\begin{exam}{}{exam:linrep:tensoring-exact-for-vector-spaces}
  Since vector spaces are free
  (\Cref{prop:linalg:lin-indep-extends-to-basis}),
  tensoring is exact
...
In due time (\Cref{sec:homol:derived-functors} and OLD:CHECK THIS 8) we will prove
...
\]
This property characterizes PIDs
(\Cref{prop:linalg:pid-characterization-via-presentations}). If
...
\begin{exer}{}{exer:linrep:f-tensor-f-separable-galois-criteria}
  ...
  \end{itemize}
  (Use \Cref{coro:linrep:tensor-with-quotient-by-ideal} to `compute' the tensor.
  The CRT from
...
\begin{exer}{}{exer:linrep:localization-is-flat}
  ...
  is flat over $R$. (Hint:
  \Cref{
    exer:linrep:localization-is-tensor-with-localized-ring,
    exer:linrep:localization-is-exact-preserves-homology
  }.)
\end{exer}
...
\begin{exer}{}{exer:linrep:tor1-of-quotients-iso-to-intersection-over-product}
  ...
  $\Tor^R_1$ vanishes if $I + J = R$, by
  \Cref{lem:irred:pairwise-comaximal-product-equals-intersection}.)
  Prove
...
\begin{exer}{$\lnot$}{exer:linrep:flatness-is-local-property}
  ...
  \Cref{
    exer:linrep:localization-is-exact-preserves-homology,
    exer:linrep:tensor-and-localization-commute
  }.
  The
...
\begin{proof}
  ...
  $\Hom_S(N, P)$. Thus $\phi$ is $R$-balanced. By
  \Cref{lem:linrep:balanced-map-factors-through-tensor},
  ...
  $m \otimes n$,
  \begin{align*}
    \tilde\phi(s(m \otimes n))
    &= \tilde\phi(m \otimes (sn))
      = \phi(m, sn) = \alpha(m)(sn) \\
...
If $R = S$, we recover the adjunction formula of
\Cref{lem:linrep:hom-tensor-adjunction};
...
\Cref{exam:linrep:exact-functor-definition-example},
this hints that $f_*$ may have both a
...
\begin{proof}
  ...
  \end{align*}
  where I have used
  \Cref{lem:linrep:hom-tensor-adjunction-bimodule}.
  These bijections are canonical, proving that $f^*$
  is left-adjoint to $f_*$.

  Similarly, there is a canonical isomorphism $N \cong N \otimes_S S$
  (\Cref{exam:linrep:tensor-with-base-ring-is-identity});
  thus $N \otimes_S S \cong f_*(N)$ as
  ...
  \begin{align*}
    ...
    &\cong \Hom_S(N, \Hom_R(S, M))
      = \Hom_S(N, f^!(M))
  \end{align*}
  again by \Cref{lem:linrep:hom-tensor-adjunction-bimodule}.
  This shows that $f^!$ is right-adjoint
...
\begin{exer}{}{exer:linrep:composition-rules-for-base-change-functors}
  ...
  \Cref{rem:linrep:notation-warning-pushforward}), one would actually
  work with categories
  which are opposite to the category of commutative rings; cf.\
  \Cref{exam:linrep:category-of-affine-algebraic-sets}. A ring
  homomorphism $f : R \to S$
...
\begin{exer}{}{exer:linrep:morita-equivalence-and-center}
  ...
  \end{itemize}
  Two rings are \emph{Morita equivalent}\index{Morita equivalent}
  if their categories of
