...
The limited exposure to the Tor and Ext functors given in
\Cref{sec:linrep:tensor-products-and-the-tor-functors} and
\Cref{sec:linrep:projective-and-injective-modules-and-the-ext-functors}
should assist the reader in occasional encounters with these
functors. However, it is very worthwhile to go back, dotting all i's
and crossing all t's as necessary to prove the wonderful facts
...
about Tor and Ext stated in \Cref{chap:linrep} will be recovered as
very particular cases of the results presented in this chapter.
...
(co)homology': what information in a complex (see
\Cref{sec:rings:complexes-and-homology}) is really responsible for
its homology. This will motivate most constructions in this chapter,
including derived functors. A guiding beacon throughout the chapter
will be the notion of derived category: I will not construct derived
categories in full generality, but the reader should nevertheless
emerge with a basic understanding of what they are. One reason for
this compromise is that I felt that delving into triangulated
categories would take us too far; they are only mentioned in passing
at the end of the chapter.  Balancing Tor and Ext will motivate the
introduction of double complexes, and these in turn will motivate a
quick treatment of spectral sequences, in the last section.
...
\item Products, coproducts may not exist (if the product of $\F_2$ and
  $\F_3$ existed in $\categ{Fld}$, what would its characteristic be?).
...
\item Even when there are kernels and cokernels, monomorphisms need
  ...
  \Cref{subsec:groups:kernel-iff-normal}), and epimorphisms need
  not be cokernels (the injection $\Z \to \Q$ is an epimorphism in
  $\categ{Ring}$; see
  \Cref{subsec:rings:monomorphisms-and-epimorphisms}).
...
exact sequences, and the like: the material covered in
\Cref{sec:rings:complexes-and-homology} will be our starting
point. That material was developed for the category
...
\Cref{subsec:homol:what-is-missing}) will provide a mathematical
reason supporting this psychological fact.

\subsection{Additive categories}
\label{subsec:homol:additive-categories}

\begin{defn}{}{defn:homol:additive-category}
  ...
  Hom-sets.
\end{defn}
None of the categories mentioned in the list in
\Cref{subsec:homol:undesirable-features-of-otherwise-reasonable-categories}
is additive.  Even $\categ{Grp}$ (which has kernels and cokernels,
products, etc.)  is not an additive category: the Hom-sets in
$\categ{Grp}$ do not have a natural abelian group structure. Of course
$\categ{Ab}$ is additive, and so are all categories
$R\text{-}\categ{Mod}$.  If a category $\categ{A}$ is additive, it
makes sense to talk about `zero-morphisms' (which I will denote by
$0$) and to add or subtract morphisms; two morphisms $\alpha$, $\beta$
are equal if and only if $\alpha - \beta = 0$. One can also talk about
kernels and cokernels in $\categ{A}$, by adopting the categorical
definitions as suitable limits and colimits
(\Cref{exam:linrep:equalizer-as-limit}). Here are those definitions again, expanded
for intelligibility:
\begin{defn}{}{defn:homol:kernel}
...
$\coker\varphi$.  We have to get used to the fact that morphisms may
have `many' kernels (or cokernels), uniquely identified with each
other by virtue of being answers to a universal question
(\Cref{prop:prelims:initial-final-unique}); it is common to harmlessly
abuse language and talk about the kernel and the cokernel of a
morphism. Further, we have to get used to the fact that the kernel
...
one cannot talk about `subobjects' or `inclusions'; the closest one
can get to these notions is monomorphism. Similarly, `surjective' is
not really an option; epimorphism is the appropriate
replacement. Recall
(\Cref{subsec:prelims:functions:monomorphisms-and-epimorphisms})
...
\begin{lem}{}{lem:homol:mono-epi-via-zero-morphisms}
...
\begin{lem}{}{lem:homol:kernels-monos-cokernels-epis}
...
this: the opposite (cf.\ \Cref{exer:prelim:op-categ-def}) of an
additive category is additive, and kernels, monomorphisms, etc., in
one correspond to cokernels, epimorphisms, etc., in the other. Thus,
proving one of these statements for all additive categories
establishes at the same time the truth of its dual statement.
...
\begin{proof}
  Let $\varphi : A \to B$ be a morphism in an additive category
  $\categ{A}$, and let $\coker\varphi : B \to C$ be its cokernel. Let
  $\gamma : C \to Z$ be a morphism such that
  $\gamma \circ \coker\varphi = 0$. The composition
  $(\gamma \circ \coker\varphi) \circ \varphi$ is $0$; by definition
  of cokernel, $\gamma \circ \coker\varphi$ factors uniquely through
  $C$:
  \[\begin{tikzcd}
      A \arrow[r, "\varphi"] & B \arrow[r, "\gamma \circ \coker\varphi
      = 0"]
      \arrow[d, "\coker\varphi"'] & Z \\
      & C \arrow[ur, dashed, "\gamma"', "\exists!"']
    \end{tikzcd}\] Since
  $\gamma \circ \coker\varphi = 0 = 0 \circ \coker\varphi$, the
  uniqueness forces $\gamma = 0$.  This proves that $\coker\varphi$ is
  an epimorphism, by \Cref{lem:homol:mono-epi-via-zero-morphisms}.

  The proof that kernels are monomorphisms is analogous and is left to
  the reader (\Cref{exer:homol:basic-abelian-category-lemmas}).
\end{proof}
% ?
...
$\ker\varphi$ and $\coker\varphi$. This is the case for monomorphisms
and epimorphisms:
\begin{lem}{}{lem:homol:mono-epi-via-trivial-kernel-cokernel}
...
\begin{proof}
  ...
  \Cref{lem:homol:mono-epi-via-zero-morphisms}, and in particular $\zeta$ factors (uniquely)
  ...
  (\Cref{exer:homol:basic-abelian-category-lemmas}).
\end{proof}
% ?
In view of \Cref{lem:homol:mono-epi-via-trivial-kernel-cokernel}, we should be able to use diagrams
\[
...
\begin{defn}{}{defn:homol:abelian-category}
...
\Cref{lem:homol:kernels-monos-cokernels-epis}), we see that in an \emph{abelian} category we
can adopt a mantra entirely analogous to the useful `kernel $\iff$
submodule' of \Cref{subsec:rings:submodules-and-quotients} vintage:
in abelian categories, the slogan would be `kernel $\iff$
monomorphism' (and similarly for cokernels vs.  epimorphisms).
\begin{rem}{}{rem:homol:subobject-quotient-notation}
  ...
  to denote (the target of) $\coker\varphi$. We will have no real use
  for this notation in this section, but it will come in handy later
  on.
\end{rem}
The very existence of kernels and cokernels links these two notions
tightly in an abelian category:

\begin{lem}{}{lem:homol:kernel-is-kernel-of-its-cokernel}
...
\end{proof}
% ?
Putting together \Cref{lem:homol:kernel-is-kernel-of-its-cokernel} and \Cref{lem:homol:mono-epi-via-trivial-kernel-cokernel}, we
can rephrase \Cref{defn:homol:abelian-category} by listing the following
...
abelian category:
\begin{lem}{}{lem:homol:mono-and-epi-implies-iso}
...
\begin{rem}{}{rem:homol:mono-epi-iso-fails-in-additive-categories}
  This fact does not hold in general additive categories. In fact, the
  reader will encounter in
  \Cref{exer:homol:category-of-short-exact-sequences-properties} an
  example of an additive category with kernels and cokernels (but
  nevertheless not abelian) and with morphisms that are both
  monomorphisms and epimorphisms, without being isomorphisms.
\end{rem}
% ?
\begin{proof}
  By \Cref{lem:homol:mono-epi-via-trivial-kernel-cokernel} the kernel of $\varphi$ is $0 \to A$, since
  $\varphi$ is a monomorphism. Similarly, $B \to 0$ is a cokernel of
  $\varphi$. Further, $\varphi$ is the cokernel of $0 \to A$ and the
  kernel of $B \to 0$, by \Cref{lem:homol:kernel-is-kernel-of-its-cokernel}.
  ...
  \[\begin{tikzcd}
      & A & & \\
      0 \arrow[r] & A \arrow[u, "\id"] \arrow[r, "\varphi", tail, two
      heads] & B \arrow[r] & 0
    \end{tikzcd}\]
  ...
  \[\begin{tikzcd}
      & A & & \\
      0 \arrow[r] & A \arrow[u, "\id"] \arrow[r, "\varphi", tail, two
      heads] & B \arrow[r] \arrow[ul, dashed, "\exists!  \eta"'] & 0
    \end{tikzcd}\]

  This says $\eta\varphi = \id_A$, so $\varphi$ has a left-inverse as
  well.
...
(possibly) simpler argument: by \Cref{prop:rings:kernels-cokernels},
monomorphisms are injective and epimorphisms are surjective; therefore
$\varphi$ is bijective, and bijective homomorphisms are isomorphisms
by \Cref{exer:rings:bijective-iso}. Such set-theoretic arguments are
not an option in an arbitrary abelian category (at least until we
develop the material of
\Cref{sec:homol:working-in-abelian-categories}), since the objects
of an abelian category are not given as `sets'. Judicious use of
appropriate universal properties accomplishes the same goal and may be
argued to convey a `deeper' sense of why the proven statement is true.
...
\begin{exam}{}{exam:homol:pullbacks-exist-in-abelian-category}
  ...
  (\Cref{exer:homol:pullback-kernel-comparison-iso}).
...
requirements for products and coproducts:
\Cref{prop:rings:direct-sum-product-coproduct}. But the direct sum of
two modules $M_1$, $M_2$ is defined by giving a module structure to
the Cartesian product $M_1 \times M_2$. Again, this strategy cannot be
applied in a general abelian category, so we have to come up with an
alternative. This gives us another example of the contrast between
set-theoretic and arrow-theoretic arguments.  The coproduct
$A \sqcup B$ is endowed with morphisms
\[
...
and an epimorphism. By \Cref{lem:homol:mono-and-epi-implies-iso}, this (and a simple
induction) will prove

\begin{prop}{}{prop:homol:finite-products-coproducts-coincide}
...
straightforward than what follows
(\Cref{exer:homol:finite-products-coproducts-coincide-additive}). Here
I will prove directly that the morphism $(\pi_A, \pi_B)$ is a
monomorphism, leaving to the reader the similar arguments needed to
show that the same morphism is an epimorphism. In the process, we
recover for these objects all the properties we expect on the basis of
our experience with $R\text{-}\categ{Mod}$.
\begin{itemize}
\item $\pi_B : A \sqcup B \to B$ is the cokernel of
  ...
  $\pi_B \circ i_B = \id_B$.  We have to show that $\gamma$ factors
  uniquely through $\pi_B$.  Note that if it factors at all, then the
  factorization is unique: indeed, if $\gamma = \delta \circ \pi_B$,
  then
...
\item $i_A : A \to A \sqcup B$ is the kernel of
  ...
  hence it is the kernel of its cokernel (\Cref{lem:homol:kernel-is-kernel-of-its-cokernel}).
...
\item The morphism $(\pi_A, \pi_B) : A \sqcup B \to A \times B$
  ...
  composition $K \to B \xrightarrow{\id} B$. That is, $K \to B$ is
  actually the zero-morphism; a fortiori, $\iota = 0$: the kernel of
  $A \sqcup B \to A \times B$ is $0$, so this morphism is indeed a
  monomorphism (by \Cref{lem:homol:mono-epi-via-trivial-kernel-cokernel}).
\end{itemize}
...
I will happily leave this to the reader
(\Cref{exer:homol:coproduct-to-product-map-is-epi}).

This will conclude the proof of \Cref{prop:homol:finite-products-coproducts-coincide}, by
\Cref{lem:homol:mono-and-epi-implies-iso}.
% ?
In view of \Cref{prop:homol:finite-products-coproducts-coincide}, we can adopt for any abelian
...
the decompositions studied in $\categ{Set}$
(\Cref{subsec:prelims:canonical-decomposition}), $\categ{Grp}$
(\Cref{subsec:groups:canonical-decomposition}), $\categ{Ring}$
(\Cref{subsec:rings:canonical-decomposition-and-consequences}),
$R\text{-}\categ{Mod}$
(\Cref{subsec:rings:canonical-decomposition-and-isomorphism-theorems}). In
each of these previous examples we could simply argue that the
decomposition was the original decomposition obtained in
$\categ{Set}$, enriched as the case may be with additional structures
adapted to the category under examination. This is not an option in
the context of an abelian category, and yet the decomposition holds
just as well: every morphism may be written as an epimorphism,
followed by an isomorphism, followed by a monomorphism.  Here is the
precise statement:
\begin{thm}{}{thm:homol:canonical-decomposition-of-morphism}
...
structure.\footnote{However, we will soon recover a sense in which
  ...
  terms: see \Cref{lem:homol:kernel-image-via-elements}.} But look at your mental image of what
the image of a function $\varphi$ is; mine looks like this:
\begin{center}
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
and $\image\varphi$ is the `smallest' such subobject; that is, it is
initial with the property of being a subobject of $B$ through which
$\varphi$ factors. This intuition is precisely captured by
$\ker(\coker\varphi)$, once we adapt to thinking of `subobjects' as
`monomorphisms':
\begin{lem}{}{lem:homol:kernel-of-cokernel-factorization-properties}
...
By \Cref{lem:homol:kernels-monos-cokernels-epis}, $\iota$ is a monomorphism. It is clear that
$\varphi$ factors through $\iota$: the composition
$A \to B \to \coker\varphi$ is the zero-morphism, so there is a
naturally induced $A \to K$ by the universal property of kernels. The
more interesting part of the statement of \Cref{lem:homol:kernel-of-cokernel-factorization-properties} asserts
that if $\lambda : L \rightarrowtail B$ is any monomorphism through
which $\varphi$ also factors, then $\iota$ must factor uniquely
through $\lambda$:
\[\begin{tikzcd}
...
\begin{proof}
  ...
  universal property of $\coker\varphi$, we have an induced map:
  \[\begin{tikzcd}
      & L \arrow[dr, "\lambda"] & & \\
      A \arrow[ur] \arrow[r] & K \arrow[r, hook] & B \arrow[r,
      "\coker\varphi"] \arrow[dr, "\coker\lambda"'] &
  ...
  \[
    K \to B \xrightarrow{\coker\lambda} \operatorname{Cok}\lambda
  \]
  is the zero-morphism. Since $\lambda : L \to B$ is the kernel of its
  cokernel (\Cref{lem:homol:kernel-is-kernel-of-its-cokernel}), it follows that there is a unique
  morphism $K \to L$ making the diagram commute, as stated.
\end{proof}

\begin{defn}{}{defn:homol:image-and-coimage}
  Let $\varphi : A \to B$ be a morphism in an abelian category. The
  \emph{image} of $\varphi$, denoted $\image\varphi$, is
  $\ker(\coker\varphi)$. The \emph{coimage} of $\varphi$, denoted
  $\operatorname{coim}\varphi$, is $\coker(\ker\varphi)$.
\end{defn}

\Cref{lem:homol:kernel-of-cokernel-factorization-properties} motivates the first definition: every
monomorphism through which $\varphi$ factors must factor uniquely
through $\image\varphi$. (Of course in this context the source of
$\image\varphi$ is only defined up to (unique) isomorphism, as is
every solution to a universal problem.) As for the second definition,
the reader will formulate the universal property satisfied by
$\operatorname{coim}\varphi$
(\Cref{exer:homol:universal-property-of-coimage}). With the convention
mentioned in \Cref{rem:homol:subobject-quotient-notation} and allowing for some abuse of language,
the (target of the) coimage of $\varphi : A \to B$ is the `quotient'
$A/\ker\varphi$.
...
\begin{lem}{}{lem:homol:image-coimage-induced-morphisms}
  Let $\varphi : A \to B$ be a morphism in an abelian category, and
...
\begin{proof}
  ...
  half to the reader (\Cref{exer:homol:coimage-to-target-is-mono}.)

  To verify that $\overline{\varphi} : A \to K$ is an epimorphism,
  consider its image $K' \to K$:
  \[\begin{tikzcd}
      K' \arrow[r, hook, "\image\overline{\varphi} =
      \ker\coker\overline{\varphi}"] & K \arrow[r, two heads,
      "\coker\overline{\varphi}"] & \operatorname{Cok}.
    \end{tikzcd}\] Recall that every epimorphism is the cokernel of
  its kernel (\Cref{lem:homol:kernel-is-kernel-of-its-cokernel}); in particular,
  $\coker\overline{\varphi} = \coker(\image\overline{\varphi})$.  Now,
  $K' \to K$ is a monomorphism through which $\varphi$ factors:
  \[\begin{tikzcd}
      A \arrow[r] \arrow[rr, bend left, "\varphi"] & K' \arrow[r,
      hook, "\image\overline{\varphi}"] & K \arrow[r, hook] & B.
    \end{tikzcd}\] Therefore, $K' \to B$ is a monomorphism through
  which $\varphi$ factors and `preceding' $\image\varphi : K \to
  B$. Since the image of $\varphi$ is initial among such morphisms (as
  proven in \Cref{lem:homol:kernel-of-cokernel-factorization-properties}), necessarily
  $\image\overline{\varphi}$ is an isomorphism. It follows that
  $0 = \coker\image\overline{\varphi} = \coker\overline{\varphi}$,
  proving that $\overline{\varphi}$ is an epimorphism by
  \Cref{lem:homol:mono-epi-via-trivial-kernel-cokernel}.
\end{proof}
% ?
...
\begin{proof}[Proof of \cref{thm:homol:canonical-decomposition-of-morphism}]
  A unique morphism $\psi : K \to C$ making the above diagram commute
  exists, by the universal property of $\image\varphi$
  (\Cref{lem:homol:kernel-of-cokernel-factorization-properties}), as well as by the universal property of
  $\operatorname{coim}\varphi$. Since
  $\underline{\varphi} \circ \psi = \image\varphi$ is a monomorphism,
  so is $\psi$. Since
  $\psi \circ \overline{\varphi} = \operatorname{coim}\varphi$ is an
  epimorphism, so is $\psi$. It follows that $\psi$ is an isomorphism
  (\Cref{lem:homol:mono-and-epi-implies-iso}), and letting $\tilde{\varphi} : C \to K$ be
...
\begin{exer}{}{exer:homol:composite-epi-mono-factor-properties}
...
\end{exer}
\begin{exer}{}{exer:homol:negative-of-morphism-mono-epi-iff-original}
  Let $\varphi : A \to B$ be a morphism in an additive category. Prove
  that $-\varphi$ is a monomorphism, resp., epimorphism, if and only
  if $\varphi$ is.
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:preadditive-one-object-is-ring}
  ...
  functor' between preadditive categories makes sense.
  [\ref{exer:homol:r-mod-equiv-additive-functors-on-r}]
\end{exer}
\begin{exer}{}{exer:homol:end-a-has-natural-ring-structure}
...
\end{exer}
\begin{exer}{}{exer:homol:zero-morphism-via-zero-object}
...
\end{exer}
\begin{exer}{}{exer:homol:zero-obj-iff-id-is-zero-morphism}
  \exerused{} Prove that an object $A$ of an additive category
  $\categ{A}$ is a zero-object if and only if $\id_A$ equals the
  zero-morphism $A \to
  A$. [\Cref{subsec:homol:quasi-isomorphisms-and-derived-categories}]
\end{exer}
\begin{exer}{}{exer:homol:categorical-cokernel-definition}
  Give a categorical definition of cokernel in the spirit of the
  definition for kernel given in \Cref{exam:linrep:equalizer-as-limit}, and verify
  that the ordinary notion of cokernel in $R\text{-}\categ{Mod}$
  satisfies this categorical requirement. Verify that your definition
  agrees with the one given in \Cref{defn:homol:kernel}.
\end{exer}
\begin{exer}{}{exer:homol:kernel-characterized-by-hom-exactness}
  Let $\varphi : A \to B$ be a morphism in an additive category
  ...
  cokernels.
\end{exer}
\begin{exer}{}{exer:homol:basic-abelian-category-lemmas}
  ...
  \item Let $\iota : K \to A$ be a kernel in $\categ{A}$; prove that
    $\iota$ is a monomorphism. (Cf.\ \Cref{lem:homol:kernels-monos-cokernels-epis}.)
  ...
  \item If $\categ{A}$ is abelian, prove that every kernel in
    ...
    \Cref{lem:homol:kernel-is-kernel-of-its-cokernel}.)
  \end{itemize} [\Cref{subsec:homol:abelian-categories}]
\end{exer}
\begin{exer}{}{exer:homol:opposite-of-abelian-is-abelian}
  \exerused{} Prove that the opposite $\categ{A}^{op}$ (cf.\
  \Cref{exer:prelim:op-categ-def}) of an abelian category is an
  abelian category.
  [\Cref{subsec:homol:complexes-of-projective-and-injective-objects}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:functor-category-into-abelian-is-abelian}
  Let $\categ{A}$ be an abelian category, and let $\categ{C}$ be any
  small category. Prove that the functor category
  $\categ{A}^{\categ{C}}$ (cf.\
  \Cref{exer:linrep:yoneda-embedding-is-functor}) is an abelian
  category.
  ...
  an exact functor
  $\mathscr{X} : \categ{A}^{\categ{C}} \to
  \categ{A}$. [\ref{exer:homol:r-mod-equiv-additive-functors-on-r},
  \ref{exer:homol:presheaves-valued-in-abelian-form-abelian},
  \ref{exer:homol:exp-map-not-epi-of-presheaves},
  \ref{exer:homol:contravariant-additive-functors-category-abelian},
  \ref{exer:homol:right-adjoint-functor-category-construction}]
\end{exer}
\begin{exer}{}{exer:homol:r-mod-equiv-additive-functors-on-r}
  For a ring $R$, let $\categ{R}$ denote the preadditive category with
  a single object determined by $R$
  (\Cref{exer:homol:preadditive-one-object-is-ring}). Prove that the
  ...
  category $\categ{Ab}^{\categ{R}}$ (cf.\
  \Cref{exer:homol:functor-category-into-abelian-is-abelian}). What
  about the category of right-$R$-modules?
\end{exer}
\begin{exer}{}{exer:homol:fin-gen-modules-over-noeth-abelian}
...
\end{exer}
\begin{exer}{}{exer:homol:presheaves-valued-in-abelian-form-abelian}
  ...
  (\Cref{exam:linrep:presheaf-of-sets}). Define the \emph{category} of
  ...
  \Cref{exer:homol:functor-category-into-abelian-is-abelian}.)
\end{exer}
\begin{exer}{}{exer:homol:exp-map-not-epi-of-presheaves}
  Consider the presheaf of continuous complex-valued functions
  $\mathscr{C}$ on a circle $S^1$ (cf.\
  \Cref{exer:linrep:presheaf-continuous-fns-gluing}) and the presheaf
  $\mathscr{C}^*$ of continuous complex-valued functions that do not
  vanish anywhere along the circle; the first is a sheaf of abelian
  groups (under $+$), and so is the second (under $\cdot$).  Use the
  exponential map to define a morphism
  ...
  presheaves\footnote{Watch out! Both presheaves $\mathscr{C}$,
    ...
    \Cref{exer:linrep:presheaf-continuous-fns-gluing}. Sheaves of
    abelian groups form a full subcategory of the category of
    presheaves, and this category is also abelian, but the inclusion
    functor is in general \emph{not} exact. For example, the
    exponential map $\mathscr{C} \to \mathscr{C}^*$ considered above
    \emph{is} an epimorphism of sheaves.
    ...
    particular case of
    \Cref{exer:homol:functor-category-into-abelian-is-abelian}), but
    the example given above shows that it is not exact in general on
    sheaves. This lack of exactness is responsible for the existence
    of sheaf cohomology.}: show that its cokernel has value $0$ over
  every proper open set of $S^1$ but is nonzero over $S^1$.
\end{exer}
\begin{exer}{}{exer:homol:pullback-kernel-comparison-iso}
  ...
  State and prove an analogous result for
  push-outs. [\Cref{subsec:homol:products-coproducts-and-direct-sums},
  \Cref{subsec:homol:the-snake-lemma-again},
  \Cref{subsec:homol:from-objects-to-complexes}]
\end{exer}
\begin{exer}{}{exer:homol:coproduct-to-product-map-is-epi}
  \exerused{} Prove that the morphism $A \amalg B \to A \times B$
  defined for objects $A$, $B$ of an abelian category is an
  epimorphism. [\Cref{subsec:homol:products-coproducts-and-direct-sums}]
\end{exer}
\begin{exer}{}{exer:homol:intersection-of-natural-monos-in-direct-sum}
...
\end{exer}
\begin{exer}{}{exer:homol:universal-property-of-coimage}
  \exerused{} Formulate the universal property satisfied by the
  coimage of a morphism in an abelian
  category. [\Cref{subsec:homol:images-canonical-decomposition-of-morphisms}]
\end{exer}
\begin{exer}{}{exer:homol:coimage-to-target-is-mono}
  \exerused{} Let $A \to B$ be a morphism in an abelian category, with
  coimage $A \to C$, and let $C \to B$ be the induced morphism. Prove
  that $C \to B$ is a monomorphism. (Cf.\ \Cref{lem:homol:image-coimage-induced-morphisms}; you
  should not use \Cref{thm:homol:canonical-decomposition-of-morphism}, since
  \Cref{lem:homol:image-coimage-induced-morphisms} was used in its proof.)
  [\Cref{subsec:homol:images-canonical-decomposition-of-morphisms}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:epi-mono-factorization-is-coim-image}
  Let $\varphi : A \to B$ be a morphism in an abelian category, and
  ...
  $\pi = \operatorname{coim}\varphi$ and $i =
  \image\varphi$. [\Cref{subsec:homol:long-exact-sequence-of-derived-functors}]
\end{exer}
\begin{exer}{}{exer:homol:finite-products-coproducts-coincide-additive}
  ...
  [\Cref{subsec:homol:products-coproducts-and-direct-sums}]
\end{exer}
...
the universal property of images (\Cref{lem:homol:kernel-of-cokernel-factorization-properties}) yields a
unique factorization of $\image\varphi$ through $\ker\psi$.
Similarly, the second condition tells us that $\ker\psi$ factors
through $\ker(\coker\varphi) = \image\varphi$.  This implies that
$\image\varphi$ and $\ker\psi$ coincide.  The conditions defining
exactness can therefore be summarized as
\begin{itemize}
\item $\image\varphi = \ker\psi$.
\end{itemize}
Of course this recovers precisely the condition used to define the
notion of exactness of a sequence of $R$-modules, in
\Cref{subsec:rings:complexes-and-exact-sequences}. All expected
elementary connotations of exactness hold in the context of abelian
categories as in $R\text{-}\categ{Mod}$, as envisioned already in
\Cref{subsec:homol:additive-categories}--1.3. With notation as
above, if the sequence is exact at $B$, then $\psi$ is a monomorphism
if and only if $\varphi$ is the zero-morphism, and $\varphi$ is an
epimorphism if and only if $\psi$ is the zero-morphism; the sequence
\[\begin{tikzcd}
...
\begin{exam}{}{exam:homol:direct-sum-exact-sequence}
  ...
  \Cref{subsec:homol:products-coproducts-and-direct-sums}.
\end{exam}
\begin{exam}{}{exam:homol:fibered-diagram-exact-sequence}
  ...
  \item the sequence obtained by adding a $0$ to the left,
    ...
    \Cref{exam:homol:pullbacks-exist-in-abelian-category});
  ...
  mentioned in \Cref{exam:homol:pullbacks-exist-in-abelian-category}
  (up to an isomorphism accounting for the signs in the case of
  coproducts).
\end{exam}
...
category of all the statements given in
\Cref{sec:rings:complexes-and-homology} for sequences of
...
reviewed in \Cref{sec:homol:complexes-and-homology-again}. However,
note that while we can now \emph{state} the snake lemma
(\Cref{lem:rings:snake-lemma}) in any abelian category, the (sketch of
the) \emph{proof} given in \Cref{sec:rings:complexes-and-homology}
cannot be directly lifted from that context and applied to this one,
or so it would seem, since the definition of the `snaking'
homomorphism $\delta$ given in
\Cref{sec:rings:complexes-and-homology} makes use of
`elements'. Barring more sophisticated alternatives, we should provide
an arrow-theoretic alternative for the definition of $\delta$.

This is perhaps not completely immediate.

As practice, note the following:
\begin{lem}{}{lem:homol:epimorphisms-stable-under-pullback}
  Let
  \[\begin{tikzcd}
      A \times_C B \arrow[r, "\varphi'"] \arrow[d, "\psi'"'] &
      B \arrow[d, "\psi"] \\
...
\begin{proof}
  ...
  the map $A \oplus B \to C$ considered in \Cref{exam:homol:fibered-diagram-exact-sequence}.
  Since epimorphisms are cokernels in an abelian category and
  cokernels are cokernels of their kernels (\Cref{lem:homol:kernel-is-kernel-of-its-cokernel}), we
  ...
  (\Cref{lem:homol:mono-epi-via-zero-morphisms}) to show that if $\zeta : B \to Z$ is a
...
(\Cref{exam:homol:fibered-diagram-exact-sequence}). By
\Cref{exer:homol:pullback-kernel-comparison-iso},
$\coker\varphi' = \coker\varphi = 0$; therefore $\varphi'$ is an
epimorphism. The reader should have no difficulty now proving the
statement dual to \Cref{lem:homol:epimorphisms-stable-under-pullback}, for fibered coproducts
(\Cref{exer:homol:pushout-preserves-monomorphism}).  Going back to the
snake lemma, start from a commutative diagram linking two exact
sequences in an abelian category:
\[\begin{tikzcd}
...
morphism\footnote{To avoid unbearably heavy notation, I will write
  ...
  etc.}  $\delta : \ker\nu \to \coker\lambda$; once this is done, we
should prove the exactness of the sequence displayed in
\Cref{lem:rings:snake-lemma}. I will leave this latter step to the
more enterprising reader (who may want to wait until we go through
\Cref{subsec:homol:working-with-elements-in-a-small-abelian-category}).
...
and \emph{from} $\coker\lambda$. There's the rub: how on earth are we
going to get a morphism \emph{from} $\ker\nu$ \emph{to}
$\coker\lambda$? (Last chance to try on your own!)

The answer is to view $\ker\nu$ as a cokernel and $\coker\lambda$ as a
kernel. This can be achieved by constructing suitable pull-back,
resp., push-out, diagrams:
\[\begin{tikzcd}
  ...
  \begin{tikzcd}
    ...
    \coker\lambda \arrow[r, "\alpha_0'"'] & (\coker\lambda)
    \amalg_{L_0} M_0.
  \end{tikzcd}\] By \Cref{lem:homol:epimorphisms-stable-under-pullback}, $\beta_1'$ is an
epimorphism since $\beta_1$ is an epimorphism, and $\ker\beta_1$,
$\ker\beta_1'$ have matching sources by
\Cref{exer:homol:pullback-kernel-comparison-iso}. Analogous (dual)
statements hold for the second diagram
(\Cref{exer:homol:pushout-preserves-monomorphism,exer:homol:pullback-kernel-comparison-iso}).
Putting everything into one commutative diagram,
\[\begin{tikzcd}[column sep=small]
    ...
    (\coker\lambda) \amalg_{L_0} M_0 \arrow[r, "\tau"'] & N_0
    \arrow[r] & 0
...
\]
Since $\coker\lambda$ plays the role of kernel in the bottom row and
$\tau \circ \epsilon' : \ker\nu \to N_0$ is the zero-morphism (because
...
\Cref{subsec:rings:homology-and-the-snake-lemma}. Together with
maps induced by simpler considerations, we obtain the sequence of
morphisms
\[\begin{tikzcd}[column sep=small]
    0 \arrow[r] & \ker\lambda \arrow[r] & \ker\mu \arrow[r] & \ker\nu
    \arrow[r, "\delta"] & \coker\lambda \arrow[r] & \coker\mu
    \arrow[r] & \coker\nu \arrow[r] & 0
...
However, assuming that we have already done this work in
\Cref{subsec:rings:homology-and-the-snake-lemma}, by chasing
elements, is it really necessary to go back and redo it with arrows,
in order to check the exactness of the sequence in an abelian
category?
...
construction in
\Cref{subsec:rings:homology-and-the-snake-lemma}. Such issues are
usually resolved once and for all at the start, when the key universal
properties are established, and it is not necessary to belabor them
later.  Still, it is true that element-theoretic arguments are usually
easier to concoct. \Cref{lem:homol:epimorphisms-stable-under-pullback} appears to be a good example:
in terms of elements the statement of this lemma is completely
obvious, less so from the diagrammatic point of
view. \Cref{exer:homol:quotient-iso-image-kernel-two-morphisms} gives
...
theorem (discussed in \Cref{subsec:homol:what-is-missing}) will
obliterate the actual usefulness of the construction, since it
provides us with a much stronger result.  Thus, I will ask the reader
to wade through some material, with the understanding that the result
that I will quote thereafter (\Cref{thm:homol:freyd-mitchell-theorem}) will make that
material immediately obsolete.

My excuse is that while proving \Cref{thm:homol:freyd-mitchell-theorem} is beyond the scope
of this book, the construction which I am about to describe is
entirely within our reach, and good propaganda for the philosophy
underlying the functor of points; this is important in, e.g.,
algebraic geometry and takes some practice to get used to. In the end,
the reader is likely to gain a better understanding of the situation
by appreciating a weaker result that we can prove entirely than by
leaning on a stronger result to be taken on faith. However, the
hurried reader should feel free to skip this material and jump
directly to \Cref{subsec:homol:what-is-missing}.  The idea is to
define an appropriate notion of an `element' of an object of an
abelian category.\footnote{I will systematically put quotes around
  this term, i.e., write `element', to indicate that I am referring to
  the relatively fancy notion I will introduce. The `elements' will be
  ordinary elements of an ordinary (pointed) set determined by the
  object of the category.} The reader should look back at the brief
discussion at the end of \Cref{subsec:linrep:examples-of-functors}:
the `functor of points' $h_A = \Hom_{\categ{A}}(\_, A)$ associates to
every object $Z$ the set of morphisms $Z \to A$; in several
...
preliminary note, recall that in the distant past
(\Cref{exam:prelims:pointed-sets}) we considered the category
$\categ{Set}^*$ of `pointed sets', whose objects consist of sets
endowed with the choice of a distinguished point (here I will denote
by $0$ the distinguished point). All the algebraic structures we have
encountered are pointed sets, often in more than one way: for example,
stripping an abelian group of all its structure except its supporting
set and the choice of the identity element leaves us with a pointed
set (in other words, this defines an evident forgetful functor
...
with the aid of the connecting morphism defined in
\Cref{subsec:homol:the-snake-lemma-again} may be verified by
chasing `elements', since this verifies exactness in
...
really strictly necessary; I will come back to this point in
\Cref{subsec:homol:what-is-missing}.  The construction can be given
as an application of the abstract language developed in
\Cref{sec:linrep:preliminaries-reprise}; but I promise that the
resulting notion will be easy to contemplate, independently of this
language.  The idea is simply to take the functor of points up to a
relation identifying the `element' $Z \to A$ with all `elements'
...
$\categ{Set}^*$.
\begin{defn}{}{defn:homol:elements-of-an-object}
...
reader should check carefully
(\Cref{exer:homol:element-relation-is-equivalence-relation}).  Now I
want to make the assignment $A \mapsto \hat{A}$ into a covariant
...
\begin{lem}{}{lem:homol:element-relation-characterization}
...
\begin{proof}
  ...
  (\Cref{lem:homol:mono-epi-via-zero-morphisms}), as claimed.
...
$0$, which we can do\footnote{Note that I am not saying that
  ...
  \Cref{exer:homol:element-functor-not-faithful}).} at the level of
pointed sets (hence, by chasing elements!) by \Cref{lem:homol:element-relation-characterization}.

Concerning monomorphisms and epimorphisms,

\begin{lem}{}{lem:homol:mono-epi-via-elements}
...
\begin{proof}
  ...
  the first statement
  (\Cref{exer:homol:mono-iff-injective-on-elements}), and I will prove
  the second.
  ...
  \[\begin{tikzcd}
      ...
      \arrow[u, "z"']
    \end{tikzcd}\] By \Cref{lem:homol:epimorphisms-stable-under-pullback}, $\varphi'$ is an
  ...
  there is an `element' of $\hat{A}$ mapping to $\id_B$. This
  `element' is represented by a morphism $z : Z \to A$, and the
  condition $\hat{\varphi}(z) = \id_B$ means that there are
...
\begin{lem}{}{lem:homol:kernel-image-via-elements}
  ...
  $\image\varphi : I \to B$, be the kernel and the image of $\varphi$,
  respectively. Then
  \begin{itemize}
  \item $\widehat{\ker\varphi}$ identifies $\hat{K}$ with the subset
    $\hat{\varphi}^{-1}(0)$ of $\hat{A}$;
  \item $\widehat{\image\varphi}$ identifies $\hat{I}$ with the image
    of $\hat{\varphi}$ in $\hat{B}$.
  \end{itemize}
\end{lem}
...
\begin{proof}
  ...
  (\Cref{exer:homol:elements-of-kernel-identified-with-preimage}). For
  the image, recall that we have a decomposition of $\varphi$,
  \[\begin{tikzcd}
      \varphi : A \arrow[r, two heads] & I \arrow[r, "\image\varphi",
      hook] & B,
    \end{tikzcd}\] and that $\image\varphi : I \to B$ is a kernel of
  $\coker\varphi$ (this is the definition of `image' (cf.\
  \Cref{subsec:homol:images-canonical-decomposition-of-morphisms});
  the fact that $A \to I$ is an epimorphism is part of the content of
  \Cref{thm:homol:canonical-decomposition-of-morphism}).

  To simplify the notation, let $j = \image\varphi$. Then (by
  \Cref{lem:homol:mono-epi-via-elements}) $\hat{\jmath}$ is an injective function
  ...
  \Cref{lem:homol:epimorphisms-stable-under-pullback}, since the top map in the fiber square is an
  ...
  \[\begin{tikzcd}
      ...
      A \arrow[rr, "\varphi"] & & B \arrow[r, "\coker\varphi"] &
      \operatorname{Cok}
    \end{tikzcd}\] Note that
  $\coker\varphi \circ z \circ w_2 = (\coker\varphi \circ \varphi)
  \circ z' \circ w_1 = 0$; since $w_2$ is an epimorphism, it follows
  that $\coker\varphi \circ z = 0$. By the universal property of
  kernels, this says that $z$ factors uniquely through
...
\begin{prop}{}{prop:homol:exactness-via-elements}
...
\begin{proof}
  This now follows immediately from \Cref{lem:homol:kernel-image-via-elements}: exactness
  in $\categ{A}$ means that $\image\varphi = \ker\psi$, and exactness
  in $\categ{Set}^*$ means that the image of $\hat{\varphi}$ equals
  $\hat{\psi}^{-1}(0)$. By \Cref{lem:homol:kernel-image-via-elements}, these conditions are
  equivalent.
\end{proof}
...
\item While we can check that a diagram commutes in $\categ{A}$ by
  ...
  following \Cref{lem:homol:element-relation-characterization}), I have stopped short of claiming
  ...
  (\Cref{exer:homol:element-functor-not-faithful}).
\item The functor $\categ{A} \to \categ{Set}^*$ is not \emph{full};
  ...
  performed in \Cref{subsec:homol:the-snake-lemma-again} in order
  to construct the connecting morphism remains necessary (even if the
  properties of this morphism can then be verified by using
  `elements').
\end{itemize}
...
\begin{thm}{Freyd--Mitchell theorem}{thm:homol:freyd-mitchell-theorem}
...
\Cref{defn:linrep:faithful-full-functor}. Briefly, a functor $F : \categ{A} \to \categ{C}$ is
fully faithful if it induces a bijection
...
exactness (cf.\ \Cref{exam:linrep:exact-functor-definition-example}) of the functor
$\categ{A} \to R\text{-}\categ{Mod}$ ensures that kernels and
cokernels match in the two environments, and so on (note that the
functor is automatically faithfully exact; see
\Cref{exer:homol:faithful-exact-implies-faithfully-exact}): everything
we laboriously checked in
\Cref{subsec:homol:working-with-elements-in-a-small-abelian-category}
for our poor man's functor $\categ{A} \to \categ{Set}^*$ holds for the
much fancier functor provided by the Freyd--Mitchell theorem.  This
functor is fully faithful: this means that one can in fact construct
morphisms in an arbitrary (small) abelian category by working with
...
morphism' discussed in
\Cref{subsec:homol:the-snake-lemma-again}. Monomorphisms really can
be assimilated with `subobjects', and `quotients' in an abelian
category really are quotients in the ordinary sense.  This is
impressive: it says that doing homological algebra in the category of
left-modules over a ring is essentially as general as doing it in an
arbitrary (small) abelian category, at least for what concerns
constructions that do not require the category to have special objects
(such considerations will play a role in
\Cref{sec:homol:the-homotopic-category-complexes-of-projectives-and}
and following). In the rest of the chapter I will adopt the common
...
unapologetically, and often so would the simpler considerations of
\Cref{subsec:homol:working-with-elements-in-a-small-abelian-category}.

\subsection*{Exercises}

\begin{exer}{}{exer:homol:exactness-via-same-kernel-cokernel}
  In an abelian category, prove that
  $A \xrightarrow{\varphi} B \xrightarrow{\psi} C$ is exact at $B$ if
  and only if $\psi$ and $\coker\varphi$ (both of which are morphisms
  with $B$ as source) have the same kernel, if and only if $\varphi$
  and $\ker\psi$ (both of which are morphisms with $B$ as target) have
  the same cokernel.
\end{exer}
\begin{exer}{}{exer:homol:pushout-preserves-monomorphism}
  ...
  \Cref{lem:homol:epimorphisms-stable-under-pullback}.)
  [\Cref{subsec:homol:the-snake-lemma-again}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:four-lemma-in-abelian-categories}
  Prove the `four-lemma' (cf.\
  \Cref{exer:rings:four-lemma-first-version,exer:rings:four-lemma-second-version})
  in
  ...
  automatically. [\ref{exer:rings:four-lemma-second-version}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:short-five-lemma}
  Prove the `short five-lemma' (cf.\
  \Cref{exer:rings:short-five-lemma-via-snake-lemma}) in any abelian
  ...
  diagram. [\ref{exer:homol:five-lemma}]
\end{exer}
\begin{exer}{}{exer:homol:element-relation-is-equivalence-relation}
  \exerused{} Prove that the relation used in
  \Cref{subsec:homol:working-with-elements-in-a-small-abelian-category}
  to define the notion of `element' of an object of an abelian
  category is indeed an equivalence
  relation. [\Cref{subsec:homol:working-with-elements-in-a-small-abelian-category}]
\end{exer}
\begin{exer}{}{exer:homol:mono-iff-injective-on-elements}
  ...
  `elements'. (Cf.\ \Cref{lem:homol:mono-epi-via-elements}.)
  [\Cref{subsec:homol:working-with-elements-in-a-small-abelian-category}]
\end{exer}
\begin{exer}{}{exer:homol:elements-of-kernel-identified-with-preimage}
  ...
  \Cref{lem:homol:kernel-image-via-elements}.)
  [\Cref{subsec:homol:working-with-elements-in-a-small-abelian-category}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:id-and-neg-id-same-element}
  Let $\categ{A}$ be an abelian category, and let $A$ be an object of
  $\categ{A}$. Prove that $\id_A : A \to A$ and $-\id_A : A \to A$
  determine the same `element' of $A$ in the sense of
  \Cref{subsec:homol:working-with-elements-in-a-small-abelian-category}. [\ref{exer:homol:element-functor-not-faithful}]
\end{exer}
\begin{exer}{}{exer:homol:element-relation-not-compatible-with-addition}
  Prove that the equivalence relation $\sim$ introduced in
  \Cref{subsec:homol:working-with-elements-in-a-small-abelian-category}
  does not preserve the addition in Hom-sets, in the sense that
  $\varphi \sim \psi \not\Rightarrow (\varphi + \eta) \sim (\psi +
  \eta)$.
\end{exer}
\begin{exer}{}{exer:homol:element-functor-not-faithful}
  \exerused{} Let $\categ{A}$ denote an abelian category. Prove that
  the functor $\categ{A} \to \categ{Set}^*$, $A \mapsto \hat{A}$
  defined in
  \Cref{subsec:homol:working-with-elements-in-a-small-abelian-category}
  is not faithful in general. (Use
  \Cref{exer:homol:id-and-neg-id-same-element}.)
  [\Cref{subsec:homol:working-with-elements-in-a-small-abelian-category},
  \Cref{subsec:homol:what-is-missing}]
\end{exer}
\begin{exer}{}{exer:homol:five-lemma}
  Upgrade the result of \Cref{exer:homol:short-five-lemma}, by proving
  the \emph{five-lemma}: if
  \[\begin{tikzcd}
  ...
  `elements' and the material developed in
  \Cref{subsec:homol:working-with-elements-in-a-small-abelian-category}.
\end{exer}
\begin{exer}{}{exer:homol:quotient-iso-image-kernel-two-morphisms}
  ...
  \[\begin{tikzcd}
    ...
    $\ker\nu$, etc.; see \Cref{rem:homol:subobject-quotient-notation} for `quotients'.}
  \[
  ...
  to prove it in $R\text{-}\categ{Mod}$.)
  [\Cref{subsec:homol:working-with-elements-in-a-small-abelian-category},
  \Cref{subsec:homol:spectral-sequences}]
\end{exer}
\begin{exer}{}{exer:homol:fin-dim-vector-spaces-not-fully-embeddable-in-ab}
  By refining the construction in
  \Cref{subsec:homol:working-with-elements-in-a-small-abelian-category},
  one can show that every small abelian category can be embedded in
  $\categ{Ab}$. However, it is not always possible to do it
  \emph{fully}. Indeed, prove that the abelian category of
  finite-dimensional $\R$-vector spaces is not equivalent to any full
  subcategory of $\categ{Ab}$.  (Hint: If it were, the one-dimensional
  vector space $\R$ would correspond to an abelian group $A$ such that
  $\End_{\categ{Ab}}(A) \cong
  \End_{\R\text{-}\categ{Vect}}(\R)$.  But look at
  \Cref{exer:linalg:end-a-field-char-0-implies-a-iso-q}.)
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:full-abelian-subcategory-characterization}
  ...
  \ref{exer:homol:category-of-cochain-complexes-is-abelian}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:contravariant-additive-functors-category-abelian}
  ...
  morphisms, and show that it is abelian. (Use
  \Cref{exer:homol:functor-category-into-abelian-is-abelian,exer:homol:full-abelian-subcategory-characterization}.)
  [\ref{exer:homol:yoneda-embedding-left-exact-reflects-exactness}]
\end{exer}
\begin{exer}{}{exer:homol:faithful-exact-implies-faithfully-exact}
  ...
  $\categ{B}$. [\Cref{subsec:homol:what-is-missing}]
\end{exer}

\begin{exer}{}{exer:homol:yoneda-embedding-left-exact-reflects-exactness}
  Upgrade the Yoneda lemma (\Cref{exer:linrep:yoneda-lemma}) to prove
  that every small abelian category $\categ{A}$ is equivalent to a
  full subcategory of the category $\categ{F}$ of
  \Cref{exer:homol:contravariant-additive-functors-category-abelian},
  by means of the functor assigning to each object $X$ in $\categ{A}$
  the functor $h_X = \Hom_{\categ{A}}(\_, X)$.
  ...
  thus, \Cref{exer:homol:full-abelian-subcategory-characterization}
  doesn't help), and the Yoneda embedding of $\categ{A}$ in
  $\categ{L}$ is exact. Finally, one proves that $\categ{L}$ is
  equivalent to the category of modules over a ring (the endomorphism
  ring of a `faithfully projective' object), and the Freyd--Mitchell
  theorem follows.
\end{exer}
...
fixed abelian category $\categ{A}$; by what we have seen in
\Cref{sec:homol:working-in-abelian-categories}, we are allowed to
pretend that the objects of $\categ{A}$ are ordinary modules over a
ring, at least if $\categ{A}$ is small. Thus we will deal with objects
and morphisms with the aid of usual set-theoretic
...
I will start off with a reminder of what we did in
\Cref{subsec:rings:complexes-and-exact-sequences}. Once more, as we
now know, everything can in fact be done in the more general setting
...
of an application. In \Cref{sec:rings:complexes-and-homology} I
chose homology; in this chapter I will choose cohomology: thus,
indices will be increasing upper indices. Setting $M_i = M^{-i}$
reconciles the two conventions.  Thus, a chain complex
$(M_\bullet, d_\bullet)$ may be (and usually is) viewed as the cochain
complex $(M^{-\bullet}, d^{-\bullet})$.

The condition $d^i \circ d^{i-1} = 0$ is equivalent to
$\image d^{i-1} \subseteq \ker d^i$. The complex is \emph{exact} at
$M^i$ if $\image d^{i-1} = \ker d^i$; it is `exact' if it is exact
everywhere. The $i$-th \emph{cohomology} of a complex
$(M^\bullet, d^\bullet)$ measures its `deviation from exactness' at
$M^i$:
\[
...
\begin{rem}{}{rem:homol:cohomology-definition-parsing}
  In an abelian category, the definition of cohomology should be
  parsed as follows (cf. \Cref{rem:homol:subobject-quotient-notation}). Let $I \to M^i$ be
  $\image d^{i-1}$, and let $K \to M^i$ be $\ker d^i$; since
  $M^\bullet$ is a complex, $\image d^{i-1}$ factors through
...
\begin{defn}{}{defn:homol:resolution-preliminary}
...
(\Cref{defn:homol:quasi-isomorphism}) will give us the correct framework in
...
\[
  ...
  \longrightarrow \dots
\]
is a resolution of $A$ if $\coker d^{-1} \cong A$, and the complex is
exact otherwise. Down the road we will restrict our attention to
bounded (-above or -below) resolutions. In a sense, this is not a very
serious restriction: any resolution may be truncated (at the price of
changing the degree-0 term) in order to produce a bounded resolution
(\Cref{exer:homol:resolution-gives-exact-truncated-complexes}). Note
that the datum of a bounded-above resolution is equivalent to that of
an exact complex
\[
...
$\Tor^R_i(M, N)$
(cf. \Cref{subsec:linrep:the-tor-functors}). Algebraic topology is
a source of many other examples and was in fact historically the main
motivation driving the development of homological algebra. In the
...
have in fact claimed (in \Cref{subsec:linrep:the-ext-functors})
that resolutions by \emph{projective} modules, or even \emph{flat}
...
associated to the given mathematical object, and one of our main goals
here will be to show that (for a large class of interesting examples)
the (co)homology of the complex will not depend on this choice.
...
\begin{itemize}
\item
  $\Obj(\categ{C}(\categ{A})) = \{\text{cochain complexes in }
  \categ{A}\}$;
...
\begin{lem}{}{lem:homol:category-of-complexes-is-abelian}
  $\categ{C}(\categ{A})$ is an abelian category.
\end{lem}

\begin{proof}
  I will leave to the reader the careful verification of this fact
  (\Cref{exer:homol:category-of-cochain-complexes-is-abelian}). In
  broad terms, morphisms between two given complexes form an abelian
  group, essentially because if $\alpha^i$ and $\beta^i : M^i \to N^i$
  are both collections of morphisms making the appropriate diagram
  commute, so is the collection of their sums $\alpha^i +
  \beta^i$. Finite products and coproducts exist in
  ...
  \[\begin{tikzcd}
      ...
      N^i \arrow[rr, "d^i_{N^\bullet}"'] \arrow[d, "\coker\alpha^i"']
      &
  ...
  $\coker\alpha^i : N^i \to C^i$ give morphisms of complexes
  $\ker\alpha^\bullet$, $\coker\alpha^\bullet$ satisfying the
  universal property for kernels and cokernels in
...
(\Cref{subsec:homol:complexes-of-complexes}).
...
\item $\categ{C}^+(\categ{A})$ denote the full\footnote{That is,
    ...
    \Cref{exer:prelim:subcat}.} subcategory of $\categ{C}(\categ{A})$
  determined by complexes $L^\bullet$ which are bounded \emph{below},
  i.e., for which $L^i = 0$ for $i \ll 0$.
...
(see
\Cref{subsec:homol:complexes-of-projective-and-injective-objects});
thus, $\categ{C}^{\leq 0}(\categ{P})$, $\categ{C}^{\geq 0}(\categ{I})$
and other variations on the same themes are fair game\footnote{But
  note that $\categ{P}$ and $\categ{I}$ are not abelian
  categories!}. In the generalities that follow I will only deal with
$\categ{C}(\categ{A})$, to keep notation under control. The reader
should have no difficulty determining what extends immediately to the
different variants of $\categ{C}(\categ{A})$ and what does not.
...
defined by setting\footnote{The sign in the differentials is inserted
  ...
  change the isomorphism class of the complex
  (\Cref{exer:homol:sign-change-differential-isomorphic-complex}).}
...
\begin{lem}{}{lem:homol:cohomology-is-additive-functor}
...
\begin{proof}
  ...
  \]
  This morphism is $0$ when restricted to $\image d^{i-1}$, by the
  commutativity of the square on the left. Therefore $\alpha^i$
  induces a morphism
  \[
...
\Cref{subsec:rings:complexes-and-exact-sequences} is the famous
snake lemma, \Cref{lem:rings:snake-lemma}, which has come back to
entertain us in \Cref{subsec:homol:the-snake-lemma-again}. Written
cohomologically and with the benefit of the language introduced in
\Cref{subsec:homol:the-category-of-complexes}, the snake lemma
takes the following form. Consider a commutative diagram
\[\begin{tikzcd}[column sep=small]
...
\Cref{subsec:rings:homology-and-the-snake-lemma} and can be
summarized (cohomologically speaking) as follows:
\begin{itemize}
...
We checked in \Cref{subsec:rings:homology-and-the-snake-lemma} that
choosing a different preimage for $n$ in $M^0$, while it may change
the element $\ell$, does not change its image in
$\coker(\lambda^0)$. Thus $\delta$ is well-defined. As we have seen in
\Cref{subsec:homol:the-snake-lemma-again}, it is relatively
straightforward to provide an arrow-theoretic version of this
construction; but we have also seen (\Cref{thm:homol:freyd-mitchell-theorem}) that we do not
need to do that, so I will stay with elements in what follows.  Now we
will upgrade this construction, in what should come across as a very
natural development. In fact, what we are going to do could be
absorbed into the snake lemma itself (cf.\
\Cref{exer:homol:reduce-long-exact-seq-to-snake-lemma}); but I have
...
$\mu^i(m) \in \image\mu^i$. The image of $\mu^i(m)$ in $M^{i+2}$ must
be $0$, because the central column is a complex:
...
$\image\nu^{i-1}$? I claim it is $0$. Indeed, if $n$ comes from
$N^{i-1}$, then a preimage in $M^i$ of $\nu^{i-1}(n)$ may be obtained
by taking a preimage of $n$ in $M^{i-1}$ and mapping it to $M^i$; but
since the central column is a complex, this preimage maps to $0$ in
$M^{i+1}$. It follows that $\delta$ maps $\nu^{i-1}(n)$ to $0$:
% TODO: diagram showing the degenerate chase producing delta(n) = 0
% when n comes from im nu^{i-1} (page 599)
Therefore, $\delta$ factors through the quotient by $\image\nu^{i-1}$.  Since
$\ker\nu^i / \image\nu^{i-1}$ is nothing but the cohomology
$H^i(N^\bullet)$, we have obtained a morphism
\[
  \delta^i : H^i(N^\bullet) \to H^{i+1}(L^\bullet).
\]
On the other hand, each $H^i$ is a functor (\Cref{lem:homol:cohomology-is-additive-functor}); thus
...
$\categ{A}$
(\Cref{exer:homol:ses-of-complexes-non-exact-cohomology-example}). In
other words, the $i$-th cohomology is not an exact functor even if, as
we will see, the cohomology sequence \emph{is} exact at
$H^i(M^\bullet)$. Understanding the exactness and failure of exactness
of the functors $H^i$ is, in a sense, what this discussion is all
about.
...
\begin{thm}{Long exact cohomology sequence}{thm:homol:long-exact-cohomology-sequence}
...
(or conversely, depending on your taste; cf.\
\Cref{exer:homol:reduce-long-exact-seq-to-snake-lemma}).
...
\begin{proof}
  ...
  for the reader
  (\Cref{exer:homol:details-of-long-exact-cohomology-sequence}). But I
  will stress the extent to which $H^i$ is exact `on the nose': part
  ...
  $\ker\beta \subseteq \image\alpha$. If $\bar m$ is a class in
  $H^i(M^\bullet)$ such that $\beta(\bar m) = 0$ in $H^i(N)$, $\bar m$
  is represented by an element $m \in \ker\mu^i$ such that
  $\beta^i(m) \in \image\nu^{i-1}$: $\beta^i(m) = \nu^{i-1}(n)$ for
  some $n \in N^{i-1}$. There is an $m' \in M^{i-1}$ such that
  $\beta^{i-1}(m') = n$, and $m - \mu^{i-1}(m')$ represents the same
  class $\bar m$ in $H^i(M)$ as $m$. By the commutativity of the
  diagram,
  \[
  ...
  and is left to the reader.\footnote{The reader is welcome to look
    ...
    \Cref{sec:homol:working-in-abelian-categories}, element-based
    proofs suffice to prove the statement in any (small) abelian
    category.}
\end{proof}
...
\Cref{subsec:homol:the-long-exact-cohomology-sequence} is the fact
that we can connect $N^\bullet$ to $L^\bullet$ in an interesting way,
after taking cohomology and allowing for a shift.  Well, if we are
willing to settle for something less interesting, we can connect
$N^\bullet$ to $L^\bullet$ right away, by simply mapping $N^\bullet$
to $0$ within $L^\bullet$:
\[
...
exact sequence of complexes. In
\Cref{subsec:homol:the-long-exact-cohomology-sequence} we have
obtained from this \emph{another} long exact sequence,
\[
  \dots \longrightarrow H^{i-1}(N^\bullet) \xrightarrow{\delta^{i-1}}
  H^i(L^\bullet) \longrightarrow H^i(M^\bullet) \longrightarrow
  H^i(N^\bullet) \xrightarrow{\delta^i} H^{i+1}(L^\bullet)
...
see the end of \Cref{subsec:homol:the-category-of-complexes}).
...
later\footnote{Beware that some authors call the distinguished
  ...
  from this viewpoint.}
(\Cref{sec:homol:the-homotopic-category-complexes-of-projectives-and}). As
with the `special' triangles given above, distinguished triangles lead
to long exact sequences in cohomology, and in a somewhat more direct
fashion. Distinguished triangles satisfy a number of axioms, which
define $\categ{K}(\categ{A})$ as a \emph{triangulated
  category}. Exploring these concepts at any level of depth is well
beyond the scope of this book, but towards the end of the chapter I
will try to clarify these last cryptic remarks
(\Cref{subsec:homol:triangulated-categories}).

\subsection*{Exercises}

\begin{exer}{\exerused{}}{exer:homol:resolution-gives-exact-truncated-complexes}
  ...
  \]
  \[
    \dots \longrightarrow 0 \longrightarrow A \longrightarrow \coker
    d^{-1} \longrightarrow M^1 \xrightarrow{d^1} M^2 \xrightarrow{d^2}
    \dots
  \]
  and that replacing $A$ by $0$ in these complexes produces new
  resolutions of
  $A$. [\Cref{subsec:homol:reminder-of-basic-definitions-general-strategy},
  \Cref{subsec:homol:recovering-a}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:cochain-complexes-as-functor-subcategory}
  Let $\categ{I}$ be the category whose objects are the integers and
  where $\Hom_{\categ{I}}(m, n)$ consists of a singleton if $m \leq n$
  and is $\emptyset$ otherwise (cf.\
  \Cref{exam:prelims:set-is-a-category}). Show that the category
  $\categ{C}(\categ{A})$ of cochain complexes is a full subcategory of
  the abelian category
  $\categ{A}^{\categ{I}}$. [\ref{exer:homol:category-of-cochain-complexes-is-abelian}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:category-of-cochain-complexes-is-abelian}
  Verify that the category $\categ{C}(\categ{A})$ of cochain complexes
  in an abelian category, defined in
  \Cref{subsec:homol:the-category-of-complexes}, is itself an
  abelian category. (You can do this either `by hand' or by applying
  \Cref{exer:homol:cochain-complexes-as-functor-subcategory,exer:homol:full-abelian-subcategory-characterization}.)
  [\Cref{subsec:homol:the-category-of-complexes}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:category-of-short-exact-sequences-properties}
  ...
  \item Show that, for every object $X$ of $\categ{A}$,
    \[\begin{tikzcd}[column sep=small]
        0 \arrow[r] & 0 \arrow[r] & X \arrow[r, "\id_X"] \arrow[d,
        "\id_X"'] &
        X \arrow[r] & 0 \\
        0 \arrow[r] & X \arrow[r, "\id_X"'] & X \arrow[r] & 0
        \arrow[r] & 0 \arrow[from=1-4, to=2-2, "\id_X"']
  ...
  \end{itemize} [\Cref{subsec:homol:abelian-categories},
  \ref{exer:homol:long-exact-cohomology-sequence-is-functorial}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:sign-change-differential-isomorphic-complex}
  ...
  isomorphic to $(L^\bullet,
  d^\bullet_{L^\bullet})$. [\Cref{subsec:homol:the-category-of-complexes}]
\end{exer}
\begin{exer}{}{exer:homol:arrow-theoretic-proof-of-snake-lemma-variant}
  Provide an arrow-theoretic proof of \Cref{lem:homol:cohomology-is-additive-functor} (cf.\
  \Cref{rem:homol:cohomology-definition-parsing}).
\end{exer}
\begin{exer}{}{exer:homol:exact-functors-commute-with-cohomology}
  ...
  functor is still an exact complex (cf.\
  \Cref{exer:linrep:exact-functor-characterization}).
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:ses-of-complexes-non-exact-cohomology-example}
  ...
  [\Cref{subsec:homol:universal-property-of-the-derived-functor}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:details-of-long-exact-cohomology-sequence}
  Provide the gory details for the long exact cohomology sequence
  (\Cref{thm:homol:long-exact-cohomology-sequence}). [\Cref{subsec:homol:the-long-exact-cohomology-sequence},
  \ref{exer:homol:reduce-long-exact-seq-to-snake-lemma}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:reduce-long-exact-seq-to-snake-lemma}
  Here is a way to reduce
  \Cref{exer:homol:details-of-long-exact-cohomology-sequence} to the
  snake lemma itself.

  Every complex $(L^\bullet, \lambda^\bullet)$ determines a complex
  $\ker^\bullet$, with the object\footnote{Of course $\ker$ and
    $\coker$ mean the source and target of the arrows $\ker$ and
    $\coker$, respectively.}  $\ker\lambda^{i+1}$ in degree $i$,
  connected by zero-morphisms, and a complex $\coker^\bullet$, with
  the object $\coker\lambda^{i-1}$ in degree $i$, also connected by
  zero-morphisms.

  Prove that $\lambda^\bullet$ induces a morphism
  $\overline{\lambda}^\bullet : \coker^\bullet \to \ker^\bullet$ in
  the abelian category of complexes, such that
  $\ker\overline{\lambda}^\bullet \cong H^\bullet(L^\bullet)$ and
  $\coker\overline{\lambda}^\bullet \cong H(L^\bullet)[1]^\bullet$.

  Now work out the whole long exact cohomology sequence as a
  consequence of the snake lemma. (You will use the version of the
  snake lemma given in
  \Cref{rem:rings:snake-lemma-relaxed-hypotheses}.)
  [\Cref{subsec:homol:the-long-exact-cohomology-sequence},
  \ref{exer:homol:differential-category-is-abelian}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:long-exact-cohomology-sequence-is-functorial}
  ...
  (cf.\
  \Cref{exer:homol:category-of-short-exact-sequences-properties}) of
  complexes to the category of
  complexes. [\ref{exer:homol:delta-functor-definition-derived-functors-are}]
\end{exer}
\begin{exer}{}{exer:homol:two-exact-columns-implies-third-abelian-category}
  Redo
  \Cref{exer:rings:infinite-ladder-two-exact-columns-implies-third}.
\end{exer}
\begin{exer}{}{exer:homol:differential-category-is-abelian}
  The viewpoint in
  \Cref{exer:homol:reduce-long-exact-seq-to-snake-lemma} admits the
  ...
  \item For any $(A, d)$ in $d\categ{A}$, prove that $d$ induces a
    morphism $\bar d : \coker d \to \ker d$.
  ...
  \end{itemize}
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:exact-vertex-implies-isomorphic-cohomologies}
  Prove that if one of the vertices of a `special' triangle arising
  from a short exact sequence of complexes (as in
  \Cref{subsec:homol:triangles}) is exact, then the other two
  vertices have isomorphic cohomologies, possibly up to a
  shift. [\Cref{subsec:homol:the-mapping-cone-of-a-morphism}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:grothendieck-grp-euler-char-abelian-category}
  ...
  $\chi(M^\bullet) = \chi(L^\bullet) +
  \chi(N^\bullet)$. [\ref{exer:homol:euler-char-of-mapping-cone},
  \ref{exer:homol:euler-char-additive-on-distinguished-triangles}]
\end{exer}
...
compensate for this lack of exactness; we will see how in
\Cref{subsec:homol:the-mapping-cone-of-a-morphism}. We will also
begin exploring a very important condition (`homotopy') guaranteeing
that two given morphisms of complexes induce the same morphism in
cohomology. Homotopies will be so important that they will lead us
later on to change our notion of morphisms between complexes and
construct a new `homotopic category' of complexes (see
\Cref{sec:homol:the-homotopic-category-complexes-of-projectives-and}).
...
constructed in
\Cref{subsec:homol:the-long-exact-cohomology-sequence}. In fact,
the mapping cone will give us the third vertex of a `special' triangle
(cf.\ the end of \Cref{subsec:homol:triangles}):
\[
  (\dagger) \quad
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    \node (M) at (1,1.3)
...
`special' triangles encountered in \Cref{subsec:homol:triangles});
but taking cohomology
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    \node (M) at (1.7,1.3)
    {$H^\bullet(M^\bullet)$}; \node (L) at (0,0)
...
prototype of the `distinguished triangles' mentioned in
\Cref{subsec:homol:triangles} (cf.\
\Cref{subsec:homol:triangulated-categories}).
...
(\Cref{exer:homol:cone-of-morphism-is-a-complex}); this is where the
sign comes in. The mapping cone is a special case of the `total
complex' determined by a double complex, to be explored later
(\Cref{subsec:homol:complexes-of-complexes}).
...
\]
is exact.
\begin{prop}{}{prop:homol:mapping-cone-exact-triangle}
...
\begin{proof}
  ...
  \Cref{thm:homol:long-exact-cohomology-sequence}; all we have to check is that the connecting
  morphism indeed agrees with the morphism induced by
  $\alpha^\bullet$.  Chasing the diagram
  \[\begin{tikzcd}[column sep=small]
      & \vdots & \vdots & \vdots & \\
...
\begin{coro}{}{coro:homol:quasi-iso-iff-mapping-cone-exact}
  Let $\alpha^\bullet : L^\bullet \to M^\bullet$ be a morphism of
  ...
  (Cf.\
  \Cref{exer:homol:exact-vertex-implies-isomorphic-cohomologies}.)
\end{coro}
...
\Cref{coro:homol:quasi-iso-iff-mapping-cone-exact} provides us with such a reason. The morphisms singled
out in that statement have a name:

\begin{defn}{}{defn:homol:quasi-isomorphism}
  A morphism $\alpha^\bullet$ of cochain complexes is a
  \emph{quasi-isomorphism} if it induces an isomorphism in cohomology.
\end{defn}

By \Cref{coro:homol:quasi-iso-iff-mapping-cone-exact}, this condition is equivalent to the exactness of
the mapping cone of $\alpha^\bullet$.

\begin{exam}{}{exam:homol:resolution-as-quasi-isomorphism}
  The datum of a resolution $M^\bullet$ of an object $A$ of an abelian
  category $\categ{A}$, as in \Cref{defn:homol:resolution-preliminary} and with $M^i = 0$ for
...
resolution as in \Cref{exam:homol:resolution-as-quasi-isomorphism} is obtained (as the reader should
check) by shifting the complex `one step to the left' and completing
it with $A$, obtaining the \emph{exact} complex:
\[
  \dots \longrightarrow M^{-1} \xrightarrow{-d^{-1}_{M^\bullet}} M^0
  \xrightarrow{\coker d^{-1}_{M^\bullet}} A \longrightarrow 0
  \longrightarrow \dots.
\]
This is nothing but our other point of view on resolutions (as in
\Cref{subsec:linalg:finitely-presented-modules-and-free-resolutions}).

\begin{rem}{}{rem:homol:quasi-isomorphisms-form-a-category}
  ...
  as in \Cref{exam:homol:resolution-as-quasi-isomorphism}, form a category.
\end{rem}
...
\begin{exam}{}{exam:homol:quasi-iso-not-homotopy-invertible}
...
\Cref{defn:homol:enough-projectives-injectives}).
...
triangles' (cf.\ \Cref{subsec:homol:triangles}) abstracting exact
sequences and giving long exact cohomology sequences. The derived
category is a \emph{triangulated category}, like the more manageable
homotopic category $\categ{K}(\categ{A})$ that we will soon
define. Its uses go beyond the confines of algebra or even
mathematics: an approach to the understanding of `D-branes' in string
theory is based on derived categories.
...
\begin{lem}{}{lem:homol:functor-invariant-under-quasi-iso-basic-properties}
...
\begin{proof}
  ...
  \Cref{exer:homol:zero-obj-iff-id-is-zero-morphism}.
\end{proof}
...
\begin{defn}{}{defn:homol:homotopy}
...
dimensions up, in \Cref{defn:homol:homotopy} it shifts degrees down.
...
\begin{defn}{}{defn:homol:homotopy-equivalence}
...
that `homotopy equivalence of complexes' is also
(\Cref{exer:homol:homotopy-and-homotopy-equiv-are-equiv-relations}).
Further, these relations are clearly compatible with simple operations
of morphisms
(\Cref{exer:homol:homotopy-compatible-with-direct-sum,exer:homol:homotopy-compatible-with-composition}).

\begin{prop}{}{prop:homol:homotopic-morphisms-agree-on-cohomology}
...
\begin{proof}
  ...
  homotopic, according to \Cref{defn:homol:homotopy} there are morphisms $h^i$
  such that
  \[
  ...
  shows that
  \[
    \beta^i(\ell) - \alpha^i(\ell) \in \image d^{i-1}_{M^\bullet},
  \]
...
\Cref{coro:homol:quasi-iso-iff-mapping-cone-exact} the cone of $\alpha^\bullet$ must be exact. It is a
good exercise to verify this fact directly
(\Cref{exer:homol:cone-of-null-homotopic-morphism-is-exact}).

\begin{coro}{}{coro:homol:homotopy-equivalent-complexes-isomorphic-cohomology}
...
\begin{proof}
  ...
  \Cref{prop:homol:homotopic-morphisms-agree-on-cohomology}.
\end{proof}

\begin{rem}{}{rem:homol:homotopy-equivalence-stronger-than-quasi-iso}
  In other words, homotopy equivalences of complexes are
  quasi-isomorphisms. This does not yet solve the `problem' that
  quasi-isomorphisms are not invertible, since \Cref{exam:homol:quasi-iso-not-homotopy-invertible} shows
  that quasi-isomorphisms may well not be invertible even up to
  homotopy.  In other words, `homotopy equivalence' is a more
  restrictive notion than `quasi-isomorphism'. One reason to study
  complexes of injective/projective modules, as we will do in
  \Cref{subsec:homol:complexes-of-projective-and-injective-objects},
  is precisely that quasi-isomorphism and homotopy equivalence are
  equivalent notions for (bounded) complexes of injective or
  projective modules. Since homotopy equivalence is a ready-made
  equivalence relation on complexes, this will bypass any technicality
  needed to make sense of `quasi-isomorphism' as an equivalence
  relation.
\end{rem}

\Cref{coro:homol:homotopy-equivalent-complexes-isomorphic-cohomology} is the main technical reason that makes the strategy
presented in
\Cref{subsec:homol:reminder-of-basic-definitions-general-strategy}
work, in a class of interesting examples. As we will see in due time,
in these examples the cohomology of the relevant cochain complexes
will end up being independent of the choices, because these choices
will produce homotopic complexes.
...
\]
is an additive functor (\Cref{defn:homol:additive-category}). Then $\mathscr{F}$ induces
...
\begin{lem}{}{lem:homol:functor-preserves-homotopy}
...
quasi-isomorphisms
(\Cref{exer:homol:additive-vs-exact-functors-quasi-iso-preservation}):
an additive functor does not preserve quasi-isomorphisms (while an
exact functor does).

\Cref{lem:homol:functor-preserves-homotopy} implies that the conclusion of \Cref{coro:homol:homotopy-equivalent-complexes-isomorphic-cohomology}
holds true after applying an additive functor:

\begin{thm}{}{thm:homol:homotopy-equivalence-preserved-under-functor}
...
preparatory work, so it is left to the reader
(\Cref{exer:homol:homotopy-equiv-functor-cohomology-iso}).

I am gracing this statement with theorem status because it is at the
root of the strategy I am pursuing. The content of \Cref{thm:homol:homotopy-equivalence-preserved-under-functor}
is that any mechanism associating to a mathematical object a cochain
...
such complex, take cohomology, and \Cref{thm:homol:homotopy-equivalence-preserved-under-functor} guarantees that
the result will be independent of the specific chosen complex. This is
(part of) the reason underlying the various claims of independence on
choices made in \Cref{subsec:linrep:the-tor-functors} and
\Cref{subsec:linrep:the-ext-functors}, concerning the definition of
the Tor and Ext functors, as we will see later in the chapter
(\Cref{sec:homol:derived-functors}, especially Examples~7.6
and~7.7).

\subsection*{Exercises}

\begin{exer}{\exerused{}}{exer:homol:cone-of-morphism-is-a-complex}
  Verify that the cone of a morphism of complexes is a complex.
  [\Cref{subsec:homol:the-mapping-cone-of-a-morphism}]
\end{exer}
\begin{exer}{}{exer:homol:mapping-cone-degree-0-complexes}
...
\end{exer}
\begin{exer}{}{exer:homol:euler-char-of-mapping-cone}
  Let $\alpha^\bullet : L^\bullet \to M^\bullet$ be a morphism of
  ...
  Euler characteristic (see
  \Cref{exer:homol:grothendieck-grp-euler-char-abelian-category}). Prove
  that
  \[
    \chi(MC(\alpha)^\bullet) = \chi(M^\bullet) - \chi(L^\bullet).
  \]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:homotopy-and-homotopy-equiv-are-equiv-relations}
  ...
  equivalence relations. [\Cref{subsec:homol:homotopy},
  \Cref{sec:homol:projective-and-injective-resolutions-and-the-derived-category}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:homotopy-compatible-with-direct-sum}
  Let
  $\alpha_k^\bullet, \beta_k^\bullet : L^\bullet_k \to M_k^\bullet$,
  $k = 0, 1$, be morphisms of cochain complexes. Assume that
  $\alpha_0^\bullet \sim \beta_0^\bullet$,
  $\alpha_1^\bullet \sim \beta_1^\bullet$. Prove that
  $\alpha_0^\bullet \oplus \alpha_1^\bullet \sim \beta_0^\bullet \oplus
  \beta_1^\bullet$ as morphisms
  $L^\bullet_0 \oplus L^\bullet_1 \to M_0^\bullet \oplus M_1^\bullet$.
  [\Cref{subsec:homol:homotopy}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:homotopy-compatible-with-composition}
  Let
  ...
  \end{enumerate} [\Cref{subsec:homol:homotopy},
  \Cref{subsec:homol:definition-of-the-homotopic-category-of-complexes},
  \Cref{subsec:homol:homotopy-equivalences-vs-quasi-isomorphisms-in-k-a},
  \Cref{subsec:homol:proof-of-theorem-5-9}]
\end{exer}
\begin{exer}{}{exer:homol:parametrization-of-homotopic-morphisms}
...
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:cone-of-null-homotopic-morphism-is-exact}
  Let $\alpha^\bullet$ be a morphism of complexes, homotopic to the
  identity. Prove directly (without appealing to \Cref{coro:homol:quasi-iso-iff-mapping-cone-exact})
  that the mapping cone of $\alpha^\bullet$ is
  exact. [\Cref{subsec:homol:homotopy}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:mapping-cylinder-is-a-complex}
  Let $\alpha^\bullet : L^\bullet \to M^\bullet$ be a morphism of
  ...
  complex. [\ref{exer:homol:homotopic-maps-extend-to-cylinder-morphism},
  \ref{exer:homol:exact-sequence-cylinder-to-cone},
  \ref{exer:homol:exact-sequence-target-to-cylinder-quasi-iso},
  \ref{exer:homol:cylinder-homotopy-equivalent-to-target}]
\end{exer}
\begin{exer}{}{exer:homol:homotopic-maps-extend-to-cylinder-morphism}
  ...
  object leads to the algebraic mapping cylinder of
  \Cref{exer:homol:mapping-cylinder-is-a-complex}.
  ...
  to $Y$. Note that this cylinder is the mapping cylinder of $\id_X$.

  Prove that two cochain maps
  $\alpha^\bullet, \beta^\bullet : L^\bullet \to M^\bullet$ are
  homotopic in the sense of \Cref{defn:homol:homotopy} if and only if they
  extend to a cochain morphism
  $(-\alpha^\bullet, h^\bullet, \beta^\bullet) : MCyl(\id_{L^\bullet})
  \to M^\bullet$.
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:exact-sequence-cylinder-to-cone}
  In topology, the mapping cone may be obtained by contracting
  $X \times 0$ to a point in the mapping cylinder (cf.\
  \Cref{exer:homol:mapping-cylinder-is-a-complex} and the description
  of the mapping cone given in
  \Cref{subsec:homol:the-mapping-cone-of-a-morphism}).
  ...
  \]
  [\ref{exer:homol:replace-morphism-by-monomorphism-up-to-homotopy}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:exact-sequence-target-to-cylinder-quasi-iso}
  With notation as in \Cref{exer:homol:mapping-cylinder-is-a-complex}
  (in particular, for a cochain morphism
  $\alpha^\bullet : L^\bullet \to M^\bullet$), prove that there is an
  exact sequence of cochain complexes
  \[
    0 \longrightarrow M^\bullet \longrightarrow MCyl(\alpha)^\bullet
    \longrightarrow MC(\id_{L^\bullet})^\bullet \longrightarrow 0.
  \]
  Deduce that there is a quasi-isomorphism
  $M^\bullet \to
  MCyl(\alpha)^\bullet$. [\ref{exer:homol:cylinder-homotopy-equivalent-to-target},
  \ref{exer:homol:replace-morphism-by-monomorphism-up-to-homotopy}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:cylinder-homotopy-equivalent-to-target}
  Still with notation as in
  \Cref{exer:homol:mapping-cylinder-is-a-complex}, define maps
  ...
  morphisms. Note that
  $\sigma^\bullet \rho^\bullet = \id_{M^\bullet}$, and prove that
  ...
  equivalent. This strengthens the conclusion of
  \Cref{exer:homol:exact-sequence-target-to-cylinder-quasi-iso}.
  [\ref{exer:homol:replace-morphism-by-monomorphism-up-to-homotopy}]
\end{exer}
\begin{exer}{}{exer:homol:replace-morphism-by-monomorphism-up-to-homotopy}
  Combine
  \Cref{exer:homol:exact-sequence-cylinder-to-cone,exer:homol:exact-sequence-target-to-cylinder-quasi-iso,exer:homol:cylinder-homotopy-equivalent-to-target}
  to conclude that, up to
...
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:additive-vs-exact-functors-quasi-iso-preservation}
  Prove that, in general, additive functors do not preserve
  quasi-isomorphisms. (Hint: \Cref{exam:homol:quasi-iso-not-homotopy-invertible}.) Prove that exact
  functors do. (Hint: Use the mapping cone.)
  [\Cref{subsec:homol:homotopy},
  \Cref{sec:homol:projective-and-injective-resolutions-and-the-derived-category}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:homotopy-equiv-functor-cohomology-iso}
  Prove \Cref{thm:homol:homotopy-equivalence-preserved-under-functor}. [\Cref{subsec:homol:homotopy}]
\end{exer}
...
One message I have tried to convey in
\Cref{sec:homol:cones-and-homotopies} is that while
...
applications. This is the moral of \Cref{thm:homol:homotopy-equivalence-preserved-under-functor}: homotopy
equivalent complexes have the same cohomology for a much better reason
than complexes linked by a quasi-isomorphism. If our general aim is to
...
higher-brow considerations at the end of
\Cref{subsec:homol:quasi-isomorphisms-and-derived-categories}. Again
consider any additive functor
\[
  \mathscr{F} : \categ{C}(\categ{A}) \longrightarrow \categ{D}
\]
...
$\categ{D}(\categ{A})$. We have verified in \Cref{prop:homol:homotopic-morphisms-agree-on-cohomology} that
homotopic maps induce the same morphism in cohomology. Might this not
be the case for any functor $\mathscr{F}$ as above? It is!

\begin{lem}{}{lem:homol:quasi-iso-inverting-functor-respects-homotopy}
...
\begin{proof}
  ...
  By \Cref{lem:homol:functor-invariant-under-quasi-iso-basic-properties}, it suffices to show that $\alpha^\bullet$
  factors through an exact complex. This exact complex is the mapping
  cone $MC(\id_{L^\bullet})^\bullet$ of the identity
  $\id_{L^\bullet} : L^\bullet \to L^\bullet$. Recall
  (\Cref{subsec:homol:the-mapping-cone-of-a-morphism}) that
  \[
    MC(\id_{L^\bullet})^i = L^{i+1} \oplus L^i,
  \]
  with differential defined by
  \[
    d^i_{MC(\id_{L^\bullet})}(a, b) = \bigl(-d^{i+1}_{L^\bullet}(a),\;
    a + d^i_{L^\bullet}(b)\bigr).
  \]
  Since the identity is trivially a quasi-isomorphism,
  $MC(\id_{L^\bullet})$ is an exact complex (\Cref{coro:homol:quasi-iso-iff-mapping-cone-exact}). The
  morphism
  \[
    L^i \to MC(\id_{L^\bullet})^i, \qquad b \mapsto (0, b)
  \]
  always commutes with differentials, so it defines a morphism of
  complexes $L^\bullet \to MC(\id_{L^\bullet})^\bullet$.  By contrast,
  a morphism $MC(\id_{L^\bullet})^\bullet \to M^\bullet$ is not always
  available. Under our hypothesis, however, there is a homotopy
  ...
  \[
    \pi^i : MC(\id_{L^\bullet})^i \to M^i \qquad \text{by} \qquad (a,
    b) \mapsto h^{i+1}(a) + \alpha^i(b).
  \]
  It is clear that the composition
  $L^i \to MC(\id_{L^\bullet})^i \to M^i$ is $\alpha^i$. All we have
  to check now is that the collection $\pi^i$ defines a morphism of
  complexes: this will show that $\alpha^\bullet$ factors through the
  exact complex $MC(\id_{L^\bullet})^\bullet$, hence that
  $\mathscr{F}(\alpha^\bullet) = 0$ (by \Cref{lem:homol:functor-invariant-under-quasi-iso-basic-properties}), concluding
  the proof of this lemma. That is, we have to verify that
  \[
...
\Cref{lem:homol:quasi-iso-inverting-functor-respects-homotopy} tells us that every functor transforming
...
\begin{defn}{}{defn:homol:homotopy-category}
...
(\Cref{exer:homol:homotopy-compatible-with-composition}); in other
words, the operation of composition
\[
...
injective, objects from $\categ{A}$
(cf. \Cref{subsec:homol:complexes-of-projective-and-injective-objects})
will also play a crucially important role.

As to the general structure underlying the homotopic category,

\begin{lem}{}{lem:homol:homotopy-category-is-additive}
  Let $\categ{A}$ be an abelian category. Then the homotopic category
  $\categ{K}(\categ{A})$ of complexes is an additive category.
\end{lem}
\begin{proof}
  \Cref{exer:homol:homotopic-categories-are-additive}.
\end{proof}
...
As I already mentioned in \Cref{subsec:homol:triangles},
$\categ{K}(\categ{A})$ is a \emph{triangulated} category; the
`distinguished triangles' are the triangles arising from the cones of
morphisms, as in
\Cref{subsec:homol:the-mapping-cone-of-a-morphism}. (I will briefly
...
We can then reinterpret \Cref{lem:homol:quasi-iso-inverting-functor-respects-homotopy} as the following
`factorization' result:

\begin{prop}{}{prop:homol:functor-factors-through-homotopy-category}
  Let $\mathscr{F} : \categ{C}(\categ{A}) \longrightarrow \categ{D}$
  be an additive functor such that $\mathscr{F}(\rho^\bullet)$ is an
  isomorphism in $\categ{D}$ for all quasi-isomorphisms $\rho^\bullet$
...
\end{prop}
\begin{proof}
  \Cref{exer:homol:functor-factors-through-homotopic-category}.
\end{proof}
...
\[\begin{tikzcd}[column sep=large]
  ...
  \end{tikzcd}\] A particular case of \Cref{prop:homol:functor-factors-through-homotopy-category}
...
follows directly from \Cref{prop:homol:homotopic-morphisms-agree-on-cohomology}: homotopic maps induce the
same morphism in cohomology.
...
equivalences: keep \Cref{exam:homol:quasi-iso-not-homotopy-invertible} in mind. If we want to think of
...
projective or injective objects. Recall (\Cref{defn:linrep:projective-injective-module}) that an
...
\begin{defn}{}{defn:homol:projective-injective-object}
...
General remarks such as \Cref{lem:linrep:projective-lifting-criterion} or the comments about
splitting of sequences at the end of
\Cref{subsec:linrep:projectives-and-injectives} hold in any abelian
...
(\Cref{exer:homol:opposite-of-abelian-is-abelian}), and projectives in
one become injectives in the other. Thus, it is really only necessary
to prove statements for, say, projectives in arbitrary abelian
categories: the corresponding statements for injectives will
...
\Cref{subsec:linrep:projective-modules} and
\Cref{subsec:linrep:injective-modules} use the specific properties
of the categories $R\text{-Mod}$, so they should not be expected to
have a simple counterpart in arbitrary abelian categories.
...
\begin{exam}{}{exam:homol:finite-abelian-groups-no-proj-inj}
  The category of finite abelian groups is abelian (surprise,
  surprise), but contains no nontrivial projective or injective
  objects
  (\Cref{exer:homol:no-nontrivial-proj-inj-finite-abelian-groups}).
\end{exam}

\begin{defn}{}{defn:homol:enough-projectives-injectives}
...
in
\Cref{sec:linrep:projective-and-injective-modules-and-the-ext-functors}:
in particular, we have already observed that, for every commutative
ring $R$, $R\text{-Mod}$ has enough projectives (this is not
challenging: free modules and their direct summands are projective,
\Cref{prop:linrep:projective-iff-direct-summand-of-free}) and enough injectives (this is challenging; we
verified it in \Cref{coro:linrep:module-embeds-in-injective}).

\Cref{exam:homol:finite-abelian-groups-no-proj-inj} shows that we should not expect an
...
\begin{defn}{}{defn:homol:bounded-complexes-of-projectives-injectives}
...
\begin{thm}{}{thm:homol:quasi-iso-of-bounded-proj-inj-complexes-is-homotopy-equivalence}
...
\begin{coro}{}{coro:homol:quasi-isos-are-isomorphisms-in-bounded-proj-inj}
...
The proof of \Cref{thm:homol:quasi-iso-of-bounded-proj-inj-complexes-is-homotopy-equivalence} will require a few
...
$\id_{N^\bullet}$ and the trivial map $0$ induce the same morphism in
cohomology, as they would if they were homotopic to each other. It is
however easy to construct examples of exact complexes for which the
identity is \emph{not} homotopic to $0$. As the reader will verify
(\Cref{exer:homol:split-exact-complex-characterization}), this has to
do with whether the complex splits or not; in general, a complex
$N^\bullet$ is said to be `split exact' if $\id_{N^\bullet}$ is
homotopic to $0$. Keeping in mind that projective or injective objects
`cause' sequences to split, it may not be too surprising that for
bounded exact complexes consisting of projective or injective objects
the identity does turn out to be homotopic to $0$.

To verify this, we study special conditions that ensure that morphisms
are homotopic to $0$.
\begin{lem}{}{lem:homol:zero-cohomology-morphism-null-homotopic}
...
\begin{proof}
  ...
  $L^{i-1}$ for $i \le 0$,
  $\ker d^{i-1}_{L^\bullet} = \image d^{i-2}_{L^\bullet}$. Therefore,
  ...
  step.
\end{proof}
\begin{coro}{}{coro:homol:morphism-to-exact-complex-null-homotopic}
...
\Cref{lem:homol:zero-cohomology-morphism-null-homotopic}, since every morphism to an exact complex
has no choice but to induce the zero-morphism in cohomology.

The `injective' version of \Cref{coro:homol:morphism-to-exact-complex-null-homotopic} is that
...
\Cref{lem:homol:zero-cohomology-morphism-null-homotopic}. It is an essentially inescapable feature
of the theory.

\begin{coro}{}{coro:homol:bounded-exact-complex-homotopy-trivial}
...
\end{coro}
\begin{proof}
  \Cref{exer:homol:exact-bounded-complex-proj-inj-homotopy-trivial}.
\end{proof}

\Cref{coro:homol:bounded-exact-complex-homotopy-trivial} is the first manifestation of the
principle captured more fully by \Cref{thm:homol:quasi-iso-of-bounded-proj-inj-complexes-is-homotopy-equivalence}: we have
...
Another consequence of \Cref{lem:homol:zero-cohomology-morphism-null-homotopic} is the following
...
\begin{lem}{}{lem:homol:quasi-iso-lifting-uniqueness-setup}
  Let $\categ{A}$ be an abelian category, and let
...
\begin{proof}
  ...
  $MC(\rho)^\bullet$ of $\rho^\bullet$
  (\Cref{subsec:homol:the-mapping-cone-of-a-morphism}), and define
  morphisms
  \[
  ...
  complexes $P^\bullet \to MC(\rho)[-1]^\bullet$. Indeed\footnote{It
    ...
    defined in
    \Cref{subsec:homol:the-mapping-cone-of-a-morphism}. The
  ...
  $MC(\rho)^\bullet$ is an exact complex (\Cref{coro:homol:quasi-iso-iff-mapping-cone-exact}). Applying
  \Cref{coro:homol:morphism-to-exact-complex-null-homotopic}, it follows that $\beta^\bullet$ is
  ...
  \[\begin{tikzcd}
    ...
    \end{tikzcd}\] so
  (\Cref{exer:homol:homotopy-compatible-with-composition}, part (i))
  the fact that $\beta^\bullet \sim 0$ implies that
  $\alpha^\bullet \sim 0$, concluding the proof.
\end{proof}

\begin{exam}{}{exam:homol:composition-zero-but-morphism-nonzero-example}
  To see that $\alpha^\bullet$ may not be zero on the nose even if
  $\rho^\bullet \circ \alpha^\bullet = 0$, look back again at
  \Cref{exam:homol:quasi-iso-not-homotopy-invertible}:
  \[\begin{tikzcd}[column sep=small]
      P^\bullet: & \dots \arrow[r] & 0 \arrow[r] \arrow[d] & \Z
      \arrow[r, "\id"] \arrow[d, "\id"'] & \Z \arrow[r] \arrow[d,
      "\cdot 2"'] &
  ...
  0$. According to \Cref{lem:homol:quasi-iso-lifting-uniqueness-setup}, the (nonzero) morphism
  $\alpha^\bullet$ is homotopic to $0$. (Indeed, a homotopy is
  immediately visible.  What is it?)
\end{exam}

\subsection{Proof of \Cref{thm:homol:quasi-iso-of-bounded-proj-inj-complexes-is-homotopy-equivalence}}%
\label{subsec:homol:proof-of-theorem-5-9}

\Cref{thm:homol:quasi-iso-of-bounded-proj-inj-complexes-is-homotopy-equivalence} will follow from a more general observation: a
quasi-isomorphism \emph{to} a complex of projectives, resp.,
\emph{from} a complex of injectives\footnote{That is, going `the wrong
  way': projectives like being the sources of morphisms, and
  injectives like to be targets.}, has a \emph{right}, resp.,
\emph{left}, homotopy inverse.

\begin{prop}{}{prop:homol:quasi-iso-to-projective-complex-has-homotopy-inverse}
  ...
  $\beta^\bullet \circ \alpha^\bullet \sim \id_{Q^\bullet}$.)
\end{prop}
\begin{proof}
  Since $\alpha^\bullet : L^\bullet \to P^\bullet$ is a
  quasi-isomorphism, the mapping cone $MC(\alpha)^\bullet$ of $\alpha$
  is an exact complex (\Cref{coro:homol:quasi-iso-iff-mapping-cone-exact}). Let $\rho^\bullet$ be the
  morphism of complexes
  \[
    \rho^\bullet = (0, \id_{P^\bullet}) : P^\bullet \to L[1]^\bullet
    \oplus P^\bullet = MC(\alpha)^\bullet.
  \]
  Since $MC(\alpha)^\bullet$ is exact, $\rho^\bullet$ is homotopic to
  zero by \Cref{coro:homol:morphism-to-exact-complex-null-homotopic}. Therefore, there exist
  morphisms
  \[
  ...
  % non-commutative homotopy "lozenge" diagram of
  % \Cref{subsec:homol:homotopy}
  \[
  ...
  \item the morphisms $h^i$ give a homotopy
    $\alpha^\bullet \circ \beta^\bullet \sim \id_{P^\bullet}$.
  \end{enumerate}
...
injectives finally establishes \Cref{thm:homol:quasi-iso-of-bounded-proj-inj-complexes-is-homotopy-equivalence}:

\begin{proof}[Proof of \cref{thm:homol:quasi-iso-of-bounded-proj-inj-complexes-is-homotopy-equivalence}]
  We will give the argument for projectives.

  By \Cref{prop:homol:quasi-iso-to-projective-complex-has-homotopy-inverse}, since $P_1^\bullet$ is in
  $\categ{C}^-(\categ{P})$ and
  ...
  \[
    \alpha^\bullet \circ \beta^\bullet \sim \id_{P_1^\bullet}.
  \]
  This implies that
  $H^\bullet(\alpha^\bullet) \circ H^\bullet(\beta^\bullet) = \id$
  (\Cref{prop:homol:homotopic-morphisms-agree-on-cohomology}). Since $H^\bullet(\alpha^\bullet)$ is
  invertible, so is $H^\bullet(\beta^\bullet)$: therefore,
  $\beta^\bullet$ is a quasi-isomorphism. Now $P_0^\bullet$ is also in
  $\categ{C}^-(\categ{P})$; hence by \Cref{prop:homol:quasi-iso-to-projective-complex-has-homotopy-inverse}
  ...
  \]
  (\Cref{exer:homol:homotopy-compatible-with-composition}), it follows
  that $\beta^\bullet \circ \alpha^\bullet \sim
  \id_{P_0^\bullet}$. Thus $\alpha^\bullet$ is a homotopy equivalence,
  as stated.
\end{proof}

\subsection*{Exercises}

\begin{exer}{\exerused{}}{exer:homol:homotopic-categories-are-additive}
  Prove that the homotopic categories $\categ{K}(\categ{A})$,
  $\categ{K}^-(\categ{A})$, $\categ{K}^+(\categ{A})$ of an abelian
  category $\categ{A}$ are
  additive. [\Cref{subsec:homol:definition-of-the-homotopic-category-of-complexes}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:functor-factors-through-homotopic-category}
  Prove
  \Cref{prop:homol:functor-factors-through-homotopy-category}. [\Cref{subsec:homol:definition-of-the-homotopic-category-of-complexes}]
\end{exer}
\begin{exer}{}{exer:homol:projective-injective-in-any-category-set-example}
  ...
  $\categ{Set}$.
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:direct-sum-projectives-injectives-abelian-category}
  Upgrade
  \Cref{exer:linrep:direct-sum-of-projectives-iff-each-is,exer:linrep:direct-sum-of-injectives-iff-each-is}
  to objects of any abelian
  category. [\ref{exer:homol:functor-category-has-enough-injectives-projectives},
  \ref{exer:homol:complex-projective-iff-split-exact-of-projectives},
  \Cref{subsec:homol:long-exact-sequence-of-derived-functors}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:no-nontrivial-proj-inj-finite-abelian-groups}
  ...
  objects. [\Cref{subsec:homol:complexes-of-projective-and-injective-objects}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:left-adjoint-preserves-projectives}
  ...
  injective objects.  [\ref{exer:homol:x-hat-preserves-injectives},
  \Cref{subsec:homol:viewpoint-shift}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:right-adjoint-functor-category-construction}
  ...
  indexed by any set\footnote{Recall that `infinite' products may be
    defined as limits; see \Cref{exam:linrep:product-as-limit-over-discrete-category}.}; for example,
  ...
  information determines the whole natural transformation.)
  [\ref{exer:homol:x-hat-preserves-injectives},
  \ref{exer:homol:functor-category-has-enough-injectives-projectives}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:x-hat-preserves-injectives}
  As in \Cref{exer:homol:right-adjoint-functor-category-construction},
  let $\categ{C}$ be a small category and let $\categ{A}$ be an
  abelian category containing all products indexed by sets. Prove that
  the functor $\widehat{\mathscr{X}}$ preserves injectives for all
  objects $X$ of $\categ{C}$: if $Q$ is an injective object of
  $\categ{A}$, then $\widehat{\mathscr{X}}(Q)$ is injective in
  $\categ{A}^{\categ{C}}$. (Hint:
  \Cref{exer:homol:left-adjoint-preserves-projectives}.)
  [\ref{exer:homol:functor-category-has-enough-injectives-projectives}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:functor-category-has-enough-injectives-projectives}
  Let $\categ{C}$ be a small category, and let $\categ{A}$ be an
  ...
  \item With notation as in
    \Cref{exer:homol:right-adjoint-functor-category-construction}, let
  ...
  \item Prove that $\mathscr{Q}$ is injective in
    $\categ{A}^{\categ{C}}$.
    (\Cref{exer:homol:direct-sum-projectives-injectives-abelian-category,exer:homol:x-hat-preserves-injectives}.)
  ...
  (\categ{A}^{op})^{(\categ{C}^{op})}$.)
  [\ref{exer:homol:presheaves-have-enough-injectives-projectives}]
\end{exer}
\begin{exer}{}{exer:homol:presheaves-have-enough-injectives-projectives}
  ...
  \Cref{exer:homol:functor-category-has-enough-injectives-projectives}.)
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:split-exact-complex-characterization}
  ...
  sense analogous to the one given for modules in
  \Cref{subsec:rings:split-exact-sequences}) if and only if
  $\id_{N^\bullet}$ is homotopic to $0$ (cf.
  \Cref{prop:rings:split-sequence}). Deduce that every additive
  functor sends split exact sequences to split exact sequences.

  More generally, let $N^\bullet$ be any complex. Prove that
  $\id_{N^\bullet}$ is homotopic to $0$ (that is, $N^\bullet$ is
  `split exact' as defined in
  \Cref{subsec:homol:homotopy-equivalences-vs-quasi-isomorphisms-in-k-a})
  if and only if $N^\bullet$ is isomorphic to a complex of the form
  \[\begin{tikzcd}[column sep=large]
  ...
  \Cref{subsec:homol:long-exact-sequence-of-derived-functors}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:exact-bounded-complex-proj-inj-homotopy-trivial}
  ...
  complex of injectives is homotopy equivalent to
  zero. [\Cref{subsec:homol:homotopy-equivalences-vs-quasi-isomorphisms-in-k-a}]
\end{exer}
\begin{exer}{}{exer:homol:homotopy-inverse-of-quasi-iso-is-quasi-iso}
  Let $\categ{A}$ be an abelian category, and let
...
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:uniqueness-of-projective-resolution-up-to-homotopy}
  ...
  \item Assume that there are quasi-isomorphisms
    ...
    wait until you have covered
    \Cref{sec:homol:projective-and-injective-resolutions-and-the-derived-category},
    in particular \Cref{thm:homol:projective-resolution-functor-exists}.)
  \end{itemize}
  [\ref{exer:homol:chain-of-quasi-isos-gives-homotopy-equivalence}]
\end{exer}
...
\Cref{thm:homol:quasi-iso-of-bounded-proj-inj-complexes-is-homotopy-equivalence} is very good news. It tells us that if we
...
(\Cref{exer:homol:homotopy-and-homotopy-equiv-are-equiv-relations}),
while quasi-isomorphism is not (see the discussion following
\Cref{exam:homol:quasi-iso-not-homotopy-invertible}). Further, homotopy equivalences are preserved by
every additive functor (\Cref{thm:homol:homotopy-equivalence-preserved-under-functor}), while general
quasi-isomorphisms are not
(\Cref{exer:homol:additive-vs-exact-functors-quasi-iso-preservation}).
...
(\Cref{subsec:homol:poor-man-s-derived-category}).
...
as a full subcategory of $\categ{K}(\categ{A})$
(\Cref{exer:homol:complexes-in-degree-0-copy-of-a}). We will look for
`more interesting' copies of $\categ{A}$ in the homotopic category.

Let us begin by recalling (and sharpening) our definition of
projective/injective resolutions of an object $A$.

\begin{defn}{}{defn:homol:projective-injective-resolution}
...
\begin{rem}{}{rem:homol:resolution-terminology-caveat}
  ...
  (\Cref{exer:homol:complex-projective-iff-split-exact-of-projectives}).
\end{rem}

Projective or injective resolutions need not exist: as I pointed out
(\Cref{exam:homol:finite-abelian-groups-no-proj-inj}), an abelian category may have no nontrivial
projective or injective objects. It is however clear that such
resolutions exist \emph{if} the category has enough
projectives/injectives: if $\categ{A}$ has enough projectives, then
given any object $A$ of $\categ{A}$ there is a projective $P^0$ with
an epimorphism $\pi : P^0 \to A$; and then a projective $P^{-1}$ with
an epimorphism $d^{-1} : P^{-1} \to \ker\pi$; and then a projective
$P^{-2}$ with an epimorphism $d^{-2} : P^{-2} \to \ker d^{-1}$; and so
on. Letting $d^i_{P^\bullet}$ be the composition
...
with target $\iota(A)$ (cf. \Cref{rem:homol:quasi-isomorphisms-form-a-category}): this is a `homotopic
category of resolutions' of $A$. I claim that projective resolutions,
if they exist, are \emph{initial} in this category.  (Analogously,
injective resolutions are \emph{final} in the homotopic category of
...
\begin{lem}{}{lem:homol:resolution-lifting-property}
...
\begin{proof}
  ...
  $M^\bullet$ with its truncated version
  (cf. \Cref{exer:homol:resolution-gives-exact-truncated-complexes})
  and then extend both $P^\bullet$ and this complex as follows:
  \[\begin{tikzcd}[column sep=large]
  ...
  $\ker\mu = \image d^{-1}_{M^\bullet}$. Therefore, we have the
  diagram
  \[\begin{tikzcd}
  ...
  homotopy follows immediately from \Cref{lem:homol:zero-cohomology-morphism-null-homotopic}.
\end{proof}
...
(\Cref{prop:prelims:initial-final-unique}). Here, this says the
following:

\begin{prop}{}{prop:homol:resolutions-are-homotopy-equivalent}
  Any two projective (resp., injective) resolutions of an object $A$
  of an abelian category $\categ{A}$ are homotopy equivalent.
\end{prop}

(This is also a direct consequence of \Cref{lem:homol:resolution-lifting-property}.)
...
\begin{prop}{}{prop:homol:morphism-lifts-to-resolutions-uniquely-up-to-homotopy}
...
\begin{proof}
  ...
  homotopy, as an immediate consequence of \Cref{lem:homol:resolution-lifting-property}.
\end{proof}

The assignment provided by \Cref{prop:homol:morphism-lifts-to-resolutions-uniquely-up-to-homotopy} is clearly
...
this assertion, for it is a key point
(\Cref{exer:homol:subcategory-cohomology-degree-0-equiv-a}). If
$\categ{A}$ has enough projectives, this gives the promised `copy' of
$\categ{A}$ within $\categ{K}^-(\categ{P})$.
...
At this point we hopefully agree
(\Cref{exer:homol:subcategory-cohomology-degree-0-equiv-a}) that the
homotopic category $\categ{K}^-(\categ{P})$ contains a full
subcategory which is equivalent to $\categ{A}$: the (homotopy)
category of projective resolutions of objects of $\categ{A}$. While a
priori each object of $\categ{A}$ corresponds to many projective
resolutions, these are all homotopy equivalent to one another (by
\Cref{prop:homol:resolutions-are-homotopy-equivalent}), hence isomorphic in
$\categ{K}^-(\categ{P})$; morphisms in $\categ{A}$ correspond
precisely to homotopy classes of morphisms between projective
resolutions (by \Cref{prop:homol:morphism-lifts-to-resolutions-uniquely-up-to-homotopy}), hence to morphisms
in $\categ{K}^-(\categ{P})$.
...
equivalent complexes have the same cohomology (by \Cref{thm:homol:homotopy-equivalence-preserved-under-functor})
and additive functors preserve homotopy equivalence, objects of
...
We will follow this lead in the next section; the functors $\Tor_i$
and $\operatorname{Ext}^i$ encountered in \Cref{chap:linrep} will
arise precisely in this fashion and will stand as our poster examples
of derived functors.
...
available, in \Cref{subsec:homol:derived-categories}.
...
observed in
\Cref{subsec:homol:quasi-isomorphisms-and-derived-categories},
resolutions (in $\categ{C}^-(\categ{A})$ or $\categ{C}^+(\categ{A})$)
are just special cases of quasi-isomorphisms; the natural question is
whether every (suitably bounded) complex has a `quasi-isomorphic copy'
in $\categ{K}^-(\categ{P})$ or $\categ{K}^+(\categ{I})$, again
uniquely defined up to isomorphism in the homotopy category (that is,
up to homotopy equivalence in $\categ{C}(\categ{A})$).

This is indeed the case.

\begin{thm}{}{thm:homol:projective-resolution-functor-exists}
...
As always, there is a mirror statement to \Cref{thm:homol:projective-resolution-functor-exists}
...
\begin{proof}
  ...
  left is also an epimorphism, by \Cref{lem:homol:epimorphisms-stable-under-pullback}. A projective
  object $P^i$ with an epimorphism to the fibered product exists
  because $\categ{A}$ has enough projectives; the composition
  $\lambda^i : P^i \to L^i$ is then also an epimorphism, as
  claimed. Since the square is a pull-back, the kernel of $\pi$ maps
  isomorphically to the kernel of $d^i_{L^\bullet}$
  (\Cref{exer:homol:pullback-kernel-comparison-iso}); it follows
  easily that the new induced map
  $\hat\lambda^i : \ker d^i_{P^\bullet} \to \ker d^i_{L^\bullet}$ is
  ...
  \Cref{exam:homol:fibered-diagram-exact-sequence}) that it is a push-out as well, and
  (\Cref{exer:homol:pullback-kernel-comparison-iso} again) it follows
  that the induced morphism $\coker\pi \to \coker d^i_{L^\bullet}$ is
  an isomorphism. As noted, this is nothing but
  \[
  ...
  $\tilde\lambda^\bullet$.  \Cref{thm:homol:quasi-iso-of-bounded-proj-inj-complexes-is-homotopy-equivalence} implies then
  ...
  now established that there exists a homotopy lift\footnote{That is,
    ...
    cf. \Cref{exer:homol:true-lift-doesnt-exist-homotopy-lift-does}.}
  to any complex $P^\bullet$ mapping quasi-isomorphically to
  $L^\bullet$, since every such complex is homotopy equivalent to the
  one constructed earlier.
  ...
  \Cref{lem:homol:quasi-iso-lifting-uniqueness-setup}, and this concludes the proof.
\end{proof}
...
universal problem presented in
\Cref{subsec:homol:quasi-isomorphisms-and-derived-categories}. The
precise statement is the following.

By \Cref{prop:homol:functor-factors-through-homotopy-category}, the functor
...
\begin{thm}{}{thm:homol:derived-category-equivalence-via-resolutions}
...
\Cref{thm:homol:derived-category-equivalence-via-resolutions} is proven in full detail in any more
...
\Cref{thm:homol:derived-category-equivalence-via-resolutions} should be true, in the sense that
...
exists by \Cref{thm:homol:projective-resolution-functor-exists}) and with every morphism
...
\Cref{thm:homol:projective-resolution-functor-exists}; therefore different choices would only
move the resolutions in their isomorphism class in
$\categ{K}^-(\categ{P})$.

\begin{rem}{}{rem:homol:resolution-functor-choice-independence}
  ...
  $\categ{C}^-(\categ{A})$. By \Cref{thm:homol:projective-resolution-functor-exists}, there is a
  ...
  isomorphism\footnote{`Natural transformations' were rapidly defined
    in \Cref{subsec:linrep:comparing-functors},
    \Cref{defn:linrep:natural-transformation}. They are the most sensible notion of a
    morphism between functors. A natural \emph{isomorphism} is a
    natural transformation that is an isomorphism on each object.}
  between any two choices $\mathscr{P}$, $\mathscr{P}'$ of functors of
  projective resolutions. In a categorical context, this is
  essentially as good as uniqueness.  Thus the arbitrariness in the
  choice of $\mathscr{P}$ is less dramatic than it may seem at first.
\end{rem}
...
\begin{thm}{}{thm:homol:resolution-functor-properties}
...
\Cref{thm:homol:derived-category-equivalence-via-resolutions}) that there is `only' an equivalence of
categories between $\categ{K}^-(\categ{P})$ and
$\categ{D}^-(\categ{A})$, not a hard and fast `isomorphism'. As I
already pointed out in
\Cref{subsec:linrep:equivalence-of-categories}, this is what should
be expected in a categorical context.
...
\begin{proof}
  ...
  \Cref{thm:homol:projective-resolution-functor-exists}, $\rho^\bullet$ lifts to a morphism of
  ...
  \Cref{coro:homol:quasi-isos-are-isomorphisms-in-bounded-proj-inj}.
  ...
  \Cref{lem:homol:quasi-iso-inverting-functor-respects-homotopy}: homotopic morphisms have the same image
  ...
  \Cref{lem:homol:quasi-iso-inverting-functor-respects-homotopy}.
  ...
  $\categ{D}$). The reader will verify
  (\Cref{exer:homol:uniqueness-of-derived-functor-nat-iso}) that this
...
\Cref{thm:homol:projective-resolution-functor-exists}, gives us an entity which certainly
...
depends on the original complex $L^\bullet$ (by
\Cref{thm:homol:homotopy-equivalence-preserved-under-functor}). The cohomology of $L^\bullet$ is only one example
of the cohomological invariants that may be extracted from
$L^\bullet$; viewing $L^\bullet$ in the derived category gives direct
access to all these invariants.

We will capitalize on these considerations in the next section.

\subsection*{Exercises}

\begin{exer}{\exerused{}}{exer:homol:complexes-in-degree-0-copy-of-a}
  ...
  category $\categ{A}$. [\Cref{subsec:homol:recovering-a}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:subcategory-cohomology-degree-0-equiv-a}
  ...
  this subcategory.) [\Cref{subsec:homol:recovering-a},
  \Cref{subsec:homol:from-objects-to-complexes},
  \ref{exer:homol:nat-transformation-identity-to-p-iota-h0},
  \Cref{subsec:homol:viewpoint-shift}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:complex-projective-iff-split-exact-of-projectives}
  ...
  \item Prove that there is a morphism of complexes
    $P^\bullet \to MC(\id_{P^\bullet})[-1]$, such that for each $i$
  ...
  \item Conversely, prove that if $P^\bullet$ is a split exact complex
    ...
    $\categ{C}(\categ{A})$. (Hint: Use
    \Cref{exer:homol:split-exact-complex-characterization,exer:homol:direct-sum-projectives-injectives-abelian-category}
  ...
  \end{itemize} [\Cref{subsec:homol:recovering-a}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:resolution-functor-p-i-properties}
  ...
  complex, as in
  \Cref{subsec:homol:poor-man-s-derived-category}. Prove that
  $\mathscr{P}$ induces a functor
  $\categ{K}^-(\categ{A}) \to \categ{K}^-(\categ{P})$, which I will
  also denote by $\mathscr{P}$.  (Use \Cref{lem:homol:quasi-iso-lifting-uniqueness-setup}.) Next,
  let $\mathscr{I}$ be the embedding
  ...
  $\mathscr{I} \circ \mathscr{P} \rightsquigarrow \id_{\categ{K}^-
    (\categ{A})}$ such that the composition
  $\mathscr{P} \circ (\mathscr{I} \circ \mathscr{P}) \rightsquigarrow
  \mathscr{P}$ is an isomorphism. [\Cref{subsec:homol:viewpoint-shift},
  \Cref{subsec:homol:universal-property-of-the-derived-functor}]
\end{exer}
\begin{exer}{}{exer:homol:chain-of-quasi-isos-gives-homotopy-equivalence}
  (Cf. \Cref{exer:homol:uniqueness-of-projective-resolution-up-to-homotopy}.)
  Let $\categ{A}$ be an abelian category with enough projectives, and
  suppose there is a chain of quasi-isomorphisms
  \[\begin{tikzcd}[column sep=small]
  ...
  \dots, $L_n^\bullet$.
\end{exer}
\begin{exer}{}{exer:homol:zero-morphism-in-derived-category-characterization}
...
\end{exer}
\begin{exer}{}{exer:homol:identity-not-null-homotopic-but-zero-in-derived-category}
  ...
  $\categ{D}^-(\categ{Ab})$.
\end{exer}
\begin{exer}{}{exer:homol:explicit-projective-resolution-example}
  For any integer $m > 1$, view the short exact sequence
  \[\begin{tikzcd}
      0 \arrow[r] & \Z \arrow[r, "\cdot m"] & \Z \arrow[r] & \Z/m\Z
      \arrow[r] & 0
  ...
  argument at the beginning of the proof of \Cref{thm:homol:projective-resolution-functor-exists}.
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:nonzero-derived-morphism-zero-in-cohomology-example}
  ...
  information than `just' cohomology.)
  [\ref{exer:homol:true-lift-doesnt-exist-homotopy-lift-does},
  \ref{exer:homol:nonzero-derived-morphism-domain-example}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:true-lift-doesnt-exist-homotopy-lift-does}
  Consider the second morphism given in
  \Cref{exer:homol:nonzero-derived-morphism-zero-in-cohomology-example},
  together with a projective resolution of the bottom complex:
  ...
  \Cref{thm:homol:projective-resolution-functor-exists}). [\Cref{subsec:homol:from-objects-to-complexes}]
\end{exer}
\begin{exer}{}{exer:homol:nonzero-derived-morphism-domain-example}
  ...
  particularly easy if you have worked out
  \Cref{exer:homol:nonzero-derived-morphism-zero-in-cohomology-example}).
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:uniqueness-of-derived-functor-nat-iso}
  Complete the proof that the functor $\widetilde{\mathscr{F}}$ in
  \Cref{thm:homol:resolution-functor-properties} is unique up to natural
  isomorphism. [\Cref{subsec:homol:poor-man-s-derived-category}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:nat-transformation-identity-to-p-iota-h0}
  (Cf. \Cref{exer:homol:subcategory-cohomology-degree-0-equiv-a}.) Let
  $\categ{A}$ be an abelian category with enough projectives, and let
  $\widehat{\categ{A}}$ be the subcategory of $\categ{K}^-(\categ{P})$
  whose objects have cohomology concentrated in degree $0$. Choosing
  (arbitrarily) a projective resolution for every object $A$ of
  $\categ{A}$ and lifting morphisms as in \Cref{prop:homol:morphism-lifts-to-resolutions-uniquely-up-to-homotopy}, we
  obtain a functor $\categ{A} \to \widehat{\categ{A}}$; with notation
  as in \Cref{subsec:homol:poor-man-s-derived-category}, this is
  $\mathscr{P} \circ \iota$. It is clear that
  $H^0 \circ \mathscr{P} \circ \iota$ is the identity functor on
  $\categ{A}$.  Prove that there is a natural isomorphism from the
  identity functor on $\widehat{\categ{A}}$ to
  $\mathscr{P} \circ \iota \circ
  H^0$. [\Cref{subsec:homol:viewpoint-shift}]
\end{exer}
...
\Cref{subsec:homol:poor-man-s-derived-category}---the homotopic
category $\categ{K}^-(\categ{P})$ is a concrete realization (up to
equivalence of categories) of the derived category of the
bounded-above complexes in $\categ{A}$. Of course everything would go
through, with appropriate changes, for bounded-below complexes in the
presence of enough injectives. These assumptions are not necessary in
order to develop this material, but they simplify it substantially.
...
We have seen
(\Cref{exer:homol:subcategory-cohomology-degree-0-equiv-a,exer:homol:nat-transformation-identity-to-p-iota-h0})
that, if $\categ{A}$ has enough
...
$\widehat{\categ{A}} \to \categ{A}$. With notation as in
\Cref{subsec:homol:poor-man-s-derived-category}, the functor
$\mathscr{P} \circ \iota$ associating with every object of $\categ{A}$
a projective resolution gives a `weak' inverse to $H^0$. As pointed
out in \Cref{subsec:homol:poor-man-s-derived-category}, there are
many different possible $\mathscr{P}$; each of them will determine a
copy $\mathscr{P} \circ \iota(\categ{A})$ of $\categ{A}$ in
$\categ{K}^-(\categ{P})$.
...
\[
  ...
  \categ{C}(\categ{B})
\]
(we briefly encountered this functor in
\Cref{sec:homol:cones-and-homotopies}): the complex $L^\bullet$ is
sent to the complex $\categ{C}(\mathscr{F})(L^\bullet)$ whose objects
are $\mathscr{F}(L^i)$ and whose differentials are
...
(\Cref{lem:homol:functor-preserves-homotopy}) that homotopic morphisms in $\categ{C}(\categ{A})$
are sent to homotopic morphisms in $\categ{C}(\categ{B})$; it follows
that $\mathscr{F}$ induces a functor
...
$\categ{A}$, resp., $\categ{B}$); given our discussion in
\Cref{subsec:homol:poor-man-s-derived-category}, this would give us
a natural candidate for the `derived functor' of $\mathscr{F}$.
...
not preserve projectives\footnote{Note that tensor is right-exact, and
  ...
  \Cref{coro:linrep:tensor-hom-adjunction}. Functors that are left-adjoints to right-exact
  functors do preserve projectives:
  \Cref{exer:homol:left-adjoint-preserves-projectives}.}
(\Cref{exer:homol:no-projective-abelian-grp-is-tensor-image}).
Assuming that $\categ{B}$ has enough projectives, the natural way to
`fix' this problem is to apply a corresponding functor
$\mathscr{P}_{\categ{B}}$ constructed as in
\Cref{subsec:homol:poor-man-s-derived-category}, associating with
each complex a projective resolution. By \Cref{lem:homol:quasi-iso-lifting-uniqueness-setup},
$\mathscr{P}_{\categ{B}}$ sends homotopic morphisms to homotopic
morphisms; hence $\mathscr{P}_{\categ{B}}$ also descends to a functor
defined on the homotopic category
(cf. \Cref{exer:homol:resolution-functor-p-i-properties}).

\begin{defn}{}{defn:homol:left-derived-functor}
...
derived categories, discussed at the beginning of
\Cref{subsec:homol:viewpoint-shift}, that is, the subcategories
whose objects have cohomology concentrated in degree $0$. The point is
that there is no reason why applying $\categ{L}\mathscr{F}$ to a
complex whose cohomology is concentrated in degree $0$ should yield a
complex with the same property, unless $\mathscr{F}$ is very special
to begin with.
...
functor $\mathscr{F} : \categ{A} \to \categ{B}$ at the level of
...
\begin{exam}{}{exam:homol:exact-functor-preserves-degree-zero-objects}
  ...
  (\Cref{exer:homol:exact-functors-commute-with-cohomology}),
  \[
...
The moral of \Cref{exam:homol:exact-functor-preserves-degree-zero-objects} is that deriving exact
...
\]
Well, in general there isn't
(\Cref{exer:homol:derived-functor-nat-transformation-not-iso-example}). What
is $\categ{L}\mathscr{F}$ good for, then?

\begin{prop}{}{prop:homol:left-derived-functor-universal-property}
  The left-derived functor $\categ{L}\mathscr{F}$ satisfies the
  following universal property: There is a natural transformation
  $\eta : \categ{L}\mathscr{F} \circ \mathscr{P}_{\categ{A}}
  \rightsquigarrow \mathscr{P}_{\categ{B}} \circ
  \categ{K}(\mathscr{F})$ such that for every functor
...
\end{prop}
\begin{proof}
  If we have done our homework (and in particular
  \Cref{exer:homol:resolution-functor-p-i-properties}), then we have
  defined a certain natural transformation
  \[
    \mathscr{I}_{\categ{A}} \circ \mathscr{P}_{\categ{A}}
    \rightsquigarrow \id_{\categ{K}^-(\categ{A})};
  \]
  composing on the left by
  $\mathscr{P}_{\categ{B}} \circ \categ{K}(\mathscr{F})$ defines a
  transformation $\eta$ as stated, since
  $\categ{L}\mathscr{F} = \mathscr{P}_{\categ{B}} \circ
  \categ{K}(\mathscr{F}) \circ \mathscr{I}_{\categ{A}}$, by
  definition. Concretely, for every $L^\bullet$,
  $\eta_{L^\bullet} : \categ{L}\mathscr{F} \circ
  \mathscr{P}_{\categ{A}}(L^\bullet) = \mathscr{P}_{\categ{B}} \circ
  \categ{K}(\mathscr{F})(\mathscr{P}_{\categ{A}}(L^\bullet)) \to
  \mathscr{P}_{\categ{B}} \circ \categ{K}(\mathscr{F})(L^\bullet)$ is
  the morphism induced by the morphism
  $\mathscr{P}_{\categ{A}}(L^\bullet) \to L^\bullet$ associated with
  the chosen projective resolution.

  Every natural transformation
  $\mathscr{G} \rightsquigarrow \categ{L}\mathscr{F}$ will induce a
  natural transformation $\gamma$ as in the statement. We have to
  verify that every
  $\gamma : \mathscr{G} \circ \mathscr{P}_{\categ{A}} \rightsquigarrow
  \mathscr{P}_{\categ{B}} \circ \categ{K}(\mathscr{F})$ is induced in
  this fashion, by one and only one (up to natural isomorphism)
  natural transformation $\mathscr{G} \rightsquigarrow
  \categ{L}\mathscr{F}$. The uniqueness follows because we can
  reconstruct the transformation
  $\mathscr{G} \rightsquigarrow \categ{L}\mathscr{F}$ from $\gamma$,
  by horizontally composing on the right by
  $\id_{\mathscr{I}_{\categ{A}}}$ (and again using
  \Cref{exer:homol:resolution-functor-p-i-properties}, to obtain an
  isomorphism
  $\id_{\categ{K}^-(\categ{P}(\categ{A}))} \cong \mathscr{P}_{\categ{A}}
  \circ \mathscr{I}_{\categ{A}}$):
  \[
    \mathscr{G} \overset{\sim}{\rightsquigarrow} \mathscr{G} \circ
    \mathscr{P}_{\categ{A}} \circ \mathscr{I}_{\categ{A}}
    \overset{\gamma \cdot
      \id_{\mathscr{I}_{\categ{A}}}}{\rightsquigarrow} \mathscr{P}_{\categ{B}} \circ
    \categ{K}(\mathscr{F}) \circ \mathscr{I}_{\categ{A}} =
    \categ{L}\mathscr{F}.
  \]

  Concretely, for all $P^\bullet$ in $\categ{K}^-(\categ{P}(\categ{A}))$,
  the resolution morphism
  $\mu_{P^\bullet} : \mathscr{P}_{\categ{A}}(P^\bullet) \to P^\bullet$
  is an isomorphism in $\categ{K}^-(\categ{P}(\categ{A}))$; then
  (writing $P^\bullet$ for $\mathscr{I}_{\categ{A}}(P^\bullet)$ for
  notation's sake)
  \[
    \mathscr{G}(P^\bullet) \overset{\mathscr{G}(\mu_{P^\bullet}^{-1})}
    {\rightsquigarrow} \mathscr{G} \circ \mathscr{P}_{\categ{A}}(P^\bullet)
    \overset{\gamma_{P^\bullet}}{\rightsquigarrow} \mathscr{P}_{\categ{B}}
    \circ \categ{K}(\mathscr{F})(P^\bullet) = \categ{L}\mathscr{F}(P^\bullet).
  \]
  It is straightforward to verify that this natural transformation
  induces $\gamma$.
\end{proof}
...
\begin{rem}{}{rem:homol:composition-of-derived-functors-remark}
  The fact that there is a natural transformation
  $\mathscr{I} \circ \mathscr{P} \rightsquigarrow \id$
  (\Cref{exer:homol:resolution-functor-p-i-properties}), used in the
  proof of \Cref{prop:homol:left-derived-functor-universal-property}, has another interesting
  ...
  that $\categ{B}$ and $\categ{C}$ have enough projectives, so that
  ...
  `distributes' through compositions in this case
  (\Cref{exer:homol:composition-of-derived-functors}).
  ...
  point of view, using spectral sequences
  (\Cref{exer:homol:cartan-eilenberg-computes-composite-derived-functor,exer:homol:spectral-seq-for-composite-derived-functors}).
\end{rem}

\subsection{Taking cohomology}
\label{subsec:homol:taking-cohomology}

The content of \Cref{prop:homol:left-derived-functor-universal-property} is again in line with
our main strategy: in moving from $\categ{A}$ to $\categ{D}^-(\categ{A})$, the
left-derived functor $\categ{L}\mathscr{F}$ is the functor that preserves `as much
...
\begin{defn}{}{defn:homol:ith-left-derived-functor}
...
directly implied by
\begin{itemize}
\item \Cref{prop:homol:resolutions-are-homotopy-equivalent}, showing that any two projective
  resolutions are homotopy equivalent, in conjunction with
\item \Cref{thm:homol:homotopy-equivalence-preserved-under-functor}, showing that the images of homotopy
  equivalent complexes via an additive functor have isomorphic
  cohomology.
\end{itemize}
...
$\categ{A}^{op} \to \categ{B}$ (\Cref{defn:linrep:covariant-functor}); the roles of
...
both enough projectives and injectives (as seen in
\Cref{subsec:linrep:projective-modules} and
\Cref{subsec:linrep:injective-modules}); thus every
covariant/contravariant functor $R\text{-Mod} \to R\text{-Mod}$ can be
derived on the left and on the right.

\begin{exam}{}{exam:homol:tor-as-derived-tensor}
  Every $R$-module $N$ determines a functor
  \[
    \_ \otimes_R N : M \mapsto M \otimes_R N
  \]
  (see
  \Cref{subsec:linrep:adjunction-with-hom-and-explicit-computations}). The
  left-derived functor of $\_ \otimes_R N$ is denoted
  $\_ \otimes^{\categ{L}}_R N$ and acts
  $\categ{D}^-(R\text{-Mod}) \to \categ{D}^-(R\text{-Mod})$. The
  $i$-th left-derived functor of $\_ \otimes_R N$, viewed as a functor
  $R\text{-Mod} \to R\text{-Mod}$, is $\Tor^R_i (\_ \otimes_R N)$:
  indeed, the construction of $\Tor^R_i (M, N)$ given in
  \Cref{subsec:linrep:the-tor-functors} matches precisely the
  `concrete' interpretation of the $i$-th left-derived functor given
  above. The reader may note that in
  \Cref{subsec:linrep:the-tor-functors} we used a free resolution
  of $M$; free modules are projective, so this was simply a convenient
  way to choose a projective resolution. The fact that we could use
  \emph{any} projective resolution of $M$ to compute $\Tor^R_i(M, N)$
  was mentioned at the end of
  \Cref{subsec:linrep:projective-modules} and was attributed there
  to the `magic of homological algebra'. This piece of magic has now
  been explained in gory detail.
  ...
  \Cref{subsec:linrep:the-tor-functors}, that $\Tor_i(M, N)$ may in
  fact be computed by using a projective resolution \emph{of $N$}
  rather than $M$. Both mysteries will be dispelled in
  \Cref{sec:homol:double-complexes}.
\end{exam}

\begin{exam}{}{exam:homol:ext-as-derived-hom}
  ...
  injective resolution of $N$. As mentioned in
  \Cref{subsec:linrep:the-ext-functors}, it is also the $i$-th
  cohomology of $\Hom_R(M^\bullet, N)$, where $M^\bullet$ is a
  projective resolution of $M$. This projective resolution should
  really be viewed as an injective resolution in the opposite category
  $R\text{-Mod}^{op}$, since the functor $\Hom_R(\_, N)$ is
  contravariant.
...
The most remarkable property of the functors $\Tor_i$ and
$\operatorname{Ext}^i$ mentioned in \Cref{chap:linrep} is probably
that they `repair' the lack of exactness of $\otimes$, $\Hom$,
...
existence of these exact sequences in
\Cref{subsec:linrep:the-tor-functors} and
\Cref{subsec:linrep:the-ext-functors}, without proof (save for
indications in case the base ring $R$ is a PID); now we are ready to
understand fully why these sequences exist, in the general context of
derived functors.
...
\Cref{chap:linrep} will be particular cases. Surely the reader expects
this general fact to follow one way or the other from the long exact
cohomology sequence (\Cref{thm:homol:long-exact-cohomology-sequence}); that reader will not be
disappointed.
...
category: as I mentioned in passing in
\Cref{subsec:homol:quasi-isomorphisms-and-derived-categories},
these triangles play the role of exact sequences in the homotopic and
...
ordinary category of complexes, with the aim of using
\Cref{thm:homol:long-exact-cohomology-sequence}.  The key point is therefore the following: assuming
that $\categ{A}$ has enough projectives and that
\[\begin{tikzcd}
    0 \arrow[r] & L \arrow[r] & M \arrow[r] & N \arrow[r] & 0
...
\begin{lem}{}{lem:homol:exact-sequence-of-projective-resolutions}
  Let
  \[\begin{tikzcd}
      0 \arrow[r] & L \arrow[r] & M \arrow[r] & N \arrow[r] & 0
...
\begin{proof}
  ...
  $P_M^i := P_L^i \oplus P_N^i$; this is projective
  (\Cref{exer:homol:direct-sum-projectives-injectives-abelian-category}),
...
\Cref{lem:homol:exact-sequence-of-projective-resolutions} tells us that we can lift exact sequences of
...
not abelian, this is simply not an option. \Cref{lem:homol:exact-sequence-of-projective-resolutions}
...
(\Cref{exer:homol:mapping-cone-description-of-pm-from-pl-pn}) that
$P_M^\bullet$ is the mapping cone of a morphism
...
\[
  ...
  \end{tikzpicture}
\]
in the sense mentioned in
\Cref{subsec:homol:the-mapping-cone-of-a-morphism}. These are the
triangles giving the homotopic category the structure of `triangulated
category'. Thus, the functor $\mathscr{P}$ allows us to construct a
distinguished triangle in $\categ{K}(\categ{P})$ starting from a
`special' exact triangle
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    \node (L) at (1,1.3)
    {$\iota(L)$}; \node (N) at (0,0)
  ...
  \end{tikzpicture}
\]
in the sense of \Cref{subsec:homol:triangles}. The horseshoe lemma
shows that this functor preserves as much exactness as is allowed by
the context.
...
precise statement:
\begin{coro}{}{coro:homol:cartan-eilenberg-resolution-of-complex}
  Let
  \[\begin{tikzcd}[column sep=small]
...
\begin{proof}
  ...
  (\Cref{lem:homol:exact-sequence-of-projective-resolutions}) yields projective resolutions of $K^i$
  ...
  (\Cref{exer:homol:epi-mono-factorization-is-coim-image}). The
...
\begin{rem}{}{rem:homol:cartan-eilenberg-resolution-naturality}
  The proof of \Cref{coro:homol:cartan-eilenberg-resolution-of-complex} shows that the
  ...
  \[\begin{tikzcd}[column sep=large]
      ...
      \arrow[r] & 0,
    \end{tikzcd}\] can in fact be chosen so that $\image d^i$,
  $\ker d^i$, and the cohomology $\ker d^i / \image d^{i-1}$ are all
...
\begin{lem}{}{lem:homol:exact-sequence-with-projective-term-functor-properties}
  Let $\categ{A}$ be an abelian category, and let
  \[\begin{tikzcd}
...
\begin{proof}
  ...
  \[\begin{tikzcd}
    ...
    \end{tikzcd}\] splits (see the end of
  \Cref{subsec:linrep:projectives-and-injectives}). It follows that
  \[\begin{tikzcd}
      0 \arrow[r] & \mathscr{F}(L^i) \arrow[r] & \mathscr{F}(M^i)
      \arrow[r] & \mathscr{F}(P^i) \arrow[r] & 0
    \end{tikzcd}\] is (split and) exact for all $i$
  (\Cref{exer:homol:split-exact-complex-characterization}). Exactness
  of a sequence of complexes is determined by exactness at each
  degree, so this proves the statement.
\end{proof}

Now the long exact sequence of left-derived functors is an immediate
consequence of the long exact cohomology sequence.

\begin{thm}{}{thm:homol:long-exact-sequence-of-derived-functors}
  Let $\mathscr{F} : \categ{A} \to \categ{B}$ be an additive functor
...
necessary to prove this to the reader
(\Cref{exer:homol:morphisms-of-ses-induce-morphisms-of-les}).

\begin{proof}
  By \Cref{lem:homol:exact-sequence-of-projective-resolutions}, the given exact sequence is induced by
  an exact sequence of projective resolutions
  \[\begin{tikzcd}
      0 \arrow[r] & P_L^\bullet \arrow[r] & P_M^\bullet \arrow[r] &
      P_N^\bullet \arrow[r] & 0.
    \end{tikzcd}\] By \Cref{lem:homol:exact-sequence-with-projective-term-functor-properties}, the corresponding
  sequence
  \[\begin{tikzcd}
  ...
  determined by this sequence (by \Cref{thm:homol:long-exact-cohomology-sequence}) gives a long exact
  sequence as stated.
\end{proof}

Once more in the style of \Cref{subsec:homol:triangles},
\Cref{thm:homol:long-exact-sequence-of-derived-functors} tells us that the triangle
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    \node (L) at (1,1.3)
    {$\iota(L)$}; \node (N) at (0,0)
...
The reader may have noticed that $\otimes$ was derived \emph{to the
  left} to obtain $\Tor$, while $\Hom$ was derived \emph{to the right}
to obtain $\operatorname{Ext}$. The asymmetry mandating this choice
lies in the fact that $\otimes$ is right-exact, while $\Hom$ is
left-exact: any additive functor can be derived to the left or to the
right (in the presence of enough projectives, resp., enough
injectives), and the derived functors will fit in long exact sequences
as proven in \Cref{thm:homol:long-exact-sequence-of-derived-functors}; but only functors
...
\begin{prop}{}{prop:homol:right-exact-functor-derived-functor-degree-zero}
...
\begin{proof}
  ...
  so is its cohomology. Since $H_i = H^{-i}$
  (cf. \Cref{subsec:homol:reminder-of-basic-definitions-general-strategy}),
  ...
  \Cref{prop:homol:morphism-lifts-to-resolutions-uniquely-up-to-homotopy} there exists a morphism of
  ...
  isomorphism.
\end{proof}
Going back to \Cref{thm:homol:long-exact-sequence-of-derived-functors}, we see that if
...
$\categ{R}^i\mathscr{F}$ vanish in this case
(\Cref{exer:homol:exact-functor-vanishing-higher-derived-functors}). As
already pointed out in \Cref{exam:homol:exact-functor-preserves-degree-zero-objects}, one should not
...
encountered in \Cref{subsec:rings:monoid-rings}. Objects of
$G\text{-}\categ{Mod}$ may be called $G$-modules.
...
carrying a trivial action of $G$; it is clear that the assignment
$M \mapsto M^G$
...
reader should either check this directly or do
\Cref{exer:homol:invariants-functor-right-adjoint-trivial-action} and
then remember \Cref{prop:linrep:right-adjoint-additive-functors-left-exact}. The reader should in fact
contemplate why this functor is not right-exact: if $G$ acts trivially
on a coset $[m]$ of a quotient $M/L$, there is no reason \emph{a
  priori} why $G$ should act trivially on a representative $m$.
...
\[\begin{tikzcd}
    0 \arrow[r] & L \arrow[r] & M \arrow[r] & N \arrow[r] & 0
  \end{tikzcd}\] we obtain (from \Cref{thm:homol:long-exact-sequence-of-derived-functors}) a long
...
(and as will finally be verified in
\Cref{sec:homol:double-complexes}), this can be computed by an
injective resolution of $M$ or by a projective resolution of $\Z$.
\begin{exam}{}{exam:homol:group-cohomology-resolution-for-z}
...
\begin{exam}{Finite cyclic groups}{exam:homol:group-cohomology-resolution-for-cyclic-group}
...
that $(\dagger)$ is exact
(\Cref{exer:homol:exactness-of-group-cohomology-complex-low-degrees}),
at least for the part relevant to the discussion that
follows. Therefore,
\[\begin{tikzcd}[column sep=large]
...
\begin{prop}{}{prop:homol:group-cohomology-via-cochain-complex}
...
\]
\begin{exam}{}{exam:homol:galois-group-acts-on-multiplicative-group}
...
\begin{prop}{Hilbert's theorem}{prop:homol:hilberts-theorem-90}
  $H^1(G, F^*) = 0$.
\end{prop}

Indeed, with notation as above we have
$H^1(G, F^*) \cong \ker d_G^1 / \image d_G^0$, and we can compute this
quotient explicitly. Let $\alpha \in C^1(G, F^*)$; denote by
$\alpha_g$ the image of $g$ in $F^*$ by $\alpha$. The `cocycle
...
so by \Cref{exer:fields:distinct-automorphisms-linearly-independent}
they must be linearly independent.  Therefore, there exists a
$\gamma \in F$ such that
\[
...
image of $d_G^0$, proving that $\ker d_G^1 / \image d_G^0 = 0$, as
claimed.

\Cref{prop:homol:hilberts-theorem-90} is significant: it goes under the name of
...
prove this particular case in
\Cref{exer:fields:hilberts-theorem-90-multiplicative} and now will
enjoy the chance to verify that indeed \Cref{prop:homol:hilberts-theorem-90}
implies Hilbert's result
(\Cref{exer:homol:hilbert-90-from-vanishing-group-cohomology}).
...
\Cref{exam:homol:group-cohomology-resolution-for-z} we verified that
$H^1(\Z, M) = M_{\Z} = H_0(\Z, M)$. The reader should now have no
difficulty computing simple examples, such as the homology
$H_\bullet(\Z, M)$
(\Cref{exer:homol:group-homology-of-z-with-coefficients}).

\subsection*{Exercises}

\begin{exer}{\exerused{}}{exer:homol:no-projective-abelian-grp-is-tensor-image}
  ...
  any abelian group $P$. [\Cref{subsec:homol:viewpoint-shift}]
\end{exer}
\begin{exer}{}{exer:homol:functor-of-complexes-preserves-mapping-cone}
...
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:derived-functor-nat-transformation-not-iso-example}
  With notation as in \Cref{prop:homol:left-derived-functor-universal-property}, prove that the natural
  transformation
  $\categ{L}\mathscr{F} \circ \mathscr{P}_{\categ{A}} \rightsquigarrow
  \mathscr{P}_{\categ{B}} \circ \categ{K}(\mathscr{F})$ is not
  necessarily an isomorphism. (Hint: View the quasi-isomorphism in
  \Cref{exam:homol:quasi-iso-not-homotopy-invertible} as a projective resolution in
  $\Z\text{-}\categ{Mod}$, and take
  $\mathscr{F} : \Z\text{-}\categ{Mod} \to
  (\Z/2\Z)\text{-}\categ{Mod}$ to be $\_\otimes_{\Z} (\Z/2\Z)$.)
  [\Cref{subsec:homol:universal-property-of-the-derived-functor}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:composition-of-derived-functors}
  (Cf.\ \Cref{rem:homol:composition-of-derived-functors-remark}.) Let $\categ{A}$, $\categ{B}$,
  $\categ{C}$ be abelian categories, and let
  $\mathscr{F} : \categ{A} \to \categ{B}$,
  $\mathscr{G} : \categ{B} \to \categ{C}$ be additive functors. Assume
  ...
  isomorphic. [\Cref{subsec:homol:universal-property-of-the-derived-functor}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:mapping-cone-description-of-pm-from-pl-pn}
  With notation as in \Cref{lem:homol:exact-sequence-of-projective-resolutions}, show that there is a
  morphism $\rho^\bullet : P_N[-1]^\bullet \to P_L^\bullet$ such that
  ...
  define morphisms $P_N^i \to P_L^{i+1}$.)
  [\Cref{subsec:homol:relating-f-li-f-rif}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:cartan-eilenberg-resolution-computes-cohomology}
  Let $\categ{B}$, $\categ{C}$ be abelian categories, and assume
  $\categ{B}$ has enough projectives, so that Cartan-Eilenberg
  resolutions can be constructed, as in \Cref{coro:homol:cartan-eilenberg-resolution-of-complex} (cf.\
  also \Cref{rem:homol:cartan-eilenberg-resolution-naturality}). Let
  $\mathscr{G} : \categ{B} \to \categ{C}$ be an additive functor, and
  let $M^\bullet : \dots \to M^{-2} \to M^{-1} \to M^0 \to 0$ be a
  complex in $\categ{B}$. Let
  ...
  0$. (Hint: With notation as in the proof of \Cref{coro:homol:cartan-eilenberg-resolution-of-complex},
  for all $i$ and $j$ we have exact sequences
  ...
  resolution of $H^q(M^\bullet)$; cf.\ \Cref{rem:homol:cartan-eilenberg-resolution-naturality}. This
  will be an ingredient in the comparison of derived functors for the
  composition of two functors by means of spectral sequences; cf.\
  \Cref{exer:homol:cartan-eilenberg-computes-composite-derived-functor,exer:homol:spectral-seq-for-composite-derived-functors}.
  [\ref{exer:homol:spectral-seq-for-composite-derived-functors}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:morphisms-of-ses-induce-morphisms-of-les}
  Complete the proof of \Cref{thm:homol:long-exact-sequence-of-derived-functors} by showing that
  morphisms of short exact sequences induce morphisms of the
  corresponding long exact sequences of derived
  functors. [\Cref{subsec:homol:long-exact-sequence-of-derived-functors}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:exact-functor-vanishing-higher-derived-functors}
  Let $\categ{A}$, $\categ{B}$ be abelian categories, and let
  ...
  0$. [\Cref{subsec:homol:relating-f-li-f-rif}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:derived-functor-of-projective-object}
  Let $\categ{A}$, $\categ{B}$ be abelian categories, and let
  ...
  0$. [\Cref{subsec:homol:resolution-by-acyclic-objects}]
\end{exer}
\begin{exer}{}{exer:homol:l0f-right-exact-same-higher-derived-functors}
  Let $\categ{A}$, $\categ{B}$ be abelian categories, and let
  $\mathscr{F} : \categ{A} \to \categ{B}$ be an additive functor.
  Assume $\categ{A}$ has enough projectives, so that
  ...
  $\mathscr{F}$.
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:dimension-shifting-derived-functors}
  Let $\categ{A}$, $\categ{B}$ be abelian categories, and let
  $\mathscr{F} : \categ{A} \to \categ{B}$ be an additive functor;
  ...
  all $i > 1$. [\ref{exer:homol:generalized-dimension-shifting}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:generalized-dimension-shifting}
  Generalize
  \Cref{exer:homol:dimension-shifting-derived-functors} as
  follows: with the same notation, assume
  \[\begin{tikzcd}
  ...
  all $i > k$. [\Cref{subsec:homol:resolution-by-acyclic-objects},
  \ref{exer:homol:projective-dimension-equivalent-characterizations}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:delta-functor-definition-derived-functors-are}
  ...
  \item the assignment of a complex $(\ddagger)$ for every short exact
    ...
    \Cref{exer:homol:long-exact-cohomology-sequence-is-functorial}).
  \end{itemize}
  ...
  example. [\ref{exer:homol:effaceable-delta-functor-morphism-extension},
  \ref{exer:homol:bifunctor-derived-functor-construction}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:effaceable-delta-functor-morphism-extension}
  `Morphisms' $\{\mathscr{T}^i\} \to \{\mathscr{S}^i\}$ of
  $\delta$-functors
  (\Cref{exer:homol:delta-functor-definition-derived-functors-are})
  are natural transformations of the individual functors
  $\mathscr{T}^i$, $\mathscr{S}^i$ that induce morphisms of the
  corresponding long exact sequences $(\ddagger)$, for every short
  exact sequence $(\dagger)$ (in other words, that preserve the
  connecting morphisms $\delta$).
  ...
  Universal $\delta$-functors $\{\mathscr{T}^i\}$ with fixed
  $\mathscr{T}^0$ are of course unique up to isomorphism.
  ...
  is effaced by $\iota : A \to B$, let $C = \coker\iota$ and consider
  the exact sequence $0 \to A \to B \to C \to 0$.)
  [\ref{exer:homol:derived-functors-are-universal-delta-functors}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:derived-functors-are-universal-delta-functors}
  Assume the result stated in
  \Cref{exer:homol:effaceable-delta-functor-morphism-extension}. Prove
  ...
  right before the statement of \Cref{prop:homol:mapping-cone-exact-triangle}.)
  ...
  sheaf
  cohomology. [\ref{exer:homol:bifunctor-derived-functor-construction}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:invariants-functor-right-adjoint-trivial-action}
  Prove that the functor $\cdot^G$ defined in
  \Cref{subsec:homol:example-a-little-group-cohomology} is
  ...
  functor
  $\cdot_G$. [\Cref{subsec:homol:example-a-little-group-cohomology}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:exactness-of-group-cohomology-complex-low-degrees}
  Consider the complex $(\dagger)$ defined in
  \Cref{subsec:homol:example-a-little-group-cohomology}. Define
  maps $h^{-1}$, $h^0$, $h$ in the diagram
  \[\begin{tikzcd}[column sep=large]
      \dots \arrow[r] & \Z[G]^{m^2} \arrow[r, "d^{-2}"] & \Z[G]^m
  ...
  $\Z[G]^m$. [\Cref{subsec:homol:example-a-little-group-cohomology}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:hilbert-90-from-vanishing-group-cohomology}
  Deduce Hilbert's theorem 90 (as stated in
  \Cref{exer:fields:hilberts-theorem-90-multiplicative}) as a
  consequence of the vanishing of \Cref{prop:homol:hilberts-theorem-90}.  (Hint:
  Use the result of \Cref{exam:homol:group-cohomology-resolution-for-cyclic-group}.)
  [\Cref{subsec:homol:example-a-little-group-cohomology}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:group-homology-of-z-with-coefficients}
  Compute the group homology $H_\bullet(\Z, M)$ for any abelian group
  $M$ carrying a
  $\Z$-action. [\Cref{subsec:homol:example-a-little-group-cohomology}]
\end{exer}
...
(\Cref{subsec:homol:resolution-by-acyclic-objects}). But everything
is clarified substantially by going one step further and considering
\emph{double complexes}, as we do in the remaining subsections.
...
$i \ge 1$ if $P$ is projective (cf.\
\Cref{exer:homol:derived-functor-of-projective-object}): if
$P$ is projective, then $\iota(P)$:
\[\begin{tikzcd}
...
\begin{defn}{}{defn:homol:f-acyclic-object}
...
on the right (cf.\ \Cref{subsec:homol:relating-f-li-f-rif}), so in
context we can just talk about `$\mathscr{F}$-acyclic' objects.
...
\begin{exam}{}{exam:homol:flat-modules-are-tor-acyclic}
  Let $R$ be a commutative ring. Recall (\Cref{defn:linrep:flat-module}) that an
  ...
  a very strong sense: if $N$ is flat, then $\Tor_i(M, N) = 0$ for
  $i \ne 0$ and all $M$; see \Cref{subsec:linrep:the-tor-functors}
  to be reminded of why, or---better---prove it anew.  Further, since
  (as we will finally prove in this section) Tor functors may be
  computed by resolving either argument, we also know that,
  `symmetrically', if $M$ is flat, then $\Tor_i(M, N) = 0$ for all
  $i \ne 0$ and all modules $N$. Thus, a flat module is
  $\mathscr{F}$-acyclic for every functor $\mathscr{F}$ defined by
  $\_ \otimes_R N$, for all modules $N$.
\end{exam}

Here is the punch line. In \Cref{subsec:linrep:the-ext-functors} I
had claimed that flat resolutions could be used in place of free or
projective resolutions, in order to compute Tor. We are going to
verify that $\mathscr{F}$-acyclic resolutions suffice in order to
compute $\categ{L}_i\mathscr{F}$.

\begin{thm}{}{thm:homol:derived-functor-via-acyclic-resolution}
...
\begin{proof}
  ...
  \[
    ...
    \end{tikzcd}
  \]
  Applying \Cref{thm:homol:long-exact-sequence-of-derived-functors} (and
  \Cref{prop:homol:right-exact-functor-derived-functor-degree-zero}), we obtain a long exact sequence
  ending with
  \[\begin{tikzcd}
  ...
  \Cref{exer:homol:cohomology-right-exact-functor-exact-complex} to
  the exact complex
  \[\begin{tikzcd}
  ...
  obtained by applying \Cref{thm:homol:long-exact-sequence-of-derived-functors} to $(*)$ give
  \[\begin{tikzcd}
  ...
  \]
  (also cf.\ \Cref{exer:homol:generalized-dimension-shifting})
  and concludes the proof.
\end{proof}

\begin{exam}{}{exam:homol:flat-resolution-computes-tor}
  ...
  (\Cref{exam:homol:flat-modules-are-tor-acyclic}), so this is now a consequence of
  \Cref{thm:homol:derived-functor-via-acyclic-resolution}.
\end{exam}

\Cref{thm:homol:derived-functor-via-acyclic-resolution} raises an interesting possibility: since
...
\Cref{exer:homol:bifunctor-derived-functor-construction} for a typical
situation.)

There is an alternative viewpoint on the question addressed by
\Cref{thm:homol:derived-functor-via-acyclic-resolution}. Let $A^\bullet$ be an
...
\[\begin{tikzcd}[column sep=small]
  ...
  \end{tikzcd}\] The result of \Cref{thm:homol:derived-functor-via-acyclic-resolution} is that
...
I warned the reader as far back as
\Cref{subsec:homol:the-category-of-complexes} that we would examine
...
of complexes leading to the long exact cohomology sequence
(\Cref{subsec:homol:the-long-exact-cohomology-sequence}) and the
construction of the mapping cone
(\Cref{subsec:homol:the-mapping-cone-of-a-morphism}). The latter
case generalizes in a straightforward way, as we will see now.
...
The mapping cone $MC(\alpha)^\bullet$ constructed in
\Cref{subsec:homol:the-mapping-cone-of-a-morphism} is obtained by
\begin{itemize}
...
% \Cref{subsec:homol:the-mapping-cone-of-a-morphism}, no new
% mathematical content beyond what the surrounding prose states.
...
\begin{defn}{}{defn:homol:double-complex}
...
to an isomorphic double complex,
\Cref{exer:homol:sign-flip-gives-isomorphic-double-complex}; in fact,
other choices may be convenient depending on the situation.
...
\begin{defn}{}{defn:homol:total-complex}
...
\begin{prop}{}{prop:homol:total-complex-is-a-complex}
...
\begin{proof}
  ...
  \[
    ...
    \circ \delta_v = 0.
  \]
  The reader will formalize this computation
  (\Cref{exer:homol:total-differential-squares-to-zero}).
\end{proof}
...
\begin{exam}{}{exam:homol:hom-tensor-double-complex-example}
  ...
  (\Cref{exer:homol:sign-flip-gives-isomorphic-double-complex}), and
  by a slight abuse of language they can both be denoted
  $TC(\Hom_{\categ{A}}(L, M))^\bullet$. The degree-$i$ piece of this
  total complex, i.e.,
  \[
  ...
  reader will verify
  (\Cref{exer:homol:cohomology-of-total-hom-complex-is-hom-in-k})
...
computed by an extremely clever device called \emph{spectral
  sequence}, which we will (briefly) encounter in
\Cref{subsec:homol:spectral-sequences}. There are standard
situations, however, in which one can conclude immediately that the
total complex is exact, and this is at the root of the applications I
will present. The statement is neat and memorable:

\begin{thm}{}{thm:homol:acyclic-assembly-lemma}
  Let $\categ{A}$ be an abelian category, and let
  \[
    (*) \qquad
    \begin{tikzcd}
...
(\Cref{exer:homol:vanishing-total-complex-cohomology-from-exact-rows}).

\emph{Mutatis mutandis}, \Cref{thm:homol:acyclic-assembly-lemma} holds for
...
In terms of double complexes, \Cref{thm:homol:acyclic-assembly-lemma} says that
...
\begin{proof}
  ...
  \Cref{exer:homol:sign-flip-gives-isomorphic-double-complex}).
...
\begin{exam}{}{exam:homol:acyclic-assembly-application}
  Here is a taste of how convenient \Cref{thm:homol:acyclic-assembly-lemma} is.
  ...
  given in \Cref{thm:homol:acyclic-assembly-lemma} (in the symmetrically bounded
  version).  According to \Cref{thm:homol:acyclic-assembly-lemma}, the
  corresponding total complex is exact; as seen in
  \Cref{exam:homol:hom-tensor-double-complex-example} (cf.\
  \Cref{exer:homol:cohomology-of-total-hom-complex-is-hom-in-k}), this
  says that $\Hom_{\categ{K}(\categ{A})}(P^\bullet, L[i]^\bullet) = 0$
  for all $i$. In other words, \emph{every cochain morphism from a
    bounded-above complex of projectives to a bounded-below exact
    complex is homotopic to $0$}. This recovers
  \Cref{coro:homol:morphism-to-exact-complex-null-homotopic}, up to easy adjustments, taking care of the
...
\Cref{exam:homol:acyclic-assembly-application} illustrates well the power of results
such as \Cref{thm:homol:acyclic-assembly-lemma}: the nuts-and-bolts-style
arguments used in
\Cref{sec:homol:the-homotopic-category-complexes-of-projectives-and}
are bypassed, and statements such as \Cref{coro:homol:morphism-to-exact-complex-null-homotopic} are placed
in a clarifying context\footnote{But I strongly believe in the
  pedagogic usefulness of first going through the nuts and
  bolts!}. The same strategy will allow us to revisit the question
examined in \Cref{subsec:homol:resolution-by-acyclic-objects},
letting a double complex draw the conclusion of
\Cref{thm:homol:derived-functor-via-acyclic-resolution}. We will then finally be able to go back
to the last remaining mystery concerning Tor and Ext, dealing with why
we can compute these functors `by resolving either argument'. This
will conclude a large circle of ideas, begun in \Cref{chap:linrep}.
...
We have studied in \Cref{subsec:homol:from-objects-to-complexes}
the problem of constructing complexes of projectives (for example)
that are quasi-isomorphic to a given complex $N^\bullet$:
\Cref{thm:homol:projective-resolution-functor-exists} accomplishes this. I have called the
...
\begin{prop}{}{prop:homol:total-complex-of-cone-is-mapping-cone-of-totalizations}
...
(\Cref{exer:homol:total-complex-of-mn-is-mapping-cone}). The following
consequence compares the different notions of `resolution', as
promised a moment ago.

\begin{thm}{}{thm:homol:balancing-derived-functors}
...
The statement of \Cref{thm:homol:balancing-derived-functors} extends that of
\Cref{thm:homol:acyclic-assembly-lemma}; its proof is a nearly immediate
consequence of the latter. As I mentioned at the beginning of
\Cref{subsec:homol:exactness-of-the-total-complex}, these
statements are vastly generalized by the machinery of \emph{spectral
  sequences}, which provide a general framework for the computation of
the cohomology of the total complex of a double complex. We will
recover \Cref{thm:homol:balancing-derived-functors} once we have learned a bit about
spectral sequences in \Cref{subsec:homol:spectral-sequences}, but
it is not hard (and it is a good exercise) to prove this statement by
hand, as I proceed to do.

\begin{proof}
  The second statement follows from the first, by flipping the
  corresponding double complex about the main diagonal.

  The first statement follows from \Cref{thm:homol:acyclic-assembly-lemma} and
  \Cref{prop:homol:total-complex-of-cone-is-mapping-cone-of-totalizations}. Indeed, let
  ...
  \[\begin{tikzcd}
      ...
      \arrow[r] & \dots
    \end{tikzcd}\] by \Cref{thm:homol:acyclic-assembly-lemma}, $TC(M_N)^\bullet$
  is exact; by \Cref{prop:homol:total-complex-of-cone-is-mapping-cone-of-totalizations} this is the mapping
  cone of the induced morphism $T^\bullet \to N^\bullet$, and it
  follows (\Cref{coro:homol:quasi-iso-iff-mapping-cone-exact}) that this morphism is a
  quasi-isomorphism, as needed.
\end{proof}

To appreciate the usefulness of \Cref{thm:homol:balancing-derived-functors}, again in
the context of our (hard) work in
\Cref{sec:homol:projective-and-injective-resolutions-and-the-derived-category},
note that it provides us with a convenient bridge between resolving
\emph{objects} (see \Cref{lem:homol:resolution-lifting-property}) and resolving
\emph{complexes} (as in \Cref{thm:homol:projective-resolution-functor-exists}).  Indeed, by
\Cref{coro:homol:cartan-eilenberg-resolution-of-complex} (which relies on the horseshoe lemma, therefore
on resolving individual objects of $\categ{A}$) every bounded-above
...
\Cref{thm:homol:balancing-derived-functors}. This reproduces the main content of
\Cref{thm:homol:projective-resolution-functor-exists}, bypassing the nastiest details of the proof
given in
\Cref{sec:homol:projective-and-injective-resolutions-and-the-derived-category}.

As mentioned in \Cref{rem:homol:cartan-eilenberg-resolution-naturality}, double complexes of projectives
(or, working in $\categ{C}^{\ge 0}$, injectives) resolving a given
complex (and satisfying the additional condition explained in
\Cref{rem:homol:cartan-eilenberg-resolution-naturality}) are called \emph{Cartan-Eilenberg
  resolutions}. Their systematic use can simplify the material I
presented in
\Cref{sec:homol:projective-and-injective-resolutions-and-the-derived-category}
but at the price of dealing early with double complexes.
...
complexes), go back to the question we studied in
\Cref{subsec:homol:resolution-by-acyclic-objects}: we ended that
subsection by lining up the complexes obtained by applying an additive
functor $\mathscr{F}$ to an $\mathscr{F}$-acyclic resolution
$A^\bullet$ of an object $M$ and to a projective resolution of the
same object, as sides of an array.  \Cref{coro:homol:cartan-eilenberg-resolution-of-complex} tells us how
to fill the rest of the array: start from
\[
  (*) \qquad
  \begin{tikzcd}
  ...
  \end{tikzcd}
\]
which is exact by hypothesis; using \Cref{coro:homol:cartan-eilenberg-resolution-of-complex}, view $(*)$
as the complex induced in $H^0$ by an exact complex
\[
  (**) \qquad
  \begin{tikzcd}
...
\Cref{thm:homol:balancing-derived-functors} to the complex
\[\begin{tikzcd}
...
\]
which was the conclusion of \Cref{thm:homol:derived-functor-via-acyclic-resolution}.
...
\begin{thm}{}{thm:homol:tor-is-balanced}
...
in \Cref{subsec:linrep:the-tor-functors} (where we used free
resolutions); see also
\Cref{exam:homol:tor-as-derived-tensor}. \Cref{thm:homol:tor-is-balanced} makes good on our
old promise (also made in \Cref{subsec:linrep:the-tor-functors}) to
show that the Tor functors could be computed by resolving the second
factor rather than the first.

\begin{proof}
  Apply \Cref{thm:homol:balancing-derived-functors} to the complex
  \[
    (*) \qquad
    \begin{tikzcd}
  ...
  \]
  by the first statement in \Cref{thm:homol:balancing-derived-functors}. On the other
  ...
  \]
  by the second statement in \Cref{thm:homol:balancing-derived-functors}. The result
  follows.
\end{proof}

\begin{thm}{}{thm:homol:ext-is-balanced}
  Let $M, N$ be modules over a commutative ring $R$, and let
...
The proof of \Cref{thm:homol:ext-is-balanced} is left to the reader
(\Cref{exer:homol:ext-via-proj-or-inj-resolution-agree}):
\Cref{exam:homol:hom-tensor-double-complex-example} and the strategy extensively discussed
above will hopefully make this a very easy task.

This statement completes the verification of all claims concerning Tor
and Ext made in \Cref{chap:linrep}, in the sense that the extent to
...
Hopefully, the same will apply to other encounters the reader may have
with simple applications of homological algebra.

In any case, this seems a fitting place to end the main body of this
last chapter. In \Cref{sec:homol:further-topics} we will give a
brief look at material that really lies beyond the scope of these
notes.

\subsection*{Exercises}

\begin{exer}{\exerused{}}{exer:homol:cohomology-right-exact-functor-exact-complex}
  ...
  \] [\Cref{subsec:homol:resolution-by-acyclic-objects}]
\end{exer}
\begin{exer}{}{exer:homol:derived-functor-via-image-objects}
  ...
  resolution of $M$, and let $J^i := \image d_A^i$. Prove that
  \[
    \categ{L}_i\mathscr{F}(M) \cong \categ{L}_1\mathscr{F}(J^{-i+1})
  \]
  for all $i > 1$.
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:total-differential-squares-to-zero}
  Verify carefully that $d_{tot} \circ d_{tot} = 0$ (cf.\
  \Cref{prop:homol:total-complex-is-a-complex}). [\Cref{subsec:homol:complexes-of-complexes}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:sign-flip-gives-isomorphic-double-complex}
  ...
  array.) [\Cref{subsec:homol:complexes-of-complexes},
  \Cref{subsec:homol:exactness-of-the-total-complex}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:cohomology-of-total-hom-complex-is-hom-in-k}
  With notation as in \Cref{exam:homol:hom-tensor-double-complex-example}, prove that the
  ...
  These isomorphisms require careful sign adjustments.)
  [\Cref{subsec:homol:complexes-of-complexes},
  \Cref{subsec:homol:exactness-of-the-total-complex}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:vanishing-total-complex-cohomology-from-exact-rows}
  ...
  $i + j = k$. [\Cref{subsec:homol:exactness-of-the-total-complex}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:total-complex-of-mn-is-mapping-cone}
  Prove
  \Cref{prop:homol:total-complex-of-cone-is-mapping-cone-of-totalizations}. [\Cref{subsec:homol:total-complexes-and-resolutions}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:cartan-eilenberg-computes-composite-derived-functor}
  ...
  $\mathscr{F}M^\bullet$ (as constructed in
  \Cref{coro:homol:cartan-eilenberg-resolution-of-complex}). Consider the complex
  \[
    (\dagger) \qquad
    \begin{tikzcd}
  ...
  concentrated in degree $0$, and apply \Cref{thm:homol:balancing-derived-functors}.)
  [\Cref{subsec:homol:universal-property-of-the-derived-functor},
  \ref{exer:homol:cartan-eilenberg-resolution-computes-cohomology},
  \ref{exer:homol:spectral-seq-for-composite-derived-functors}]
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:ext-via-proj-or-inj-resolution-agree}
  Prove \Cref{thm:homol:ext-is-balanced}: Let $M, N$ be modules over a
  ...
  given in
  \Cref{subsec:linrep:the-ext-functors}. [\Cref{subsec:homol:acyclic-resolutions-again-and-balancing-tor-and-ext}]
\end{exer}
\begin{exer}{}{exer:homol:ext-characterization-of-projective-injective}
  ...
  $\categ{A}$ and all $i \ge 1$.
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:projective-dimension-equivalent-characterizations}
  ...
  adapt \Cref{exer:homol:generalized-dimension-shifting}.)
  [\ref{exer:homol:injective-dimension-equivalent-characterizations},
  \ref{exer:homol:global-dimension-pd-id-equality}]
\end{exer}
\begin{exer}{$\lnot$}{exer:homol:injective-dimension-equivalent-characterizations}
  Let $\categ{A}$ be an abelian category with enough injectives and
  projectives. The \emph{injective dimension} of an object $B$ of
  $\categ{A}$ is the minimum number $d = \id(B)$ such that there
  exists an injective resolution
  \[\begin{tikzcd}
  ...
  \end{enumerate}
  (See
  \Cref{exer:homol:projective-dimension-equivalent-characterizations}.)
  [\ref{exer:homol:global-dimension-pd-id-equality}]
\end{exer}
\begin{exer}{}{exer:homol:global-dimension-pd-id-equality}
  (See
  \Cref{exer:homol:projective-dimension-equivalent-characterizations,exer:homol:injective-dimension-equivalent-characterizations}.)
  Let $\categ{A}$ be an abelian category with enough injectives and
  projectives. Prove that the supremum of $\operatorname{pd}(A)$,
  $A \in \Obj(\categ{A})$, equals the supremum of $\id(B)$,
  $B \in \Obj(\categ{A})$, and give an Ext interpretation for this
  number.
...
\end{exer}
\begin{exer}{\exerused{}}{exer:homol:bifunctor-derived-functor-construction}
  ...
  \item Let
    ...
    (Hint: Mimic the proof of \Cref{thm:homol:derived-functor-via-acyclic-resolution}.)
  \end{itemize}
  ...
  \Cref{exer:homol:derived-functors-are-universal-delta-functors}
  and the uniqueness of universal
  $\delta$-functors). [\Cref{subsec:homol:resolution-by-acyclic-objects}]
\end{exer}
...
\Cref{subsec:homol:spectral-sequences}, illustrating the spectral
sequence associated to a double complex, since this can be done
reasonably quickly (but delving into any other application of spectral
sequences would simply take us too far). For the other themes in this
section (derived and triangulated categories) I will really do no more
than a little propaganda.

\subsection{Derived categories}
\label{subsec:homol:derived-categories}
Derived categories made their appearance in
\Cref{subsec:homol:quasi-isomorphisms-and-derived-categories} and
have since served as a guiding concept motivating the natural
development of the subject, but we have stopped short of constructing
them in general. The closest we have come to a concrete description of
the derived category is in the `poor man's version' of
\Cref{subsec:homol:poor-man-s-derived-category}, which should
actually be labeled the `rich man's version', since it relies on the
fortunate presence of enough projectives/injectives. Also, the
...
of the field of fractions (see
\Cref{subsec:irred:the-field-of-fractions-of-an-integral-domain}). This
process, in a more general version dealing with inverting
`multiplicative subsets' of a ring, is called \emph{localization}
(\Cref{exer:irred:localization-construction}). Categories may be
localized as well. For example, the homotopic category
...
identified to one another and to $\id_A$ in the localized category;
the roof
\[\begin{tikzcd}
...
\Cref{subsec:homol:quasi-isomorphisms-and-derived-categories}. If
$\categ{A}$ has (for example) enough projectives, then the bounded
version $\categ{D}^-(\categ{A})$ is equivalent to the homotopy
category $\categ{K}^-(\categ{P})$ of complexes of projectives, as I
have endeavored to justify in
\Cref{subsec:homol:poor-man-s-derived-category}. The latter should
be viewed as a convenient way to `compute' the former in favorable
circumstances.  Summarizing, a morphism $L^\bullet \to M^\bullet$ in
the derived category $\categ{D}(\categ{A})$ of an abelian category
$\categ{A}$ can be represented as a roof diagram
\[\begin{tikzcd}
...
through the homotopic category (\Cref{prop:homol:functor-factors-through-homotopy-category}). In
...
\Cref{subsec:homol:quasi-isomorphisms-and-derived-categories}. However,
the real benefits of introducing the derived category lie deeper, for
example in its structure as a \emph{triangulated category}. A brief
discussion of this concept follows.

The reader should check that the composition of two roofs is indeed a
roof
(\Cref{exer:homol:upside-down-roof-implies-regular-roof});
this may be easier after acquiring a little familiarity with
distinguished triangles. Also, note that the diagram in
\Cref{exer:homol:upside-down-roof-implies-regular-roof} does
not commute in $\categ{C}(\categ{A})$, but it does in
...
homotopic categories of complexes. I have pointed out in
\Cref{subsec:homol:definition-of-the-homotopic-category-of-complexes}
that these categories should not be expected to be (and indeed are
not) \emph{abelian}, but they preserve enough structure to make sense
of, for example, long exact cohomology sequences. The fact that
derived functors ended up having long exact sequences associated to
them (\Cref{subsec:homol:long-exact-sequence-of-derived-functors})
bears witness to this fact.  The essential ingredients of a
...
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    ...
    below] {$\id_A$}; \draw[->] (C) -- (T) node[midway, left] {$+1$};
  \end{tikzpicture}
\]
...
$MC(\alpha)^\bullet \to L[1]^\bullet$ studied in
\Cref{subsec:homol:the-mapping-cone-of-a-morphism}.
...
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    ...
    node[midway, below]
    {$\id_{L^\bullet}$}; \draw[->] (Z) -- (T) node[midway, left]
    {$+1$};
  \end{tikzpicture}
\]
...
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    ...
    {$\id_{L^\bullet}$}; \draw[->] (B) -- (Z); \draw[->] (Z) -- (L)
    node[midway, left] {$+1$};
  \end{tikzpicture}
\]
...
$0$. But the mapping cone of the identity is \emph{homotopically
  equivalent} to $0$
(\Cref{exer:homol:mapping-cone-of-isomorphism-homotopy-trivial}),
so the rotated triangle is indeed distinguished in
$\categ{K}(\categ{A})$, as it should be, according to the axioms. More
generally, it is easy to see
(\Cref{exer:homol:mapping-cone-of-natural-morphism-is-shift})
that if $\alpha^\bullet : L^\bullet \to M^\bullet$ is a cochain
morphism, so that we have a distinguished triangle,
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    \node (L) at (1,1.3)
    {$L^\bullet$}; \node (MC) at (0,0)
...
\[
  ...
  \end{tikzpicture}
\]
obtained in \Cref{prop:homol:mapping-cone-exact-triangle}. In general, a
...
will not surprise the reader. The functors $\Hom(A,\_)$ and
$\Hom(\_,A)$ (to $\categ{Ab}$) are cohomological functors on every
triangulated category, for every object $A$.
...
mysterious remarks at the end of \Cref{subsec:homol:triangles}. The
`special triangles'
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    \node (L) at (1,1.3)
  ...
  \end{tikzpicture}
\]
arising as in \Cref{subsec:homol:triangles} from a short exact
sequence
\[
  0 \longrightarrow L^\bullet \xrightarrow{\alpha^\bullet} M^\bullet
  \xrightarrow{\beta^\bullet} N^\bullet \longrightarrow 0
...
hand, the reader can now verify
(\Cref{exer:homol:mapping-cone-of-mono-quasi-iso-to-cokernel})
that there is a quasi-isomorphism $MC(\alpha)^\bullet \to N^\bullet$:
this can be shown by computing explicitly another mapping cone and
applying \Cref{coro:homol:quasi-iso-iff-mapping-cone-exact}. Thus, in the derived category the
...
\Cref{exer:homol:connecting-morphism-in-derived-category-from-ses}). Applying
the cohomology functor to this triangle directly gives the exact
triangle
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
    \node (L) at (1,1.3)
  ...
  \end{tikzpicture}
\]
expressing the long exact cohomology sequence as in
\Cref{subsec:homol:triangles}, without having to single out the
$+1$ morphism for special consideration. In retrospect, the
zero-morphism I insisted on using when folding `special' triangles in
\Cref{subsec:homol:triangles} turns out to be an artifact: placing
...
\to \categ{D}^\pm(\categ{B})$ encountered in
\Cref{subsec:homol:universal-property-of-the-derived-functor} are
also functors of triangulated categories.  This fact is responsible
for the good properties of derived functors, such as the long exact
sequences studied in
\Cref{subsec:homol:long-exact-sequence-of-derived-functors}.

\subsection{Spectral sequences}
\label{subsec:homol:spectral-sequences}
I mentioned in \Cref{subsec:homol:exactness-of-the-total-complex}
that `spectral sequences' may be used to compute the cohomology of a
double complex. There unfortunately does not seem to be an easy entry
point in the subject of spectral sequences, which is marred by
intrinsic notational complexity. I will limit myself to a very scant
description of the concept and some information linking it back to
\Cref{subsec:homol:total-complexes-and-resolutions}. My motivation
is that, in the literature, references to results such as
\Cref{thm:homol:balancing-derived-functors} are often replaced by sentences such as `by an
immediate spectral sequence argument\dots', and I should try to
explain to the reader what this means.
...
\[
  ...
  \end{tikzpicture}
\]
be an exact triangle\footnote{That is, $\image\alpha = \ker\beta$,
  $\image\beta = \ker\gamma$, and $\image\gamma = \ker\alpha$.} in an
abelian category; I am placing no assumption on the `degree' of
$\gamma$ (and indeed, I am not assuming we have defined a translation
functor).
...
\end{itemize}
where $[e]$ denotes the class of $e \in \ker d$ in $E'$. (Since
...
(\Cref{exer:homol:derived-exact-couple-triangle-is-exact}).
...
exact triangle (couple) obtained in \Cref{fact:homol:9.1} is the
...
differential $d_i$, such that $E_{i+1} = \ker d_i / \image d_i$.

\begin{defn}{}{defn:homol:spectral-sequence}
...
$\overline{B}_{i+1} = \image d_i$; define $Z_{i+1}$, $B_{i+1}$ as the
...
Define\footnote{Here I am implicitly working in a category
  ...
  \Cref{thm:homol:freyd-mitchell-theorem}). The infinite unions and intersections used
...
(\Cref{exer:homol:limit-of-spectral-sequence-from-exact-couple})
that if the spectral sequence arises from an exact couple, as above,
then
\[
  Z_\infty = \bigcap_r \gamma^{-1}(\image \alpha^r), \quad B_\infty =
  \bigcup_r \beta(\ker \alpha^r),
\]
...
\Cref{subsec:groups2:the-jordan-h-older-theorem} are filtrations of
$G$. If $M$ is an object of an abelian category, a filtration as above
determines an associated \emph{graded object}
\[
...
\Cref{defn:homol:double-complex}) that the diagram anticommutes, so that the
...
\]
As we know since
\Cref{subsec:homol:the-long-exact-cohomology-sequence}, this
determines an exact triangle (couple!)
\[
  \begin{tikzpicture}[baseline=(current bounding box.center)]
...
\begin{defn}{}{defn:homol:spectral-sequence-of-double-complex}
...
\begin{rem}{}{rem:homol:spectral-sequence-differential-degree}
  ...
  after \Cref{fact:homol:9.1}), so it does not change $k$ but
  increases $m$ by $(r-1)$.
\end{rem}
...
\begin{thm}{}{thm:homol:spectral-sequence-of-a-double-complex}
  ...
  complex, as I glibly announced in
  \Cref{subsec:homol:exactness-of-the-total-complex}, but it
  computes the graded object determined by a filtration on the
  cohomology of the total complex, and this is good enough for many
  applications.
\end{thm}
...
\begin{proof}
  ...
  couple
  (\Cref{exer:homol:limit-of-spectral-sequence-from-exact-couple}),
  as a subquotient of ${}_vE_1$: it is the quotient
  $Z_\infty/B_\infty$, where
  \[
    Z_\infty = \bigcap_r \gamma^{-1}(\image\alpha^r), \qquad B_\infty
    = \beta\Big(\bigcup_r \ker\alpha^r\Big).
  \]
  ...
  By the innocent
  \Cref{exer:homol:quotient-iso-image-kernel-two-morphisms},
  \[
  ...
  \[
    ...
    \image(H^\bullet(T_{m+1}^\bullet) \to H^\bullet(T_m^\bullet)),
  \]
  hence
  \[
  ...
  as stated.
\end{proof}
\Cref{thm:homol:spectral-sequence-of-a-double-complex} is a substantial generalization of
\Cref{thm:homol:acyclic-assembly-lemma,thm:homol:balancing-derived-functors}. To see that it implies \Cref{thm:homol:balancing-derived-functors},
note again that
\[
...
and by \Cref{thm:homol:spectral-sequence-of-a-double-complex} this says that
...
statement in \Cref{thm:homol:balancing-derived-functors}. This is what is meant by the
incantation `by an immediate spectral sequence argument' I mentioned
in the beginning of this subsection.  This reasoning and other
applications of the spectral sequence of a double complex are easier
to understand if the various degrees and shifts are visualized in a
more compelling way. View the original double complex
\[\begin{tikzcd}
    \vdots & \vdots & \vdots & \vdots & \\
...
$1$ (\Cref{rem:homol:spectral-sequence-differential-degree}); that is, $(i,j) \mapsto (i+r, j-(r-1))$.
...
sequence. The argument sketched above, deriving \Cref{thm:homol:balancing-derived-functors}
from \Cref{thm:homol:spectral-sequence-of-a-double-complex}, is probably clearer from this point of
view: if the cohomology of the columns is concentrated in degree $0$,
then ${}_vE_1$ is concentrated along the bottom row of the array. It
is then clear that all differentials $d_i$ for $i \ge 2$ will have to
be zero, since either their source or their target is zero. Thus the
sequence collapses at ${}_vE_2$ and yields the cohomology of the total
complex by \Cref{thm:homol:spectral-sequence-of-a-double-complex}. The other situation considered in
\Cref{thm:homol:balancing-derived-functors} can be analyzed by using the sequence ${}_hE_i$
obtained from the horizontal filtration.
...
derived functor of the composition of two functors\footnote{For a
  different viewpoint on this question, see \Cref{rem:homol:composition-of-derived-functors-remark}.}:
for example, if $\mathscr{F} : \categ{A} \to \categ{B}$ and
...
$\Tor^S_p(\Tor^R_q(A,S),B) \Rightarrow \Tor^R_\bullet(A,B)$. In
topology, the \emph{Serre spectral sequence} can be used to compute
the homology of a fibration in terms of the homology of the base and
of the fiber. It would be completely futile for me to attempt to do
any justice to the range of applications of spectral sequences: the
set of mathematicians $X$ such that there is an important $X$-spectral
...
put together by working out
\Cref{exer:homol:spectral-seq-for-composite-derived-functors}.

\subsection*{Exercises}

\begin{exer}{}{exer:homol:euler-char-additive-on-distinguished-triangles}
  ...
  bounded complexes in these categories (see
  \Cref{exer:homol:grothendieck-grp-euler-char-abelian-category}). Prove
  that if $A$, $B$, $C$ are bounded complexes in $\categ{A}$ and
  \[
    \begin{tikzpicture}[baseline=(current bounding box.center)]
      \node (A) at (1,1.3) {$A$}; \node (C) at (0,0)
      {$C$}; \node (B) at (2,0)
...
\begin{exer}{\exerused{}}{exer:homol:upside-down-roof-implies-regular-roof}
  ...
  $MC(\beta)^\bullet$ is exact, and rotate a triangle.)
  [\Cref{subsec:homol:derived-categories}]
\end{exer}

\begin{exer}{\exerused{}}{exer:homol:mapping-cone-of-isomorphism-homotopy-trivial}
  Let $\alpha^\bullet : L^\bullet \to M^\bullet$ be an isomorphism of
  cochain complexes. Prove that $MC(\alpha)^\bullet$ is homotopy
  equivalent to $0$. [\Cref{subsec:homol:triangulated-categories},
  \ref{exer:homol:mapping-cone-of-natural-morphism-is-shift}]
\end{exer}

\begin{exer}{\exerused{}}{exer:homol:mapping-cone-of-natural-morphism-is-shift}
  ...
  \Cref{exer:homol:mapping-cone-of-isomorphism-homotopy-trivial}
  first.) [\Cref{subsec:homol:triangulated-categories}]
\end{exer}

\begin{exer}{\exerused{}}{exer:homol:mapping-cone-of-mono-quasi-iso-to-cokernel}
  Let
  \[
    0 \longrightarrow L^\bullet \xrightarrow{\alpha^\bullet} M^\bullet
    \xrightarrow{\beta^\bullet} N^\bullet \longrightarrow 0
  ...
  \item Conclude that $MC(\alpha)^\bullet$ is quasi-isomorphic to
    $N^\bullet$.
  \end{itemize} [\Cref{subsec:homol:triangulated-categories}]
\end{exer}

\begin{exer}{\exerused{}}{exer:homol:connecting-morphism-in-derived-category-from-ses}
  Let
  \[
    0 \longrightarrow L^\bullet \xrightarrow{\alpha^\bullet} M^\bullet
    \xrightarrow{\beta^\bullet} N^\bullet \longrightarrow 0
  ...
  $H^i(N^\bullet) \to H^{i+1}(L^\bullet)$ in
  cohomology. [\Cref{subsec:homol:triangulated-categories}]
\end{exer}

\begin{exer}{\exerused{}}{exer:homol:derived-exact-couple-triangle-is-exact}
  Prove
  \Cref{fact:homol:9.1}. [\Cref{subsec:homol:spectral-sequences}]
\end{exer}

\begin{exer}{\exerused{}}{exer:homol:limit-of-spectral-sequence-from-exact-couple}
  ...
  $B_{r+1} = \beta(\ker\alpha^r)$.)
  [\Cref{subsec:homol:spectral-sequences}]
\end{exer}

\begin{exer}{}{exer:homol:stabilization-of-vertical-spectral-sequence}
  Let ${}_vE_r$ be the spectral sequence of a first-quadrant double
  complex from the vertical filtration, as in
  \Cref{subsec:homol:spectral-sequences}. Prove that
  ${}_vE_r^{i,j} = {}_vE_\infty^{i,j}$ for $r > \max(i, j+1)$.
\end{exer}

\begin{exer}{\exerused{}}{exer:homol:spectral-seq-for-composite-derived-functors}
  Let $\categ{A}$, $\categ{B}$, $\categ{C}$ be abelian categories, and
  let $\mathscr{F} : \categ{A} \to \categ{B}$,
  $\mathscr{G} : \categ{B} \to \categ{C}$ be additive functors. Assume
  that $\categ{A}$ and $\categ{B}$ have enough projectives, that
  ...
  This is precisely the set-up of
  \Cref{exer:homol:cartan-eilenberg-computes-composite-derived-functor}. Now
  take the double complex associated with $(\dagger)$, and construct
  the spectral sequence corresponding to the horizontal
  filtration. Prove that the $E_2$ term of this sequence has terms
  \[ {}_hE_2^{p,q} =
    \categ{L}_p\mathscr{G}(\categ{L}_q\mathscr{F}(A)).
  \]
  (Use
  \Cref{exer:homol:cartan-eilenberg-resolution-computes-cohomology} to
  compute the ${}_hE_1$ term.)

  This is the Grothendieck spectral sequence mentioned at the end of
  \Cref{subsec:homol:spectral-sequences}. Prove that it abuts to
  $\categ{L}_\bullet(\mathscr{G} \circ \mathscr{F})(A)$.

  (Use the `horizontal' version of \Cref{thm:homol:spectral-sequence-of-a-double-complex}, together
  with the computation of the cohomology of the total complex from
  \Cref{exer:homol:cartan-eilenberg-computes-composite-derived-functor}.)
  [\Cref{subsec:homol:universal-property-of-the-derived-functor},
  \ref{exer:homol:cartan-eilenberg-resolution-computes-cohomology},
  \Cref{subsec:homol:spectral-sequences}]
\end{exer}
