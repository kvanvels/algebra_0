...
applications (for example, in Galois theory; cf.\ \ref{sec:fields:short-march-through-applications-of-galois-theory}), and it
...
established back in~\ref{sec:groups:group-actions}.
...
\[
  \rho(g, a) = gag^{-1}.
\]
As we know (\ref{subsec:groups:actions-on-sets}), this datum is equivalent to the datum of a
...
\begin{proof}
  ...
  \]
  (cf.\ \ref{subsec:groups:the-index-and-lagrange-s-theorem}).
\end{proof}
...
to be able to tell whether a group is simple or not.\footnote{In fact,
  ...
  classification result mentioned at the end of \ref{subsec:groups:example-subgroup-generated-by-a-subset}, arguably
...
\begin{proof}[Proof of \cref{prop:groups2:sylow-i-strong}]
  ...
  quotient (\ref{subsec:groups:subgroups-of-quotients}, especially \Cref{prop:groups:subgroups-of-quotients-bijection}), this subgroup
...
\begin{proof}
  ...
  (cf.\ \ref{subsec:groups:subgroups-of-quotients}) of the form $H'/H$ for a subgroup $H'$ of
...
form a multiset\footnote{See~\ref{subsec:prelims:examples-multisets-indexed-sets} for a reminder on multisets:
...
(\ref{subsec:fields:solvability-of-polynomial-equations-by-radicals}).  But we can already appreciate it in the way it relates
...
$\categ{Set}$) of the set $\{1,\dots,n\}$. In~\ref{sec:groups:examples-of-groups} we denoted
...
agreed in~\ref{subsec:groups:symmetric-groups} that we would let our permutations act on the
...
Corollary VII.7.16). Note that $A_2$ is trivial, $A_3 \cong \Z/3\Z$ is
...
We already know that products exist in \textsf{Grp} (see~\ref{subsec:groups:products-et-al});
...
Recall from~\ref{subsec:groups:products-et-al} that the \emph{(direct) product} of two groups
...
in~\ref{subsec:rings:complexes-and-exact-sequences}. A (short) exact sequence of groups is a sequence of
...
$\Z$-modules) from \ref{subsec:rings:complexes-and-exact-sequences}; a notational difference is that here
...
We encountered this terminology in~\ref{subsec:rings:split-exact-sequences} for modules, thus for
...
theorem for finite abelian groups mentioned in~\ref{subsec:groups:example-subgroup-generated-by-a-subset}.
...
In due time (Proposition VI.2.11, VI.2.19) we will in fact be
...
least as far back as\footnote{In fact, this observation will really
  ...
  Remainder Theorem, Theorem V.6.1.} \Cref{exer:groups:cmn-iso-cm-times-cn}.
...
\begin{exer}{}{exer:groups2:6.10}
  ...
  \item Prove that if $G$ is a finite abelian group, then
    ...
    case.)
  \end{itemize}
  In~\ref{subsec:linrep:ext-Z-G-Z} we will encounter another notion of `dual' of a
  group.
\end{exer}
\begin{exer}{}{exer:groups2:6.11}
  ...
  \item Prove that if $p$ is prime, all finite modules over $\Z/p\Z$
    are free.\footnote{As we will see in Proposition VI.4.10, this
