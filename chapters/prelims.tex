...
objects in the category $\mathsf{C}_{A,B}$ considered in Example~3.9, for
$\mathsf{C} = \mathsf{Set}$.
...
definition of products given in \S{}1.4 only makes sense in $\mathsf{Set}$ (and
possibly in other categories where one has a notion of `elements'). We say that
a category $\mathsf{C}$ \emph{has (finite) products}, or is a category `with
(finite) products', if for all objects $A$, $B$ in $\mathsf{C}$ the category
$\mathsf{C}_{A,B}$ considered in Example~3.9 has final objects. Such a final
object consists of the data of an object of $\mathsf{C}$, usually denoted
...
Just as products are final objects in the categories $\mathsf{C}_{A,B}$ obtained
by considering morphisms in $\mathsf{C}$ with common source, whose targets are
...
cf.\ \Cref{exer:3.11}.} $\mathsf{C}^{A,B}$ of morphisms with common target,
whose sources are $A$ and $B$. Dear reader, look away and spell this universal
property out before we do.
Here it is. Let $A$, $B$ be objects of a category $\mathsf{C}$. A
\textbf{coproduct} $A \sqcup B$ of $A$ and $B$ will be an object of
$\mathsf{C}$, endowed with two morphisms $i_A \colon A \to A \sqcup B$,
$i_B \colon B \to A \sqcup B$ and satisfying the following universal property:
...
\end{tikzcd}
\]
there exists a unique morphism $\sigma \colon A \sqcup B \to Z$ such that the
diagram
\[
\begin{tikzcd}
  A \arrow[dr, "i_A"'] \arrow[ddr, "f_A"'] & & B \arrow[dl, "i_B"] \arrow[ddl, "f_B"] \\
  & A \sqcup B \arrow[d, "\sigma"] & \\
  & Z &
\end{tikzcd}
\]
commutes. The symmetry with the universal property of products is hopefully
completely apparent. We say that a category $\mathsf{C}$ \emph{has coproducts}
if this universal problem has a solution for all pairs of objects $A$ and $B$.
Is the reader familiar with any coproduct? Yes!

\begin{prop}{The disjoint union is a coproduct in $\mathsf{Set}$.}{prop:prelims:disjoint-union-coproduct}
\end{prop}
\begin{proof}
Recall (\S{}1.4) that the disjoint union $A \sqcup B$ is defined as the union
...
\[
  \sigma \colon A \sqcup B = (\{0\} \times A) \cup (\{1\} \times B) \to Z
\]
by
\[
...
\end{proof}
This observation tells us that the category $\mathsf{Set}$ has coproducts and
...
beautiful' singleton in $\mathsf{Set}$ (cf.\ \S{}5.1).
...
\begin{exer}{}{exer:5.1}
Prove that a final object in a category $\mathsf{C}$ is initial in the opposite
category $\mathsf{C}^{op}$ (cf.\ \Cref{exer:3.1}).
\end{exer}

\begin{exer}{\exerused}{exer:5.2}
Prove that $\emptyset$ is the unique initial object in $\mathsf{Set}$. [\S{}5.1]
\end{exer}
...
\begin{exer}{}{exer:5.8}
Show that in every category $\mathsf{C}$ the products $A \times B$ and
...
\begin{exer}{}{exer:5.9}
Let $\mathsf{C}$ be a category with products. Find a reasonable candidate for
the universal property that the product $A \times B \times C$ of three objects
of $\mathsf{C}$ ought to satisfy, and prove that both $(A \times B) \times C$
...
$\mathsf{Set}$? It is common to denote the product $A \times \cdots \times A$
($n$ times) by $A^n$.
\end{exer}
...
objects of the categories $\mathsf{C}_{\alpha,\beta}$,
$\mathsf{C}^{\alpha,\beta}$ considered in Example~3.10 (cf.\ also
\Cref{exer:3.11}), by stating carefully the corresponding universal properties.
As it happens, $\mathsf{Set}$ has both fibered products and coproducts. Define
